...
/// Norm-definiteness: at every Ext level, if the norm (a²-d*b²) is structurally
/// zero then both a and b are structurally zero. This captures "d is non-square
/// in the field generated by lower levels."
///
/// For depth-1 towers (Rat components), this follows from dts_nonsquare_radicands.
/// For deeper towers, it's a stronger condition about radicand independence.
    ...
    decreases x,
{
    match x {
        ...
        DynTowerSpec::Ext(re, im, d) =>
            ...
            && dts_norm_definite(*d)
            && (dts_is_zero(dts_sub(dts_mul(*re, *re), dts_mul(*d, dts_mul(*im, *im))))
                ==> (dts_is_zero(*re) && dts_is_zero(*im))),
    }
}
