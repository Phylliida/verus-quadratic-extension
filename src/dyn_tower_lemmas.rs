///  Axiom proof lemmas for DynTowerSpec<T>.
///
///  Each lemma follows the pattern:
///  - Base case (Rat): delegate to T: OrderedField axiom
///  - Inductive case (Ext): recurse on components
///  - Cross-depth (Rat↔Ext): recurse into the Ext's re component
use vstd::prelude::*;
use verus_algebra::traits::*;
// (generic T: OrderedField - no Rational import needed)
use crate::dyn_tower::*;

verus! {

//  ═══════════════════════════════════════════════════════════════════
//   Generic ordered field helper: nonneg(r) ∧ nonneg(neg(r)) → r.eqv(zero)
//  ═══════════════════════════════════════════════════════════════════

///  If both r and neg(r) are non-negative in T, then r ≡ zero.
///  This replaces the old Rational-specific `.num`/`.den` reasoning.
    ...
    ensures r.eqv(T::zero()),
{
    let zero = T::zero();
    let neg_r = r.neg();
    //  From zero.le(neg_r), add r to both sides: zero.add(r).le(neg_r.add(r))
    T::axiom_le_add_monotone(zero, neg_r, r);
    //  zero.add(r) ≡ r
    T::axiom_add_commutative(zero, r);
    T::axiom_add_zero_right(r);
    T::axiom_eqv_transitive(zero.add(r), r.add(zero), r);
    //  neg_r.add(r) ≡ zero
    T::axiom_add_commutative(neg_r, r);
    T::axiom_add_inverse_right(r);
    T::axiom_eqv_transitive(neg_r.add(r), r.add(neg_r), zero);
    //  le_congruence: zero.add(r) ≡ r, neg_r.add(r) ≡ zero → r.le(zero)
    T::axiom_le_congruence(zero.add(r), r, neg_r.add(r), zero);
    //  antisymmetric: zero.le(r) && r.le(zero) → zero.eqv(r) → r.eqv(zero)
    T::axiom_le_antisymmetric(zero, r);
    T::axiom_eqv_symmetric(zero, r);
}
...
pub proof fn lemma_dts_eqv_zero_implies_is_zero<T: OrderedField>(x: DynTowerSpec<T>)
...
pub proof fn lemma_dts_is_zero_implies_eqv_zero<T: OrderedField>(x: DynTowerSpec<T>)
...
pub proof fn lemma_dts_is_zero_congruence<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    ...
    decreases a, b,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            ...
            T::axiom_eqv_transitive(r2, r1, T::zero());
        },
...
pub proof fn lemma_dts_is_zero_eqv<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    ...
    decreases a, b,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            T::axiom_eqv_symmetric(r2, T::zero());
            T::axiom_eqv_transitive(r1, T::zero(), r2);
        },
        (DynTowerSpec::Ext(re1, im1, _), DynTowerSpec::Ext(re2, im2, _)) => {
...
pub proof fn lemma_dts_add_is_zero_left<T: OrderedField>(x: DynTowerSpec<T>, y: DynTowerSpec<T>)
    requires dts_is_zero(x),
    ...
    decreases x, y,
{
    match (x, y) {
        (DynTowerSpec::Rat(r), DynTowerSpec::Rat(s)) => {
            ...
            T::axiom_add_congruence_left(r, T::zero(), s);
            //  Now: (r+s).eqv(0+s)
            T::axiom_add_commutative(T::zero(), s);
            //  (0+s).eqv(s+0)
            T::axiom_add_zero_right(s);
            //  (s+0).eqv(s)
...
pub proof fn lemma_dts_eqv_reflexive<T: OrderedField>(a: DynTowerSpec<T>)
    ensures dts_eqv(a, a),
    decreases a,
{
    match a {
...
pub proof fn lemma_dts_eqv_symmetric<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
pub proof fn lemma_dts_eqv_transitive<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>)
...
pub proof fn lemma_dts_eq_implies_eqv<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires a == b,
    ensures dts_eqv(a, b),
{
...
pub proof fn lemma_dts_add_commutative<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    ensures dts_eqv(dts_add(a, b), dts_add(b, a)),
    decreases a, b,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            T::axiom_add_commutative(r1, r2);
        },
        (DynTowerSpec::Ext(re1, im1, _), DynTowerSpec::Ext(re2, im2, _)) => {
...
pub proof fn lemma_dts_add_associative<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>)
    ...
    decreases a, b, c,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            match c {
                DynTowerSpec::Rat(r3) => {
                    T::axiom_add_associative(r1, r2, r3);
                },
                DynTowerSpec::Ext(re3, im3, _) => {
...
pub proof fn lemma_dts_add_zero_right<T: OrderedField>(a: DynTowerSpec<T>)
...
pub proof fn lemma_dts_add_congruence_left<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>)
    ...
    decreases a, b, c,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            match c {
                DynTowerSpec::Rat(r3) => {
                    T::axiom_add_congruence_left(r1, r2, r3);
                },
                DynTowerSpec::Ext(re3, im3, _) => {
...
pub proof fn lemma_dts_neg_congruence<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_eqv(a, b),
...
pub proof fn lemma_dts_neg_preserves_is_zero<T: OrderedField>(x: DynTowerSpec<T>)
    requires dts_is_zero(x),
    ...
    decreases x,
{
    match x {
        DynTowerSpec::Rat(r) => {
            //  r.eqv(zero) → neg(r).eqv(neg(zero)) → neg(r).eqv(zero)
...
pub proof fn lemma_dts_neg_mul_neg<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
pub proof fn lemma_dts_neg_square<T: OrderedField>(a: DynTowerSpec<T>)
...
#[verifier::rlimit(40)]
pub proof fn lemma_dts_add_closed<T: OrderedField>(
...
#[verifier::rlimit(40)]
pub proof fn lemma_dts_mul_closed<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>,
)
    requires dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
    ensures
...
pub proof fn lemma_dts_neg_well_formed<T: OrderedField>(a: DynTowerSpec<T>)
    requires dts_well_formed(a),
...
///  - Ext case: norm(x*y) ≡ norm(x)*norm(y) via norm_mul.
///    is_zero(x*y) → is_zero(norm(x*y)) → is_zero(norm(x)*norm(y)).
///    ¬is_zero(y) → ¬is_zero(norm(y)) [contrapositive of norm_definite].
///    Induction at lower depth: is_zero(norm(x)) → is_zero(x) [norm_definite].
pub proof fn lemma_dts_mul_cancel_zero<T: OrderedField>(
    x: DynTowerSpec<T>, y: DynTowerSpec<T>,
)
    requires
        dts_is_zero(dts_mul(x, y)),
        !dts_is_zero(y),
        dts_well_formed(x), dts_well_formed(y),
        dts_same_radicand(x, y),
    ...
    decreases dts_depth(x),
{
    match (x, y) {
        (DynTowerSpec::Rat(rx), DynTowerSpec::Rat(ry)) => {
            //  In T (OrderedField): rx*ry ≡ 0 and ry ≢ 0 → rx ≡ 0 via cancel_left.
            T::axiom_mul_commutative(rx, ry);
            T::axiom_eqv_symmetric(rx.mul(ry), ry.mul(rx));
            T::axiom_eqv_transitive(ry.mul(rx), rx.mul(ry), T::zero());
            T::axiom_mul_zero_right(ry);
            T::axiom_eqv_symmetric(ry.mul(T::zero()), T::zero());
            T::axiom_eqv_transitive(ry.mul(rx), T::zero(), ry.mul(T::zero()));
            verus_algebra::lemmas::field_lemmas::lemma_mul_cancel_left::<T>(rx, T::zero(), ry);
        }
        (DynTowerSpec::Ext(xre, xim, xd), DynTowerSpec::Ext(yre, yim, _)) => {
            let dd = *xd;
            ...
            lemma_dts_neg_well_formed(dts_mul(dd, dts_mul(*xim, *xim)));
            lemma_dts_same_radicand_neg(dts_mul(dd, dts_mul(*xim, *xim)));
            ...
            lemma_dts_same_radicand_symmetric(*yre, dts_mul(*yre, *yre));
            lemma_dts_same_radicand_transitive(dts_mul(*yre, *yre), *yre, dd);
            lemma_dts_same_radicand_transitive(dts_mul(*yre, *yre), dd, dts_mul(dd, dts_mul(*yim, *yim)));
            lemma_dts_neg_well_formed(dts_mul(dd, dts_mul(*yim, *yim)));
            lemma_dts_same_radicand_neg(dts_mul(dd, dts_mul(*yim, *yim)));
        ...
        (DynTowerSpec::Ext(xre, xim, _), DynTowerSpec::Rat(r)) => {
            //  mul(Ext, Rat) = Ext(xre*r, xim*r, d).
            lemma_dts_mul_cancel_zero(*xre, DynTowerSpec::Rat(r));
            lemma_dts_mul_cancel_zero(*xim, DynTowerSpec::Rat(r));
        }
    }
}
...
///  Combined with a² ≤ b²: a² = b². Difference of squares: (b-a)(b+a) = 0.
///  Either a+b = 0 → a=b=0, or integral domain → b-a = 0 → a ≤ b.
pub proof fn lemma_dts_square_le_implies_le_fuel<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, fuel: nat,
)
    requires
        fuel >= dts_depth(a) + 1,
    ...
    decreases fuel, 3nat,
{
    ...
    let ab = dts_sub(a, b);
    let sum = dts_add(b, a);
    //  le_total: either b ≥ a or a ≥ b
    lemma_dts_add_closed(a, b);
    lemma_dts_same_radicand_symmetric(a, b);
    ...
    lemma_dts_depth_add_le(a, dts_neg(b));
    lemma_dts_nonneg_radicands_neg(a);
    lemma_dts_nonneg_radicands_neg(b);
    lemma_dts_nonneg_radicands_add(b, dts_neg(a));
    lemma_dts_nonneg_radicands_add(a, dts_neg(b));
    lemma_norm_definite_neg(a);
    lemma_norm_definite_neg(b);
    lemma_norm_definite_add(b, dts_neg(a));
    lemma_norm_definite_add(a, dts_neg(b));
    lemma_dts_nonneg_or_neg_nonneg_fuel(ba, fuel);
    if dts_nonneg_fuel(ba, fuel) {
        return;  //  b ≥ a directly
    }
    //  ¬nonneg(b-a) → nonneg(a-b) from le_total
    //  a ≥ b: square_le_square(b, a) → a² ≥ b²
    //  neg(ba) ≡ ab via neg_sub_swap. same_radicand: neg(ba) ~ ba ~ b ~ a ~ ab.
    lemma_dts_neg_sub_swap(b, a);
    lemma_dts_neg_well_formed(ba);
    lemma_dts_same_radicand_neg(ba);
    lemma_dts_same_radicand_symmetric(ba, dts_neg(ba));
    lemma_dts_same_radicand_symmetric(b, ba);
    lemma_dts_same_radicand_transitive(dts_neg(ba), ba, b);
    lemma_dts_same_radicand_transitive(dts_neg(ba), b, a);
    lemma_dts_same_radicand_symmetric(a, ab);
    lemma_dts_same_radicand_transitive(dts_neg(ba), a, ab);
    lemma_dts_nonneg_fuel_congruence(dts_neg(ba), ab, fuel);
    lemma_dts_square_le_square_fuel(b, a, fuel);
    //  Both a² ≥ b² and b² ≥ a²: le_antisymmetric → is_zero(b²-a²)
    ...
    lemma_dts_nonneg_radicands_neg(dts_mul(a, a));
    lemma_dts_nonneg_radicands_add(dts_mul(b, b), dts_neg(dts_mul(a, a)));
    lemma_norm_definite_mul(a, a);
    lemma_norm_definite_mul(b, b);
    lemma_norm_definite_neg(dts_mul(a, a));
    lemma_norm_definite_add(dts_mul(b, b), dts_neg(dts_mul(a, a)));
    lemma_dts_depth_mul_le(a, a);
    lemma_dts_depth_mul_le(b, b);
    lemma_dts_depth_neg(dts_mul(a, a));
    lemma_dts_depth_add_le(dts_mul(b, b), dts_neg(dts_mul(a, a)));
    //  sub(a², b²) via neg_sub_swap of sub(b², a²)
    lemma_dts_neg_sub_swap(dts_mul(b, b), dts_mul(a, a));
    //  same_radicand(neg(sub(b²,a²)), sub(a²,b²)): chain through b² and a²
    let b2a2 = dts_sub(dts_mul(b, b), dts_mul(a, a));
    let a2b2 = dts_sub(dts_mul(a, a), dts_mul(b, b));
    lemma_dts_neg_well_formed(b2a2);
    lemma_dts_same_radicand_neg(b2a2);
    lemma_dts_same_radicand_symmetric(b2a2, dts_neg(b2a2));
    lemma_dts_same_radicand_symmetric(dts_mul(b, b), b2a2);
    lemma_dts_same_radicand_transitive(dts_neg(b2a2), b2a2, dts_mul(b, b));
    //  Build a2b2 = sub(a², b²) infrastructure
    lemma_dts_same_radicand_symmetric(a, dts_mul(a, a));
    lemma_dts_same_radicand_transitive(dts_mul(a, a), a, b);
    lemma_dts_same_radicand_transitive(dts_mul(a, a), b, dts_mul(b, b));
    lemma_dts_neg_well_formed(dts_mul(b, b));
    lemma_dts_same_radicand_neg(dts_mul(b, b));
    lemma_dts_same_radicand_transitive(dts_mul(a, a), dts_mul(b, b),
        dts_neg(dts_mul(b, b)));
    lemma_dts_nonneg_radicands_neg(dts_mul(b, b));
    lemma_dts_add_closed(dts_mul(a, a), dts_neg(dts_mul(b, b)));
    //  Chain: neg(b2a2) ~ b2a2 ~ b² ~ a ~ a² ~ a2b2
    lemma_dts_same_radicand_symmetric(dts_mul(b, b), b2a2);
    lemma_dts_same_radicand_transitive(dts_neg(b2a2), dts_mul(b, b), a);
    lemma_dts_same_radicand_symmetric(a, dts_mul(a, a));
    lemma_dts_same_radicand_transitive(dts_neg(b2a2), a, dts_mul(a, a));
    lemma_dts_same_radicand_symmetric(dts_mul(a, a), a2b2);
    lemma_dts_same_radicand_transitive(dts_neg(b2a2), dts_mul(a, a), a2b2);
    //  a2b2 depth + nonneg_radicands + norm_definite (for le_antisymmetric later)
    lemma_dts_nonneg_radicands_neg(dts_mul(b, b));
    lemma_dts_nonneg_radicands_add(dts_mul(a, a), dts_neg(dts_mul(b, b)));
    lemma_dts_depth_add_le(dts_mul(a, a), dts_neg(dts_mul(b, b)));
    lemma_norm_definite_neg(dts_mul(b, b));
    lemma_norm_definite_add(dts_mul(a, a), dts_neg(dts_mul(b, b)));
    //  mul(b,b) ~ a chain for the neg(b2a2) ~ a2b2 chain
    lemma_dts_same_radicand_symmetric(b, dts_mul(b, b));
    lemma_dts_same_radicand_symmetric(a, b);
    lemma_dts_same_radicand_transitive(dts_mul(b, b), b, a);
    ...
    lemma_dts_is_zero_congruence(
        ...
        dts_mul(ba, sum));
    //  Case: is_zero(b+a) → both a, b are zero → b-a is zero → nonneg(b-a)
    //  Case: !is_zero(b+a) → integral domain → is_zero(b-a) → nonneg(b-a)
    if dts_is_zero(sum) {
        //  sum = b+a ≡ 0. With nonneg(a), nonneg(b), nonneg_add:
        //  Both a and b are ≡ 0 (le_antisymmetric argument).
...
///  Nonneg(re_val) from neg(norm) conditions: dd*b² ≥ a² for both factors.
///  le_mul chain: (dd*b1²)*(dd*b2²) ≥ a1²*a2². After congruence:
///  (dd*b1*b2)² ≥ (a1*a2)². square_le → dd*b1*b2 ≥ |a1*a2| → re_val ≥ 0.
///  Calls le_transitive_raw with b/a swapped (reversed inequality direction).
proof fn lemma_cauchy_nonneg_re_from_neg_norms<T: OrderedField>(
    ...
    requires
        ...
        dts_nonneg(dd),
        //  neg(norm) ≥ 0: neg(sub(a², dd*b²)) nonneg for both factors
    ...
    decreases f, 5nat,
{
    //  le_transitive_raw with swapped a/b gives (dd*b1²)*(dd*b2²) ≥ a1²*a2²
    //  i.e., nonneg(sub((dd*b1²)*(dd*b2²), a1²*a2²))
    //  This is the REVERSE of what le_transitive_raw produces.
    //  Reuse le_transitive_raw by swapping: call with (b1, b2, a1, a2, dd) gives
    //  nonneg(sub((dd*a1²)*(dd*a2²) ... no, le_transitive_raw uses specific sub patterns.
    //
    //  Simplest: just inline the le_mul calls.
    //  le_mul(a1_sq, db1_sq, db2_sq): sub(db1_sq, a1_sq) ≥ 0 + nonneg(db2_sq) → sub(db1*db2, a1*db2) ≥ 0
    //  le_mul(a2_sq, db2_sq, a1_sq): sub(db2_sq, a2_sq) ≥ 0 + nonneg(a1_sq) → sub(db2*a1, a2*a1) ≥ 0
    //  After commuting: transitivity → sub(db1*db2, a1*a2) ≥ 0
    //  Congruences: db1*db2 ≡ (dd*b1*b2)², a1*a2 ≡ (a1*a2)²... no, the result is
    //  sub((dd*b1²)*(dd*b2²), a1²*a2²) ≥ 0 directly.
    //  Then square_mul congruences → sub((dd*b1*b2)², (a1*a2)²) ≥ 0
    //  Then nonneg(dd*b1*b2) from nonneg_mul(dd, b1*b2).
    //  nonneg(neg(a1*a2)) via nonneg_or_neg.
    //  square_le_implies_le(neg(a1*a2), dd*b1*b2) → sub(dd*b1*b2, neg(a1*a2)) ≥ 0
    //  sub(dd*b1*b2, neg(a1*a2)) = add(dd*b1*b2, neg(neg(a1*a2))) = add(dd*b1*b2, a1*a2) via neg_invol
    //  = add(a1*a2, dd*b1*b2) via commutative = re_val.
    //
    //  For now: Z3 should derive nonneg(re_val) from the nonneg_fuel structural
    //  expansion of the factors + the neg_sub_swap conditions.
    //  If not: implement the full le_mul chain.
    //  Z3 hint: square_nonneg + le_mul are available.
    //  Convert neg(sub(a², dd*b²)) to sub(dd*b², a²) via neg_sub_swap + congruence
    ...
    //  neg_sub_swap: eqv(neg(sub(a1², dd*b1²)), sub(dd*b1², a1²))
    //  Transfer: nonneg(neg(sub(a1², dd*b1²))) → nonneg(sub(dd*b1², a1²))
    //  same_radicand chain for congruence (reuse from nonneg_component helper pattern)
    ...
    lemma_dts_same_radicand_transitive(nx, dts_mul(a1, a1), dd);
    lemma_dts_same_radicand_transitive(nx, dd, dts_mul(dd, dts_mul(b1, b1)));
    lemma_dts_same_radicand_symmetric(nx, dts_neg(nx));
    lemma_dts_same_radicand_transitive(dts_neg(nx), nx, dts_mul(dd, dts_mul(b1, b1)));
    lemma_dts_neg_well_formed(dts_mul(a1, a1));
    lemma_dts_same_radicand_neg(dts_mul(a1, a1));
    lemma_dts_same_radicand_symmetric(dts_mul(a1, a1), dts_neg(dts_mul(a1, a1)));
    lemma_dts_same_radicand_symmetric(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)));
    lemma_dts_same_radicand_transitive(dts_mul(dd, dts_mul(b1, b1)),
        dts_mul(a1, a1), dts_neg(dts_mul(a1, a1)));
    lemma_dts_add_closed(dts_mul(dd, dts_mul(b1, b1)), dts_neg(dts_mul(a1, a1)));
    lemma_dts_same_radicand_transitive(dts_neg(nx), dts_mul(dd, dts_mul(b1, b1)),
        dts_sub(dts_mul(dd, dts_mul(b1, b1)), dts_mul(a1, a1)));
    lemma_dts_nonneg_fuel_congruence(dts_neg(nx),
        dts_sub(dts_mul(dd, dts_mul(b1, b1)), dts_mul(a1, a1)), f);
    //  Similarly for ny
    ...
    lemma_dts_neg_sub_swap(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
    ...
    lemma_dts_same_radicand_transitive(ny, dts_mul(a2, a2), dd);
    lemma_dts_same_radicand_transitive(ny, dd, dts_mul(dd, dts_mul(b2, b2)));
    lemma_dts_same_radicand_symmetric(ny, dts_neg(ny));
    lemma_dts_same_radicand_transitive(dts_neg(ny), ny, dts_mul(dd, dts_mul(b2, b2)));
    lemma_dts_neg_well_formed(dts_mul(a2, a2));
    lemma_dts_same_radicand_neg(dts_mul(a2, a2));
    lemma_dts_same_radicand_symmetric(dts_mul(a2, a2), dts_neg(dts_mul(a2, a2)));
    lemma_dts_same_radicand_symmetric(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
    lemma_dts_same_radicand_transitive(dts_mul(dd, dts_mul(b2, b2)),
        dts_mul(a2, a2), dts_neg(dts_mul(a2, a2)));
    lemma_dts_add_closed(dts_mul(dd, dts_mul(b2, b2)), dts_neg(dts_mul(a2, a2)));
    lemma_dts_same_radicand_transitive(dts_neg(ny), dts_mul(dd, dts_mul(b2, b2)),
    ...
    //  Now have: nonneg(sub(dd*b1², a1²)) and nonneg(sub(dd*b2², a2²))
    //  Proceed with le_mul chain
    ...
    //  dd*b1*b2 ≥ 0
    //  a1*a2: might not be nonneg. Dispatch:
    lemma_dts_depth_mul_le(a1, a2);
    lemma_dts_nonneg_or_neg_nonneg_fuel(dts_mul(a1, a2), f);
    if dts_nonneg_fuel(dts_mul(a1, a2), f) {
        //  Both nonneg: re_val = a1*a2 + dd*b1*b2 ≥ 0 by nonneg_add
        ...
        //  neg(a1*a2) ≥ 0. Use Cauchy-Schwarz: dd*b1*b2 ≥ |a1*a2|.
        //  le_chain_neg gives (dd*b1²)*(dd*b2²) ≥ a1²*a2².
        //  neg_a1a2_square_le converts to nonneg(re_val) via congruence + square_le.
        lemma_cauchy_le_chain_neg_norms(a1, a2, b1, b2, dd, f);
        lemma_cauchy_neg_a1a2_square_le(a1, a2, b1, b2, dd, f);
    }
}
...
///  Derive nonneg(a) from nonneg(norm) + Ext(a,b,d) nonneg + norm_definite.
///  C3 requires nonneg(neg(norm)). With nonneg(norm) too:
///  - If also nonneg(neg(norm)): le_antisymmetric → is_zero(norm) → is_zero(a) → nonneg(a).
///  - If !nonneg(neg(norm)): nonneg_component_from_ext → nonneg(a).
///  C1/C2 both give nonneg(a) directly.
proof fn lemma_dts_nonneg_re_from_nonneg_norm<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, d: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(a) + 1, f >= dts_depth(b) + 1, f >= dts_depth(d) + 1,
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(d),
        dts_same_radicand(a, b), dts_same_radicand(a, d),
        dts_nonneg_radicands(a), dts_nonneg_radicands(b), dts_nonneg_radicands(d),
        dts_norm_definite(a), dts_norm_definite(b), dts_norm_definite(d),
        dts_norm_definite(DynTowerSpec::Ext(Box::new(a), Box::new(b), Box::new(d))),
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a), Box::new(b), Box::new(d)), (f + 1) as nat),
        dts_nonneg_fuel(dts_sub(dts_mul(a, a), dts_mul(d, dts_mul(b, b))), f),
    ensures
        dts_nonneg_fuel(a, f),
    decreases f, 2nat,
{
    let nx = dts_sub(dts_mul(a, a), dts_mul(d, dts_mul(b, b)));
    //  Infrastructure for nx (for le_antisymmetric / nonneg_component_from_ext)
    lemma_dts_same_radicand_reflexive(a);
    lemma_dts_same_radicand_reflexive(b);
    lemma_dts_mul_closed(a, a);
    lemma_dts_mul_closed(b, b);
    ...
    lemma_dts_same_radicand_symmetric(b, dts_mul(b, b));
    lemma_dts_same_radicand_transitive(d, b, dts_mul(b, b));
    lemma_dts_mul_closed(d, dts_mul(b, b));
    lemma_dts_same_radicand_symmetric(a, dts_mul(a, a));
    lemma_dts_same_radicand_transitive(dts_mul(a, a), a, d);
    lemma_dts_same_radicand_symmetric(d, dts_mul(d, dts_mul(b, b)));
    lemma_dts_same_radicand_transitive(dts_mul(a, a), d, dts_mul(d, dts_mul(b, b)));
    lemma_dts_neg_well_formed(dts_mul(d, dts_mul(b, b)));
    lemma_dts_same_radicand_neg(dts_mul(d, dts_mul(b, b)));
    lemma_dts_same_radicand_transitive(dts_mul(a, a), dts_mul(d, dts_mul(b, b)),
        dts_neg(dts_mul(d, dts_mul(b, b))));
    lemma_dts_add_closed(dts_mul(a, a), dts_neg(dts_mul(d, dts_mul(b, b))));
    lemma_dts_nonneg_radicands_mul(a, a);
    lemma_dts_nonneg_radicands_mul(b, b);
    lemma_dts_nonneg_radicands_mul(d, dts_mul(b, b));
    lemma_dts_nonneg_radicands_neg(dts_mul(d, dts_mul(b, b)));
    lemma_dts_nonneg_radicands_add(dts_mul(a, a), dts_neg(dts_mul(d, dts_mul(b, b))));
    lemma_norm_definite_mul(a, a);
    lemma_norm_definite_mul(b, b);
    lemma_norm_definite_mul(d, dts_mul(b, b));
    lemma_norm_definite_neg(dts_mul(d, dts_mul(b, b)));
    lemma_norm_definite_add(dts_mul(a, a), dts_neg(dts_mul(d, dts_mul(b, b))));
    lemma_dts_depth_mul_le(a, a);
    lemma_dts_depth_mul_le(b, b);
    lemma_dts_depth_mul_le(d, dts_mul(b, b));
    lemma_dts_depth_neg(dts_mul(d, dts_mul(b, b)));
    lemma_dts_depth_add_le(dts_mul(a, a), dts_neg(dts_mul(d, dts_mul(b, b))));
    //  Case split: is neg(norm) also nonneg?
    lemma_dts_nonneg_or_neg_nonneg_fuel(nx, f);
    if dts_nonneg_fuel(dts_neg(nx), f) {
        //  Both nonneg(nx) and nonneg(neg(nx)) → is_zero(nx) → is_zero(a) → nonneg(a)
        lemma_dts_le_antisymmetric_fuel(nx, f);
        lemma_dts_is_zero_implies_eqv_zero(a);
        lemma_dts_nonneg_fuel_zero(a, f);
    } else {
        //  norm strictly positive → nonneg_component_from_ext → nonneg(a)
        lemma_dts_nonneg_component_from_ext_fuel(a, b, d, f);
    }
}

///  Derive nonneg(b) from neg(norm) ≥ 0 and Ext(a,b,d) nonneg + norm_definite.
///  C2 requires nonneg(norm) + !is_zero(b). With neg(norm)≥0 too, le_antisymmetric →
///  is_zero(norm) → norm_definite → is_zero(b) contradicts C2's !is_zero(b).
///  C1/C3 both give nonneg(b).
proof fn lemma_dts_nonneg_im_from_neg_norm<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, d: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(a) + 1, f >= dts_depth(b) + 1, f >= dts_depth(d) + 1,
        ...
        dts_norm_definite(DynTowerSpec::Ext(Box::new(a), Box::new(b), Box::new(d))),
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a), Box::new(b), Box::new(d)), (f + 1) as nat),
        dts_nonneg_fuel(dts_neg(dts_sub(dts_mul(a, a), dts_mul(d, dts_mul(b, b)))), f),
    ensures
        dts_nonneg_fuel(b, f),
    decreases f, 2nat,
{
    //  Z3 unfolds Ext nonneg to C1||C2||C3. C1/C3 give nonneg(b) directly.
    //  Eliminate C2 by showing nonneg(norm) + neg(norm)≥0 → is_zero(norm) → is_zero(b).
    let nx = dts_sub(dts_mul(a, a), dts_mul(d, dts_mul(b, b)));
    if dts_nonneg_fuel(nx, f) {
        //  Both nonneg(nx) and nonneg(neg(nx)). Infrastructure for le_antisymmetric(nx).
        lemma_dts_same_radicand_reflexive(a);
        lemma_dts_same_radicand_reflexive(b);
        lemma_dts_mul_closed(a, a);
        lemma_dts_mul_closed(b, b);
        ...
        lemma_dts_depth_add_le(dts_mul(a, a), dts_neg(dts_mul(d, dts_mul(b, b))));
        lemma_dts_le_antisymmetric_fuel(nx, f);
        //  is_zero(nx) → norm_definite(Ext) → is_zero(b) → nonneg(b)
        lemma_dts_is_zero_implies_eqv_zero(b);
        lemma_dts_nonneg_fuel_zero(b, f);
    }
    //  else: !nonneg(nx) → C2 requires nonneg(nx) → C2 eliminated.
    //  C1/C3 both include nonneg(b). Z3 derives.
}

///  Mixed-sign norms: nonneg(neg(nx*ny)) + is_zero(im) → is_zero(re_val) → nonneg(re_val).
///  Via norm_mul eqv: sub(re², dd*im²) ≡ nx*ny. With im=0: re² ≡ nx*ny.
///  neg(nx*ny)≥0 + re²≥0 → is_zero(re²) → is_zero(re_val).
proof fn lemma_dts_iszero_re_from_neg_norm_product<T: OrderedField>(
    ...
    requires
        ...
        dts_norm_definite(a1), dts_norm_definite(a2),
        dts_norm_definite(b1), dts_norm_definite(b2), dts_norm_definite(dd),
        dts_norm_definite(DynTowerSpec::Ext(Box::new(a1), Box::new(b1), Box::new(dd))),
    ...
    decreases f, 3nat,
{
    ...
    //  ═══ Simplify sub(re², dd*im²) ≡ re² using is_zero(im) ═══
    lemma_dts_same_radicand_reflexive(re_val);
    lemma_dts_mul_closed(re_val, re_val);
    lemma_dts_same_radicand_reflexive(im_val);
    lemma_dts_mul_closed(im_val, im_val);
    ...
    lemma_dts_neg_congruence(dts_mul(dd, dts_mul(im_val, im_val)), dts_zero());
    verus_algebra::lemmas::additive_group_lemmas::lemma_neg_zero::<T>();
    lemma_dts_eqv_transitive(
        dts_neg(dts_mul(dd, dts_mul(im_val, im_val))), dts_neg(dts_zero()), dts_zero());
    //  sub(re², dd*im²) = add(re², neg(dd*im²)) ≡ add(re², 0) ≡ re²
    lemma_dts_add_congruence_right(dts_mul(re_val, re_val),
        dts_neg(dts_mul(dd, dts_mul(im_val, im_val))), dts_zero());
    lemma_dts_add_zero_right(dts_mul(re_val, re_val));
    lemma_dts_eqv_transitive(
        dts_sub(dts_mul(re_val, re_val), dts_mul(dd, dts_mul(im_val, im_val))),
        dts_add(dts_mul(re_val, re_val), dts_zero()), dts_mul(re_val, re_val));
    //  ═══ Chain: eqv(nx*ny, re²) ═══
    //  eqv(sub(re², dd*im²), nx*ny) from norm_mul. symmetric → eqv(nx*ny, sub(re²,dd*im²)).
    //  eqv(sub(re²,dd*im²), re²) from above. transitive → eqv(nx*ny, re²).
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_mul_closed(a1, a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_mul_closed(a2, a2);
    ...
    lemma_dts_eqv_symmetric(
        dts_sub(dts_mul(re_val, re_val), dts_mul(dd, dts_mul(im_val, im_val))),
        dts_mul(nx, ny));
    lemma_dts_eqv_transitive(dts_mul(nx, ny),
        dts_sub(dts_mul(re_val, re_val), dts_mul(dd, dts_mul(im_val, im_val))),
        dts_mul(re_val, re_val));
    //  ═══ neg_congruence: eqv(neg(nx*ny), neg(re²)) ═══
    lemma_dts_neg_congruence(dts_mul(nx, ny), dts_mul(re_val, re_val));
    //  ═══ nonneg_fuel_congruence: nonneg(neg(nx*ny)) → nonneg(neg(re²)) ═══
    ...
    //  sr(neg(nx*ny), neg(re²)): neg(nx*ny) ~ nx*ny ~ nx ~ a1 ~ re_val ~ re² ~ neg(re²)
    lemma_dts_same_radicand_symmetric(dts_mul(nx, ny), dts_neg(dts_mul(nx, ny)));
    lemma_dts_same_radicand_symmetric(dts_mul(a1, a1), nx);
    lemma_dts_same_radicand_transitive(nx, dts_mul(a1, a1), a1);
    lemma_dts_same_radicand_symmetric(nx, dts_mul(nx, ny));
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(nx, ny)), dts_mul(nx, ny), nx);
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(nx, ny)), nx, a1);
    //  sr(a1, re_val): a1 ~ mul(a1, a2) ~ re_val
    lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, a2));
    ...
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(nx, ny)), a1, re_val);
    lemma_dts_same_radicand_symmetric(re_val, dts_mul(re_val, re_val));
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(nx, ny)), re_val,
        dts_mul(re_val, re_val));
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(nx, ny)), dts_mul(re_val, re_val),
        dts_neg(dts_mul(re_val, re_val)));
    lemma_dts_nonneg_fuel_congruence(dts_neg(dts_mul(nx, ny)),
        dts_neg(dts_mul(re_val, re_val)), f);
    //  ═══ square_nonneg(re_val): nonneg(re²) ═══
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_dts_nonneg_radicands_mul(b1, b2);
    lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b2));
    ...
    //  ═══ mul_cancel_zero → is_zero(re_val) → nonneg(re_val) ═══
    if dts_is_zero(re_val) {
        lemma_dts_is_zero_implies_eqv_zero(re_val);
        lemma_dts_nonneg_fuel_zero(re_val, f);
    } else {
        lemma_dts_mul_cancel_zero(re_val, re_val);
        //  is_zero(re_val) contradicts !is_zero(re_val) → false.
        lemma_dts_is_zero_implies_eqv_zero(re_val);
        lemma_dts_nonneg_fuel_zero(re_val, f);
    }
}

///  Congruence bridge: convert Q²≥P² from na2_sq*a1_sq form to a1_sq*a2_sq form,
///  then call cauchy_neg_a1a2_square_le. Clean-context wrapper to avoid Z3 pollution.
proof fn lemma_cauchy_neg_a1a2_from_na2sq<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>,
    b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_norm_definite(b1), dts_norm_definite(b2), dts_norm_definite(dd),
        dts_nonneg(dd),
        //  Q²≥P² in na2_sq*a1_sq form: sub(db1sq*db2sq, na2sq*a1sq) ≥ 0
        dts_nonneg_fuel(dts_sub(
            dts_mul(dts_mul(dd, dts_mul(b1, b1)), dts_mul(dd, dts_mul(b2, b2))),
            dts_mul(dts_mul(dts_neg(a2), dts_neg(a2)), dts_mul(a1, a1))), f),
        dts_nonneg_fuel(dts_neg(dts_mul(a1, a2)), f),
        dts_nonneg_fuel(b1, f),
        dts_nonneg_fuel(b2, f),
    ensures
        dts_nonneg_fuel(dts_add(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2))), f),
    decreases f, 5nat,
{
    ...
    let na2_sq = dts_mul(na2, na2);
    let db1_sq = dts_mul(dd, dts_mul(b1, b1));
    let db2_sq = dts_mul(dd, dts_mul(b2, b2));
    //  Infrastructure
    lemma_dts_neg_well_formed(a2);
    lemma_dts_same_radicand_neg(a2);
    lemma_dts_same_radicand_reflexive(na2);
    lemma_dts_mul_closed(na2, na2);
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_mul_closed(a1, a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_mul_closed(a2, a2);
    ...
    lemma_dts_same_radicand_transitive(dd, b2, dts_mul(b2, b2));
    lemma_dts_mul_closed(dd, dts_mul(b2, b2));
    //  sr(na2_sq, a2_sq): na2_sq ~ na2 ~ a2 ~ a2_sq
    lemma_dts_same_radicand_symmetric(na2, na2_sq);
    lemma_dts_same_radicand_symmetric(a2, na2);  //  sr(a2, na2) → sr(na2, a2)
    lemma_dts_same_radicand_transitive(na2_sq, na2, a2);
    lemma_dts_same_radicand_symmetric(a2, a2_sq);
    lemma_dts_same_radicand_transitive(na2_sq, a2, a2_sq);
    //  sr(a1_sq, a2_sq): a1_sq ~ a1 ~ a2 ~ a2_sq
    ...
    //  sr(na2_sq, a1_sq): na2_sq already ~ a2, need a2 ~ a1
    lemma_dts_same_radicand_symmetric(a1, a2);  //  sr(a1, a2) → sr(a2, a1)
    lemma_dts_same_radicand_transitive(na2_sq, a2, a1);
    lemma_dts_same_radicand_symmetric(a1, a1_sq);
    lemma_dts_same_radicand_transitive(na2_sq, a1, a1_sq);
    //  ═══ Congruence: na2_sq*a1_sq ≡ a1_sq*a2_sq ═══
    //  Step 1: eqv(na2_sq, a2_sq) via neg_mul_neg
    lemma_dts_neg_mul_neg(a2, a2);
    //  Step 2: mul_congruence_left: eqv(na2_sq*a1_sq, a2_sq*a1_sq)
    lemma_dts_mul_congruence_left(na2_sq, a2_sq, a1_sq);
    //  Step 3: mul_commutative: eqv(a2_sq*a1_sq, a1_sq*a2_sq)
    lemma_dts_same_radicand_symmetric(a1_sq, a2_sq);  //  sr(a1_sq, a2_sq) → sr(a2_sq, a1_sq)
    lemma_dts_mul_commutative(a2_sq, a1_sq);
    //  Step 4: chain
    lemma_dts_eqv_transitive(dts_mul(na2_sq, a1_sq),
        dts_mul(a2_sq, a1_sq), dts_mul(a1_sq, a2_sq));
    //  ═══ sub_congruence: sub(db1sq*db2sq, na2sq*a1sq) ≡ sub(db1sq*db2sq, a1sq*a2sq) ═══
    //  sr(db1_sq, db2_sq)
    lemma_dts_same_radicand_symmetric(dd, db1_sq);
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);
    lemma_dts_mul_closed(db1_sq, db2_sq);
    lemma_dts_eqv_reflexive(dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_reflexive(dts_mul(db1_sq, db2_sq));
    //  sr(na2_sq*a1_sq, a1_sq*a2_sq)
    ...
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), na2_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), a1_sq,
        dts_mul(a1_sq, a2_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(db1_sq, db2_sq), dts_mul(na2_sq, a1_sq),
        dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq));
    //  ═══ Transfer nonneg via congruence ═══
    //  sr for the sub expressions: sub(db1sq*db2sq, X) where X changes
    //  sub(A, B) = add(A, neg(B)). sr(add(A, neg(B))) comes from add_closed.
    lemma_dts_neg_well_formed(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(na2_sq, dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), na2_sq, a2);
    lemma_dts_same_radicand_transitive(a2, a1, dd);
    lemma_dts_same_radicand_symmetric(a2, a1);
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), a2, a1);
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), a1, dd);
    //  sr(dd, mul(db1_sq, db2_sq)): dd ~ db1_sq ~ mul(db1_sq, db2_sq)
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dd, db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(dd, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(na2_sq, a1_sq), dd,
        dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(na2_sq, a1_sq),
        dts_neg(dts_mul(na2_sq, a1_sq)));
    //  sr(db1_sq*db2_sq, na2_sq*a1_sq) from symmetric of the transitive above
    lemma_dts_same_radicand_symmetric(dts_mul(na2_sq, a1_sq), dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq),
        dts_mul(na2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_neg_well_formed(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, a1);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1, dd);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), dd,
        dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, a2_sq),
        dts_neg(dts_mul(a1_sq, a2_sq)));
    //  sr(db1_sq*db2_sq, a1_sq*a2_sq) from symmetric
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, a2_sq), dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq),
        dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    //  sr between the two sub expressions
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(na2_sq, a1_sq)),
        dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    //  nonneg_fuel_congruence transfer
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(na2_sq, a1_sq)),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)), f);
    //  Now have Q²≥P² in a1_sq*a2_sq form. Call cauchy_neg_a1a2_square_le.
    lemma_cauchy_neg_a1a2_square_le(a1, a2, b1, b2, dd, f);
}
...
proof fn lemma_dts_dd_sq_product_eqv<T: OrderedField>(
    b1: DynTowerSpec<T>, b2: DynTowerSpec<T>, dd: DynTowerSpec<T>,
)
    requires
        dts_well_formed(b1), dts_well_formed(b2), dts_well_formed(dd),
            ...
            dts_mul(dts_mul(dd, dts_mul(b1, b2)), dts_mul(dd, dts_mul(b1, b2)))),
{
    ...
    let dd_sq = dts_mul(dd, dd);

    //  Same-radicand infrastructure
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    ...
    lemma_dts_mul_closed(b1, b2);
    lemma_dts_same_radicand_symmetric(b1, dd);
    lemma_dts_same_radicand_transitive(dd, b1, b2);
    lemma_dts_same_radicand_symmetric(b1, b1_sq);
    lemma_dts_same_radicand_transitive(dd, b1, b1_sq);
    lemma_dts_same_radicand_symmetric(dd, b1_sq);  //  b1_sq ~ dd
    ...
    lemma_dts_same_radicand_symmetric(dd, b2_sq);  //  b2_sq ~ dd
    lemma_dts_mul_closed(dd, b2_sq);
    lemma_dts_same_radicand_symmetric(b1, b1b2);
    lemma_dts_same_radicand_transitive(dd, b1, b1b2);
    lemma_dts_mul_closed(dd, b1b2);
    lemma_dts_same_radicand_transitive(b1_sq, b1, b2);
    lemma_dts_same_radicand_transitive(b1_sq, b2, b2_sq);
    lemma_dts_mul_closed(b1_sq, b2_sq);

    //  ═══ Chain A: db1_sq*db2_sq ≡ dd²*(b1_sq*b2_sq) ═══
    //  A1: db1_sq*db2_sq ≡ dd*(b1_sq*db2_sq) via associative
    lemma_dts_same_radicand_symmetric(dd, db1_sq);
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);
    ...
    //  A2: b1_sq*db2_sq ≡ dd*(b1_sq*b2_sq)
    lemma_dts_same_radicand_symmetric(b1_sq, dd);
    lemma_dts_mul_associative(b1_sq, dd, b2_sq);
    lemma_dts_eqv_symmetric(dts_mul(b1_sq, dts_mul(dd, b2_sq)),
        dts_mul(dts_mul(b1_sq, dd), b2_sq));
    lemma_dts_mul_commutative(b1_sq, dd);
    lemma_dts_mul_closed(b1_sq, dd);
    lemma_dts_same_radicand_symmetric(b1_sq, dts_mul(b1_sq, dd));
    ...
    lemma_dts_eqv_transitive(dts_mul(b1_sq, db2_sq),
        dts_mul(dts_mul(b1_sq, dd), b2_sq), dts_mul(db1_sq, b2_sq));
    lemma_dts_eqv_transitive(dts_mul(b1_sq, db2_sq),
        dts_mul(db1_sq, b2_sq), dts_mul(dd, dts_mul(b1_sq, b2_sq)));

    //  A3: dd*(b1_sq*db2_sq) ≡ dd*(dd*(b1_sq*b2_sq)) via congruence_right
    ...
    lemma_dts_same_radicand_symmetric(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq)));
    lemma_dts_same_radicand_transitive(dts_mul(b1_sq, db2_sq), dd,
        dts_mul(dd, dts_mul(b1_sq, b2_sq)));
    lemma_dts_mul_congruence_right(dts_mul(b1_sq, db2_sq),
        dts_mul(dd, dts_mul(b1_sq, b2_sq)), dd);

    //  A4: dd*(dd*(b1_sq*b2_sq)) ≡ dd²*(b1_sq*b2_sq) via associative
    lemma_dts_same_radicand_symmetric(dd, dts_mul(b1_sq, b2_sq));
    lemma_dts_mul_associative(dd, dd, dts_mul(b1_sq, b2_sq));

    //  Full chain A
    lemma_dts_eqv_transitive(dts_mul(db1_sq, db2_sq),
        dts_mul(dd, dts_mul(b1_sq, db2_sq)),
        dts_mul(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq))));
    lemma_dts_eqv_transitive(dts_mul(db1_sq, db2_sq),
        dts_mul(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq))),
        dts_mul(dd_sq, dts_mul(b1_sq, b2_sq)));

    //  ═══ Chain B: db1b2² ≡ dd²*(b1_sq*b2_sq) via square_mul ═══
    lemma_dts_square_mul(dd, b1b2);
    lemma_dts_square_mul(b1, b2);
    //  sr chain for mul_congruence_right(b1b2², b1_sq*b2_sq, dd²)
    ...
    lemma_dts_same_radicand_transitive(dts_mul(b1b2, b1b2), b1_sq,
        dts_mul(b1_sq, b2_sq));
    lemma_dts_mul_congruence_right(dts_mul(b1b2, b1b2),
        dts_mul(b1_sq, b2_sq), dd_sq);
    //  db1b2² ≡ dd²*(b1b2)² ≡ dd²*(b1_sq*b2_sq)
    lemma_dts_eqv_transitive(dts_mul(db1b2, db1b2),
        dts_mul(dd_sq, dts_mul(b1b2, b1b2)),
        dts_mul(dd_sq, dts_mul(b1_sq, b2_sq)));

    //  ═══ Combined: db1_sq*db2_sq ≡ db1b2² ═══
    lemma_dts_eqv_symmetric(dts_mul(db1b2, db1b2),
        dts_mul(dd_sq, dts_mul(b1_sq, b2_sq)));
    lemma_dts_eqv_transitive(dts_mul(db1_sq, db2_sq),
        dts_mul(dd_sq, dts_mul(b1_sq, b2_sq)),
        dts_mul(db1b2, db1b2));
...
///  Le-chain for neg norms: from dd*bi² ≥ ai² (reversed direction), derives
///  nonneg(sub((dd*b1²)*(dd*b2²), a1²*a2²)) via le_mul chain + algebra identity.
///  Mirror of le_transitive_raw with swapped inequality direction.
proof fn lemma_cauchy_le_chain_neg_norms<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>,
    b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    ...
    ensures
        dts_nonneg_fuel(dts_sub(
            dts_mul(dts_mul(dd, dts_mul(b1, b1)), dts_mul(dd, dts_mul(b2, b2))),
            dts_mul(dts_mul(a1, a1), dts_mul(a2, a2))), f),
    decreases f, 3nat,
{
    let a1_sq = dts_mul(a1, a1);
    let a2_sq = dts_mul(a2, a2);
    ...
    //  ═══ Part A: Same-radicand infrastructure ═══
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    ...
    lemma_dts_same_radicand_symmetric(a1_sq, db1_sq);  //  db1_sq ~ a1_sq
    lemma_dts_same_radicand_symmetric(dd, db2_sq);
    lemma_dts_same_radicand_transitive(a1_sq, dd, db2_sq);
    //  db1_sq ~ db2_sq
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);
    //  a2_sq chains
    lemma_dts_same_radicand_symmetric(a2, a2_sq);
    lemma_dts_same_radicand_transitive(a2_sq, a2, dd);
    lemma_dts_same_radicand_transitive(a2_sq, dd, db2_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, db2_sq);  //  db2_sq ~ a2_sq
    //  db2_sq ~ a1_sq: db2_sq ~ dd (from symmetric) and a1_sq ~ dd → dd ~ a1_sq
    lemma_dts_same_radicand_symmetric(a1_sq, dd);  //  dd ~ a1_sq
    lemma_dts_same_radicand_transitive(db2_sq, dd, a1_sq);  //  db2_sq ~ a1_sq

    //  ═══ Part B: le_mul steps ═══
    //  Establish nonneg(db2_sq) and nonneg(a1_sq) for le_mul preconditions
    ...
    lemma_dts_nonneg_radicands_mul(dd, b1_sq);
    lemma_dts_nonneg_radicands_mul(dd, b2_sq);
    lemma_norm_definite_mul(b1, b1);
    lemma_norm_definite_mul(b2, b2);
    lemma_norm_definite_mul(a1, a1);
    lemma_norm_definite_mul(a2, a2);
    lemma_norm_definite_mul(dd, b1_sq);
    lemma_norm_definite_mul(dd, b2_sq);
    lemma_dts_depth_mul_le(b1, b1);
    lemma_dts_depth_mul_le(b2, b2);
    lemma_dts_depth_mul_le(a1, a1);
    lemma_dts_depth_mul_le(a2, a2);
    lemma_dts_depth_mul_le(dd, b1_sq);
    lemma_dts_depth_mul_le(dd, b2_sq);
    //  nonneg(db2_sq): dd ≥ 0, b2² ≥ 0 → dd*b2² ≥ 0
    lemma_dts_square_nonneg(b2, f);
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b2_sq, f);
    //  nonneg(a1_sq): a1² ≥ 0 from square_nonneg
    lemma_dts_square_nonneg(a1, f);
    //  Step 1: le_mul(a1_sq, db1_sq, db2_sq) → nonneg(sub(db1_sq*db2_sq, a1_sq*db2_sq))
    lemma_dts_le_mul_nonneg_monotone_fuel(a1_sq, db1_sq, db2_sq, f);
    //  Step 2: le_mul(a2_sq, db2_sq, a1_sq) → nonneg(sub(db2_sq*a1_sq, a2_sq*a1_sq))
    lemma_dts_le_mul_nonneg_monotone_fuel(a2_sq, db2_sq, a1_sq, f);

    //  ═══ Part C: Commute middle term + nonneg_add for le_transitive ═══
    //  sub(db1_sq*db2_sq, a1_sq*db2_sq): commute a1_sq*db2_sq ≡ db2_sq*a1_sq
    lemma_dts_same_radicand_transitive(a1_sq, dd, db2_sq);
    lemma_dts_mul_commutative(a1_sq, db2_sq);
    lemma_dts_mul_closed(a1_sq, db2_sq);
    lemma_dts_mul_closed(db2_sq, a1_sq);
    lemma_dts_mul_closed(db1_sq, db2_sq);
    lemma_dts_eqv_reflexive(dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, db2_sq), a1_sq, db2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, db2_sq), db2_sq,
        dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_reflexive(dts_mul(db1_sq, db2_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, db2_sq),
        dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, db2_sq)),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq)), f);
    //  Now: nonneg(sub(db1_sq*db2_sq, db2_sq*a1_sq)) — aligned with step 2
    let sub1 = dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq));
    let sub2 = dts_sub(dts_mul(db2_sq, a1_sq), dts_mul(a2_sq, a1_sq));
    //  nonneg_add(sub1, sub2): need well_formed + same_radicand + etc.
    lemma_dts_neg_well_formed(dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq), db1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, db2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db2_sq,
        dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq),
        dts_neg(dts_mul(db2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
    lemma_dts_nonneg_radicands_mul(db1_sq, db2_sq);
    lemma_dts_nonneg_radicands_mul(db2_sq, a1_sq);
    //  a2_sq ~ a1_sq chain: a2_sq ~ a2 ~ a1 ~ a1_sq
    lemma_dts_same_radicand_transitive(a2_sq, a2, a1);
    lemma_dts_same_radicand_symmetric(a1, a1_sq);
    lemma_dts_same_radicand_transitive(a2_sq, a1, a1_sq);
    lemma_dts_mul_closed(a2_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, a1_sq);
    lemma_dts_nonneg_radicands_mul(a2_sq, a1_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(db2_sq, a1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
    lemma_dts_neg_well_formed(dts_mul(a2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(a2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), db2_sq, a2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), a2_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, a2_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), a2_sq, dts_mul(a2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), dts_mul(a2_sq, a1_sq),
        dts_neg(dts_mul(a2_sq, a1_sq)));
    lemma_dts_nonneg_radicands_neg(dts_mul(a2_sq, a1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(db2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
    lemma_norm_definite_mul(db1_sq, db2_sq);
    lemma_norm_definite_mul(db2_sq, a1_sq);
    lemma_norm_definite_mul(a2_sq, a1_sq);
    lemma_norm_definite_neg(dts_mul(db2_sq, a1_sq));
    lemma_norm_definite_add(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
    lemma_norm_definite_neg(dts_mul(a2_sq, a1_sq));
    lemma_norm_definite_add(dts_mul(db2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(db2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
    //  same_radicand(sub1, sub2)
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq), sub1);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(db1_sq, db2_sq), db2_sq);
    lemma_dts_same_radicand_transitive(sub1, db2_sq, dts_mul(db2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(db2_sq, a1_sq), sub2);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(db2_sq, a1_sq), sub2);
    //  depth bounds
    lemma_dts_depth_mul_le(db1_sq, db2_sq);
    lemma_dts_depth_mul_le(db2_sq, a1_sq);
    lemma_dts_depth_mul_le(a2_sq, a1_sq);
    lemma_dts_depth_neg(dts_mul(db2_sq, a1_sq));
    lemma_dts_depth_add_le(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
    lemma_dts_depth_neg(dts_mul(a2_sq, a1_sq));
    lemma_dts_depth_add_le(dts_mul(db2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
    lemma_dts_nonneg_add_closed_fuel(sub1, sub2, f);

    //  ═══ Part D: Algebra identity: add(sub(A,B), sub(B,C)) ≡ sub(A,C) ═══
    let neg_b = dts_neg(dts_mul(db2_sq, a1_sq));
    let neg_c = dts_neg(dts_mul(a2_sq, a1_sq));
    lemma_dts_add_associative(dts_mul(db1_sq, db2_sq), neg_b,
        dts_add(dts_mul(db2_sq, a1_sq), neg_c));
    lemma_dts_add_associative(neg_b, dts_mul(db2_sq, a1_sq), neg_c);
    lemma_dts_eqv_symmetric(dts_add(dts_add(neg_b, dts_mul(db2_sq, a1_sq)), neg_c),
        dts_add(neg_b, dts_add(dts_mul(db2_sq, a1_sq), neg_c)));
    lemma_dts_add_commutative(neg_b, dts_mul(db2_sq, a1_sq));
    lemma_dts_add_inverse_right(dts_mul(db2_sq, a1_sq));
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_mul(db2_sq, a1_sq)),
        dts_add(dts_mul(db2_sq, a1_sq), neg_b), dts_zero());
    lemma_dts_add_congruence_left(dts_add(neg_b, dts_mul(db2_sq, a1_sq)), dts_zero(), neg_c);
    lemma_dts_add_commutative(dts_zero(), neg_c);
    lemma_dts_add_zero_right(neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_zero(), neg_c), dts_add(neg_c, dts_zero()), neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_add(neg_b, dts_mul(db2_sq, a1_sq)), neg_c),
        dts_add(dts_zero(), neg_c), neg_c);
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_add(dts_mul(db2_sq, a1_sq), neg_c)),
        dts_add(dts_add(neg_b, dts_mul(db2_sq, a1_sq)), neg_c), neg_c);
    lemma_dts_add_congruence_right(dts_mul(db1_sq, db2_sq),
        dts_add(neg_b, dts_add(dts_mul(db2_sq, a1_sq), neg_c)), neg_c);
    lemma_dts_eqv_transitive(
        dts_add(sub1, sub2),
        dts_add(dts_mul(db1_sq, db2_sq),
            dts_add(neg_b, dts_add(dts_mul(db2_sq, a1_sq), neg_c))),
        dts_add(dts_mul(db1_sq, db2_sq), neg_c));

    //  ═══ Part E: Commute a2_sq*a1_sq ≡ a1_sq*a2_sq + transfer ═══
    lemma_dts_mul_commutative(a2_sq, a1_sq);
    lemma_dts_mul_closed(a1_sq, a2_sq);
    lemma_dts_neg_congruence(dts_mul(a2_sq, a1_sq), dts_mul(a1_sq, a2_sq));
    //  sub(A, a2_sq*a1_sq) ≡ sub(A, a1_sq*a2_sq) via congruence on neg
    lemma_dts_eqv_reflexive(dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(a2_sq, a1_sq)),
        dts_mul(a2_sq, a1_sq), a2_sq);
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(a2_sq, a1_sq)), a2_sq, a1_sq);
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(a2_sq, a1_sq)), a1_sq,
        dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_neg(dts_mul(a2_sq, a1_sq)),
        dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_add_congruence_right(dts_mul(db1_sq, db2_sq), neg_c,
        dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_eqv_transitive(dts_add(sub1, sub2),
        dts_add(dts_mul(db1_sq, db2_sq), neg_c),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    //  Transfer nonneg
    lemma_dts_add_closed(sub1, sub2);
    lemma_dts_same_radicand_symmetric(sub1, dts_add(sub1, sub2));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), sub1,
        dts_mul(db1_sq, db2_sq));
    lemma_dts_neg_well_formed(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, a1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), a1_sq,
        dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq),
        dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_add(sub1, sub2),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)), f);
}

///  Neg(a1*a2) case: given (dd*b1²)*(dd*b2²) ≥ a1²*a2² and neg(a1*a2) ≥ 0,
///  derives nonneg(add(a1*a2, dd*(b1*b2))) via congruence chains + square_le_implies_le.
proof fn lemma_cauchy_neg_a1a2_square_le<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>,
    b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    ...
    decreases f, 4nat,
{
    ...
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_dts_same_radicand_transitive(b1, a1, dd);
    lemma_dts_same_radicand_symmetric(b1, dd);  //  dd ~ b1
    lemma_dts_mul_closed(a1, a2);
    lemma_dts_mul_closed(b1, b2);
    lemma_dts_same_radicand_symmetric(b1, b1b2);
    lemma_dts_same_radicand_transitive(dd, b1, b1b2);
    ...
    //  ═══ Congruence: db1_sq*db2_sq ≡ db1b2² via dd_sq_product_eqv ═══
    lemma_dts_dd_sq_product_eqv(b1, b2, dd);

    //  ═══ Congruence: a1_sq*a2_sq ≡ neg(a1*a2)² ═══
    //  square_mul: (a1*a2)² ≡ a1²*a2²
    lemma_dts_square_mul(a1, a2);
    lemma_dts_eqv_symmetric(dts_mul(a1a2, a1a2), dts_mul(a1_sq, a2_sq));
    //  neg_mul_neg: neg(a1*a2)² ≡ (a1*a2)²
    lemma_dts_same_radicand_reflexive(a1a2);
    lemma_dts_neg_mul_neg(a1a2, a1a2);
    //  Chain: a1_sq*a2_sq ≡ (a1*a2)² ≡ neg(a1*a2)²
    lemma_dts_eqv_symmetric(dts_mul(neg_a1a2, neg_a1a2), dts_mul(a1a2, a1a2));
    lemma_dts_eqv_transitive(dts_mul(a1_sq, a2_sq), dts_mul(a1a2, a1a2),
        dts_mul(neg_a1a2, neg_a1a2));

    //  ═══ sub_congruence: sub(db1_sq*db2_sq, a1_sq*a2_sq) ≡ sub(db1b2², neg_a1a2²) ═══
    lemma_dts_mul_closed(a1, a1);
    lemma_dts_mul_closed(a2, a2);
    lemma_dts_same_radicand_reflexive(db1b2);
    lemma_dts_mul_closed(db1b2, db1b2);
    lemma_dts_neg_well_formed(a1a2);
    lemma_dts_same_radicand_neg(a1a2);
    lemma_dts_same_radicand_symmetric(a1a2, neg_a1a2);
    lemma_dts_same_radicand_reflexive(neg_a1a2);
    lemma_dts_mul_closed(neg_a1a2, neg_a1a2);
    //  db1_sq/db2_sq infrastructure
    lemma_dts_same_radicand_reflexive(dd);
    lemma_dts_same_radicand_reflexive(b2);
    lemma_dts_mul_closed(b1, b1);  //  sr(b1, b1_sq), wf(b1_sq)
    lemma_dts_mul_closed(b2, b2);  //  sr(b2, b2_sq), wf(b2_sq)
    lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, b1));  //  b1_sq ~ b1
    lemma_dts_same_radicand_transitive(dd, b1, dts_mul(b1, b1));  //  dd ~ b1_sq
    lemma_dts_mul_closed(dd, dts_mul(b1, b1));  //  sr(dd, db1_sq), wf(db1_sq)
    lemma_dts_same_radicand_symmetric(a1, b2);
    lemma_dts_same_radicand_transitive(b2, a1, dd);
    lemma_dts_same_radicand_symmetric(b2, dd);  //  dd ~ b2
    lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, b2));  //  b2_sq ~ b2
    lemma_dts_same_radicand_transitive(dd, b2, dts_mul(b2, b2));  //  dd ~ b2_sq
    lemma_dts_mul_closed(dd, dts_mul(b2, b2));  //  sr(dd, db2_sq), wf(db2_sq)
    lemma_dts_same_radicand_symmetric(dd, db1_sq);  //  db1_sq ~ dd
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);  //  db1_sq ~ db2_sq
    //  sr(db1_sq*db2_sq, db1b2²) for sub_congruence_both
    lemma_dts_mul_closed(db1_sq, db2_sq);
    lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, a1));
    lemma_dts_same_radicand_transitive(dts_mul(a1, a1), a1, dd);
    lemma_dts_same_radicand_symmetric(dd, db1_sq);
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dd, db1b2);
    lemma_dts_same_radicand_symmetric(db1b2, dts_mul(db1b2, db1b2));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1b2,
        dts_mul(db1b2, db1b2));
    //  sr(a1_sq*a2_sq, neg_a1a2²) for sub_congruence_both
    //  a1_sq ~ a2_sq chain: a1_sq ~ a1 ~ a2 ~ a2_sq
    lemma_dts_same_radicand_symmetric(a1, a1_sq);  //  a1_sq ~ a1
    lemma_dts_same_radicand_transitive(a1_sq, a1, a2);  //  a1_sq ~ a2
    lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, a2));  //  a2_sq ~ a2
    lemma_dts_same_radicand_transitive(a1_sq, a2, a2_sq);  //  a1_sq ~ a2_sq
    lemma_dts_mul_closed(a1_sq, a2_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(a1, a1a2);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, a1);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1, a1a2);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1a2, neg_a1a2);
    lemma_dts_same_radicand_symmetric(neg_a1a2, dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), neg_a1a2,
        dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_sub_congruence_both(
        dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq),
        dts_mul(db1b2, db1b2), dts_mul(neg_a1a2, neg_a1a2));
    //  Transfer nonneg — need sr chains for add_closed (= sub well_formed)
    //  sr(db1_sq*db2_sq, neg(a1_sq*a2_sq)): db1_sq*db2_sq ~ db1_sq ~ dd ~ a1 ~ a1_sq ~ a1_sq*a2_sq ~ neg(...)
    lemma_dts_neg_well_formed(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, dd);
    lemma_dts_same_radicand_symmetric(dd, a1);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dd, a1);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), a1, a1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq),
        dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    //  sr(db1b2², neg(neg_a1a2²)): db1b2² ~ db1b2 ~ dd ~ a1 ~ a1a2 ~ neg_a1a2 ~ neg_a1a2² ~ neg(...)
    lemma_dts_neg_well_formed(dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_same_radicand_neg(dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_same_radicand_symmetric(db1b2, dts_mul(db1b2, db1b2));
    lemma_dts_same_radicand_symmetric(dd, db1b2);  //  db1b2 ~ dd (before transitive)
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), db1b2, dd);
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), dd, a1);
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), a1, a1a2);
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), a1a2, neg_a1a2);
    lemma_dts_same_radicand_symmetric(neg_a1a2, dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), neg_a1a2,
        dts_mul(neg_a1a2, neg_a1a2));
    lemma_dts_same_radicand_transitive(dts_mul(db1b2, db1b2), dts_mul(neg_a1a2, neg_a1a2),
        dts_neg(dts_mul(neg_a1a2, neg_a1a2)));
    lemma_dts_add_closed(dts_mul(db1b2, db1b2), dts_neg(dts_mul(neg_a1a2, neg_a1a2)));
    lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)),
        dts_mul(db1_sq, db2_sq), dts_mul(db1b2, db1b2));
    lemma_dts_same_radicand_symmetric(dts_mul(db1b2, db1b2),
        dts_sub(dts_mul(db1b2, db1b2), dts_mul(neg_a1a2, neg_a1a2)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)),
        dts_mul(db1b2, db1b2),
        dts_sub(dts_mul(db1b2, db1b2), dts_mul(neg_a1a2, neg_a1a2)));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)),
        dts_sub(dts_mul(db1b2, db1b2), dts_mul(neg_a1a2, neg_a1a2)), f);

    //  ═══ square_le_implies_le(neg_a1a2, db1b2) → nonneg(sub(db1b2, neg_a1a2)) ═══
    //  Setup: nonneg(db1b2) = nonneg(dd*(b1*b2))
    lemma_dts_nonneg_radicands_mul(b1, b2);
    lemma_norm_definite_mul(b1, b2);
    lemma_dts_depth_mul_le(b1, b2);
    lemma_dts_nonneg_mul_closed_fuel(b1, b2, f);
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b1b2, f);
    //  sr(neg_a1a2, db1b2) for square_le_implies_le
    lemma_dts_same_radicand_transitive(neg_a1a2, a1a2, a1);
    lemma_dts_same_radicand_transitive(neg_a1a2, a1, dd);
    lemma_dts_same_radicand_transitive(neg_a1a2, dd, db1b2);
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_dts_nonneg_radicands_neg(a1a2);
    lemma_dts_nonneg_radicands_mul(dd, b1b2);
    lemma_norm_definite_mul(a1, a2);
    lemma_norm_definite_neg(a1a2);
    lemma_norm_definite_mul(dd, b1b2);
    lemma_dts_depth_neg(a1a2);
    lemma_dts_depth_mul_le(a1, a2);
    lemma_dts_depth_mul_le(dd, b1b2);
    lemma_dts_square_le_implies_le_fuel(neg_a1a2, db1b2, f);
    //  → nonneg(sub(db1b2, neg_a1a2))

    //  ═══ sub(db1b2, neg_a1a2) ≡ add(a1*a2, db1b2) = re_val ═══
    //  sub(db1b2, neg_a1a2) = add(db1b2, neg(neg_a1a2)) = add(db1b2, neg(neg(a1a2)))
    //  neg_involution: neg(neg(a1a2)) ≡ a1a2
    lemma_dts_neg_involution(a1a2);
    //  add_congruence_right: add(db1b2, neg(neg(a1a2))) ≡ add(db1b2, a1a2)
    lemma_dts_neg_well_formed(neg_a1a2);
    lemma_dts_same_radicand_neg(neg_a1a2);
    lemma_dts_same_radicand_symmetric(a1a2, neg_a1a2);  //  neg_a1a2 ~ a1a2
    lemma_dts_same_radicand_symmetric(neg_a1a2, dts_neg(neg_a1a2));  //  neg(neg_a1a2) ~ neg_a1a2
    lemma_dts_same_radicand_transitive(dts_neg(neg_a1a2), neg_a1a2, a1a2);
    lemma_dts_add_congruence_right(db1b2, dts_neg(neg_a1a2), a1a2);
    //  add_commutative: add(db1b2, a1a2) ≡ add(a1a2, db1b2)
    lemma_dts_add_commutative(db1b2, a1a2);
    //  Chain: sub(db1b2, neg_a1a2) ≡ add(db1b2, a1a2) ≡ add(a1a2, db1b2)
    lemma_dts_eqv_transitive(dts_sub(db1b2, neg_a1a2),
        dts_add(db1b2, a1a2), dts_add(a1a2, db1b2));
    //  Transfer nonneg — sr chains for add_closed
    //  sr(db1b2, neg(neg_a1a2)): db1b2 ~ dd ~ a1 ~ a1a2 ~ neg_a1a2 ~ neg(neg_a1a2)
    lemma_dts_same_radicand_symmetric(a1, dd);  //  dd ~ a1
    lemma_dts_same_radicand_transitive(db1b2, dd, a1);
    lemma_dts_same_radicand_symmetric(a1, a1a2);  //  a1a2 ~ a1
    lemma_dts_same_radicand_transitive(db1b2, a1, a1a2);
    lemma_dts_same_radicand_transitive(db1b2, a1a2, neg_a1a2);
    lemma_dts_same_radicand_transitive(db1b2, neg_a1a2, dts_neg(neg_a1a2));
    lemma_dts_add_closed(db1b2, dts_neg(neg_a1a2));
    //  sr(db1b2, a1a2) already established above
    lemma_dts_add_closed(db1b2, a1a2);
    //  sr(a1a2, db1b2) from symmetric
    lemma_dts_same_radicand_symmetric(db1b2, a1a2);
    lemma_dts_add_closed(a1a2, db1b2);
    lemma_dts_same_radicand_symmetric(db1b2, dts_sub(db1b2, neg_a1a2));
    lemma_dts_same_radicand_symmetric(a1a2, dts_add(a1a2, db1b2));
    lemma_dts_same_radicand_transitive(dts_sub(db1b2, neg_a1a2), db1b2, a1a2);
    lemma_dts_same_radicand_transitive(dts_sub(db1b2, neg_a1a2), a1a2,
        dts_add(a1a2, db1b2));
    lemma_dts_nonneg_fuel_congruence(dts_sub(db1b2, neg_a1a2),
        dts_add(a1a2, db1b2), f);
}
    ...
    decreases f, 3nat,
{
    ...
    lemma_dts_same_radicand_transitive(a2, a1, dd);
    lemma_dts_same_radicand_symmetric(a2, dd);
    //  a1_sq ~ a2_sq ~ db1_sq ~ db2_sq chains
    lemma_dts_same_radicand_symmetric(a1, a1_sq);
    lemma_dts_same_radicand_transitive(a1_sq, a1, dd);
    lemma_dts_same_radicand_symmetric(dd, db1_sq);
    lemma_dts_same_radicand_transitive(a1_sq, dd, db1_sq);
    lemma_dts_same_radicand_symmetric(a2, a2_sq);
    lemma_dts_same_radicand_transitive(a2_sq, a2, dd);
    lemma_dts_same_radicand_transitive(a2_sq, dd, db1_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, db1_sq);
    lemma_dts_same_radicand_symmetric(dd, db2_sq);
    lemma_dts_same_radicand_transitive(a1_sq, dd, db2_sq);
    //  Additional chains needed by le_mul calls
    lemma_dts_same_radicand_symmetric(a1_sq, db1_sq);  //  db1_sq ~ a1_sq (le_mul step 1 arg a~b)
    lemma_dts_same_radicand_transitive(a1_sq, a1, a2);  //  a1_sq ~ a2
    lemma_dts_same_radicand_transitive(a1_sq, a2, a2_sq);  //  a1_sq ~ a2_sq (le_mul step 1 arg b~c)
    lemma_dts_same_radicand_transitive(a2_sq, dd, db2_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, db2_sq);  //  db2_sq ~ a2_sq (le_mul step 2 arg a~b)

    //  ═══ Part B: le_mul steps (redo in clean context) ═══
    //  Step 1: le_mul(db1_sq, a1_sq, a2_sq) → nonneg(sub(a1_sq*a2_sq, db1_sq*a2_sq))
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(a2, a2);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    lemma_dts_nonneg_radicands_mul(b2, b2);
    ...
    lemma_dts_depth_mul_le(dd, b1_sq);
    lemma_dts_depth_mul_le(dd, b2_sq);
    lemma_dts_square_nonneg(a2, f);
    lemma_dts_le_mul_nonneg_monotone_fuel(db1_sq, a1_sq, a2_sq, f);
    //  Step 2: le_mul(db2_sq, a2_sq, db1_sq) → nonneg(sub(a2_sq*db1_sq, db2_sq*db1_sq))
    lemma_dts_square_nonneg(b1, f);
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b1_sq, f);
    lemma_dts_le_mul_nonneg_monotone_fuel(db2_sq, a2_sq, db1_sq, f);

    //  ═══ Part C: Commute middle term + nonneg_add for le_transitive ═══
    //  sub(a1_sq*a2_sq, db1_sq*a2_sq): commute db1_sq*a2_sq ≡ a2_sq*db1_sq
    lemma_dts_mul_commutative(db1_sq, a2_sq);
    lemma_dts_mul_closed(db1_sq, a2_sq);
    lemma_dts_mul_closed(a2_sq, db1_sq);
    lemma_dts_mul_closed(a1_sq, a2_sq);
    lemma_dts_eqv_reflexive(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, a2_sq), db1_sq, a2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, a2_sq), a2_sq, dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_reflexive(dts_mul(a1_sq, a2_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(a1_sq, a2_sq), dts_mul(db1_sq, a2_sq),
        dts_mul(a1_sq, a2_sq), dts_mul(a2_sq, db1_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(db1_sq, a2_sq)),
        dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(a2_sq, db1_sq)), f);
    //  Now: nonneg(sub(a1_sq*a2_sq, a2_sq*db1_sq)) — aligned with step 2
    let sub1 = dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(a2_sq, db1_sq));
    let sub2 = dts_sub(dts_mul(a2_sq, db1_sq), dts_mul(db2_sq, db1_sq));
    //  nonneg_add(sub1, sub2): need well_formed + same_radicand + etc.
    lemma_dts_neg_well_formed(dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_neg(dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, a2_sq), a1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, a2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a2_sq, dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), dts_mul(a2_sq, db1_sq),
        dts_neg(dts_mul(a2_sq, db1_sq)));
    lemma_dts_add_closed(dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a2_sq, db1_sq)));
    lemma_dts_nonneg_radicands_mul(a1_sq, a2_sq);
    lemma_dts_nonneg_radicands_mul(a2_sq, db1_sq);
    lemma_dts_same_radicand_transitive(db2_sq, dd, db1_sq);  //  db2_sq ~ db1_sq
    lemma_dts_mul_closed(db2_sq, db1_sq);
    lemma_dts_nonneg_radicands_mul(db2_sq, db1_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(a2_sq, db1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a2_sq, db1_sq)));
    lemma_dts_neg_well_formed(dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_neg(dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_symmetric(db2_sq, db1_sq);  //  db1_sq ~ db2_sq
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(db1_sq, db2_sq, dts_mul(db2_sq, db1_sq));  //  db1_sq ~ mul(db2_sq, db1_sq)
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, db1_sq), a2_sq, db1_sq);  //  mul(a2_sq, db1_sq) ~ db1_sq
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, db1_sq), db1_sq, dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, db1_sq), dts_mul(db2_sq, db1_sq),
        dts_neg(dts_mul(db2_sq, db1_sq)));
    lemma_dts_nonneg_radicands_neg(dts_mul(db2_sq, db1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(a2_sq, db1_sq), dts_neg(dts_mul(db2_sq, db1_sq)));
    lemma_norm_definite_mul(a1_sq, a2_sq);
    lemma_norm_definite_mul(a2_sq, db1_sq);
    lemma_norm_definite_mul(db2_sq, db1_sq);
    lemma_norm_definite_neg(dts_mul(a2_sq, db1_sq));
    lemma_norm_definite_add(dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a2_sq, db1_sq)));
    lemma_norm_definite_neg(dts_mul(db2_sq, db1_sq));
    lemma_norm_definite_add(dts_mul(a2_sq, db1_sq), dts_neg(dts_mul(db2_sq, db1_sq)));
    lemma_dts_add_closed(dts_mul(a2_sq, db1_sq), dts_neg(dts_mul(db2_sq, db1_sq)));
    //  same_radicand(sub1, sub2)
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, a2_sq), sub1);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(a1_sq, a2_sq), a2_sq);
    lemma_dts_same_radicand_transitive(sub1, a2_sq, dts_mul(a2_sq, db1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a2_sq, db1_sq), sub2);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(a2_sq, db1_sq), sub2);
    //  depth bounds
    lemma_dts_depth_mul_le(a1_sq, a2_sq);
    lemma_dts_depth_mul_le(a2_sq, db1_sq);
    lemma_dts_depth_mul_le(db2_sq, db1_sq);
    lemma_dts_depth_neg(dts_mul(a2_sq, db1_sq));
    lemma_dts_depth_add_le(dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a2_sq, db1_sq)));
    lemma_dts_depth_neg(dts_mul(db2_sq, db1_sq));
    lemma_dts_depth_add_le(dts_mul(a2_sq, db1_sq), dts_neg(dts_mul(db2_sq, db1_sq)));
    lemma_dts_nonneg_add_closed_fuel(sub1, sub2, f);
    //  → nonneg(add(sub1, sub2))

    //  ═══ Part D: Algebra identity: add(sub(A,B), sub(B,C)) ≡ sub(A,C) ═══
    let neg_b = dts_neg(dts_mul(a2_sq, db1_sq));
    let neg_c = dts_neg(dts_mul(db2_sq, db1_sq));
    //  Step D1: associative outer
    lemma_dts_add_associative(dts_mul(a1_sq, a2_sq), neg_b,
        dts_add(dts_mul(a2_sq, db1_sq), neg_c));
    //  Step D2: inner neg_b + B → zero
    lemma_dts_add_associative(neg_b, dts_mul(a2_sq, db1_sq), neg_c);
    lemma_dts_eqv_symmetric(dts_add(dts_add(neg_b, dts_mul(a2_sq, db1_sq)), neg_c),
        dts_add(neg_b, dts_add(dts_mul(a2_sq, db1_sq), neg_c)));
    lemma_dts_add_commutative(neg_b, dts_mul(a2_sq, db1_sq));
    lemma_dts_add_inverse_right(dts_mul(a2_sq, db1_sq));
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_mul(a2_sq, db1_sq)),
        dts_add(dts_mul(a2_sq, db1_sq), neg_b), dts_zero());
    //  Step D3: add(zero, neg_c) ≡ neg_c
    lemma_dts_add_congruence_left(dts_add(neg_b, dts_mul(a2_sq, db1_sq)), dts_zero(), neg_c);
    lemma_dts_add_commutative(dts_zero(), neg_c);
    lemma_dts_add_zero_right(neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_zero(), neg_c), dts_add(neg_c, dts_zero()), neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_add(neg_b, dts_mul(a2_sq, db1_sq)), neg_c),
        dts_add(dts_zero(), neg_c), neg_c);
    //  Step D4: chain inner
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_add(dts_mul(a2_sq, db1_sq), neg_c)),
        dts_add(dts_add(neg_b, dts_mul(a2_sq, db1_sq)), neg_c), neg_c);
    //  Step D5: propagate to outer
    lemma_dts_add_congruence_right(dts_mul(a1_sq, a2_sq),
        dts_add(neg_b, dts_add(dts_mul(a2_sq, db1_sq), neg_c)), neg_c);
    //  Step D6: full chain: add(sub1, sub2) ≡ add(A, neg_c) = sub(A, C)
    lemma_dts_eqv_transitive(
        dts_add(sub1, sub2),
        dts_add(dts_mul(a1_sq, a2_sq), dts_add(neg_b, dts_add(dts_mul(a2_sq, db1_sq), neg_c))),
        dts_add(dts_mul(a1_sq, a2_sq), neg_c));
    //  Transfer: nonneg(add(sub1, sub2)) → nonneg(sub(A, C))
    lemma_dts_add_closed(sub1, sub2);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, db2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), db2_sq, dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), dts_mul(db2_sq, db1_sq), neg_c);
    lemma_dts_add_closed(dts_mul(a1_sq, a2_sq), neg_c);
    lemma_dts_same_radicand_symmetric(sub1, dts_add(sub1, sub2));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), sub1, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, a2_sq),
        dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(db2_sq, db1_sq)));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), dts_mul(a1_sq, a2_sq),
        dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(db2_sq, db1_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_add(sub1, sub2),
    ...
    decreases f, 4nat,
{
    ...
    lemma_dts_square_mul(a1, a2);
    lemma_dts_eqv_symmetric(dts_mul(a1a2, a1a2), dts_mul(a1_sq, a2_sq));
    //  E2: eqv(db2_sq*db1_sq, neg(db1b2)²) via chain
    //  First: b1~b2 for mul_closed(b1, b2)
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_dts_mul_closed(b1, b2);
    //  dd~b1b2 for mul_closed(dd, b1b2)
    lemma_dts_same_radicand_symmetric(b1, b1b2);
    lemma_dts_same_radicand_transitive(dd, b1, b1b2);
    lemma_dts_mul_closed(dd, b1b2);
    lemma_dts_same_radicand_reflexive(db1b2);
    lemma_dts_mul_closed(db1b2, db1b2);
    //  neg_mul_neg: neg(db1b2)² ≡ db1b2²
    lemma_dts_neg_mul_neg(db1b2, db1b2);
    //  square_mul chain: db1b2² ≡ dd²*(b1b2)² ≡ dd²*(b1_sq*b2_sq)
    lemma_dts_square_mul(dd, b1b2);
    lemma_dts_square_mul(b1, b2);
    ...
    lemma_dts_eqv_transitive(dts_mul(db1b2, db1b2),
        ...
        dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)));
    //  Now: C = db2_sq*db1_sq ≡ dd²*(b1_sq*b2_sq) via mul algebra
    //  db2_sq*db1_sq → db1_sq*db2_sq [commutative]
    lemma_dts_same_radicand_transitive(db2_sq, dd, db1_sq);
    ...
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(db1_sq, db2_sq),
        dts_mul(dd, dts_mul(b1_sq, db2_sq)));
    //  b1_sq*db2_sq → dd*(b1_sq*b2_sq) via associative+commutative chain
    lemma_dts_same_radicand_symmetric(b1_sq, b1);
    lemma_dts_same_radicand_transitive(b1_sq, b1, dd);
    lemma_dts_same_radicand_transitive(b1_sq, dd, db2_sq);
    lemma_dts_mul_associative(b1_sq, dd, b2_sq);
    ...
    lemma_dts_mul_congruence_right(dts_mul(b1_sq, db2_sq),
        dts_mul(dd, dts_mul(b1_sq, b2_sq)), dd);
    //  → eqv(dd*(b1_sq*db2_sq), dd*(dd*(b1_sq*b2_sq)))
    //  dd*(dd*(b1_sq*b2_sq)) → dd²*(b1_sq*b2_sq) [associative reversed]
    lemma_dts_same_radicand_symmetric(dd, dts_mul(b1_sq, b2_sq));
    lemma_dts_mul_associative(dd, dd, dts_mul(b1_sq, b2_sq));
    lemma_dts_eqv_symmetric(dts_mul(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq))),
        dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)));
    //  Full chain C → dd²*(b1_sq*b2_sq) → db1b2² → neg(db1b2)²
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(dd, dts_mul(b1_sq, db2_sq)),
        dts_mul(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq))));
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(dd, dts_mul(dd, dts_mul(b1_sq, b2_sq))),
        dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)));
    //  reverse db1b2² chain
    lemma_dts_eqv_symmetric(dts_mul(db1b2, db1b2), dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)));
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)),
        dts_mul(db1b2, db1b2));
    //  db1b2² → neg(db1b2)²: eqv_symmetric of neg_mul_neg
    lemma_dts_eqv_symmetric(dts_mul(dts_neg(db1b2), dts_neg(db1b2)), dts_mul(db1b2, db1b2));
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(db1b2, db1b2),
        dts_mul(dts_neg(db1b2), dts_neg(db1b2)));
    //  E3: same_radicand(A, a1a2²) and same_radicand(C, neg(db1b2)²)
    lemma_dts_mul_closed(a1, a2);
    lemma_dts_same_radicand_symmetric(a1, a1a2);
    lemma_dts_same_radicand_reflexive(a1a2);
    lemma_dts_mul_closed(a1a2, a1a2);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, a1);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1, a1a2);
    lemma_dts_same_radicand_symmetric(a1a2, dts_mul(a1a2, a1a2));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1a2, dts_mul(a1a2, a1a2));
    //  C ~ neg(db1b2)²
    lemma_dts_neg_well_formed(db1b2);
    lemma_dts_same_radicand_neg(db1b2);
    lemma_dts_same_radicand_reflexive(dts_neg(db1b2));
    lemma_dts_mul_closed(dts_neg(db1b2), dts_neg(db1b2));
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, db1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, db1_sq), db2_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, db1_sq), dd, db1b2);
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, db1_sq), db1b2, dts_neg(db1b2));
    lemma_dts_same_radicand_symmetric(dts_neg(db1b2), dts_mul(dts_neg(db1b2), dts_neg(db1b2)));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, db1_sq), dts_neg(db1b2),
        dts_mul(dts_neg(db1b2), dts_neg(db1b2)));
    //  sub_congruence_both: sub(a1_sq*a2_sq, db2_sq*db1_sq) ≡ sub(a1a2², neg(db1b2)²)
    lemma_dts_sub_congruence_both(dts_mul(a1_sq, a2_sq), dts_mul(db2_sq, db1_sq),
        dts_mul(a1a2, a1a2), dts_mul(dts_neg(db1b2), dts_neg(db1b2)));
    //  Transfer nonneg
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(a1_sq, a2_sq), dts_mul(db2_sq, db1_sq)),
        dts_sub(dts_mul(a1a2, a1a2), dts_mul(dts_neg(db1b2), dts_neg(db1b2))), f);
    //  → nonneg(sub(a1a2², neg(db1b2)²))

    //  ═══ Part F: square_le_implies_le + le_antisymmetric → is_zero(re_val) ═══
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_norm_definite_mul(a1, a2);
    lemma_dts_nonneg_radicands_mul(b1, b2);
    lemma_norm_definite_mul(b1, b2);
    ...
    //  db1b2 ~ a1a2 chain: db1b2 ~ dd ~ a1 ~ a1a2
    lemma_dts_same_radicand_symmetric(dd, db1b2);
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_transitive(db1b2, dd, a1);
    lemma_dts_same_radicand_symmetric(a1, a1a2);
    lemma_dts_same_radicand_transitive(db1b2, a1, a1a2);
    //  a1a2 ~ db1b2 for norm_definite_add
    lemma_dts_same_radicand_symmetric(db1b2, a1a2);
    ...
    //  Z3 should derive nonneg(re_val) from neg_involution + congruence.
    //  le_antisymmetric(re_val): nonneg(re_val) + nonneg(neg(re_val)) → is_zero(re_val)
    lemma_dts_nonneg_radicands_add(a1a2, db1b2);
    lemma_norm_definite_add(a1a2, db1b2);
    lemma_dts_depth_add_le(a1a2, db1b2);
    lemma_dts_add_closed(a1a2, db1b2);
    //  Connect sub(a1a2, neg(db1b2)) to re_val via neg_involution + congruence
    lemma_dts_neg_involution(db1b2);
    lemma_dts_add_congruence_right(a1a2, dts_neg(dts_neg(db1b2)), db1b2);
    //  eqv(sub(a1a2, neg(db1b2)), re_val)
    lemma_dts_same_radicand_symmetric(a1, a1a2);
    lemma_dts_same_radicand_transitive(a1a2, a1, dd);
    lemma_dts_same_radicand_transitive(a1a2, dd, db1b2);
    lemma_dts_same_radicand_neg(db1b2);
    //  sub(a1a2, neg(db1b2)) = add(a1a2, neg(neg(db1b2)))
    //  Need same_radicand for the congruence transfer to re_val
    lemma_dts_neg_well_formed(db1b2);
    lemma_dts_same_radicand_transitive(a1a2, db1b2, dts_neg(db1b2));
    lemma_dts_same_radicand_neg(dts_neg(db1b2));
    lemma_dts_same_radicand_transitive(a1a2, dts_neg(db1b2), dts_neg(dts_neg(db1b2)));
    lemma_dts_neg_well_formed(dts_neg(db1b2));
    lemma_dts_add_closed(a1a2, dts_neg(dts_neg(db1b2)));  //  same_radicand(a1a2, sub(a1a2, neg(db1b2)))
    lemma_dts_same_radicand_symmetric(a1a2, dts_sub(a1a2, dts_neg(db1b2)));
    lemma_dts_same_radicand_symmetric(a1a2, re_val);
    lemma_dts_same_radicand_transitive(dts_sub(a1a2, dts_neg(db1b2)), a1a2, re_val);
    lemma_dts_nonneg_fuel_congruence(dts_sub(a1a2, dts_neg(db1b2)), re_val, f);
    assert(dts_nonneg_fuel(dts_neg(re_val), f));  //  from requires
    lemma_dts_le_antisymmetric_fuel(re_val, f);
}

///  Reverse Cauchy: from positive norms (a²≥d*b²), nonneg(a1*a2), nonneg(neg(dd*b1*b2)),
///  derives nonneg(add(a1*a2, dd*b1*b2)) = nonneg(re_val).
///  Same le_mul chain + congruence + square_le as cauchy_neg_db1b2_case, but
///  ensures nonneg(re_val) instead of is_zero(re_val) — no le_antisymmetric step.
proof fn lemma_reverse_cauchy_nonneg_re_from_pos_norms<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>,
    b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    ...
    decreases f, 5nat,
{
    ...
    //  ═══ Congruence: sub(a1_sq*a2_sq, db2_sq*db1_sq) → sub(a1a2², neg(db1b2)²) ═══
    //  E1: eqv(a1_sq*a2_sq, a1a2²) from square_mul
    lemma_dts_square_mul(a1, a2);
    lemma_dts_eqv_symmetric(dts_mul(a1a2, a1a2), dts_mul(a1_sq, a2_sq));
    //  E2: eqv(db2_sq*db1_sq, neg(db1b2)²) via chain
    ...
    lemma_dts_eqv_transitive(dts_mul(db1b2, db1b2),
        dts_mul(dts_mul(dd, dd), dts_mul(b1b2, b1b2)),
        dts_mul(dts_mul(dd, dd), dts_mul(b1_sq, b2_sq)));
    //  C = db2_sq*db1_sq ≡ dd²*(b1_sq*b2_sq) via mul algebra
    lemma_dts_same_radicand_transitive(db2_sq, dd, db1_sq);
    lemma_dts_mul_commutative(db2_sq, db1_sq);
    lemma_dts_same_radicand_symmetric(db2_sq, db1_sq);
    lemma_dts_mul_closed(b1_sq, dd);
    lemma_dts_mul_associative(dd, b1_sq, db2_sq);
    lemma_dts_eqv_symmetric(dts_mul(dd, dts_mul(b1_sq, db2_sq)), dts_mul(db1_sq, db2_sq));
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(db1_sq, db2_sq),
    ...
    lemma_dts_eqv_transitive(dts_mul(db2_sq, db1_sq), dts_mul(db1b2, db1b2),
        dts_mul(dts_neg(db1b2), dts_neg(db1b2)));
    //  sr(A, a1a2²) and sr(C, neg(db1b2)²) for sub_congruence_both
    lemma_dts_mul_closed(a1, a2);
    lemma_dts_same_radicand_symmetric(a1, a1a2);
    lemma_dts_same_radicand_reflexive(a1a2);
    lemma_dts_mul_closed(a1a2, a1a2);
    ...
    //  ═══ square_le_implies_le(neg(db1b2), a1a2) → nonneg(sub(a1a2, neg(db1b2))) ═══
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_norm_definite_mul(a1, a2);
    lemma_dts_nonneg_radicands_mul(b1, b2);
    lemma_norm_definite_mul(b1, b2);
    ...
    //  ═══ sub(a1a2, neg(db1b2)) ≡ add(a1a2, db1b2) = re_val ═══
    lemma_dts_nonneg_radicands_add(a1a2, db1b2);
    lemma_norm_definite_add(a1a2, db1b2);
    lemma_dts_depth_add_le(a1a2, db1b2);
    lemma_dts_add_closed(a1a2, db1b2);
    ...
    lemma_dts_same_radicand_transitive(a1a2, dts_neg(db1b2), dts_neg(dts_neg(db1b2)));
    lemma_dts_neg_well_formed(dts_neg(db1b2));
    lemma_dts_add_closed(a1a2, dts_neg(dts_neg(db1b2)));
    lemma_dts_same_radicand_symmetric(a1a2, dts_sub(a1a2, dts_neg(db1b2)));
    lemma_dts_same_radicand_symmetric(a1a2, re_val);
    lemma_dts_same_radicand_transitive(dts_sub(a1a2, dts_neg(db1b2)), a1a2, re_val);
    lemma_dts_nonneg_fuel_congruence(dts_sub(a1a2, dts_neg(db1b2)), re_val, f);
...
///  Neg norms handler: both norms ≤ 0 → nonneg(re_val) via Cauchy bridge.
///  From neg(norm)≥0 + norm_definite → nonneg(b1), nonneg(b2).
///  Case split on a1*a2: nonneg → nonneg_add; neg → cauchy_neg_a1a2_from_na2sq.
proof fn lemma_neg_norms_nonneg_re_val<T: OrderedField>(
    ...
    decreases f, 6nat,
{
    //  Derive nonneg(b1), nonneg(b2) from neg(norm) + norm_definite.
    ...
    lemma_dts_nonneg_or_neg_nonneg_fuel(dts_mul(a1, a2), f);
    if dts_nonneg_fuel(dts_mul(a1, a2), f) {
        //  nonneg(a1*a2) + nonneg(dd*b1*b2) → nonneg(re_val) via nonneg_add.
        lemma_dts_same_radicand_symmetric(a1, b1);
        lemma_dts_same_radicand_transitive(b1, a1, b2);
        lemma_dts_mul_closed(b1, b2);
        lemma_dts_nonneg_radicands_mul(b1, b2);
        ...
        lemma_dts_nonneg_mul_closed_fuel(b1, b2, f);
        lemma_dts_nonneg_fuel_stabilize(dd, f);
        lemma_dts_same_radicand_symmetric(a1, dd);  //  sr(dd, a1)
        lemma_dts_same_radicand_transitive(dd, a1, b1);  //  sr(dd, b1)
        lemma_dts_same_radicand_transitive(dd, b1, dts_mul(b1, b2));
        lemma_dts_mul_closed(dd, dts_mul(b1, b2));
        lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b2));
        lemma_norm_definite_mul(dd, dts_mul(b1, b2));
        ...
        //  neg(a1*a2)≥0: build le chain → sub(db1sq*db2sq, a1sq*a2sq) ≥ 0,
        //  then call cauchy_neg_a1a2_square_le directly.
        let a1_sq = dts_mul(a1, a1);
        let a2_sq = dts_mul(a2, a2);
        let db1_sq = dts_mul(dd, dts_mul(b1, b1));
        let db2_sq = dts_mul(dd, dts_mul(b2, b2));
        ...
        //  neg_sub_swap: neg(nx) ≥ 0 → sub(dd*b1², a1²) ≥ 0
        lemma_dts_neg_sub_swap(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)));
        lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, a1));
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1), a1, dd);
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1), dd, dts_mul(dd, dts_mul(b1, b1)));
        ...
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)),
            dts_neg(dts_mul(dd, dts_mul(b1, b1))));
        lemma_dts_add_closed(dts_mul(a1, a1), dts_neg(dts_mul(dd, dts_mul(b1, b1))));
        lemma_dts_same_radicand_neg(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))));
        //  sr(a1_sq, neg(sub(a1_sq, db1_sq))) from transitive chain
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1),
            dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))),
            dts_neg(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)))));
        lemma_dts_same_radicand_symmetric(dts_mul(a1, a1),
            dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))));
        ...
        lemma_dts_neg_well_formed(dts_mul(a1, a1));
        lemma_dts_same_radicand_neg(dts_mul(a1, a1));
        lemma_dts_same_radicand_symmetric(dd, db1_sq);  //  sr(db1_sq, dd) for transitive below
        lemma_dts_same_radicand_transitive(db1_sq, dd, a1);
        lemma_dts_same_radicand_symmetric(a1, a1_sq);
        lemma_dts_same_radicand_transitive(db1_sq, a1, a1_sq);
        lemma_dts_same_radicand_transitive(db1_sq, a1_sq, dts_neg(a1_sq));
        lemma_dts_add_closed(db1_sq, dts_neg(a1_sq));
        lemma_dts_same_radicand_symmetric(dd, db1_sq);
        lemma_dts_same_radicand_symmetric(dts_mul(a1, a1),
            dts_neg(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)))));
        lemma_dts_same_radicand_transitive(
            dts_neg(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)))),
            dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)));
        lemma_dts_same_radicand_transitive(
            dts_neg(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)))),
            dts_mul(dd, dts_mul(b1, b1)), dts_neg(a1_sq));
        lemma_dts_same_radicand_symmetric(db1_sq, dts_sub(db1_sq, a1_sq));
        lemma_dts_same_radicand_transitive(
            dts_neg(dts_sub(a1_sq, db1_sq)),
            dts_sub(a1_sq, db1_sq), a1_sq);
        lemma_dts_same_radicand_transitive(
            dts_neg(dts_sub(a1_sq, db1_sq)), a1_sq, db1_sq);
        lemma_dts_same_radicand_symmetric(db1_sq, dts_sub(db1_sq, a1_sq));
        lemma_dts_same_radicand_transitive(
            dts_neg(dts_sub(a1_sq, db1_sq)), db1_sq, dts_sub(db1_sq, a1_sq));
        lemma_dts_nonneg_fuel_congruence(
            dts_neg(dts_sub(a1_sq, db1_sq)), dts_sub(db1_sq, a1_sq), f);
        //  Similarly for neg(ny) → sub(dd*b2², a2²) ≥ 0
        lemma_dts_neg_sub_swap(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
        lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, a2));
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_transitive(a2, a1, dd);
        ...
        lemma_dts_same_radicand_symmetric(dd, db2_sq);
        lemma_dts_same_radicand_symmetric(a2, dd);
        lemma_dts_same_radicand_transitive(db2_sq, dd, a2);
        lemma_dts_same_radicand_transitive(db2_sq, a2, a2_sq);
        lemma_dts_same_radicand_transitive(db2_sq, a2_sq, dts_neg(a2_sq));
        lemma_dts_add_closed(db2_sq, dts_neg(a2_sq));
        lemma_dts_same_radicand_neg(dts_sub(a2_sq, db2_sq));
        lemma_dts_same_radicand_symmetric(dts_sub(a2_sq, db2_sq), dts_neg(dts_sub(a2_sq, db2_sq)));
        lemma_dts_same_radicand_symmetric(dts_mul(a2, a2), dts_sub(a2_sq, db2_sq));
        lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)),
            dts_sub(a2_sq, db2_sq), a2_sq);
        lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)), a2_sq, db2_sq);
        lemma_dts_same_radicand_symmetric(db2_sq, dts_sub(db2_sq, a2_sq));
        lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)), db2_sq,
            dts_sub(db2_sq, a2_sq));
        lemma_dts_nonneg_fuel_congruence(
            dts_neg(dts_sub(a2_sq, db2_sq)), dts_sub(db2_sq, a2_sq), f);
        //  ═══ Le chain: db1sq*db2sq ≥ a1sq*a2sq ═══
        //  Step 1: le_mul(a1_sq, db1_sq, db2_sq): sub(db1_sq, a1_sq)≥0, db2_sq≥0
        lemma_dts_same_radicand_symmetric(a1, a1_sq);
        lemma_dts_same_radicand_transitive(a1_sq, a1, dd);
        lemma_dts_same_radicand_transitive(a1_sq, dd, db1_sq);
        lemma_dts_same_radicand_symmetric(a1_sq, db1_sq);
        lemma_dts_same_radicand_transitive(a1_sq, dd, db2_sq);  //  sr(a1_sq, db2_sq)
        lemma_dts_same_radicand_transitive(db1_sq, a1_sq, db2_sq);
        //  Wait, need sr(db1_sq, db2_sq): db1_sq ~ dd ~ db2_sq
        lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);
        lemma_dts_nonneg_fuel_stabilize(dd, f);
        lemma_dts_square_nonneg(b2, f);
        lemma_dts_nonneg_mul_closed_fuel(dd, dts_mul(b2, b2), f);
        lemma_dts_square_nonneg(a1, f);
        lemma_dts_le_mul_nonneg_monotone_fuel(a1_sq, db1_sq, db2_sq, f);
        //  Step 2: le_mul(a2_sq, db2_sq, a1_sq): sub(db2_sq, a2_sq)≥0, a1_sq≥0
        lemma_dts_same_radicand_symmetric(a2, a2_sq);
        lemma_dts_same_radicand_transitive(a2_sq, a2, dd);
        lemma_dts_same_radicand_transitive(a2_sq, dd, db2_sq);
        lemma_dts_same_radicand_symmetric(a2_sq, db2_sq);
        //  sr(a2_sq, a1_sq): a2_sq ~ a2 ~ a1 ~ a1_sq
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_transitive(a2_sq, a2, a1);
        lemma_dts_same_radicand_transitive(a2_sq, a1, a1_sq);
        lemma_dts_same_radicand_transitive(db2_sq, a2_sq, a1_sq);
        //  Wait, need sr(db2_sq, a1_sq): db2_sq ~ dd ~ a1 ~ a1_sq
        lemma_dts_same_radicand_symmetric(dd, a1);
        lemma_dts_same_radicand_transitive(db2_sq, dd, a1);
        lemma_dts_same_radicand_transitive(db2_sq, a1, a1_sq);
        lemma_dts_le_mul_nonneg_monotone_fuel(a2_sq, db2_sq, a1_sq, f);
        //  Commute + transitivity: same pattern as cauchy_le_transitive_raw Part C+D.
        //  Middle: a1_sq*db2_sq and db2_sq*a1_sq (commutative)
        lemma_dts_mul_commutative(a1_sq, db2_sq);
        lemma_dts_mul_closed(a1_sq, db2_sq);
        lemma_dts_mul_closed(db2_sq, a1_sq);
        lemma_dts_mul_closed(db1_sq, db2_sq);
        ...
        lemma_dts_eqv_reflexive(dts_mul(db1_sq, db2_sq));
        lemma_dts_same_radicand_reflexive(dts_mul(db1_sq, db2_sq));
        //  sub_congruence for step 1: sub(db1_sq*db2_sq, a1_sq*db2_sq) → sub(db1_sq*db2_sq, db2_sq*a1_sq)
        lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, db2_sq));
        lemma_dts_same_radicand_transitive(dts_mul(a1_sq, db2_sq), a1_sq, db2_sq);
        lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(a1_sq, db2_sq), db2_sq, dts_mul(db2_sq, a1_sq));
        lemma_dts_sub_congruence_both(
            dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, db2_sq),
            dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq));
        lemma_dts_nonneg_fuel_congruence(
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, db2_sq)),
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq)), f);
        //  nonneg_add: sub(db1sq*db2sq, db2sq*a1sq) + sub(db2sq*a1sq, a2sq*a1sq) ≥ 0
        let sub1 = dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq));
        let sub2 = dts_sub(dts_mul(db2_sq, a1_sq), dts_mul(a2_sq, a1_sq));
        //  Infrastructure for nonneg_add
        lemma_dts_neg_well_formed(dts_mul(db2_sq, a1_sq));
        lemma_dts_same_radicand_neg(dts_mul(db2_sq, a1_sq));
        lemma_dts_nonneg_radicands_mul(db1_sq, db2_sq);
        lemma_dts_nonneg_radicands_mul(db2_sq, a1_sq);
        ...
        lemma_norm_definite_mul(a2_sq, a1_sq);
        lemma_norm_definite_neg(dts_mul(db2_sq, a1_sq));
        //  sr chains for norm_definite_add and add_closed — establish BEFORE calling them
        lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));  //  sr(mul, db1_sq)
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, db2_sq);
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db2_sq, dts_mul(db2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq),
            dts_neg(dts_mul(db2_sq, a1_sq)));
        lemma_norm_definite_add(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
        lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(db2_sq, a1_sq)));
        //  sr chains for the second add pair
        lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), db2_sq, a2_sq);
        lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(db2_sq, a1_sq), a2_sq, dts_mul(a2_sq, a1_sq));
        ...
        //  sr(sub1, sub2)
        lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq), sub1);
        lemma_dts_same_radicand_transitive(sub1, dts_mul(db1_sq, db2_sq), dts_mul(db2_sq, a1_sq));
        lemma_dts_same_radicand_symmetric(dts_mul(db2_sq, a1_sq), sub2);
        lemma_dts_same_radicand_transitive(sub1, dts_mul(db2_sq, a1_sq), sub2);
        lemma_dts_depth_mul_le(db1_sq, db2_sq);
        lemma_dts_depth_mul_le(db2_sq, a1_sq);
        ...
        lemma_dts_depth_add_le(dts_mul(db2_sq, a1_sq), dts_neg(dts_mul(a2_sq, a1_sq)));
        lemma_dts_nonneg_add_closed_fuel(sub1, sub2, f);
        //  Telescoping identity: add(sub1, sub2) ≡ sub(db1sq*db2sq, a2sq*a1sq)
        let neg_b = dts_neg(dts_mul(db2_sq, a1_sq));
        let neg_c = dts_neg(dts_mul(a2_sq, a1_sq));
        lemma_dts_add_associative(dts_mul(db1_sq, db2_sq), neg_b,
            dts_add(dts_mul(db2_sq, a1_sq), neg_c));
        ...
        lemma_dts_eqv_transitive(
            dts_add(sub1, sub2),
            dts_add(dts_mul(db1_sq, db2_sq), dts_add(neg_b, dts_add(dts_mul(db2_sq, a1_sq), neg_c))),
            dts_add(dts_mul(db1_sq, db2_sq), neg_c));
        //  Transfer nonneg
        lemma_dts_add_closed(sub1, sub2);
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db2_sq, a2_sq);
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), a2_sq, dts_mul(a2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq), neg_c);
        lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), neg_c);
        lemma_dts_same_radicand_symmetric(sub1, dts_add(sub1, sub2));
        lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), sub1, dts_mul(db1_sq, db2_sq));
        lemma_dts_same_radicand_symmetric(dts_mul(db1_sq, db2_sq),
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq)));
        lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), dts_mul(db1_sq, db2_sq),
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq)));
        lemma_dts_nonneg_fuel_congruence(dts_add(sub1, sub2),
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq)), f);
        //  Commute a2sq*a1sq → a1sq*a2sq for cauchy_neg_a1a2_square_le
        lemma_dts_same_radicand_transitive(a2_sq, a2, a1);
        lemma_dts_same_radicand_transitive(a2_sq, a1, a1_sq);
        lemma_dts_mul_commutative(a2_sq, a1_sq);
        lemma_dts_same_radicand_symmetric(a2_sq, a1_sq);  //  sr(a1_sq, a2_sq)
        lemma_dts_mul_closed(a1_sq, a2_sq);
        lemma_dts_eqv_reflexive(dts_mul(db1_sq, db2_sq));
        lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, a1_sq));
        lemma_dts_same_radicand_transitive(dts_mul(a2_sq, a1_sq), a2_sq, a1_sq);
        lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
        lemma_dts_same_radicand_transitive(dts_mul(a2_sq, a1_sq), a1_sq, dts_mul(a1_sq, a2_sq));
        lemma_dts_sub_congruence_both(
            dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq),
            dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq));
        lemma_dts_nonneg_fuel_congruence(
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a2_sq, a1_sq)),
            dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)), f);
        //  Call cauchy_neg_a1a2_square_le
        lemma_cauchy_neg_a1a2_square_le(a1, a2, b1, b2, dd, f);
    }
}

///  Mixed norms (nx≥0, neg(ny)≥0): derive nonneg(im_val) via (a1*b2)²≥(b1*a2)².
///  From positive-norm factor: nonneg(a1). From negative-norm factor: nonneg(b2).
///  Le chain: a1²*b2² ≥ dd*b1²*b2² ≡ dd*b2²*b1² ≥ a2²*b1² → (a1*b2)²≥(b1*a2)².
///  Then square_le_implies_le → nonneg(add(a1*b2, b1*a2)) = nonneg(im_val).
#[verifier::rlimit(40)]
proof fn lemma_mixed_norms_nonneg_im_val<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_norm_definite(a2), dts_norm_definite(b2), dts_norm_definite(dd),
        dts_norm_definite(DynTowerSpec::Ext(Box::new(a1), Box::new(b1), Box::new(dd))),
        //  norm_definite(Ext(a2,b2,dd)) derivable from components + dd forall from Ext(a1,b1,dd)
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a1), Box::new(b1), Box::new(dd)), (f + 1) as nat),
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a2), Box::new(b2), Box::new(dd)), (f + 1) as nat),
        //  Mixed norms: nx ≥ 0, neg(ny) ≥ 0
        dts_nonneg_fuel(dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))), f),
        dts_nonneg_fuel(dts_neg(dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)))), f),
    ensures
    ...
    decreases f, 6nat,
{
    //  ═══ Step 1: derive nonneg(a1) and nonneg(b2) ═══
    ...
    //  nonneg(a1*b2) from nonneg(a1) and nonneg(b2)
    lemma_dts_mul_closed(a1, b2);
    ...
    lemma_dts_nonneg_or_neg_nonneg_fuel(dts_mul(b1, a2), f);
    if dts_nonneg_fuel(dts_mul(b1, a2), f) {
        //  Both a1*b2 ≥ 0 and b1*a2 ≥ 0 → nonneg(im_val) via nonneg_add.
        lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, b2));
        lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(dts_mul(a1, b2), a1, b1);
        lemma_dts_same_radicand_transitive(dts_mul(a1, b2), b1, dts_mul(b1, a2));
        ...
        lemma_dts_nonneg_add_closed_fuel(dts_mul(a1, b2), dts_mul(b1, a2), f);
        return;
    }
    //  neg(b1*a2) ≥ 0. Need (a1*b2)² ≥ (b1*a2)² via mixed-norm le chain.
    let a1_sq = dts_mul(a1, a1);
    let a2_sq = dts_mul(a2, a2);
    let b1_sq = dts_mul(b1, b1);
    let b2_sq = dts_mul(b2, b2);
    let db1_sq = dts_mul(dd, b1_sq);
    let db2_sq = dts_mul(dd, b2_sq);
    let a1b2 = dts_mul(a1, b2);
    let b1a2 = dts_mul(b1, a2);
    let neg_b1a2 = dts_neg(b1a2);
    //  ═══ Step 2: le_mul chain ═══
    //  Same-radicand infrastructure
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(a2);
    ...
    lemma_dts_same_radicand_symmetric(a1, a1_sq);
    lemma_dts_same_radicand_symmetric(a2, a2_sq);
    //  a1_sq ~ b2_sq, db1_sq ~ a2_sq chains
    lemma_dts_same_radicand_transitive(a1_sq, a1, b2);
    lemma_dts_same_radicand_transitive(a1_sq, b2, b2_sq);
    lemma_dts_same_radicand_symmetric(dd, db1_sq);  //  sr(db1_sq, dd)
    lemma_dts_same_radicand_transitive(a1_sq, a1, dd);  //  sr(a1_sq, dd)
    lemma_dts_same_radicand_transitive(a1_sq, dd, db1_sq);  //  sr(a1_sq, db1_sq)
    lemma_dts_same_radicand_symmetric(a1, a2);
    lemma_dts_same_radicand_transitive(a2, a1, dd);
    lemma_dts_same_radicand_symmetric(a2, dd);  //  sr(dd, a2)
    lemma_dts_same_radicand_transitive(a2_sq, a2, dd);
    lemma_dts_same_radicand_transitive(db1_sq, dd, a2);
    lemma_dts_same_radicand_transitive(db1_sq, a2, a2_sq);
    //  Step 2a: le_mul(db1_sq, a1_sq, b2_sq): nonneg(nx) + nonneg(b2²)
    //  → nonneg(sub(a1_sq*b2_sq, db1_sq*b2_sq))
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    lemma_dts_nonneg_radicands_mul(b2, b2);
    lemma_dts_nonneg_radicands_mul(a2, a2);
    ...
    lemma_dts_square_nonneg(b2, f);
    lemma_dts_same_radicand_symmetric(a1_sq, db1_sq);
    lemma_dts_le_mul_nonneg_monotone_fuel(db1_sq, a1_sq, b2_sq, f);
    //  Step 2b: le_mul(a2_sq, db2_sq, b1_sq): nonneg(neg(ny)) + nonneg(b1²)
    //  → nonneg(sub(db2_sq*b1_sq, a2_sq*b1_sq))
    lemma_dts_square_nonneg(b1, f);
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b1_sq, f);
    //  sr(db2_sq, a2_sq): db2_sq ~ dd ~ a2 ~ a2_sq
    lemma_dts_same_radicand_symmetric(dd, db2_sq);  //  sr(db2_sq, dd)
    lemma_dts_same_radicand_symmetric(a2, dd);  //  sr(dd, a2) — wait need sr(dd, a2)
    //  sr(dd, a2): from sr(a2, dd) via symmetric. sr(a2, dd) from transitive above.
    lemma_dts_same_radicand_transitive(db2_sq, dd, a2);
    lemma_dts_same_radicand_transitive(db2_sq, a2, a2_sq);
    lemma_dts_same_radicand_symmetric(db2_sq, a2_sq);  //  sr(a2_sq, db2_sq)
    //  neg_sub_swap: neg(sub(a2², dd*b2²)) ≡ sub(dd*b2², a2²)
    lemma_dts_neg_sub_swap(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
    lemma_dts_same_radicand_transitive(a2_sq, a2, dd);
    lemma_dts_same_radicand_transitive(a2_sq, dd, dts_mul(dd, b2_sq));
    lemma_dts_neg_well_formed(db2_sq);
    lemma_dts_same_radicand_neg(db2_sq);
    lemma_dts_same_radicand_transitive(a2_sq, db2_sq, dts_neg(db2_sq));
    lemma_dts_add_closed(a2_sq, dts_neg(db2_sq));
    lemma_dts_same_radicand_neg(dts_sub(a2_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(a2_sq, dts_sub(a2_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(dts_sub(a2_sq, db2_sq), dts_neg(dts_sub(a2_sq, db2_sq)));
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)), dts_sub(a2_sq, db2_sq), a2_sq);
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)), a2_sq, db2_sq);
    lemma_dts_neg_well_formed(a2_sq);
    lemma_dts_same_radicand_neg(a2_sq);
    lemma_dts_same_radicand_transitive(db2_sq, a2_sq, dts_neg(a2_sq));
    lemma_dts_add_closed(db2_sq, dts_neg(a2_sq));
    lemma_dts_same_radicand_symmetric(db2_sq, dts_sub(db2_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(a2_sq, db2_sq)), db2_sq,
    ...
    lemma_dts_nonneg_fuel_congruence(
        dts_neg(dts_sub(a2_sq, db2_sq)), dts_sub(db2_sq, a2_sq), f);
    lemma_dts_same_radicand_symmetric(a2_sq, db2_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, db2_sq);  //  sr(db2_sq, a2_sq) needed by le_mul
    //  sr(a2_sq, b1_sq): a2_sq ~ a2 ~ a1 ~ b1 ~ b1_sq
    lemma_dts_same_radicand_symmetric(a1, a2);
    lemma_dts_same_radicand_transitive(a2_sq, a2, a1);
    lemma_dts_same_radicand_transitive(a2_sq, a1, b1);
    lemma_dts_same_radicand_transitive(a2_sq, b1, b1_sq);
    lemma_dts_same_radicand_transitive(db2_sq, a2_sq, b1_sq);  //  sr(db2_sq, b1_sq)
    lemma_dts_le_mul_nonneg_monotone_fuel(a2_sq, db2_sq, b1_sq, f);
    //  ═══ Step 3: Commute middle + nonneg_add for transitive ═══
    //  Middle: db1_sq*b2_sq and db2_sq*b1_sq.
    lemma_dts_same_radicand_symmetric(b1, b1_sq);  //  sr(b1_sq, b1)
    lemma_dts_same_radicand_transitive(b1, a1, b2);  //  sr(b1, b2)
    lemma_dts_same_radicand_transitive(b1_sq, b1, b2);
    lemma_dts_same_radicand_transitive(b1_sq, b2, b2_sq);
    lemma_dts_mul_associative(dd, b1_sq, b2_sq);
    lemma_dts_mul_closed(b1_sq, b2_sq);
    lemma_dts_mul_commutative(b1_sq, b2_sq);
    lemma_dts_same_radicand_symmetric(b1_sq, dts_mul(b1_sq, b2_sq));
    lemma_dts_same_radicand_symmetric(a1, dd);  //  sr(dd, a1)
    lemma_dts_same_radicand_transitive(b1, a1, dd);  //  sr(b1, dd) — wait, need sr(b1, a1) first
    //  sr(b1, a1) from symmetric(a1, b1) → call was at line ~4208. Reestablish for safety:
    lemma_dts_same_radicand_transitive(b1_sq, b1, dd);  //  sr(b1_sq, dd) — needs sr(b1, dd) just established
    lemma_dts_same_radicand_transitive(dts_mul(b1_sq, b2_sq), b1_sq, dd);
    lemma_dts_same_radicand_symmetric(dts_mul(b1_sq, b2_sq), dd);  //  sr(dd, mul(b1_sq, b2_sq))
    //  sr(mul(b1_sq, b2_sq), mul(b2_sq, b1_sq)) for mul_congruence_right
    lemma_dts_same_radicand_symmetric(b1_sq, b2_sq);  //  sr(b2_sq, b1_sq) needed by mul_closed
    lemma_dts_mul_closed(b2_sq, b1_sq);
    lemma_dts_same_radicand_symmetric(b1_sq, dts_mul(b1_sq, b2_sq));  //  sr(mul, b1_sq)
    lemma_dts_same_radicand_transitive(dts_mul(b1_sq, b2_sq), b1_sq, b2_sq);  //  sr(mul, b2_sq)
    lemma_dts_same_radicand_symmetric(b2_sq, dts_mul(b2_sq, b1_sq));  //  sr(mul(b2_sq,b1_sq), b2_sq)
    lemma_dts_same_radicand_transitive(dts_mul(b1_sq, b2_sq), b2_sq, dts_mul(b2_sq, b1_sq));
    lemma_dts_mul_congruence_right(dts_mul(b1_sq, b2_sq), dts_mul(b2_sq, b1_sq), dd);
    lemma_dts_same_radicand_symmetric(a1, b2);
    lemma_dts_same_radicand_transitive(b2, a1, dd);  //  sr(b2, dd)
    lemma_dts_same_radicand_transitive(b2_sq, b2, dd);  //  sr(b2_sq, dd)
    lemma_dts_same_radicand_transitive(dts_mul(b2_sq, b1_sq), b2_sq, dd);
    lemma_dts_same_radicand_symmetric(dts_mul(b2_sq, b1_sq), dd);  //  sr(dd, mul) from reverse
    lemma_dts_mul_associative(dd, b2_sq, b1_sq);
    lemma_dts_eqv_symmetric(dts_mul(dd, dts_mul(b1_sq, b2_sq)), dts_mul(db1_sq, b2_sq));
    lemma_dts_eqv_transitive(dts_mul(db1_sq, b2_sq), dts_mul(dd, dts_mul(b1_sq, b2_sq)),
        dts_mul(dd, dts_mul(b2_sq, b1_sq)));
    lemma_dts_eqv_symmetric(dts_mul(dd, dts_mul(b2_sq, b1_sq)), dts_mul(db2_sq, b1_sq));
    lemma_dts_eqv_transitive(dts_mul(db1_sq, b2_sq), dts_mul(dd, dts_mul(b2_sq, b1_sq)),
        dts_mul(db2_sq, b1_sq));
    //  sub_congruence: sub(a1_sq*b2_sq, db1_sq*b2_sq) → sub(a1_sq*b2_sq, db2_sq*b1_sq)
    lemma_dts_mul_closed(a1_sq, b2_sq);
    //  sr(db1_sq, b2_sq): db1_sq ~ dd ~ b2 ~ b2_sq
    lemma_dts_same_radicand_symmetric(dd, db1_sq);  //  sr(db1_sq, dd)
    lemma_dts_same_radicand_transitive(db1_sq, dd, b2);  //  needs sr(dd, b2) from symmetric(b2, dd) established earlier
    lemma_dts_same_radicand_transitive(db1_sq, b2, b2_sq);
    lemma_dts_mul_closed(db1_sq, b2_sq);
    lemma_dts_mul_closed(db2_sq, b1_sq);
    lemma_dts_eqv_reflexive(dts_mul(a1_sq, b2_sq));
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, b2_sq), db1_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, b2_sq), dd, db2_sq);
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, b2_sq), db2_sq, dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1_sq, a1);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1, dd);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), dd, db1_sq);
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), db1_sq, dts_mul(db1_sq, b2_sq));
    lemma_dts_same_radicand_reflexive(dts_mul(a1_sq, b2_sq));  //  sr(a, c) where a=c
    lemma_dts_sub_congruence_both(dts_mul(a1_sq, b2_sq), dts_mul(db1_sq, b2_sq),
        dts_mul(a1_sq, b2_sq), dts_mul(db2_sq, b1_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(db1_sq, b2_sq)),
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(db2_sq, b1_sq)), f);
    //  nonneg_add: sub(a1_sq*b2_sq, db2_sq*b1_sq) + sub(db2_sq*b1_sq, a2_sq*b1_sq)
    //  ≡ sub(a1_sq*b2_sq, a2_sq*b1_sq) via telescoping identity
    let sub1 = dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(db2_sq, b1_sq));
    let sub2 = dts_sub(dts_mul(db2_sq, b1_sq), dts_mul(a2_sq, b1_sq));
    lemma_dts_mul_closed(a2_sq, b1_sq);
    //  Infrastructure for nonneg_add(sub1, sub2)
    lemma_dts_neg_well_formed(dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_neg(dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, b2_sq), a1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1_sq, db1_sq);
    lemma_dts_same_radicand_transitive(db1_sq, dd, db2_sq);  //  sr(db1_sq, db2_sq)
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), db1_sq, db2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), db2_sq, dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), dts_mul(db2_sq, b1_sq),
        dts_neg(dts_mul(db2_sq, b1_sq)));
    lemma_dts_add_closed(dts_mul(a1_sq, b2_sq), dts_neg(dts_mul(db2_sq, b1_sq)));
    lemma_dts_neg_well_formed(dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_neg(dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_symmetric(db2_sq, dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, b1_sq), db2_sq, a2_sq);
    lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, b1_sq), a2_sq, dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db2_sq, b1_sq), dts_mul(a2_sq, b1_sq),
        dts_neg(dts_mul(a2_sq, b1_sq)));
    lemma_dts_add_closed(dts_mul(db2_sq, b1_sq), dts_neg(dts_mul(a2_sq, b1_sq)));
    lemma_dts_nonneg_radicands_mul(a1_sq, b2_sq);
    lemma_dts_nonneg_radicands_mul(db2_sq, b1_sq);
    lemma_dts_nonneg_radicands_mul(a2_sq, b1_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(db2_sq, b1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(a1_sq, b2_sq), dts_neg(dts_mul(db2_sq, b1_sq)));
    lemma_dts_nonneg_radicands_neg(dts_mul(a2_sq, b1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(db2_sq, b1_sq), dts_neg(dts_mul(a2_sq, b1_sq)));
    lemma_norm_definite_mul(a1_sq, b2_sq);
    lemma_norm_definite_mul(db2_sq, b1_sq);
    lemma_norm_definite_mul(a2_sq, b1_sq);
    lemma_norm_definite_neg(dts_mul(db2_sq, b1_sq));
    lemma_norm_definite_add(dts_mul(a1_sq, b2_sq), dts_neg(dts_mul(db2_sq, b1_sq)));
    lemma_norm_definite_neg(dts_mul(a2_sq, b1_sq));
    lemma_norm_definite_add(dts_mul(db2_sq, b1_sq), dts_neg(dts_mul(a2_sq, b1_sq)));
    //  sr(sub1, sub2)
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, b2_sq), sub1);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(a1_sq, b2_sq), dts_mul(db2_sq, b1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(db2_sq, b1_sq), sub2);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(db2_sq, b1_sq), sub2);
    lemma_dts_depth_mul_le(a1_sq, b2_sq);
    lemma_dts_depth_mul_le(db2_sq, b1_sq);
    lemma_dts_depth_mul_le(a2_sq, b1_sq);
    lemma_dts_depth_neg(dts_mul(db2_sq, b1_sq));
    lemma_dts_depth_add_le(dts_mul(a1_sq, b2_sq), dts_neg(dts_mul(db2_sq, b1_sq)));
    lemma_dts_depth_neg(dts_mul(a2_sq, b1_sq));
    lemma_dts_depth_add_le(dts_mul(db2_sq, b1_sq), dts_neg(dts_mul(a2_sq, b1_sq)));
    lemma_dts_nonneg_add_closed_fuel(sub1, sub2, f);
    //  Telescoping: add(sub1, sub2) ≡ sub(a1_sq*b2_sq, a2_sq*b1_sq)
    //  via the standard identity add(sub(A,B), sub(B,C)) ≡ sub(A,C)
    let neg_b = dts_neg(dts_mul(db2_sq, b1_sq));
    let neg_c = dts_neg(dts_mul(a2_sq, b1_sq));
    lemma_dts_add_associative(dts_mul(a1_sq, b2_sq), neg_b,
        dts_add(dts_mul(db2_sq, b1_sq), neg_c));
    lemma_dts_add_associative(neg_b, dts_mul(db2_sq, b1_sq), neg_c);
    lemma_dts_eqv_symmetric(dts_add(dts_add(neg_b, dts_mul(db2_sq, b1_sq)), neg_c),
        dts_add(neg_b, dts_add(dts_mul(db2_sq, b1_sq), neg_c)));
    lemma_dts_add_commutative(neg_b, dts_mul(db2_sq, b1_sq));
    lemma_dts_add_inverse_right(dts_mul(db2_sq, b1_sq));
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_mul(db2_sq, b1_sq)),
        dts_add(dts_mul(db2_sq, b1_sq), neg_b), dts_zero());
    lemma_dts_add_congruence_left(dts_add(neg_b, dts_mul(db2_sq, b1_sq)), dts_zero(), neg_c);
    lemma_dts_add_commutative(dts_zero(), neg_c);
    lemma_dts_add_zero_right(neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_zero(), neg_c), dts_add(neg_c, dts_zero()), neg_c);
    lemma_dts_eqv_transitive(dts_add(dts_add(neg_b, dts_mul(db2_sq, b1_sq)), neg_c),
        dts_add(dts_zero(), neg_c), neg_c);
    lemma_dts_eqv_transitive(dts_add(neg_b, dts_add(dts_mul(db2_sq, b1_sq), neg_c)),
        dts_add(dts_add(neg_b, dts_mul(db2_sq, b1_sq)), neg_c), neg_c);
    lemma_dts_add_congruence_right(dts_mul(a1_sq, b2_sq),
        dts_add(neg_b, dts_add(dts_mul(db2_sq, b1_sq), neg_c)), neg_c);
    lemma_dts_eqv_transitive(
        dts_add(sub1, sub2),
        dts_add(dts_mul(a1_sq, b2_sq), dts_add(neg_b, dts_add(dts_mul(db2_sq, b1_sq), neg_c))),
        dts_add(dts_mul(a1_sq, b2_sq), neg_c));
    //  Transfer nonneg: add(sub1, sub2) → sub(a1_sq*b2_sq, a2_sq*b1_sq)
    lemma_dts_add_closed(sub1, sub2);
    lemma_dts_same_radicand_transitive(a1_sq, a1, a2);  //  sr(a1_sq, a2)
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1_sq, a2);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a2, a2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a2_sq, dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq), neg_c);
    lemma_dts_add_closed(dts_mul(a1_sq, b2_sq), neg_c);
    lemma_dts_same_radicand_symmetric(sub1, dts_add(sub1, sub2));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), sub1, dts_mul(a1_sq, b2_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(a1_sq, b2_sq),
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq)));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), dts_mul(a1_sq, b2_sq),
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_add(sub1, sub2),
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq)), f);
    //  → nonneg(sub(a1_sq*b2_sq, a2_sq*b1_sq))

    //  ═══ Step 4: Congruence to sub((a1*b2)², (b1*a2)²) ═══
    //  a1_sq*b2_sq ≡ (a1*b2)² via square_mul
    lemma_dts_square_mul(a1, b2);
    lemma_dts_eqv_symmetric(dts_mul(a1b2, a1b2), dts_mul(a1_sq, b2_sq));
    //  a2_sq*b1_sq ≡ (a2*b1)² via square_mul → (b1*a2)² via commutative
    lemma_dts_same_radicand_transitive(a2, a1, b1);
    lemma_dts_mul_commutative(a2, b1);
    lemma_dts_square_mul(a2, b1);
    //  (a2*b1)² ≡ a2_sq*b1_sq
    lemma_dts_eqv_symmetric(dts_mul(dts_mul(a2, b1), dts_mul(a2, b1)), dts_mul(a2_sq, b1_sq));
    //  Commute a2_sq*b1_sq → b1_sq*a2_sq, then square_mul + neg_mul_neg.
    lemma_dts_same_radicand_transitive(a2_sq, a2, b1);
    lemma_dts_same_radicand_transitive(a2_sq, b1, b1_sq);
    lemma_dts_mul_commutative(a2_sq, b1_sq);
    lemma_dts_same_radicand_transitive(b1_sq, b1, a1);  //  sr(b1_sq, a1) — need sr(b1, a1). Have from symmetric(a1, b1)
    lemma_dts_same_radicand_transitive(b1_sq, a1, a2);  //  sr(b1_sq, a2)
    lemma_dts_same_radicand_transitive(b1_sq, a2, a2_sq);  //  sr(b1_sq, a2_sq)
    lemma_dts_mul_closed(b1_sq, a2_sq);
    //  b1_sq*a2_sq ≡ (b1*a2)² from square_mul(b1, a2)
    lemma_dts_square_mul(b1, a2);
    lemma_dts_eqv_symmetric(dts_mul(b1a2, b1a2), dts_mul(b1_sq, a2_sq));
    //  Chain: a2_sq*b1_sq ≡ b1_sq*a2_sq ≡ (b1*a2)²
    lemma_dts_mul_closed(a2_sq, b1_sq);
    lemma_dts_eqv_transitive(dts_mul(a2_sq, b1_sq), dts_mul(b1_sq, a2_sq),
        dts_mul(b1a2, b1a2));
    //  neg_mul_neg: neg(b1*a2)² ≡ (b1*a2)²
    lemma_dts_neg_well_formed(b1a2);
    lemma_dts_same_radicand_neg(b1a2);
    lemma_dts_same_radicand_reflexive(neg_b1a2);
    lemma_dts_mul_closed(neg_b1a2, neg_b1a2);
    lemma_dts_same_radicand_reflexive(b1a2);
    lemma_dts_mul_closed(b1a2, b1a2);
    lemma_dts_neg_mul_neg(b1a2, b1a2);
    //  chain: a2_sq*b1_sq ≡ (b1*a2)² ≡ neg(b1*a2)²
    lemma_dts_eqv_symmetric(dts_mul(neg_b1a2, neg_b1a2), dts_mul(b1a2, b1a2));
    lemma_dts_eqv_transitive(dts_mul(a2_sq, b1_sq), dts_mul(b1a2, b1a2),
        dts_mul(neg_b1a2, neg_b1a2));
    //  sr chains for sub_congruence_both
    lemma_dts_same_radicand_reflexive(a1b2);
    lemma_dts_mul_closed(a1b2, a1b2);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1_sq, a1);
    lemma_dts_same_radicand_symmetric(a1, a1b2);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1, a1b2);
    lemma_dts_same_radicand_symmetric(a1b2, dts_mul(a1b2, a1b2));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, b2_sq), a1b2, dts_mul(a1b2, a1b2));
    lemma_dts_same_radicand_symmetric(a2_sq, dts_mul(a2_sq, b1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, b1_sq), a2_sq, a2);
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, b1_sq), a2, b1);
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, b1_sq), b1, b1a2);
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, b1_sq), b1a2, neg_b1a2);
    lemma_dts_same_radicand_symmetric(neg_b1a2, dts_mul(neg_b1a2, neg_b1a2));
    lemma_dts_same_radicand_transitive(dts_mul(a2_sq, b1_sq), neg_b1a2,
        dts_mul(neg_b1a2, neg_b1a2));
    lemma_dts_sub_congruence_both(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq),
        dts_mul(a1b2, a1b2), dts_mul(neg_b1a2, neg_b1a2));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(a1_sq, b2_sq), dts_mul(a2_sq, b1_sq)),
        dts_sub(dts_mul(a1b2, a1b2), dts_mul(neg_b1a2, neg_b1a2)), f);
    //  → nonneg(sub((a1*b2)², neg(b1*a2)²))

    //  ═══ Step 5: square_le_implies_le(neg(b1*a2), a1*b2) → nonneg(sub(a1*b2, neg(b1*a2))) ═══
    lemma_dts_nonneg_radicands_mul(a1, b2);
    lemma_dts_nonneg_radicands_mul(b1, a2);
    lemma_dts_nonneg_radicands_neg(b1a2);
    lemma_norm_definite_mul(a1, b2);
    lemma_norm_definite_mul(b1, a2);
    lemma_norm_definite_neg(b1a2);
    lemma_dts_same_radicand_symmetric(b1a2, neg_b1a2);
    lemma_dts_same_radicand_symmetric(b1, b1a2);
    lemma_dts_same_radicand_transitive(neg_b1a2, b1a2, b1);
    lemma_dts_same_radicand_transitive(neg_b1a2, b1, a1);
    lemma_dts_same_radicand_symmetric(a1, a1b2);
    lemma_dts_same_radicand_transitive(neg_b1a2, a1, a1b2);
    lemma_dts_depth_neg(b1a2);
    lemma_dts_depth_mul_le(a1, b2);
    lemma_dts_depth_mul_le(b1, a2);
    lemma_dts_square_le_implies_le_fuel(neg_b1a2, a1b2, f);
    //  → nonneg(sub(a1b2, neg_b1a2))

    //  ═══ Step 6: sub(a1*b2, neg(b1*a2)) ≡ add(a1*b2, b1*a2) = im_val ═══
    lemma_dts_neg_involution(b1a2);
    lemma_dts_same_radicand_transitive(a1b2, a1, b1);
    lemma_dts_same_radicand_transitive(a1b2, b1, b1a2);
    lemma_dts_same_radicand_transitive(a1b2, b1a2, neg_b1a2);
    lemma_dts_same_radicand_neg(neg_b1a2);
    lemma_dts_same_radicand_transitive(a1b2, neg_b1a2, dts_neg(neg_b1a2));
    lemma_dts_neg_well_formed(neg_b1a2);
    lemma_dts_add_closed(a1b2, dts_neg(neg_b1a2));
    lemma_dts_add_congruence_right(a1b2, dts_neg(neg_b1a2), b1a2);
    //  eqv(sub(a1b2, neg_b1a2), add(a1b2, b1a2))
    lemma_dts_add_closed(a1b2, b1a2);
    lemma_dts_same_radicand_symmetric(a1b2, dts_sub(a1b2, neg_b1a2));
    lemma_dts_same_radicand_symmetric(a1b2, dts_add(a1b2, b1a2));
    lemma_dts_same_radicand_transitive(dts_sub(a1b2, neg_b1a2), a1b2,
        dts_add(a1b2, b1a2));
    lemma_dts_nonneg_fuel_congruence(dts_sub(a1b2, neg_b1a2),
        dts_add(a1b2, b1a2), f);
}

///  Cauchy-Schwarz conclusion: nonneg(re_val) from nx≥0, ny≥0, and factor signs.
proof fn lemma_cauchy_schwarz_is_zero_re<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    ...
    decreases f, 5nat,
{
    ...
    //  ═══ Step 1: nonneg(a1*a2) ═══
    //  From nonneg_fuel of factors: Z3 extracts C1/C2/C3 signs of a1, a2.
    //  With nx > 0 (a1² > dd*b1²): factor 1 must be C1 or C2 (a1 ≥ 0).
    //  With ny > 0 (a2² > dd*b2²): factor 2 must be C1 or C2 (a2 ≥ 0).
    //  Both a1 ≥ 0 and a2 ≥ 0 → a1*a2 ≥ 0.
    //  Z3 should derive from nonneg_fuel expansion + nx/ny positivity.

    //  ═══ Step 2: le_mul chain → (a1*a2)² ≥ (dd*b1*b2)² ═══
    //  le_mul_nonneg_monotone(dd*b1², a1², a2²): nx≥0 + nonneg(a2²) → nonneg(sub(a1²*a2², dd*b1²*a2²))
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    ...
    lemma_dts_same_radicand_transitive(dd, b2, dts_mul(b2, b2));
    lemma_dts_mul_closed(dd, dts_mul(b2, b2));
    //  square_nonneg(a2) for le_mul step 1
    lemma_dts_square_nonneg(a2, f);
    //  nonneg_radicands + norm_definite for le_mul args
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(a2, a2);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b1));
    ...
    lemma_dts_depth_mul_le(dd, dts_mul(b1, b1));
    lemma_dts_depth_mul_le(dd, dts_mul(b2, b2));
    //  Step 1: le_mul_nonneg_monotone(dd*b1², a1², a2²)
    lemma_dts_le_mul_nonneg_monotone_fuel(
    ...
    //  le_mul_nonneg_monotone(dd*b2², a2², dd*b1²): needs same_radicand(dd*b2², a2²)
    //  and same_radicand(a2², dd*b1²).
    lemma_dts_same_radicand_symmetric(dts_mul(dd, dts_mul(b1, b1)), dts_mul(a2, a2));
    lemma_dts_le_mul_nonneg_monotone_fuel(
        dts_mul(dd, dts_mul(b2, b2)), dts_mul(a2, a2), dts_mul(dd, dts_mul(b1, b1)), f);
    //  → nonneg(sub(a2²*dd*b1², dd*b2²*dd*b1²))
    //  ═══ Step 3: Transitivity ═══
    //  nonneg(sub(a1²*a2², dd*b1²*a2²)) from step 1
    //  nonneg(sub(dd*b1²*a2², dd*b2²*dd*b1²)) from step 2 (after commuting a2²*dd*b1² to dd*b1²*a2²)
    //  Z3 should chain: a1²*a2² ≥ dd*b1²*a2² ≥ dd*b1²*dd*b2²
    //  For the square_le_implies_le call: need nonneg(sub((a1*a2)², (dd*b1*b2)²)).
    //  (a1*a2)² = a1²*a2² and (dd*b1*b2)² = dd²*b1²*b2² = dd*b1²*dd*b2² (via congruences).
    //  Z3 should derive from the structural mul + sub definitions.
    //  Call square_le_implies_le(dd*b1*b2, a1*a2, f):
    //  But dd*b1*b2 might not be nonneg! Use neg(b2) approach.
    //  Actually: just use le_antisymmetric on re_val directly.
    //  nonneg(sub(a1²*a2², dd*b1²*dd*b2²)) from transitivity.
    ...
    if dts_nonneg_fuel(db1b2, f) {
        ...
        //  neg(db1b2) ≥ 0: use le_transitive chain + square_le_implies_le.
        //  Extracted to helper for clean Z3 context (Fix 1).
        lemma_cauchy_neg_db1b2_case(a1, a2, b1, b2, dd, f);
    }
}
...
pub proof fn lemma_dts_mul_is_zero_right<T: OrderedField>(x: DynTowerSpec<T>, c: DynTowerSpec<T>)
    requires dts_is_zero(x),
    ...
    decreases c, x,
{
    match (c, x) {
        (DynTowerSpec::Rat(rc), DynTowerSpec::Rat(rx)) => {
            ...
            T::axiom_mul_congruence_left(rx, zero, rc);
            //  mul(0, rc).eqv(0) by mul_zero_left
            verus_algebra::lemmas::ring_lemmas::lemma_mul_zero_left::<T>(rc);
            //  Chain: mul(rx, rc).eqv(0)
            T::axiom_eqv_transitive(rx.mul(rc), zero.mul(rc), zero);
            //  mul(rc, rx) ≡ mul(rx, rc) by commutativity
            T::axiom_mul_commutative(rc, rx);
            //  Chain: mul(rc, rx).eqv(0)
            T::axiom_eqv_transitive(rc.mul(rx), rx.mul(rc), zero);
        },
...
pub proof fn lemma_dts_mul_is_zero_left<T: OrderedField>(x: DynTowerSpec<T>, c: DynTowerSpec<T>)
    requires dts_is_zero(x),
...
pub proof fn lemma_dts_add_preserves_same_radicand_both<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
)
...
pub proof fn lemma_dts_mul_preserves_same_radicand_right<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
...
pub proof fn lemma_dts_mul_congruence_right<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
        ...
        (DynTowerSpec::Rat(ra), DynTowerSpec::Rat(rb)) => {
            ...
            match c {
                DynTowerSpec::Rat(rc) => {
                    ...
                    T::axiom_mul_commutative(rc, ra);
                    //  rc*ra ≡ ra*rc
                    T::axiom_mul_congruence_left(ra, rb, rc);
                    //  ra*rc ≡ rb*rc
                    T::axiom_eqv_transitive(rc.mul(ra), ra.mul(rc), rb.mul(rc));
                    //  rc*ra ≡ rb*rc
                    T::axiom_mul_commutative(rb, rc);
                    //  rb*rc ≡ rc*rb
                    T::axiom_eqv_transitive(rc.mul(ra), rb.mul(rc), rc.mul(rb));
                },
                DynTowerSpec::Ext(re_c, im_c, _) => {
                    //  mul(Ext_c, Rat_a) = Ext(mul(re_c,Rat_a), mul(im_c,Rat_a), d_c)
                    //  mul(Ext_c, Rat_b) = Ext(mul(re_c,Rat_b), mul(im_c,Rat_b), d_c)
...
pub proof fn lemma_dts_mul_congruence_left<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
    requires dts_eqv(a, b), dts_same_radicand(a, b),
...
pub proof fn lemma_dts_mul_preserves_same_radicand_left<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
    requires dts_same_radicand(a, b),
...
proof fn lemma_dts_add_both_zero<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_is_zero(a), dts_is_zero(b),
...
proof fn lemma_dts_add_is_zero_right<T: OrderedField>(y: DynTowerSpec<T>, x: DynTowerSpec<T>)
    requires dts_is_zero(x),
...
pub proof fn lemma_dts_add_congruence_right<T: OrderedField>(c: DynTowerSpec<T>, a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_eqv(a, b),
...
pub proof fn lemma_dts_add_inverse_right<T: OrderedField>(a: DynTowerSpec<T>)
...
pub proof fn lemma_dts_sub_is_add_neg<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
///  mul(a, one()) ≡ a.
pub proof fn lemma_dts_mul_one_right<T: OrderedField>(a: DynTowerSpec<T>)
...
///  mul(a, zero()) ≡ zero().
pub proof fn lemma_dts_mul_zero_right<T: OrderedField>(a: DynTowerSpec<T>)
...
///  one() ≢ zero().
pub proof fn lemma_dts_one_ne_zero<T: OrderedField>()
    ensures !dts_eqv(dts_one::<T>(), dts_zero::<T>()),
{
    T::axiom_one_ne_zero();
}
...
pub proof fn lemma_dts_div_is_mul_recip<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
pub proof fn lemma_dts_nonneg_fuel_zero<T: OrderedField>(x: DynTowerSpec<T>, fuel: nat)
    requires
        dts_eqv(x, dts_zero()),
        fuel >= dts_depth(x) + 1,
    ...
    decreases x,
{
    match x {
        DynTowerSpec::Rat(r) => {
            //  nonneg_fuel(Rat(r), fuel) = 0.le(r)
            //  r ≡ 0, so 0 ≡ r by symmetry, and 0 ≤ 0 by reflexivity, so 0 ≤ r by congruence
            let zero = T::zero();
            T::axiom_le_reflexive(zero);
            T::axiom_eqv_reflexive(zero);
            T::axiom_eqv_symmetric(r, zero);
            T::axiom_le_congruence(zero, zero, zero, r);
        },
        DynTowerSpec::Ext(re, im, d) => {
...
pub proof fn lemma_dts_nonneg_zero<T: OrderedField>(x: DynTowerSpec<T>)
    requires dts_eqv(x, dts_zero()),
...
pub proof fn lemma_dts_le_reflexive<T: OrderedField>(a: DynTowerSpec<T>)
    ensures dts_le(a, a),
{
...
pub proof fn lemma_dts_lt_iff<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
//  ===========================================================================
//   Ordered field infrastructure for DynTowerSpec<T>
//  ===========================================================================

///  same_radicand is symmetric.
pub proof fn lemma_dts_same_radicand_symmetric<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_same_radicand(a, b),
...
pub proof fn lemma_dts_same_radicand_transitive<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
...
pub proof fn lemma_dts_same_radicand_reflexive<T: OrderedField>(a: DynTowerSpec<T>)
...
pub proof fn lemma_dts_same_radicand_neg<T: OrderedField>(a: DynTowerSpec<T>)
...
pub proof fn lemma_dts_neg_preserves_same_radicand<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_same_radicand(a, b),
...
pub proof fn lemma_dts_depth_neg<T: OrderedField>(x: DynTowerSpec<T>)
...
pub proof fn lemma_dts_depth_mul_le<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
pub proof fn lemma_dts_depth_add_le<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
...
pub proof fn lemma_dts_nonneg_fuel_monotone<T: OrderedField>(x: DynTowerSpec<T>, f1: nat, f2: nat)
...
pub proof fn lemma_dts_nonneg_fuel_congruence<T: OrderedField>(
    x: DynTowerSpec<T>, y: DynTowerSpec<T>, fuel: nat,
)
...
pub proof fn lemma_dts_nonneg_fuel_stabilize<T: OrderedField>(x: DynTowerSpec<T>, f: nat)
...
pub proof fn lemma_dts_nonneg_congruence<T: OrderedField>(
    x: DynTowerSpec<T>, y: DynTowerSpec<T>,
)
...
pub proof fn lemma_dts_sub_congruence_both<T: OrderedField>(
...
///  Total ordering: every DynTowerSpec<T> value is nonneg or its negation is.
    ...
    decreases fuel, 2nat,
{
    match x {
        DynTowerSpec::Rat(r) => {
            // (generic T: OrderedField - no Rational import needed)
            T::axiom_le_total(T::zero(), r);
            if !T::zero().le(r) {
                verus_algebra::lemmas::ordered_ring_lemmas::lemma_neg_nonpos_iff::<T>(r);
            }
        }
        DynTowerSpec::Ext(re, im, d) => {
...
pub proof fn lemma_dts_mul_commutative<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
...
pub proof fn lemma_dts_rational_nonzero_preserved<T: OrderedField>(r: T)
    requires !r.eqv(T::zero()),
...
pub proof fn lemma_dts_nonneg_conclude_re_fuel<T: OrderedField>(
    re: DynTowerSpec<T>, im: DynTowerSpec<T>, d: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(re) + 1,
        f >= dts_depth(im) + 1,
...
pub proof fn lemma_dts_nonneg_conclude_im_fuel<T: OrderedField>(
    re: DynTowerSpec<T>, im: DynTowerSpec<T>, d: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(re) + 1,
...
pub proof fn lemma_dts_add_exchange<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>,
    c: DynTowerSpec<T>, d: DynTowerSpec<T>)
    ensures dts_eqv(dts_add(dts_add(a, b), dts_add(c, d)),
                     dts_add(dts_add(a, c), dts_add(b, d))),
{
    //  (a+b)+(c+d) ≡ a+(b+(c+d)) [assoc]
    lemma_dts_add_associative(a, b, dts_add(c, d));
    //  b+(c+d) ≡ (b+c)+d [assoc, symmetric]
    lemma_dts_add_associative(b, c, d);
    lemma_dts_eqv_symmetric(dts_add(dts_add(b, c), d), dts_add(b, dts_add(c, d)));
...
pub proof fn lemma_dts_neg_add<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    ensures dts_eqv(dts_neg(dts_add(a, b)), dts_add(dts_neg(a), dts_neg(b))),
{
    verus_algebra::lemmas::additive_group_lemmas::lemma_neg_add::<DynTowerSpec<T>>(a, b);
}

///  neg_mul_right: mul(a, neg(b)) ≡ neg(mul(a, b)).
///  Self-contained structural induction — d sub-field calls always decrease.
pub proof fn lemma_dts_neg_mul_right<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
    ...
    decreases a, b,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            verus_algebra::lemmas::ring_lemmas::lemma_mul_neg_right::<T>(r1, r2);
        }
        (DynTowerSpec::Ext(re, im, d), DynTowerSpec::Rat(r)) => {
...
pub proof fn lemma_dts_neg_mul_left<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
...
pub proof fn lemma_dts_mul_distributes_left<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
    requires
    ...
    decreases a, b, c,
{
    ...
    match (a, b, c) {
        (DynTowerSpec::Rat(r), DynTowerSpec::Rat(s), DynTowerSpec::Rat(t)) => {
            T::axiom_mul_distributes_left(r, s, t);
        }
...
#[verifier::rlimit(60)]
pub proof fn lemma_dts_difference_of_squares<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires
...
pub proof fn lemma_dts_mul_associative<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
...
#[verifier::rlimit(400)]
proof fn lemma_dts_mul_associative_eee<T: OrderedField>(
    a1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    c1: DynTowerSpec<T>, c2: DynTowerSpec<T>, dd: DynTowerSpec<T>,
)
    requires
    ...
    decreases a1, b1, c1,
{
    ...
    assert(dts_eqv(dts_add(dts_add(t1, t2), dts_add(t3, t4)),
        ...
        lemma_dts_add_commutative(u4, u2);
    ...
    assert(dts_eqv(dts_add(dts_add(s1, s2), dts_add(s3, s4)),
        ...
        lemma_dts_add_commutative(v4, v2);
...
pub proof fn lemma_dts_mul_distributes_over_sub<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
                ...
                dts_sub(dts_mul(a, b), dts_mul(a, c))),
{
    ...
    lemma_dts_sub_is_add_neg(dts_mul(a, b), dts_mul(a, c));
...
pub proof fn lemma_dts_sub_mul_right<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
        dts_same_radicand(a, b), dts_same_radicand(a, c),
    ensures
...
pub proof fn lemma_dts_square_mul<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
    ensures
...
pub proof fn lemma_dts_four_commute<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b),
...
proof fn lemma_dts_norm_mul_cross<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>, d: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
...
proof fn lemma_dts_norm_mul_re_sq<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>, d: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
    ...
    },
{
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence::<DynTowerSpec<T>>(
...
proof fn lemma_dts_norm_mul_pr_sub<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>, d: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
...
proof fn lemma_dts_norm_mul_qs_sub<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>, d: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
    ...
    },
{
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_antisymmetric::<DynTowerSpec<T>>(de2, c2);
    lemma_dts_neg_well_formed(ny);
    lemma_dts_same_radicand_neg(ny);
...
proof fn lemma_dts_norm_mul_dim_sq<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, e: DynTowerSpec<T>, d: DynTowerSpec<T>,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(c),
    ...
    },
{
    ...
    //  (p+q)-(r+s) ≡ (p-r)+(q-s)
    verus_algebra::determinant::lemma_sub_pairs::<DynTowerSpec<T>>(p, q, r, s);
    //  p-r: sub_pairs then cancel ac·dbe - d·ae·bc ≡ 0
    verus_algebra::determinant::lemma_sub_pairs::<DynTowerSpec<T>>(
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_self::<DynTowerSpec<T>>(
        dts_mul(d, dts_mul(ae, bc)));
    ...
    assert(dts_eqv(dts_add(dts_zero(), qs_sub), dts_add(qs_sub, dts_zero()))) by {
        lemma_dts_add_commutative(dts_zero(), qs_sub);
    };
    ...
    //  neg(d·b2·ny) ≡ neg(db2·ny) by neg_congruence from X ≡ Y
    assert(dts_eqv(dts_neg(dts_mul(d, dts_mul(b2, ny))), dts_neg(dts_mul(db2, ny)))) by {
        lemma_dts_neg_congruence(dts_mul(d, dts_mul(b2, ny)), dts_mul(db2, ny));
    };
    //  LEGACY: the rest of the original by-block was cleaning up sub(zero, X) ≡ neg(X).
    //  With generic T, neg_congruence is direct. Skip the sub_congruence detour.
    if false {  //  dead code — preserved for reference during migration
        lemma_dts_eqv_symmetric(
            dts_neg(dts_mul(d, dts_mul(b2, ny))),
...
pub proof fn lemma_dts_square_le_square_fuel<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, fuel: nat,
)
    requires
    ...
    decreases fuel, 2nat,
{
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<DynTowerSpec<T>>(b, a);
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_then_add_cancel::<DynTowerSpec<T>>(b, a);
    //  sub(b,a).add(a) ≡ b → add(sub(b,a), a) ≡ b
    lemma_dts_add_commutative(a, dts_sub(b, a));
...
pub proof fn lemma_dts_le_mul_nonneg_monotone_fuel<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, fuel: nat,
)
    requires
...
///  Cancellation: from c > 0 (nonneg ∧ !is_zero) and a*c ≤ b*c, conclude a ≤ b.
///  Proof by contradiction via le_antisymmetric + mul_cancel_zero (integral domain).
#[verifier::rlimit(80)]
pub proof fn lemma_dts_le_mul_cancel_pos_fuel<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, c: DynTowerSpec<T>, fuel: nat,
)
    requires
        fuel >= dts_depth(a) + 1, fuel >= dts_depth(b) + 1, fuel >= dts_depth(c) + 1,
    ...
    decreases fuel, 3nat,
{
    ...
    //  ─── Trichotomy: nonneg(b-a) || nonneg(neg(b-a)) ───
    lemma_dts_nonneg_or_neg_nonneg_fuel(dts_sub(b, a), fuel);
    if dts_nonneg_fuel(dts_sub(b, a), fuel) {
        return;
    }

    //  ─── nonneg(neg(b-a)). Apply le_mul_nonneg_monotone(b, a, c, fuel) ───
    //  Requires: nonneg(sub(a, b)) [from neg(sub(b,a)) via congruence]
    lemma_dts_neg_well_formed(b);
    lemma_dts_same_radicand_neg(b);
    ...
    //  neg(sub(b, a)) ≡ sub(a, b)
    lemma_dts_neg_sub_swap(b, a);
    //  Build sr chain neg(sub(b,a)) ~ sub(a,b)
    lemma_dts_neg_well_formed(dts_sub(b, a));
    lemma_dts_same_radicand_neg(dts_sub(b, a));
    lemma_dts_same_radicand_symmetric(dts_sub(b, a), dts_neg(dts_sub(b, a)));
    lemma_dts_nonneg_radicands_neg(dts_sub(b, a));
    lemma_norm_definite_neg(dts_sub(b, a));
    lemma_dts_depth_neg(dts_sub(b, a));
    //  sr(neg(sub(b,a)), sub(a,b)): neg(sub(b,a)) ~ sub(b,a) ~ b ~ a ~ sub(a,b)
    lemma_dts_same_radicand_symmetric(b, dts_sub(b, a));
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(b, a)), dts_sub(b, a), b);
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(b, a)), b, a);
    lemma_dts_same_radicand_symmetric(a, dts_sub(a, b));
    lemma_dts_same_radicand_transitive(dts_neg(dts_sub(b, a)), a, dts_sub(a, b));
    lemma_dts_nonneg_fuel_congruence(dts_neg(dts_sub(b, a)), dts_sub(a, b), fuel);
    //  Now: nonneg(sub(a, b)) at fuel.

    //  Apply le_mul_nonneg_monotone(b, a, c): nonneg(sub(a,b)) ∧ nonneg(c) → nonneg(sub(a*c, b*c))
    lemma_dts_same_radicand_transitive(a, b, c);  //  sr(a, c)
    lemma_dts_le_mul_nonneg_monotone_fuel(b, a, c, fuel);
    //  → nonneg(sub(a*c, b*c))

    //  ─── Now: nonneg(sub(b*c, a*c)) and nonneg(sub(a*c, b*c)). ───
    //  Transfer the latter to nonneg(neg(sub(b*c, a*c))) via congruence, then apply le_antisymmetric.
    lemma_dts_mul_closed(b, c);
    lemma_dts_mul_closed(a, c);
    //  wf for sub(b*c, a*c)
    lemma_dts_neg_well_formed(dts_mul(a, c));
    lemma_dts_same_radicand_neg(dts_mul(a, c));
    lemma_dts_same_radicand_symmetric(dts_mul(a, c), dts_neg(dts_mul(a, c)));
    //  sr(b*c, a*c): b*c ~ b ~ a ~ a*c
    lemma_dts_same_radicand_symmetric(b, dts_mul(b, c));
    lemma_dts_same_radicand_transitive(dts_mul(b, c), b, a);
    lemma_dts_same_radicand_symmetric(a, dts_mul(a, c));
    lemma_dts_same_radicand_transitive(dts_mul(b, c), a, dts_mul(a, c));
    lemma_dts_same_radicand_transitive(
        dts_mul(b, c), dts_mul(a, c), dts_neg(dts_mul(a, c)));
    lemma_dts_add_closed(dts_mul(b, c), dts_neg(dts_mul(a, c)));
    //  Symmetric: sr(a*c, b*c) — needed below
    lemma_dts_same_radicand_symmetric(dts_mul(b, c), dts_mul(a, c));
    lemma_dts_nonneg_radicands_mul(b, c);
    lemma_dts_nonneg_radicands_mul(a, c);
    lemma_dts_nonneg_radicands_neg(dts_mul(a, c));
    lemma_dts_nonneg_radicands_add(dts_mul(b, c), dts_neg(dts_mul(a, c)));
    lemma_norm_definite_mul(b, c);
    lemma_norm_definite_mul(a, c);
    lemma_norm_definite_neg(dts_mul(a, c));
    lemma_norm_definite_add(dts_mul(b, c), dts_neg(dts_mul(a, c)));
    lemma_dts_depth_mul_le(b, c);
    lemma_dts_depth_mul_le(a, c);
    lemma_dts_depth_neg(dts_mul(a, c));
    lemma_dts_depth_add_le(dts_mul(b, c), dts_neg(dts_mul(a, c)));
    //  wf for sub(a*c, b*c)
    lemma_dts_neg_well_formed(dts_mul(b, c));
    lemma_dts_same_radicand_neg(dts_mul(b, c));
    lemma_dts_same_radicand_symmetric(dts_mul(b, c), dts_neg(dts_mul(b, c)));
    lemma_dts_same_radicand_transitive(
        dts_mul(a, c), dts_mul(b, c), dts_neg(dts_mul(b, c)));
    lemma_dts_add_closed(dts_mul(a, c), dts_neg(dts_mul(b, c)));
    lemma_dts_nonneg_radicands_neg(dts_mul(b, c));
    lemma_dts_nonneg_radicands_add(dts_mul(a, c), dts_neg(dts_mul(b, c)));
    lemma_norm_definite_neg(dts_mul(b, c));
    lemma_norm_definite_add(dts_mul(a, c), dts_neg(dts_mul(b, c)));
    lemma_dts_depth_neg(dts_mul(b, c));
    lemma_dts_depth_add_le(dts_mul(a, c), dts_neg(dts_mul(b, c)));
    //  wf/sr for neg(sub(b*c, a*c))
    lemma_dts_neg_well_formed(dts_sub(dts_mul(b, c), dts_mul(a, c)));
    lemma_dts_same_radicand_neg(dts_sub(dts_mul(b, c), dts_mul(a, c)));
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(b, c), dts_mul(a, c)),
        dts_neg(dts_sub(dts_mul(b, c), dts_mul(a, c))));
    lemma_dts_nonneg_radicands_neg(dts_sub(dts_mul(b, c), dts_mul(a, c)));
    lemma_norm_definite_neg(dts_sub(dts_mul(b, c), dts_mul(a, c)));
    lemma_dts_depth_neg(dts_sub(dts_mul(b, c), dts_mul(a, c)));

    //  neg_sub_swap: neg(sub(b*c, a*c)) ≡ sub(a*c, b*c)
    lemma_dts_neg_sub_swap(dts_mul(b, c), dts_mul(a, c));
    //  Symmetric: sub(a*c, b*c) ≡ neg(sub(b*c, a*c))
    lemma_dts_eqv_symmetric(
        dts_neg(dts_sub(dts_mul(b, c), dts_mul(a, c))),
        dts_sub(dts_mul(a, c), dts_mul(b, c)));
    //  sr(sub(a*c, b*c), neg(sub(b*c, a*c))): sub(a*c, b*c) ~ a*c ~ b*c ~ sub(b*c, a*c) ~ neg(sub(b*c, a*c))
    lemma_dts_same_radicand_symmetric(
        dts_mul(a, c), dts_sub(dts_mul(a, c), dts_mul(b, c)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(a, c), dts_mul(b, c)), dts_mul(a, c), dts_mul(b, c));
    lemma_dts_same_radicand_symmetric(
        dts_mul(b, c), dts_sub(dts_mul(b, c), dts_mul(a, c)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(a, c), dts_mul(b, c)),
        dts_mul(b, c),
        dts_sub(dts_mul(b, c), dts_mul(a, c)));
    ...
    lemma_dts_nonneg_fuel_congruence(
        ...
        dts_neg(dts_sub(dts_mul(b, c), dts_mul(a, c))),
        fuel);
    //  Now: nonneg(neg(sub(b*c, a*c))). Combined with precondition nonneg(sub(b*c, a*c)),
    //  apply le_antisymmetric → is_zero(sub(b*c, a*c))
    lemma_dts_le_antisymmetric_fuel(dts_sub(dts_mul(b, c), dts_mul(a, c)), fuel);
    //  → is_zero(sub(b*c, a*c))

    //  ─── sub(b*c, a*c) ≡ mul(sub(b, a), c) via distributivity ───
    //  (Same chain as in le_mul_nonneg_monotone.)
    //  sr(sub(b,a), c): sub(b,a) ~ b ~ c (need this BEFORE mul_commutative)
    lemma_dts_same_radicand_symmetric(b, dts_sub(b, a));
    lemma_dts_same_radicand_transitive(dts_sub(b, a), b, c);
    lemma_dts_mul_commutative(dts_sub(b, a), c);
    lemma_dts_same_radicand_symmetric(b, c);
    ...
    lemma_dts_neg_congruence(dts_mul(c, a), dts_mul(a, c));
    lemma_dts_add_congruence_left(
        dts_mul(c, b), dts_mul(b, c), dts_mul(c, dts_neg(a)));
    lemma_dts_eqv_transitive(
        dts_mul(c, dts_neg(a)),
        dts_neg(dts_mul(c, a)),
        dts_neg(dts_mul(a, c)));
    lemma_dts_add_congruence_right(
        dts_mul(b, c), dts_mul(c, dts_neg(a)), dts_neg(dts_mul(a, c)));
    lemma_dts_eqv_transitive(
        dts_mul(dts_sub(b, a), c),
        dts_mul(c, dts_sub(b, a)),
        dts_add(dts_mul(c, b), dts_mul(c, dts_neg(a))));
    lemma_dts_eqv_transitive(
        dts_add(dts_mul(c, b), dts_mul(c, dts_neg(a))),
        dts_add(dts_mul(b, c), dts_mul(c, dts_neg(a))),
    ...
    lemma_dts_eqv_transitive(
        ...
        dts_add(dts_mul(c, b), dts_mul(c, dts_neg(a))),
        dts_add(dts_mul(b, c), dts_neg(dts_mul(a, c))));
    //  Now: eqv(mul(sub(b,a), c), sub(mul(b,c), mul(a,c)))
    //  Symmetric:
    lemma_dts_eqv_symmetric(
        dts_mul(dts_sub(b, a), c),
        dts_sub(dts_mul(b, c), dts_mul(a, c)));

    //  is_zero_congruence: is_zero(sub(b*c, a*c)) + eqv → is_zero(mul(sub(b,a), c))
    lemma_dts_is_zero_congruence(
        dts_sub(dts_mul(b, c), dts_mul(a, c)),
        dts_mul(dts_sub(b, a), c));

    //  ─── Apply mul_cancel_zero: is_zero(mul(sub(b,a), c)) ∧ !is_zero(c) → is_zero(sub(b,a)) ───
    lemma_dts_mul_closed(dts_sub(b, a), c);
    //  sr(sub(b,a), c): sub(b,a) ~ b ~ c
    lemma_dts_same_radicand_symmetric(b, dts_sub(b, a));
    lemma_dts_same_radicand_transitive(dts_sub(b, a), b, c);
    lemma_dts_mul_cancel_zero(dts_sub(b, a), c);
    //  → is_zero(sub(b, a))

    //  ─── is_zero implies nonneg ───
    lemma_dts_is_zero_implies_eqv_zero(dts_sub(b, a));
    lemma_dts_nonneg_fuel_zero(dts_sub(b, a), fuel);
    //  → nonneg(sub(b, a)). Contradicts !nonneg from the if-branch. Z3 derives false → vacuous.
}

///  nonneg_radicands preserved by add.
pub proof fn lemma_dts_nonneg_radicands_add<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires
        dts_nonneg_radicands(a), dts_nonneg_radicands(b),
    ensures
...
pub proof fn lemma_dts_nonneg_radicands_neg<T: OrderedField>(a: DynTowerSpec<T>)
...
pub proof fn lemma_dts_nonneg_radicands_mul<T: OrderedField>(a: DynTowerSpec<T>, b: DynTowerSpec<T>)
    requires
        dts_nonneg_radicands(a), dts_nonneg_radicands(b),
    ensures
...
#[verifier::rlimit(120)]
pub proof fn lemma_dts_c1c2_norm_bound<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
    ...
    decreases f, 1nat,
{
    ...
    if b1_nn {
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(a1, a2);
    } else {
        lemma_dts_add_commutative(a1, a2);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(a2, a1);
        lemma_dts_add_congruence_left(
    ...
    if b1_nn {
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b1, b2);
    } else {
        lemma_dts_add_commutative(b1, b2);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b2, b1);
        lemma_dts_add_congruence_left(
    ...
    lemma_dts_add_commutative(dts_neg(s), sum_im);
    lemma_dts_eqv_transitive(
...
///  then mul_commutative on b*a → a*b, then additive rearrangement.
#[verifier::rlimit(120)]
pub proof fn lemma_dts_square_of_sum<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>,
)
    requires dts_well_formed(a), dts_well_formed(b), dts_same_radicand(a, b),
    ensures
        dts_eqv(
            dts_mul(dts_add(a, b), dts_add(a, b)),
            dts_add(
                dts_add(dts_mul(a, a), dts_mul(b, b)),
                dts_add(dts_mul(a, b), dts_mul(a, b)))),
{
    //  Infrastructure
    lemma_dts_add_closed(a, b);
    let s = dts_add(a, b);
    lemma_dts_same_radicand_symmetric(a, s);
    lemma_dts_same_radicand_transitive(s, a, b);
    lemma_dts_same_radicand_reflexive(a);
    lemma_dts_same_radicand_reflexive(b);
    lemma_dts_same_radicand_symmetric(a, b);
    ...
    lemma_dts_mul_closed(b, a);
    lemma_dts_mul_closed(b, b);
    let aa = dts_mul(a, a); let ab = dts_mul(a, b);
    let ba = dts_mul(b, a); let bb = dts_mul(b, b);
    //  sr chains for products
    lemma_dts_same_radicand_symmetric(a, aa);
    lemma_dts_same_radicand_symmetric(a, ab);
    lemma_dts_same_radicand_symmetric(b, ba);
    lemma_dts_same_radicand_symmetric(b, bb);
    //  sr(aa, ab): aa ~ a ~ ab
    lemma_dts_same_radicand_transitive(aa, a, ab);
    //  sr(aa, bb): aa ~ a ~ b ~ bb
    lemma_dts_same_radicand_transitive(aa, a, b);
    lemma_dts_same_radicand_transitive(aa, b, bb);
    //  sr(ab, bb): ab ~ a ~ b ~ bb
    lemma_dts_same_radicand_transitive(ab, a, b);
    lemma_dts_same_radicand_transitive(ab, b, bb);
    //  sr(ba, ab): ba ~ b ~ a ~ ab
    lemma_dts_same_radicand_transitive(ba, b, a);
    lemma_dts_same_radicand_transitive(ba, a, ab);
    //  sr(ba, bb): ba ~ b ~ bb
    lemma_dts_same_radicand_transitive(ba, b, bb);
    //  ═══ Step 1: (a+b)² = (a+b)*a + (a+b)*b  [left dist] ═══
    lemma_dts_mul_distributes_left(s, a, b);
    //  eqv(mul(s, s), add(mul(s, a), mul(s, b)))  — Z3 sees s == add(a,b)
    //  ═══ Step 2: s*a ≡ a² + ab  [commute then left dist on a] ═══
    lemma_dts_mul_commutative(s, a);
    lemma_dts_mul_distributes_left(a, a, b);
    //  mul(s,a) ≡ mul(a,s) ≡ add(aa, ab)
    lemma_dts_eqv_transitive(dts_mul(s, a), dts_mul(a, s), dts_add(aa, ab));
    //  ═══ Step 3: s*b ≡ ba + b² ≡ ab + b²  [commute, dist, commute ba→ab] ═══
    lemma_dts_mul_commutative(s, b);
    lemma_dts_mul_distributes_left(b, a, b);
    //  mul(s,b) ≡ mul(b,s) ≡ add(ba, bb)
    lemma_dts_eqv_transitive(dts_mul(s, b), dts_mul(b, s), dts_add(ba, bb));
    //  ba ≡ ab
    lemma_dts_mul_commutative(b, a);
    //  add(ba, bb) ≡ add(ab, bb)
    lemma_dts_add_congruence_left(ba, ab, bb);
    //  mul(s,b) ≡ add(ab, bb)
    lemma_dts_eqv_transitive(dts_mul(s, b), dts_add(ba, bb), dts_add(ab, bb));
    //  ═══ Step 4: Combine: s² ≡ add(add(aa,ab), add(ab,bb)) ═══
    lemma_dts_mul_closed(s, a);
    lemma_dts_mul_closed(s, b);
    //  sr for congruence
    lemma_dts_same_radicand_symmetric(s, dts_mul(s, a));
    lemma_dts_same_radicand_transitive(dts_mul(s, a), s, a);
    lemma_dts_same_radicand_transitive(dts_mul(s, a), a, b);
    lemma_dts_same_radicand_symmetric(s, b);
    lemma_dts_same_radicand_transitive(b, s, dts_mul(s, b));
    lemma_dts_same_radicand_symmetric(b, dts_mul(s, b));
    lemma_dts_same_radicand_transitive(dts_mul(s, a), b, dts_mul(s, b));
    //  add_congruence_left: eqv(s*a, add(aa,ab)) → eqv(add(s*a, X), add(add(aa,ab), X))
    lemma_dts_add_congruence_left(dts_mul(s, a), dts_add(aa, ab), dts_mul(s, b));
    //  add_congruence_right: eqv(s*b, add(ab,bb)) → eqv(add(Y, s*b), add(Y, add(ab,bb)))
    lemma_dts_add_closed(aa, ab);
    lemma_dts_add_congruence_right(dts_add(aa, ab), dts_mul(s, b), dts_add(ab, bb));
    //  Chain: add(s*a, s*b) ≡ add(add(aa,ab), s*b) ≡ add(add(aa,ab), add(ab,bb))
    lemma_dts_add_closed(dts_mul(s, a), dts_mul(s, b));
    //  sr(add(aa,ab), mul(s,b)) for add_closed
    lemma_dts_same_radicand_symmetric(aa, dts_add(aa, ab));
    lemma_dts_same_radicand_transitive(dts_add(aa, ab), aa, a);
    lemma_dts_same_radicand_symmetric(a, s);
    lemma_dts_same_radicand_transitive(dts_add(aa, ab), a, s);
    lemma_dts_same_radicand_transitive(dts_add(aa, ab), s, dts_mul(s, b));
    lemma_dts_add_closed(dts_add(aa, ab), dts_mul(s, b));
    //  sr(add(aa,ab), add(ab,bb)) for add_closed — re-establish sr chains locally
    lemma_dts_same_radicand_transitive(ab, a, b);  //  sr(ab, b)
    lemma_dts_same_radicand_transitive(ab, b, bb);  //  sr(ab, bb)
    lemma_dts_add_closed(ab, bb);  //  sr(ab, add(ab,bb))
    lemma_dts_same_radicand_transitive(dts_add(aa, ab), a, ab);
    lemma_dts_same_radicand_symmetric(ab, dts_add(ab, bb));
    lemma_dts_same_radicand_transitive(dts_add(aa, ab), ab, dts_add(ab, bb));
    lemma_dts_add_closed(dts_add(aa, ab), dts_add(ab, bb));
    lemma_dts_eqv_transitive(
        dts_add(dts_mul(s, a), dts_mul(s, b)),
        dts_add(dts_add(aa, ab), dts_mul(s, b)),
        dts_add(dts_add(aa, ab), dts_add(ab, bb)));
    //  s² ≡ add(s*a, s*b) ≡ add(add(aa,ab), add(ab,bb))
    lemma_dts_eqv_transitive(
        dts_mul(s, s), dts_add(dts_mul(s, a), dts_mul(s, b)),
        dts_add(dts_add(aa, ab), dts_add(ab, bb)));
    //  ═══ Step 5: Rearrange add(add(aa,ab), add(ab,bb)) ≡ add(add(aa,bb), add(ab,ab)) ═══
    //  (aa+ab) + (ab+bb) → aa + (ab + (ab+bb)) → aa + ((ab+ab) + bb)
    //  → (aa+(ab+ab)) + bb → ((ab+ab)+aa) + bb → (ab+ab)+(aa+bb) → (aa+bb)+(ab+ab)
    //  Step 5a: assoc
    lemma_dts_add_associative(aa, ab, dts_add(ab, bb));
    //  Step 5b: assoc inner + symmetric
    lemma_dts_add_associative(ab, ab, bb);
    lemma_dts_eqv_symmetric(dts_add(ab, dts_add(ab, bb)), dts_add(dts_add(ab, ab), bb));
    //  Step 5c: congruence add(aa, _)
    lemma_dts_add_congruence_right(aa, dts_add(ab, dts_add(ab, bb)),
        dts_add(dts_add(ab, ab), bb));
    //  Step 5d: assoc reverse
    lemma_dts_add_associative(aa, dts_add(ab, ab), bb);
    lemma_dts_eqv_symmetric(dts_add(aa, dts_add(dts_add(ab, ab), bb)),
        dts_add(dts_add(aa, dts_add(ab, ab)), bb));
    //  Step 5e: commute inner
    lemma_dts_add_commutative(aa, dts_add(ab, ab));
    //  Step 5f: congruence add(_, bb)
    lemma_dts_add_congruence_left(dts_add(aa, dts_add(ab, ab)),
        dts_add(dts_add(ab, ab), aa), bb);
    //  Step 5g: assoc
    lemma_dts_add_associative(dts_add(ab, ab), aa, bb);
    //  Step 5h: commute outer
    lemma_dts_add_commutative(dts_add(ab, ab), dts_add(aa, bb));
    //  Full chain via eqv_transitive (6 steps)
    let t0 = dts_add(dts_add(aa, ab), dts_add(ab, bb));
    let t1 = dts_add(aa, dts_add(ab, dts_add(ab, bb)));
    let t2 = dts_add(aa, dts_add(dts_add(ab, ab), bb));
    let t3 = dts_add(dts_add(aa, dts_add(ab, ab)), bb);
    let t4 = dts_add(dts_add(dts_add(ab, ab), aa), bb);
    let t5 = dts_add(dts_add(ab, ab), dts_add(aa, bb));
    let t6 = dts_add(dts_add(aa, bb), dts_add(ab, ab));
    lemma_dts_eqv_transitive(t0, t1, t2);
    lemma_dts_eqv_transitive(t0, t2, t3);
    lemma_dts_eqv_transitive(t0, t3, t4);
    lemma_dts_eqv_transitive(t0, t4, t5);
    lemma_dts_eqv_transitive(t0, t5, t6);
    //  Final: s² ≡ t0 ≡ t6 = add(add(aa,bb), add(ab,ab))
    lemma_dts_eqv_transitive(dts_mul(s, s), t0, t6);
}

///  Helper: dd*(b1+b2)² ≡ (dd*b1² + dd*b2²) + (dd*b1*b2 + dd*b1*b2).
///  Uses square_of_sum + mul_congruence_right + mul_distributes_left chain.
#[verifier::rlimit(300)]
pub proof fn lemma_dts_d_dist_quad<T: OrderedField>(
    dd: DynTowerSpec<T>, b1: DynTowerSpec<T>, b2: DynTowerSpec<T>,
)
    requires
        dts_well_formed(dd), dts_well_formed(b1), dts_well_formed(b2),
    ...
    ensures
        dts_eqv(
            dts_mul(dd, dts_mul(dts_add(b1, b2), dts_add(b1, b2))),
            dts_add(
                dts_add(dts_mul(dd, dts_mul(b1, b1)), dts_mul(dd, dts_mul(b2, b2))),
                dts_add(dts_mul(dd, dts_mul(b1, b2)), dts_mul(dd, dts_mul(b1, b2))))),
{
    //  ─── Build sr infrastructure ───
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    lemma_dts_same_radicand_reflexive(dd);
    lemma_dts_same_radicand_symmetric(b1, b2);
    lemma_dts_same_radicand_symmetric(b1, dd);
    let bs = dts_add(b1, b2);
    lemma_dts_add_closed(b1, b2);
    //  sr(b1, bs) and sr(bs, b1) etc.
    lemma_dts_same_radicand_symmetric(b1, bs);
    lemma_dts_same_radicand_reflexive(bs);
    lemma_dts_mul_closed(bs, bs);
    lemma_dts_same_radicand_symmetric(bs, dts_mul(bs, bs));
    //  sr(b1, bs²): b1 ~ bs ~ bs²
    lemma_dts_same_radicand_transitive(b1, bs, dts_mul(bs, bs));
    //  Build product cells & their sr connections
    lemma_dts_mul_closed(b1, b1);
    lemma_dts_mul_closed(b2, b2);
    lemma_dts_mul_closed(b1, b2);
    //  sr(b1, b1²), sr(b1, b2²), sr(b1, b1*b2)
    lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, b1));
    lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, b2));
    lemma_dts_same_radicand_transitive(b1, b2, dts_mul(b2, b2));
    lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, b2));
    //  sr(b1², b2²) and sr(b1*b2, b1*b2)
    lemma_dts_same_radicand_transitive(dts_mul(b1, b1), b1, dts_mul(b2, b2));
    //  Build outer adds: add(b1², b2²) and add(b1*b2, b1*b2)
    lemma_dts_add_closed(dts_mul(b1, b1), dts_mul(b2, b2));
    lemma_dts_same_radicand_reflexive(dts_mul(b1, b2));
    lemma_dts_add_closed(dts_mul(b1, b2), dts_mul(b1, b2));
    //  sr(b1², add(b1², b2²))
    lemma_dts_same_radicand_symmetric(
        dts_mul(b1, b1), dts_add(dts_mul(b1, b1), dts_mul(b2, b2)));
    //  sr(b1*b2, add(b1*b2, b1*b2))
    lemma_dts_same_radicand_symmetric(
        dts_mul(b1, b2), dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));
    //  sr(add(b1²,b2²), add(b1*b2, b1*b2)) via b1
    lemma_dts_same_radicand_transitive(
    ...
    //  Build the outer sum: add(add(b1²,b2²), add(b1*b2, b1*b2))
    lemma_dts_add_closed(
        dts_add(dts_mul(b1, b1), dts_mul(b2, b2)),
        dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));
    //  sr(b1, dd) → sr(dd, b1) → sr(dd, b1²) → sr(dd, add(b1²,b2²)) etc.
    lemma_dts_same_radicand_symmetric(b1, dd);
    lemma_dts_same_radicand_transitive(dd, b1, dts_mul(b1, b1));
    ...
    lemma_dts_same_radicand_transitive(dd, dts_mul(b1, b2),
        dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));
    //  sr(dd, mul(bs, bs))
    lemma_dts_same_radicand_transitive(dd, b1, dts_mul(bs, bs));

    //  ─── Step 1: square_of_sum on (b1, b2) ───
    lemma_dts_square_of_sum(b1, b2);
    //  eqv(mul(bs, bs), add(add(b1², b2²), add(b1*b2, b1*b2)))
    let big_sum = dts_add(
        dts_add(dts_mul(b1, b1), dts_mul(b2, b2)),
        dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));
    //  sr(mul(bs, bs), big_sum) via dd
    lemma_dts_same_radicand_symmetric(dd, dts_mul(bs, bs));
    lemma_dts_same_radicand_symmetric(dd,
        dts_add(dts_mul(b1, b1), dts_mul(b2, b2)));
    lemma_dts_same_radicand_transitive(
        dts_mul(bs, bs), dd, dts_add(dts_mul(b1, b1), dts_mul(b2, b2)));
    //  sr(big_sum, b1²) for outer add closure
    //  Need: sr(mul(bs, bs), big_sum). Both share radicand with dd.
    //  sr(dd, big_sum) via add(b1²,b2²) ~ big_sum (from add_closed) and dd ~ add(b1²,b2²).
    lemma_dts_same_radicand_transitive(dd,
        dts_add(dts_mul(b1, b1), dts_mul(b2, b2)), big_sum);
    lemma_dts_same_radicand_symmetric(dd, big_sum);
    lemma_dts_same_radicand_transitive(dts_mul(bs, bs), dd, big_sum);

    //  ─── Step 2: dd * mul(bs, bs) ≡ dd * big_sum  [mul_congruence_right] ───
    //  Signature: mul_congruence_right(a, b, c) requires eqv(a, b) ∧ sr(a, b),
    //  ensures eqv(mul(c, a), mul(c, b)). c is the LEFT factor.
    lemma_dts_mul_congruence_right(dts_mul(bs, bs), big_sum, dd);

    //  ─── Step 3: dd * big_sum ≡ dd*(b1²+b2²) + dd*(b1*b2 + b1*b2)  [distribute] ───
    lemma_dts_mul_distributes_left(dd,
        dts_add(dts_mul(b1, b1), dts_mul(b2, b2)),
        dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));

    //  ─── Step 4: dd*(b1²+b2²) ≡ dd*b1² + dd*b2²  [distribute] ───
    lemma_dts_same_radicand_transitive(dd, b1, dts_mul(b1, b1));
    lemma_dts_same_radicand_transitive(dd, dts_mul(b1, b1), dts_mul(b2, b2));
    lemma_dts_mul_distributes_left(dd, dts_mul(b1, b1), dts_mul(b2, b2));

    //  ─── Step 5: dd*(b1*b2 + b1*b2) ≡ dd*b1*b2 + dd*b1*b2  [distribute] ───
    lemma_dts_same_radicand_transitive(dd, b1, dts_mul(b1, b2));
    lemma_dts_same_radicand_reflexive(dts_mul(b1, b2));
    lemma_dts_mul_distributes_left(dd, dts_mul(b1, b2), dts_mul(b1, b2));

    //  ─── Step 6: chain via add_congruence + eqv_transitive ───
    let lhs1 = dts_mul(dd, dts_add(dts_mul(b1, b1), dts_mul(b2, b2)));
    let rhs1 = dts_add(dts_mul(dd, dts_mul(b1, b1)), dts_mul(dd, dts_mul(b2, b2)));
    let lhs2 = dts_mul(dd, dts_add(dts_mul(b1, b2), dts_mul(b1, b2)));
    let rhs2 = dts_add(dts_mul(dd, dts_mul(b1, b2)), dts_mul(dd, dts_mul(b1, b2)));
    //  add(lhs1, lhs2) ≡ add(rhs1, lhs2) via add_congruence_left
    lemma_dts_add_congruence_left(lhs1, rhs1, lhs2);
    //  add(rhs1, lhs2) ≡ add(rhs1, rhs2) via add_congruence_right
    lemma_dts_add_congruence_right(rhs1, lhs2, rhs2);
    //  Chain: add(lhs1, lhs2) ≡ add(rhs1, lhs2) ≡ add(rhs1, rhs2)
    lemma_dts_eqv_transitive(
        dts_add(lhs1, lhs2),
        dts_add(rhs1, lhs2),
        dts_add(rhs1, rhs2));

    //  ─── Final chain: dd*mul(bs,bs) ≡ dd*big_sum ≡ add(lhs1,lhs2) ≡ add(rhs1,rhs2) ───
    //  Step 2 gave: dd*mul(bs,bs) ≡ dd*big_sum
    //  Step 3 gave: dd*big_sum ≡ add(lhs1, lhs2)
    lemma_dts_eqv_transitive(
        dts_mul(dd, dts_mul(bs, bs)),
        dts_mul(dd, big_sum),
        dts_add(lhs1, lhs2));
    //  Combine with the add_congruence chain:
    lemma_dts_eqv_transitive(
        dts_mul(dd, dts_mul(bs, bs)),
        dts_add(lhs1, lhs2),
        dts_add(rhs1, rhs2));
...
///  Cauchy cross-term: from norm bounds + nonneg cross factors, derive nonneg cross diff.
///  Given nx = a1² - dd·b1² ≥ 0, ny = a2² - dd·b2² ≥ 0, a1·a2 ≥ 0, dd·(b1·b2) ≥ 0,
///  derive a1·a2 - dd·(b1·b2) ≥ 0.
///  Proof: monotonicity gives (a1²)(a2²) - (dd·b1²)(a2²) ≥ 0 and similar.
///  Telescoping → (a1²)(a2²) - (dd·b2²)(dd·b1²) ≥ 0.
///  Ring identities: (a1²)(a2²) ≡ (a1·a2)² and (dd·b2²)(dd·b1²) ≡ (dd·(b1·b2))².
///  Then square_le_implies_le → conclusion.
#[verifier::rlimit(800)]
proof fn lemma_dts_cauchy_cross_term<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_nonneg_fuel(
            dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2))), f),
        dts_nonneg_fuel(dts_mul(a1, a2), f),
        dts_nonneg_fuel(dts_mul(dd, dts_mul(b1, b2)), f),
    ensures
        dts_nonneg_fuel(
            dts_sub(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2))), f),
    decreases f, 4nat,
{
    ...
    let a12 = dts_mul(a1, a2);
    let b12 = dts_mul(b1, b2);
    let db12 = dts_mul(dd, b12);

    //  ─── sr/wf/nonneg_radicands/norm_definite infrastructure ───
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    ...
    lemma_dts_same_radicand_transitive(a1, a2, aa2);
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_symmetric(b1, bb1);
    lemma_dts_same_radicand_transitive(a1, b1, bb1);
    lemma_dts_same_radicand_symmetric(b2, bb2);
    lemma_dts_same_radicand_transitive(a1, b2, bb2);
    lemma_dts_same_radicand_symmetric(b1, b12);
    lemma_dts_same_radicand_transitive(a1, b1, b12);
    //  dd*bb_, dd*b12 cells
    lemma_dts_same_radicand_transitive(dd, a1, bb1);
    lemma_dts_same_radicand_symmetric(dd, a1);
    lemma_dts_same_radicand_transitive(dd, a1, bb1);
    lemma_dts_mul_closed(dd, bb1);
    lemma_dts_same_radicand_transitive(dd, a1, bb2);
    lemma_dts_mul_closed(dd, bb2);
    lemma_dts_same_radicand_transitive(dd, a1, b12);
    lemma_dts_mul_closed(dd, b12);
    //  sr(a1, dbb1), sr(a1, dbb2), sr(a1, db12)
    lemma_dts_same_radicand_symmetric(dd, dbb1);
    lemma_dts_same_radicand_transitive(a1, dd, dbb1);
    lemma_dts_same_radicand_symmetric(dd, dbb2);
    lemma_dts_same_radicand_transitive(a1, dd, dbb2);
    lemma_dts_same_radicand_symmetric(dd, db12);
    lemma_dts_same_radicand_transitive(a1, dd, db12);
    //  nonneg_radicands for products
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(a2, a2);
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    ...
    lemma_dts_nonneg_radicands_mul(dd, b12);
    ...
    lemma_norm_definite_mul(dd, b12);
    //  depth bounds for products
    lemma_dts_depth_mul_le(a1, a1);
    lemma_dts_depth_mul_le(a2, a2);
    lemma_dts_depth_mul_le(a1, a2);
    lemma_dts_depth_mul_le(b1, b1);
    ...
    lemma_dts_depth_mul_le(dd, b12);

    //  ─── nonneg(a2*a2) at fuel f via square_nonneg ───
    lemma_dts_square_nonneg(a2, f);
    //  nonneg(dbb1) at fuel f: dd*b1*b1 nonneg via nonneg_mul + square
    lemma_dts_square_nonneg(b1, f);
    //  nonneg(dd) at fuel f
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    //  nonneg(dd * bb1)
    lemma_dts_nonneg_mul_closed_fuel(dd, bb1, f);

    //  ─── Step 1: monotone(dbb1, aa1, aa2) — nonneg(aa1·aa2 - dbb1·aa2) ───
    //  Need: sr(dbb1, aa1), sr(aa1, aa2)
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(dbb1, a1, aa1);
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_transitive(aa1, a1, aa2);
    //  nonneg(sub(aa1, dbb1)) is the precondition (nx ≥ 0)
    lemma_dts_le_mul_nonneg_monotone_fuel(dbb1, aa1, aa2, f);
    //  Result: nonneg(sub(aa1·aa2, dbb1·aa2))

    //  ─── Step 2: monotone(dbb2, aa2, dbb1) — nonneg(aa2·dbb1 - dbb2·dbb1) ───
    //  Need: sr(dbb2, aa2), sr(aa2, dbb1)
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(dbb2, a1, aa2);
    lemma_dts_same_radicand_symmetric(a1, aa2);
    lemma_dts_same_radicand_transitive(aa2, a1, dbb1);
    lemma_dts_le_mul_nonneg_monotone_fuel(dbb2, aa2, dbb1, f);
    //  Result: nonneg(sub(aa2·dbb1, dbb2·dbb1))

    //  ─── Step 3: middle term commutation: dbb1·aa2 ≡ aa2·dbb1 ───
    //  sr(dbb1, aa2): both ~ a1
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(dbb1, a1, aa2);
    lemma_dts_mul_commutative(dbb1, aa2);
    //  Result: eqv(dbb1·aa2, aa2·dbb1)
    //  Also need sr for sub_congruence later
    lemma_dts_mul_closed(aa1, aa2);
    lemma_dts_mul_closed(dbb1, aa2);
    lemma_dts_mul_closed(aa2, dbb1);
    //  sr(dbb2, dbb1): both ~ a1
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(dbb2, a1, dbb1);
    lemma_dts_mul_closed(dbb2, dbb1);
    //  sr(aa1·aa2, dbb1·aa2): both ~ a1
    lemma_dts_same_radicand_symmetric(aa1, dts_mul(aa1, aa2));
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_transitive(dts_mul(aa1, aa2), aa1, a1);
    lemma_dts_same_radicand_symmetric(dbb1, dts_mul(dbb1, aa2));
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(dts_mul(dbb1, aa2), dbb1, a1);
    lemma_dts_same_radicand_symmetric(dts_mul(dbb1, aa2), a1);
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), a1, dts_mul(dbb1, aa2));
    //  sr(aa2·dbb1, dbb2·dbb1) similarly
    lemma_dts_same_radicand_symmetric(aa2, dts_mul(aa2, dbb1));
    lemma_dts_same_radicand_symmetric(a1, aa2);
    lemma_dts_same_radicand_transitive(dts_mul(aa2, dbb1), aa2, a1);
    lemma_dts_same_radicand_symmetric(dbb2, dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(dts_mul(dbb2, dbb1), dbb2, a1);
    lemma_dts_same_radicand_symmetric(dts_mul(dbb2, dbb1), a1);
    lemma_dts_same_radicand_transitive(
        dts_mul(aa2, dbb1), a1, dts_mul(dbb2, dbb1));

    //  Apply nonneg_fuel_congruence to step 1's result via dbb1·aa2 ≡ aa2·dbb1
    //  We have: nonneg(sub(aa1·aa2, dbb1·aa2)) [from step 1]
    //  Want:    nonneg(sub(aa1·aa2, aa2·dbb1))
    //  via sub_congruence_both with right_eqv = dbb1·aa2 ≡ aa2·dbb1
    //  Or directly: cong on the second arg
    //  Use sub_congruence_both: sub(aa1·aa2, dbb1·aa2) ≡ sub(aa1·aa2, aa2·dbb1)
    //  Need eqv(aa1·aa2, aa1·aa2) (refl) and eqv(dbb1·aa2, aa2·dbb1) (have)
    lemma_dts_eqv_reflexive(dts_mul(aa1, aa2));
    //  Also need sr(aa1·aa2, aa1·aa2) and sr(dbb1·aa2, aa2·dbb1)
    lemma_dts_same_radicand_reflexive(dts_mul(aa1, aa2));
    //  sr(dbb1·aa2, aa2·dbb1): both ~ a1
    //  aa2*dbb1 ~ aa2 (from mul_closed) → sym → sr(aa2*dbb1, aa2). Then aa2~a1 → sr(aa2*dbb1, a1).
    lemma_dts_same_radicand_symmetric(aa2, dts_mul(aa2, dbb1));
    lemma_dts_same_radicand_symmetric(a1, aa2);
    lemma_dts_same_radicand_transitive(dts_mul(aa2, dbb1), aa2, a1);
    lemma_dts_same_radicand_symmetric(dts_mul(aa2, dbb1), a1);
    lemma_dts_same_radicand_transitive(
        dts_mul(dbb1, aa2), a1, dts_mul(aa2, dbb1));
    lemma_dts_sub_congruence_both(
        dts_mul(aa1, aa2), dts_mul(dbb1, aa2),
        dts_mul(aa1, aa2), dts_mul(aa2, dbb1));
    //  Need norm_definite + nonneg_radicands + depth for the new sub
    lemma_dts_nonneg_radicands_mul(aa1, aa2);
    lemma_dts_nonneg_radicands_mul(dbb1, aa2);
    lemma_dts_nonneg_radicands_mul(aa2, dbb1);
    lemma_dts_nonneg_radicands_mul(dbb2, dbb1);
    lemma_norm_definite_mul(aa1, aa2);
    lemma_norm_definite_mul(dbb1, aa2);
    lemma_norm_definite_mul(aa2, dbb1);
    lemma_norm_definite_mul(dbb2, dbb1);
    //  neg/sub well_formed and depth
    lemma_dts_neg_well_formed(dts_mul(dbb1, aa2));
    lemma_dts_neg_well_formed(dts_mul(aa2, dbb1));
    lemma_dts_neg_well_formed(dts_mul(dbb2, dbb1));
    lemma_dts_depth_neg(dts_mul(dbb1, aa2));
    lemma_dts_depth_neg(dts_mul(aa2, dbb1));
    lemma_dts_depth_neg(dts_mul(dbb2, dbb1));
    lemma_dts_depth_mul_le(aa1, aa2);
    lemma_dts_depth_mul_le(dbb1, aa2);
    lemma_dts_depth_mul_le(aa2, dbb1);
    lemma_dts_depth_mul_le(dbb2, dbb1);
    lemma_dts_depth_mul_le(a1, a1);
    lemma_dts_depth_mul_le(a2, a2);
    lemma_dts_depth_mul_le(b1, b1);
    lemma_dts_depth_mul_le(b2, b2);
    //  Build sub cells for the congruence
    //  sub_congruence transfer: nonneg(sub(aa1·aa2, dbb1·aa2)) → nonneg(sub(aa1·aa2, aa2·dbb1))
    //  The sub_congruence_both gives eqv result. nonneg_fuel_congruence transfers nonneg.
    //  Need: sr between the two subs
    lemma_dts_same_radicand_neg(dts_mul(dbb1, aa2));
    lemma_dts_same_radicand_neg(dts_mul(aa2, dbb1));
    lemma_dts_same_radicand_symmetric(
        dts_mul(dbb1, aa2), dts_neg(dts_mul(dbb1, aa2)));
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), dts_mul(dbb1, aa2), dts_neg(dts_mul(dbb1, aa2)));
    lemma_dts_add_closed(dts_mul(aa1, aa2), dts_neg(dts_mul(dbb1, aa2)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa2, dbb1), dts_neg(dts_mul(aa2, dbb1)));
    //  sr(aa1·aa2, aa2·dbb1) via dbb1·aa2
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), dts_mul(dbb1, aa2), dts_mul(aa2, dbb1));
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), dts_mul(aa2, dbb1), dts_neg(dts_mul(aa2, dbb1)));
    lemma_dts_add_closed(dts_mul(aa1, aa2), dts_neg(dts_mul(aa2, dbb1)));
    //  Apply nonneg_fuel_congruence
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb1, aa2)),
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        f);
    //  Now we have nonneg(sub(aa1·aa2, aa2·dbb1)) and nonneg(sub(aa2·dbb1, dbb2·dbb1)) [step 2]

    //  ─── Step 4: telescope via 3-arg sub_add_sub (additive_group_lemmas) ───
    //  (aa1·aa2 - aa2·dbb1) + (aa2·dbb1 - dbb2·dbb1) ≡ aa1·aa2 - dbb2·dbb1
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub::<DynTowerSpec<T>>(
        dts_mul(aa1, aa2), dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1));
    //  Add the two nonneg subs to get nonneg of the LHS
    //  sr(sub(aa1·aa2, aa2·dbb1), sub(aa2·dbb1, dbb2·dbb1))
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa1, aa2),
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa2, dbb1), dts_neg(dts_mul(aa2, dbb1)));
    //  sub(aa1·aa2, aa2·dbb1) = add(aa1·aa2, neg(aa2·dbb1))
    //  sr(this, aa1·aa2) from add_closed → sym
    //  Going simpler: both subs ~ a1
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        dts_mul(aa1, aa2), a1);
    //  Same for the second sub
    lemma_dts_neg_well_formed(dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_neg(dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_symmetric(
        dts_mul(dbb2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_transitive(
        dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_add_closed(dts_mul(aa2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa2, dbb1),
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)),
        dts_mul(aa2, dbb1), a1);
    //  Combine: sr(sub1, sub2) via a1
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)), a1);
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        a1,
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)));
    //  norm_definite for the subs
    lemma_dts_neg_well_formed(dts_mul(aa2, dbb1));
    lemma_norm_definite_neg(dts_mul(aa2, dbb1));
    lemma_norm_definite_add(dts_mul(aa1, aa2), dts_neg(dts_mul(aa2, dbb1)));
    lemma_norm_definite_neg(dts_mul(dbb2, dbb1));
    lemma_norm_definite_add(dts_mul(aa2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    //  nonneg_radicands for the subs
    lemma_dts_nonneg_radicands_neg(dts_mul(aa2, dbb1));
    lemma_dts_nonneg_radicands_add(
        dts_mul(aa1, aa2), dts_neg(dts_mul(aa2, dbb1)));
    lemma_dts_nonneg_radicands_neg(dts_mul(dbb2, dbb1));
    lemma_dts_nonneg_radicands_add(
        dts_mul(aa2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    //  depth bounds
    lemma_dts_depth_neg(dts_mul(aa2, dbb1));
    lemma_dts_depth_add_le(dts_mul(aa1, aa2), dts_neg(dts_mul(aa2, dbb1)));
    lemma_dts_depth_neg(dts_mul(dbb2, dbb1));
    lemma_dts_depth_add_le(dts_mul(aa2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    //  Add nonneg
    lemma_dts_nonneg_add_closed_fuel(
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)),
        f);
    //  Now we have nonneg(add(sub1, sub2)) and via sub_add_sub eqv:
    //  add(sub1, sub2) ≡ sub(aa1·aa2, dbb2·dbb1)
    //  Apply nonneg_fuel_congruence
    //  Need sr(add(sub1, sub2), sub(aa1·aa2, dbb2·dbb1))
    lemma_dts_add_closed(
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_neg(dts_mul(dbb2, dbb1));
    ...
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa1, aa2),
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)));
    //  add(sub1, sub2) ~ sub1 ~ aa1·aa2
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
        dts_add(
            dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
            dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1))));
    //  sub(aa1·aa2, dbb2·dbb1) ~ aa1·aa2
    //  Need sr(aa1·aa2, dbb2·dbb1) first via a1
    lemma_dts_same_radicand_symmetric(dbb2, dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(dts_mul(dbb2, dbb1), dbb2, a1);
    lemma_dts_same_radicand_symmetric(dts_mul(dbb2, dbb1), a1);
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), a1, dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), dts_mul(dbb2, dbb1), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_add_closed(dts_mul(aa1, aa2), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa1, aa2),
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb2, dbb1)));
    //  Chain: add(sub1, sub2) ~ aa1·aa2 ~ sub(aa1·aa2, dbb2·dbb1)
    lemma_dts_same_radicand_transitive(
        dts_add(
            dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
            dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1))),
    ...
    lemma_dts_same_radicand_transitive(
        ...
        dts_mul(aa1, aa2),
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb2, dbb1)));
    //  norm_definite for the new sub
    lemma_dts_neg_well_formed(dts_mul(dbb2, dbb1));
    lemma_norm_definite_neg(dts_mul(dbb2, dbb1));
    lemma_norm_definite_add(dts_mul(aa1, aa2), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_nonneg_radicands_neg(dts_mul(dbb2, dbb1));
    lemma_dts_nonneg_radicands_add(
        dts_mul(aa1, aa2), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_depth_neg(dts_mul(dbb2, dbb1));
    lemma_dts_depth_add_le(dts_mul(aa1, aa2), dts_neg(dts_mul(dbb2, dbb1)));
    lemma_dts_nonneg_fuel_congruence(
        dts_add(
            dts_sub(dts_mul(aa1, aa2), dts_mul(aa2, dbb1)),
            dts_sub(dts_mul(aa2, dbb1), dts_mul(dbb2, dbb1))),
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb2, dbb1)),
        f);
    //  Now we have nonneg(sub(aa1·aa2, dbb2·dbb1))

    //  ─── Step 5: ring identity 1: aa1·aa2 ≡ (a1·a2)·(a1·a2) [square_of_product symmetric] ───
    lemma_dts_square_of_product(a1, a2);
    //  ensures: eqv((a1·a2)·(a1·a2), aa1·aa2). Symmetric:
    lemma_dts_eqv_symmetric(dts_mul(a12, a12), dts_mul(aa1, aa2));

    //  ─── Step 6: ring identity 2: dbb2·dbb1 ≡ (db12)·(db12) [d_quadruple_eq_sq] ───
    //  Need sr(b1, b2), sr(b1, dd) — have both
    lemma_dts_d_quadruple_eq_sq(dd, b1, b2);
    //  ensures: eqv(dbb2·dbb1, db12·db12).

    //  ─── Step 7: combine via sub_congruence_both ───
    //  sub(aa1·aa2, dbb2·dbb1) ≡ sub((a12·a12), (db12·db12))
    //  Need sr(aa1·aa2, a12·a12) and sr(dbb2·dbb1, db12·db12)
    lemma_dts_same_radicand_reflexive(a12);
    lemma_dts_same_radicand_reflexive(db12);
    lemma_dts_mul_closed(a12, a12);
    lemma_dts_mul_closed(db12, db12);
    //  sr(aa1·aa2, a12·a12): both ~ a1
    //  Build sr(a1, a12*a12): a1 ~ a12 (have) and a12 ~ a12*a12 (mul_closed) → trans
    lemma_dts_same_radicand_transitive(a1, a12, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
        dts_mul(aa1, aa2), a1, dts_mul(a12, a12));
    //  sr(dbb2·dbb1, db12·db12): both ~ a1
    lemma_dts_same_radicand_symmetric(db12, dts_mul(db12, db12));
    lemma_dts_same_radicand_transitive(a1, db12, dts_mul(db12, db12));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(db12, db12));
    //  sr(dbb2·dbb1, a1)
    lemma_dts_same_radicand_symmetric(dts_mul(dbb2, dbb1), a1);
    //  Wait, do we have this? Let me re-derive.
    //  dbb2 ~ a1 (built earlier). dbb2 ~ dbb2*dbb1 from mul_closed → sym → dbb2*dbb1 ~ dbb2 ~ a1
    lemma_dts_same_radicand_symmetric(dbb2, dts_mul(dbb2, dbb1));
    lemma_dts_same_radicand_transitive(dts_mul(dbb2, dbb1), dbb2, a1);
    lemma_dts_same_radicand_transitive(
        dts_mul(dbb2, dbb1), a1, dts_mul(db12, db12));
    //  Apply sub_congruence_both
    lemma_dts_sub_congruence_both(
        dts_mul(aa1, aa2), dts_mul(dbb2, dbb1),
        dts_mul(a12, a12), dts_mul(db12, db12));
    //  Result: eqv(sub(aa1·aa2, dbb2·dbb1), sub(a12·a12, db12·db12))

    //  ─── Step 8: nonneg_fuel_congruence to transfer nonneg ───
    //  Need sr between the two subs
    lemma_dts_neg_well_formed(dts_mul(db12, db12));
    lemma_dts_same_radicand_neg(dts_mul(db12, db12));
    lemma_dts_same_radicand_symmetric(
        dts_mul(db12, db12), dts_neg(dts_mul(db12, db12)));
    //  sr(a12*a12, db12*db12): both ~ a1
    lemma_dts_same_radicand_symmetric(a1, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
        dts_mul(a12, a12), a1, dts_mul(db12, db12));
    lemma_dts_same_radicand_transitive(
        dts_mul(a12, a12), dts_mul(db12, db12), dts_neg(dts_mul(db12, db12)));
    lemma_dts_add_closed(dts_mul(a12, a12), dts_neg(dts_mul(db12, db12)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(a12, a12),
        dts_sub(dts_mul(a12, a12), dts_mul(db12, db12)));
    //  norm_definite + nonneg_radicands + depth for new sub
    lemma_dts_nonneg_radicands_mul(a12, a12);
    lemma_dts_nonneg_radicands_mul(db12, db12);
    lemma_dts_nonneg_radicands_neg(dts_mul(db12, db12));
    lemma_dts_nonneg_radicands_add(
        dts_mul(a12, a12), dts_neg(dts_mul(db12, db12)));
    lemma_norm_definite_mul(a12, a12);
    lemma_norm_definite_mul(db12, db12);
    lemma_norm_definite_neg(dts_mul(db12, db12));
    lemma_norm_definite_add(
        dts_mul(a12, a12), dts_neg(dts_mul(db12, db12)));
    lemma_dts_depth_mul_le(a12, a12);
    lemma_dts_depth_mul_le(db12, db12);
    lemma_dts_depth_neg(dts_mul(db12, db12));
    lemma_dts_depth_add_le(dts_mul(a12, a12), dts_neg(dts_mul(db12, db12)));
    //  sr(sub(aa1·aa2, dbb2·dbb1), sub(a12·a12, db12·db12)) via a1
    lemma_dts_same_radicand_symmetric(
        dts_mul(aa1, aa2),
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb2, dbb1)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(aa1, aa2), dts_mul(dbb2, dbb1)),
        dts_mul(aa1, aa2), a1);
    //  Need sr(a1, sub(a12·a12, db12·db12))
    //  sr(a12*a12, sub(...)) from add_closed → sym; then trans with sr(a1, a12*a12)
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(a12, a12), dts_mul(db12, db12)),
        dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
    ...
    lemma_dts_nonneg_fuel_congruence(
        ...
        dts_sub(dts_mul(a12, a12), dts_mul(db12, db12)),
        f);
    //  Now nonneg(sub(a12·a12, db12·db12)) ≡ nonneg((a1·a2)² - (db12)²)

    //  ─── Step 9: square_le_implies_le_fuel(db12, a12, f) ───
    //  requires: nonneg(db12), nonneg(a12), nonneg(sub(a12², db12²))
    //  ensures: nonneg(sub(a12, db12))
    //  All preconditions in place
    //  Need sr(db12, a12)
    lemma_dts_same_radicand_symmetric(a1, db12);
    lemma_dts_same_radicand_transitive(db12, a1, a12);
    //  Need depth bounds
    lemma_dts_depth_mul_le(dd, b12);
    lemma_dts_depth_mul_le(a1, a2);
    //  Need nonneg_radicands for db12 and a12
    lemma_dts_nonneg_radicands_mul(dd, b12);
    lemma_dts_nonneg_radicands_mul(a1, a2);
    //  norm_definite for db12 and a12
    lemma_norm_definite_mul(dd, b12);
    lemma_norm_definite_mul(a1, a2);
    lemma_dts_square_le_implies_le_fuel(db12, a12, f);
    //  Result: nonneg(sub(a12, db12)) which is the goal!
}

///  Cauchy cross-term for NEG norms (C3+C3 case):
///  From neg-norms (d*bi² ≥ ai²), nonneg(a1*a2), nonneg(d*b1*b2),
///  derive nonneg(d*b1*b2 - a1*a2) (the "negated" cross-term sign).
///
///  Proof: uses lemma_cauchy_le_chain_neg_norms to get
///  nonneg((d*b1²)*(d*b2²) - a1²*a2²), then converts to (d*b1*b2)² ≥ (a1*a2)²
///  via dd_sq_product_eqv + square_of_product, then square_le_implies_le.
#[verifier::rlimit(500)]
proof fn lemma_dts_cauchy_cross_term_neg<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_nonneg_fuel(dts_mul(a1, a2), f),
        dts_nonneg_fuel(dts_mul(dd, dts_mul(b1, b2)), f),
        //  bi nonneg (needed by cauchy_le_chain_neg_norms)
        dts_nonneg_fuel(b1, f),
        dts_nonneg_fuel(b2, f),
    ensures
        dts_nonneg_fuel(
            dts_sub(dts_mul(dd, dts_mul(b1, b2)), dts_mul(a1, a2)), f),
    decreases f, 4nat,
{
    let a1_sq = dts_mul(a1, a1);
    let a2_sq = dts_mul(a2, a2);
    ...
    lemma_dts_same_radicand_symmetric(a1, a1_sq);
    lemma_dts_same_radicand_symmetric(a2, a2_sq);
    lemma_dts_same_radicand_transitive(a1, a2, a2_sq);
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_symmetric(b1, b1_sq);
    lemma_dts_same_radicand_transitive(a1, b1, b1_sq);
    lemma_dts_same_radicand_symmetric(b2, b2_sq);
    lemma_dts_same_radicand_transitive(a1, b2, b2_sq);
    lemma_dts_same_radicand_symmetric(b1, b12);
    lemma_dts_same_radicand_transitive(a1, b1, b12);
    //  dd*bi²/db12 cells
    lemma_dts_same_radicand_transitive(dd, a1, b1_sq);
    lemma_dts_mul_closed(dd, b1_sq);
    lemma_dts_same_radicand_transitive(dd, a1, b2_sq);
    lemma_dts_mul_closed(dd, b2_sq);
    lemma_dts_same_radicand_transitive(dd, a1, b12);
    lemma_dts_mul_closed(dd, b12);
    //  sr(a1, db1_sq), sr(a1, db2_sq), sr(a1, db12)
    lemma_dts_same_radicand_symmetric(dd, db1_sq);
    lemma_dts_same_radicand_transitive(a1, dd, db1_sq);
    lemma_dts_same_radicand_symmetric(dd, db2_sq);
    lemma_dts_same_radicand_transitive(a1, dd, db2_sq);
    lemma_dts_same_radicand_symmetric(dd, db12);
    lemma_dts_same_radicand_transitive(a1, dd, db12);
    //  nonneg_radicands for products
    lemma_dts_nonneg_radicands_mul(a1, a1);
    ...
    //  ─── Step A: Apply lemma_cauchy_le_chain_neg_norms ───
    //  → nonneg(sub((d*b1²)*(d*b2²), a1²*a2²))
    lemma_cauchy_le_chain_neg_norms(a1, a2, b1, b2, dd, f);

    //  ─── Step B: Convert (d*b1²)*(d*b2²) ≡ db12² via dd_sq_product_eqv ───
    lemma_dts_dd_sq_product_eqv(b1, b2, dd);
    //  ensures eqv(db1_sq*db2_sq, db12*db12)

    //  ─── Step C: Convert a1²*a2² ≡ a12² via square_of_product (symmetric) ───
    lemma_dts_square_of_product(a1, a2);
    //  ensures eqv(a12*a12, a1_sq*a2_sq). Symmetric:
    lemma_dts_eqv_symmetric(dts_mul(a12, a12), dts_mul(a1_sq, a2_sq));
    //  Now: eqv(a1_sq*a2_sq, a12*a12)

    //  ─── Step D: Build the squared cells, sub_congruence_both ───
    //  sr(db1_sq, db2_sq) via db1_sq ~ a1 ~ db2_sq
    lemma_dts_same_radicand_symmetric(a1, db1_sq);
    lemma_dts_same_radicand_transitive(db1_sq, a1, db2_sq);
    //  sr(a1_sq, a2_sq) via a1_sq ~ a1 ~ a2_sq
    lemma_dts_same_radicand_transitive(a1_sq, a1, a2_sq);
    lemma_dts_same_radicand_reflexive(db12);
    lemma_dts_same_radicand_reflexive(a12);
    lemma_dts_mul_closed(db1_sq, db2_sq);
    lemma_dts_mul_closed(a1_sq, a2_sq);
    lemma_dts_mul_closed(db12, db12);
    lemma_dts_mul_closed(a12, a12);
    //  sr(db1_sq*db2_sq, db12*db12) — both ~ a1
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), dd, db12);
    lemma_dts_same_radicand_symmetric(db12, dts_mul(db12, db12));
    lemma_dts_same_radicand_transitive(
        dts_mul(db1_sq, db2_sq), db12, dts_mul(db12, db12));
    //  sr(a1_sq*a2_sq, a12*a12) — both ~ a1
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1_sq, a1);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, a2_sq), a1, a12);
    lemma_dts_same_radicand_symmetric(a12, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
        dts_mul(a1_sq, a2_sq), a12, dts_mul(a12, a12));
    lemma_dts_sub_congruence_both(
        dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq),
        dts_mul(db12, db12), dts_mul(a12, a12));
    //  Result: eqv(sub(db1_sq*db2_sq, a1_sq*a2_sq), sub(db12*db12, a12*a12))

    //  ─── Step E: nonneg_fuel_congruence to transfer nonneg ───
    lemma_dts_nonneg_radicands_mul(db1_sq, db2_sq);
    lemma_dts_nonneg_radicands_mul(a1_sq, a2_sq);
    lemma_dts_nonneg_radicands_mul(db12, db12);
    lemma_dts_nonneg_radicands_mul(a12, a12);
    ...
    lemma_dts_depth_mul_le(db12, db12);
    lemma_dts_depth_mul_le(a12, a12);
    //  sub well_formed/sr/etc for both subs
    lemma_dts_neg_well_formed(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_symmetric(
        dts_mul(a1_sq, a2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    //  sr(db1_sq*db2_sq, neg(a1_sq*a2_sq)) — chain via a1
    //  db1_sq*db2_sq ~ db1_sq (mul_closed sym) ~ a1 (have)
    lemma_dts_same_radicand_symmetric(db1_sq, dts_mul(db1_sq, db2_sq));
    lemma_dts_same_radicand_symmetric(a1, db1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), db1_sq, a1);
    //  a1 ~ a1_sq*a2_sq via a1 ~ a1_sq (have, sym above) ~ a1_sq*a2_sq (mul_closed)
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(a1, a1_sq, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(db1_sq, db2_sq), a1, dts_mul(a1_sq, a2_sq));
    lemma_dts_same_radicand_transitive(
        dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq),
        dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_add_closed(dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_nonneg_radicands_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_nonneg_radicands_add(
        dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_norm_definite_neg(dts_mul(a1_sq, a2_sq));
    lemma_norm_definite_add(
        dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    lemma_dts_depth_neg(dts_mul(a1_sq, a2_sq));
    lemma_dts_depth_add_le(
        dts_mul(db1_sq, db2_sq), dts_neg(dts_mul(a1_sq, a2_sq)));
    //  Same for sub(db12², a12²)
    lemma_dts_neg_well_formed(dts_mul(a12, a12));
    lemma_dts_same_radicand_neg(dts_mul(a12, a12));
    lemma_dts_same_radicand_symmetric(
        dts_mul(a12, a12), dts_neg(dts_mul(a12, a12)));
    //  Build sr(a1, db12*db12) via a1 ~ db12 ~ db12*db12
    lemma_dts_same_radicand_symmetric(db12, dts_mul(db12, db12));
    lemma_dts_same_radicand_transitive(a1, db12, dts_mul(db12, db12));
    //  Build sr(a1, a12*a12) via a1 ~ a12 ~ a12*a12
    lemma_dts_same_radicand_symmetric(a12, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(a1, a12, dts_mul(a12, a12));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(db12, db12));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
        dts_mul(db12, db12), a1, dts_mul(a12, a12));
    lemma_dts_same_radicand_transitive(
        dts_mul(db12, db12), dts_mul(a12, a12),
        dts_neg(dts_mul(a12, a12)));
    lemma_dts_add_closed(dts_mul(db12, db12), dts_neg(dts_mul(a12, a12)));
    lemma_dts_nonneg_radicands_neg(dts_mul(a12, a12));
    lemma_dts_nonneg_radicands_add(
        dts_mul(db12, db12), dts_neg(dts_mul(a12, a12)));
    lemma_norm_definite_neg(dts_mul(a12, a12));
    lemma_norm_definite_add(
        dts_mul(db12, db12), dts_neg(dts_mul(a12, a12)));
    lemma_dts_depth_neg(dts_mul(a12, a12));
    lemma_dts_depth_add_le(
        dts_mul(db12, db12), dts_neg(dts_mul(a12, a12)));
    //  sr between the two subs (both ~ a1)
    lemma_dts_same_radicand_symmetric(
        dts_mul(db1_sq, db2_sq),
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)),
        dts_mul(db1_sq, db2_sq), a1);
    //  sub(db12², a12²) ~ a1
    lemma_dts_same_radicand_symmetric(
        dts_mul(db12, db12),
        dts_sub(dts_mul(db12, db12), dts_mul(a12, a12)));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(db12, db12));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db12, db12), dts_mul(a12, a12)),
        dts_mul(db12, db12), a1);
    //  Combine via a1
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(db12, db12), dts_mul(a12, a12)), a1);
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(db1_sq, db2_sq), dts_mul(a1_sq, a2_sq)),
        a1,
        dts_sub(dts_mul(db12, db12), dts_mul(a12, a12)));
    lemma_dts_nonneg_fuel_congruence(
        ...
        dts_sub(dts_mul(db12, db12), dts_mul(a12, a12)),
        f);
    //  Now: nonneg(sub(db12², a12²)) i.e., (db12)² ≥ (a12)²

    //  ─── Step F: square_le_implies_le_fuel(a12, db12, f) ───
    //  requires: nonneg(a12), nonneg(db12), nonneg(sub(db12², a12²))
    //  ensures:  nonneg(sub(db12, a12))
    //  Need sr(a12, db12)
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_transitive(a12, a1, db12);
    lemma_dts_square_le_implies_le_fuel(a12, db12, f);
    //  Result: nonneg(sub(db12, a12)) which is the goal!
}

///  C2+C2 norm bound: from both x and y being C2, derive nonneg of sum's norm.
///  Combines cauchy_cross_term + norm_sum_decomposition + nonneg_add chain.
#[verifier::rlimit(1000)]
proof fn lemma_dts_c2c2_norm_bound<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_norm_definite(a1), dts_norm_definite(b1),
        dts_norm_definite(a2), dts_norm_definite(b2), dts_norm_definite(dd),
        //  C2 facts for x: a1 ≥ 0, neg(b1) ≥ 0, nx ≥ 0
        dts_nonneg_fuel(a1, f),
        dts_nonneg_fuel(dts_neg(b1), f),
        dts_nonneg_fuel(
            dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))), f),
        //  C2 facts for y: a2 ≥ 0, neg(b2) ≥ 0, ny ≥ 0
        dts_nonneg_fuel(a2, f),
        dts_nonneg_fuel(dts_neg(b2), f),
        dts_nonneg_fuel(
            dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2))), f),
    ensures
        dts_nonneg_fuel(
    ...
    decreases f, 5nat,
{
    ...
    let b12 = dts_mul(b1, b2);
    let db12 = dts_mul(dd, b12);
    let nx = dts_sub(aa1, dbb1);
    let ny = dts_sub(aa2, dbb2);
    let cross = dts_sub(a12, db12);
    let ssa = dts_add(a1, a2);
    let ssb = dts_add(b1, b2);
    ...
    lemma_dts_mul_closed(b2, b2);
    lemma_dts_mul_closed(b1, b2);
    //  sr(dd, b1) via dd ~ a1 ~ b1
    lemma_dts_same_radicand_transitive(dd, a1, b1);
    //  sr(dd, bb1), sr(dd, bb2), sr(dd, b12)
    lemma_dts_same_radicand_transitive(dd, b1, bb1);
    lemma_dts_same_radicand_transitive(b1, b2, bb2);
    lemma_dts_same_radicand_transitive(dd, b1, bb2);
    lemma_dts_same_radicand_transitive(dd, b1, b12);
    lemma_dts_mul_closed(dd, bb1);
    lemma_dts_mul_closed(dd, bb2);
    lemma_dts_mul_closed(dd, b12);
    //  ssa, ssb closures
    lemma_dts_add_closed(a1, a2);
    lemma_dts_add_closed(b1, b2);
    //  nonneg_radicands + norm_definite + depth for products
    lemma_dts_nonneg_radicands_mul(a1, a2);
    lemma_dts_nonneg_radicands_mul(b1, b2);
    lemma_dts_nonneg_radicands_mul(dd, b12);
    lemma_dts_nonneg_radicands_mul(a1, a1);
    ...
    //  ─── Step A: nonneg(a1*a2) via nonneg_mul_closed ───
    lemma_dts_nonneg_mul_closed_fuel(a1, a2, f);

    //  ─── Step B: nonneg(dd*(b1*b2)) ───
    //  First: nonneg(neg(b1)*neg(b2)) via nonneg_mul_closed
    ...
    //  neg_mul_neg: neg(b1)*neg(b2) ≡ b1*b2
    lemma_dts_neg_mul_neg(b1, b2);
    //  Transfer nonneg via congruence
    lemma_dts_mul_closed(dts_neg(b1), dts_neg(b2));
    //  sr(neg(b1)*neg(b2), b1*b2): both ~ b1
    lemma_dts_same_radicand_symmetric(
        dts_neg(b1), dts_mul(dts_neg(b1), dts_neg(b2)));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_neg(b1), dts_neg(b2)), dts_neg(b1), b1);
    lemma_dts_same_radicand_symmetric(b1, b12);
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_neg(b1), dts_neg(b2)), b1, b12);
    //  Need norm_definite + nonneg_radicands + depth for the new product
    lemma_dts_nonneg_radicands_mul(dts_neg(b1), dts_neg(b2));
    lemma_norm_definite_mul(dts_neg(b1), dts_neg(b2));
    lemma_dts_depth_mul_le(dts_neg(b1), dts_neg(b2));
    lemma_dts_nonneg_fuel_congruence(
        dts_mul(dts_neg(b1), dts_neg(b2)), b12, f);
    //  Now: nonneg(b1*b2) at fuel f
    //  Apply nonneg_mul_closed_fuel(dd, b1*b2, f) — needs nonneg(dd, f)
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b12, f);
    //  Now: nonneg(dd*(b1*b2)) at fuel f

    //  ─── Step C: cauchy_cross_term → nonneg(sub(a12, db12)) = nonneg(cross) ───
    lemma_dts_cauchy_cross_term(a1, b1, a2, b2, dd, f);

    //  ─── Step D: norm_sum_decomposition → eqv(N(sum), nx + (ny + (cross + cross))) ───
    lemma_dts_norm_sum_decomposition(a1, b1, a2, b2, dd);

    //  ─── Step E: nonneg of decomposition ───
    //  nonneg(nx) given. nonneg(ny) given. nonneg(cross) from Step C.
    //  nonneg(cross + cross) via nonneg_add
    //  sr(cross, cross) reflexive
    lemma_dts_same_radicand_reflexive(cross);
    //  cross = sub(a12, db12). Need wf, nonneg_radicands, norm_definite, depth.
    lemma_dts_neg_well_formed(db12);
    lemma_dts_same_radicand_neg(db12);
    lemma_dts_same_radicand_symmetric(db12, dts_neg(db12));
    //  sr(a1, db12) via a1 ~ dd ~ db12
    lemma_dts_same_radicand_transitive(a1, dd, db12);
    //  sr(a12, db12): both ~ a1
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_transitive(a12, a1, db12);
    lemma_dts_same_radicand_transitive(a12, db12, dts_neg(db12));
    lemma_dts_add_closed(a12, dts_neg(db12));
    lemma_dts_nonneg_radicands_neg(db12);
    lemma_dts_nonneg_radicands_add(a12, dts_neg(db12));
    lemma_norm_definite_neg(db12);
    lemma_norm_definite_add(a12, dts_neg(db12));
    lemma_dts_depth_neg(db12);
    lemma_dts_depth_add_le(a12, dts_neg(db12));
    //  Now nonneg(cross + cross) via nonneg_add
    lemma_dts_nonneg_add_closed_fuel(cross, cross, f);
    //  sr(ny, cross + cross): both ~ a1
    //  ny = sub(aa2, dbb2). Setup wf, etc.
    lemma_dts_neg_well_formed(dbb2);
    lemma_dts_same_radicand_neg(dbb2);
    lemma_dts_same_radicand_symmetric(dbb2, dts_neg(dbb2));
    //  sr(a1, aa2) via a1 ~ a2 ~ aa2
    lemma_dts_same_radicand_symmetric(a2, aa2);
    lemma_dts_same_radicand_transitive(a1, a2, aa2);
    //  sr(a1, dbb2) via a1 ~ dd ~ dbb2
    lemma_dts_same_radicand_transitive(a1, dd, dbb2);
    lemma_dts_same_radicand_symmetric(a1, aa2);
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(aa2, a1, dbb2);
    lemma_dts_same_radicand_transitive(aa2, dbb2, dts_neg(dbb2));
    lemma_dts_add_closed(aa2, dts_neg(dbb2));
    lemma_dts_nonneg_radicands_neg(dbb2);
    lemma_dts_nonneg_radicands_add(aa2, dts_neg(dbb2));
    lemma_norm_definite_neg(dbb2);
    lemma_norm_definite_add(aa2, dts_neg(dbb2));
    lemma_dts_depth_neg(dbb2);
    lemma_dts_depth_add_le(aa2, dts_neg(dbb2));
    //  cross+cross also needs the same
    lemma_dts_add_closed(cross, cross);
    //  sr(ny, cross+cross): need to chain
    //  ny ~ aa2 ~ a1 ~ a12 ~ cross ~ cross+cross
    lemma_dts_same_radicand_symmetric(aa2, ny);
    lemma_dts_same_radicand_transitive(ny, aa2, a1);
    lemma_dts_same_radicand_transitive(a1, a12, cross);
    //  sr(a12, cross) needs sym
    lemma_dts_same_radicand_symmetric(a12, cross);
    lemma_dts_same_radicand_transitive(a1, a12, cross);
    lemma_dts_same_radicand_symmetric(a1, cross);
    lemma_dts_same_radicand_symmetric(cross, dts_add(cross, cross));
    lemma_dts_same_radicand_transitive(a1, cross, dts_add(cross, cross));
    lemma_dts_same_radicand_transitive(ny, a1, dts_add(cross, cross));
    //  Now nonneg(ny + (cross + cross)) via nonneg_add
    //  Need norm_definite for ny + (cross+cross)
    lemma_dts_nonneg_radicands_add(cross, cross);
    lemma_norm_definite_add(cross, cross);
    lemma_dts_depth_add_le(cross, cross);
    lemma_dts_nonneg_add_closed_fuel(ny, dts_add(cross, cross), f);
    //  Now nonneg(nx + (ny + (cross+cross)))
    //  Setup nx
    lemma_dts_neg_well_formed(dbb1);
    lemma_dts_same_radicand_neg(dbb1);
    lemma_dts_same_radicand_symmetric(dbb1, dts_neg(dbb1));
    //  sr(a1, aa1) via a1 ~ aa1 from mul_closed (already have via sym above? No — mul_closed gives sr(a1, aa1))
    //  Need sr(a1, dbb1) via a1 ~ dd ~ dbb1
    lemma_dts_same_radicand_transitive(a1, dd, dbb1);
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    ...
    //  sr(nx, ny + (cross+cross))
    //  nx ~ aa1 ~ a1 ~ ny ~ ny+(cross+cross)
    lemma_dts_same_radicand_symmetric(aa1, nx);
    lemma_dts_same_radicand_transitive(nx, aa1, a1);
    //  sr(a1, ny) via a1 ~ aa2 ~ ny
    lemma_dts_same_radicand_symmetric(aa2, ny);
    lemma_dts_same_radicand_transitive(a1, aa2, ny);
    lemma_dts_same_radicand_symmetric(a1, ny);
    lemma_dts_same_radicand_transitive(nx, a1, ny);
    lemma_dts_add_closed(ny, dts_add(cross, cross));
    lemma_dts_same_radicand_symmetric(ny, dts_add(ny, dts_add(cross, cross)));
    lemma_dts_same_radicand_transitive(
        nx, ny, dts_add(ny, dts_add(cross, cross)));
    //  Need norm_definite for ny + (cross+cross)
    lemma_dts_nonneg_radicands_add(ny, dts_add(cross, cross));
    lemma_norm_definite_add(ny, dts_add(cross, cross));
    lemma_dts_depth_add_le(ny, dts_add(cross, cross));
    lemma_dts_nonneg_add_closed_fuel(
        nx, dts_add(ny, dts_add(cross, cross)), f);
    //  Now: nonneg(nx + (ny + (cross+cross)))

    //  ─── Step F: nonneg_fuel_congruence to transfer to N(sum) ───
    //  norm_sum_decomposition gives:
    //  eqv(sub(ssa², dd*ssb²), nx + (ny + (cross+cross)))
    //  Symmetric for the direction we need
    lemma_dts_eqv_symmetric(
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))),
        dts_add(nx, dts_add(ny, dts_add(cross, cross))));
    //  Need sr(nx + (ny + (cross+cross)), sub(ssa², dd*ssb²))
    //  Both ~ a1
    //  Build: sub(ssa², dd*ssb²) ~ a1
    //  sr(ssa, ssa): reflexive needed for mul_closed
    lemma_dts_same_radicand_reflexive(ssa);
    lemma_dts_mul_closed(ssa, ssa);
    lemma_dts_same_radicand_symmetric(a1, ssa);
    lemma_dts_same_radicand_symmetric(ssa, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(a1, ssa, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_reflexive(ssb);
    lemma_dts_mul_closed(ssb, ssb);
    lemma_dts_same_radicand_transitive(a1, b1, ssb);
    lemma_dts_same_radicand_symmetric(b1, ssb);
    lemma_dts_same_radicand_symmetric(ssb, dts_mul(ssb, ssb));
    lemma_dts_same_radicand_transitive(a1, ssb, dts_mul(ssb, ssb));
    lemma_dts_same_radicand_transitive(dd, a1, dts_mul(ssb, ssb));
    lemma_dts_mul_closed(dd, dts_mul(ssb, ssb));
    //  sr(a1, dd*ssb²): a1~dd~dd*ssb² (from mul_closed)
    lemma_dts_same_radicand_transitive(a1, dd, dts_mul(dd, dts_mul(ssb, ssb)));
    //  sub(ssa², dd*ssb²) = add(ssa², neg(dd*ssb²))
    lemma_dts_neg_well_formed(dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_neg(dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_symmetric(
        dts_mul(dd, dts_mul(ssb, ssb)), dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_transitive(
        dts_mul(ssa, ssa), a1, dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_transitive(
        dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb)),
        dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    lemma_dts_add_closed(
        dts_mul(ssa, ssa), dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    lemma_dts_same_radicand_symmetric(
        dts_mul(ssa, ssa),
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))));
    //  sr(sub(ssa², dd*ssb²), a1)
    lemma_dts_same_radicand_symmetric(a1, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))),
        dts_mul(ssa, ssa), a1);
    //  sr(nx+(ny+(cross+cross)), a1): nx+... ~ nx (from add_closed) ~ a1
    //  Need add_closed(nx, ny+(cross+cross)) for sr(nx, sum)
    lemma_dts_add_closed(nx, dts_add(ny, dts_add(cross, cross)));
    lemma_dts_same_radicand_symmetric(
        nx, dts_add(nx, dts_add(ny, dts_add(cross, cross))));
    //  sr(a1, nx) via a1 ~ aa1 ~ nx
    lemma_dts_same_radicand_symmetric(a1, aa1);  //  sr(aa1, a1)
    lemma_dts_same_radicand_symmetric(aa1, nx);  //  sr(nx, aa1) (already have but harmless)
    lemma_dts_same_radicand_transitive(nx, aa1, a1);
    lemma_dts_same_radicand_symmetric(nx, a1);
    lemma_dts_same_radicand_transitive(
        dts_add(nx, dts_add(ny, dts_add(cross, cross))), nx, a1);
    //  Combine: sr(nx+(ny+(cross+cross)), sub(ssa², dd*ssb²))
    lemma_dts_same_radicand_symmetric(
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))), a1);
    lemma_dts_same_radicand_transitive(
        dts_add(nx, dts_add(ny, dts_add(cross, cross))),
        a1,
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))));
    //  norm_definite + nonneg_radicands + depth for the target sub
    lemma_dts_nonneg_radicands_add(a1, a2);  //  ssa nonneg_radicands
    lemma_dts_nonneg_radicands_add(b1, b2);  //  ssb nonneg_radicands
    lemma_norm_definite_add(a1, a2);  //  ssa norm_definite
    lemma_norm_definite_add(b1, b2);  //  ssb norm_definite
    lemma_dts_depth_add_le(a1, a2);
    lemma_dts_depth_add_le(b1, b2);
    lemma_dts_nonneg_radicands_mul(ssa, ssa);
    lemma_dts_nonneg_radicands_mul(ssb, ssb);
    lemma_dts_nonneg_radicands_mul(dd, dts_mul(ssb, ssb));
    lemma_dts_nonneg_radicands_neg(dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_nonneg_radicands_add(
        dts_mul(ssa, ssa), dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    lemma_norm_definite_mul(ssa, ssa);
    lemma_norm_definite_mul(ssb, ssb);
    lemma_norm_definite_mul(dd, dts_mul(ssb, ssb));
    lemma_norm_definite_neg(dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_norm_definite_add(
        dts_mul(ssa, ssa), dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    lemma_dts_depth_mul_le(ssa, ssa);
    lemma_dts_depth_mul_le(ssb, ssb);
    lemma_dts_depth_mul_le(dd, dts_mul(ssb, ssb));
    lemma_dts_depth_neg(dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_depth_add_le(
        dts_mul(ssa, ssa), dts_neg(dts_mul(dd, dts_mul(ssb, ssb))));
    //  Apply nonneg_fuel_congruence
    lemma_dts_nonneg_fuel_congruence(
        ...
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))),
        f);
    //  Result: nonneg(sub(ssa², dd*ssb²)) at fuel f ✓
}

///  C3+C3 neg-norm bound: from both x and y being C3, derive nonneg of sum's NEG norm.
///  Mirrors c2c2_norm_bound but uses cauchy_cross_term_neg + negated decomposition.
///  Output: nonneg(sub(d*ssb², ssa²)) which is the C3 form of "neg norm ≥ 0".
#[verifier::rlimit(1000)]
proof fn lemma_dts_c3c3_neg_norm_bound<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_norm_definite(a1), dts_norm_definite(b1),
        dts_norm_definite(a2), dts_norm_definite(b2), dts_norm_definite(dd),
        //  C3 facts for x: neg(a1) ≥ 0, b1 ≥ 0, neg_norm ≥ 0 (i.e. d*b1² ≥ a1²)
        dts_nonneg_fuel(dts_neg(a1), f),
        dts_nonneg_fuel(b1, f),
        dts_nonneg_fuel(
            dts_sub(dts_mul(dd, dts_mul(b1, b1)), dts_mul(a1, a1)), f),
        //  C3 facts for y
    ...
    decreases f, 5nat,
{
    ...
    let b12 = dts_mul(b1, b2);
    let db12 = dts_mul(dd, b12);
    //  "neg-norm" form: nx_n = sub(dbb1, aa1), ny_n = sub(dbb2, aa2),
    //  cross_n = sub(db12, a12) (negated cross)
    let nx_n = dts_sub(dbb1, aa1);
    let ny_n = dts_sub(dbb2, aa2);
    let cross_n = dts_sub(db12, a12);
    let ssa = dts_add(a1, a2);
    let ssb = dts_add(b1, b2);

    //  ─── sr/wf/nonneg_radicands/norm_definite infrastructure (same as c2c2) ───
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_reflexive(b2);
    ...
    //  ─── Step A: nonneg(a1*a2) via neg_mul_neg ───
    //  neg(a1) ≥ 0, neg(a2) ≥ 0 → neg(a1)*neg(a2) ≥ 0 → a1*a2 ≥ 0 via neg_mul_neg
    ...
    //  Transfer nonneg via congruence
    //  sr(neg(a1)*neg(a2), a1*a2): both ~ a1
    lemma_dts_same_radicand_symmetric(
        dts_neg(a1), dts_mul(dts_neg(a1), dts_neg(a2)));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_neg(a1), dts_neg(a2)), dts_neg(a1), a1);
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_neg(a1), dts_neg(a2)), a1, a12);
    lemma_dts_nonneg_radicands_mul(dts_neg(a1), dts_neg(a2));
    lemma_norm_definite_mul(dts_neg(a1), dts_neg(a2));
    lemma_dts_depth_mul_le(dts_neg(a1), dts_neg(a2));
    lemma_dts_nonneg_fuel_congruence(
        dts_mul(dts_neg(a1), dts_neg(a2)), a12, f);
    //  Now: nonneg(a12) at fuel f

    //  ─── Step B: nonneg(dd*(b1*b2)) via nonneg_mul_closed (b's nonneg directly) ───
    lemma_dts_nonneg_mul_closed_fuel(b1, b2, f);
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    lemma_dts_nonneg_mul_closed_fuel(dd, b12, f);
    //  Now: nonneg(db12) at fuel f

    //  ─── Step C: cauchy_cross_term_neg → nonneg(sub(db12, a12)) = nonneg(cross_n) ───
    lemma_dts_cauchy_cross_term_neg(a1, b1, a2, b2, dd, f);

    //  ─── Step D: norm_sum_decomposition gives N(sum) ≡ nx + (ny + (cross + cross)) ───
    //  We need to negate this to get sub(d*ssb², ssa²) ≡ nx_n + (ny_n + (cross_n + cross_n))
    lemma_dts_norm_sum_decomposition(a1, b1, a2, b2, dd);
    let nx = dts_sub(aa1, dbb1);
    let ny = dts_sub(aa2, dbb2);
    let cross = dts_sub(a12, db12);
    let decomp = dts_add(nx, dts_add(ny, dts_add(cross, cross)));
    let neg_decomp = dts_add(nx_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    let n_sum = dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb)));
    let target = dts_sub(
        dts_mul(dd, dts_mul(ssb, ssb)),
        dts_mul(ssa, ssa));
    //  norm_sum_decomposition: eqv(n_sum, decomp)

    //  ─── Step E: nonneg of neg_decomp ───
    //  We have: nonneg(nx_n), nonneg(ny_n) [given], nonneg(cross_n) [from step C].
    //  Build nonneg(cross_n + cross_n), then nonneg(ny_n + (cross_n + cross_n)),
    //  then nonneg(nx_n + (ny_n + (cross_n + cross_n))) = nonneg(neg_decomp)
    lemma_dts_same_radicand_reflexive(cross_n);
    //  cross_n = sub(db12, a12) = add(db12, neg(a12)). Need wf, sr, etc.
    lemma_dts_neg_well_formed(a12);
    lemma_dts_same_radicand_neg(a12);
    lemma_dts_same_radicand_symmetric(a12, dts_neg(a12));
    //  sr(db12, a12) via db12 ~ dd ~ a1 ~ a12
    lemma_dts_same_radicand_symmetric(dd, db12);
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_transitive(db12, dd, a1);
    lemma_dts_same_radicand_transitive(db12, a1, a12);
    lemma_dts_same_radicand_transitive(db12, a12, dts_neg(a12));
    lemma_dts_add_closed(db12, dts_neg(a12));
    lemma_dts_nonneg_radicands_neg(a12);
    lemma_dts_nonneg_radicands_add(db12, dts_neg(a12));
    lemma_norm_definite_neg(a12);
    lemma_norm_definite_add(db12, dts_neg(a12));
    lemma_dts_depth_neg(a12);
    lemma_dts_depth_add_le(db12, dts_neg(a12));
    //  Now nonneg(cross_n + cross_n)
    lemma_dts_nonneg_add_closed_fuel(cross_n, cross_n, f);

    //  Setup ny_n = sub(dbb2, aa2)
    lemma_dts_neg_well_formed(aa2);
    lemma_dts_same_radicand_neg(aa2);
    lemma_dts_same_radicand_symmetric(aa2, dts_neg(aa2));
    //  sr(dbb2, aa2) via dbb2 ~ dd ~ a1 ~ a2 ~ aa2
    lemma_dts_same_radicand_symmetric(dd, dbb2);
    lemma_dts_same_radicand_transitive(dbb2, dd, a1);
    lemma_dts_same_radicand_transitive(dbb2, a1, a2);
    lemma_dts_same_radicand_symmetric(a2, aa2);
    lemma_dts_same_radicand_transitive(dbb2, a2, aa2);
    ...
    //  cross_n + cross_n closure
    lemma_dts_add_closed(cross_n, cross_n);
    //  sr(ny_n, cross_n + cross_n): chain through a1
    //  Build sr(a1, dbb2): a1 ~ dd ~ dbb2
    lemma_dts_same_radicand_symmetric(dd, dbb2);
    lemma_dts_same_radicand_transitive(a1, dd, dbb2);
    //  ny_n ~ dbb2 (from above sr) ~ a1 ~ db12 ~ cross_n ~ cross_n + cross_n
    lemma_dts_same_radicand_symmetric(dbb2, ny_n);
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(ny_n, dbb2, a1);
    //  Build sr(a1, db12): a1 ~ dd ~ db12
    lemma_dts_same_radicand_symmetric(dd, db12);
    lemma_dts_same_radicand_transitive(a1, dd, db12);
    //  a1 ~ cross_n: a1 ~ db12 (have) ~ cross_n
    lemma_dts_same_radicand_symmetric(db12, cross_n);
    lemma_dts_same_radicand_symmetric(a1, db12);
    lemma_dts_same_radicand_transitive(a1, db12, cross_n);
    //  cross_n ~ cross_n + cross_n
    lemma_dts_same_radicand_symmetric(cross_n, dts_add(cross_n, cross_n));
    lemma_dts_same_radicand_transitive(a1, cross_n, dts_add(cross_n, cross_n));
    lemma_dts_same_radicand_transitive(ny_n, a1, dts_add(cross_n, cross_n));
    //  norm_definite for ny_n + (cross_n+cross_n)
    lemma_dts_nonneg_radicands_add(cross_n, cross_n);
    lemma_norm_definite_add(cross_n, cross_n);
    lemma_dts_depth_add_le(cross_n, cross_n);
    //  Now nonneg(ny_n + (cross_n + cross_n))
    lemma_dts_nonneg_add_closed_fuel(ny_n, dts_add(cross_n, cross_n), f);

    //  Setup nx_n = sub(dbb1, aa1)
    lemma_dts_neg_well_formed(aa1);
    lemma_dts_same_radicand_neg(aa1);
    lemma_dts_same_radicand_symmetric(aa1, dts_neg(aa1));
    //  sr(dbb1, aa1) via dbb1 ~ dd ~ a1 ~ aa1
    lemma_dts_same_radicand_symmetric(dd, dbb1);
    lemma_dts_same_radicand_transitive(dbb1, dd, a1);
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_transitive(dbb1, a1, aa1);
    lemma_dts_same_radicand_transitive(dbb1, aa1, dts_neg(aa1));
    lemma_dts_add_closed(dbb1, dts_neg(aa1));
    lemma_dts_nonneg_radicands_neg(aa1);
    lemma_dts_nonneg_radicands_add(dbb1, dts_neg(aa1));
    lemma_norm_definite_neg(aa1);
    lemma_norm_definite_add(dbb1, dts_neg(aa1));
    lemma_dts_depth_neg(aa1);
    lemma_dts_depth_add_le(dbb1, dts_neg(aa1));
    //  sr(nx_n, ny_n + (cross_n + cross_n)): both ~ a1
    //  nx_n ~ dbb1 (sym) ~ a1 (have)
    lemma_dts_same_radicand_symmetric(dbb1, nx_n);
    lemma_dts_same_radicand_transitive(nx_n, dbb1, a1);
    //  ny_n + ... ~ ny_n (from add_closed sym)
    lemma_dts_add_closed(ny_n, dts_add(cross_n, cross_n));
    lemma_dts_same_radicand_symmetric(ny_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    //  nx_n ~ a1 ~ ny_n (from chain ny_n ~ a1 above) ~ ny_n+...
    lemma_dts_same_radicand_symmetric(ny_n, a1);
    lemma_dts_same_radicand_transitive(nx_n, a1, ny_n);
    lemma_dts_same_radicand_transitive(
        nx_n, ny_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    //  norm_definite for the whole thing
    lemma_dts_nonneg_radicands_add(ny_n, dts_add(cross_n, cross_n));
    lemma_norm_definite_add(ny_n, dts_add(cross_n, cross_n));
    lemma_dts_depth_add_le(ny_n, dts_add(cross_n, cross_n));
    lemma_dts_nonneg_add_closed_fuel(
        nx_n, dts_add(ny_n, dts_add(cross_n, cross_n)), f);
    //  Now: nonneg(neg_decomp) at fuel f

    //  ─── Step F: Build the eqv chain: target ≡ neg(n_sum) ≡ neg(decomp) ≡ neg_decomp ───
    //  F1: target = sub(d*ssb², ssa²) ≡ neg(sub(ssa², d*ssb²)) = neg(n_sum)  via neg_sub_swap
    //  Need wf, sr for ssa², d*ssb²
    lemma_dts_same_radicand_reflexive(ssa);
    lemma_dts_mul_closed(ssa, ssa);
    lemma_dts_same_radicand_reflexive(ssb);
    lemma_dts_mul_closed(ssb, ssb);
    //  sr(dd, ssb²) via dd ~ b1 ~ ssb ~ ssb²
    lemma_dts_same_radicand_symmetric(b1, ssb);
    lemma_dts_same_radicand_transitive(dd, b1, ssb);
    lemma_dts_same_radicand_symmetric(ssb, dts_mul(ssb, ssb));
    lemma_dts_same_radicand_transitive(dd, ssb, dts_mul(ssb, ssb));
    lemma_dts_mul_closed(dd, dts_mul(ssb, ssb));
    lemma_dts_neg_sub_swap(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb)));
    //  ensures eqv(neg(sub(ssa², d*ssb²)), sub(d*ssb², ssa²)) = eqv(neg(n_sum), target)
    //  Symmetric:
    lemma_dts_eqv_symmetric(dts_neg(n_sum), target);
    //  Now: eqv(target, neg(n_sum))

    //  F2: neg(n_sum) ≡ neg(decomp) via neg_congruence (norm_sum_decomposition gives eqv(n_sum, decomp))
    //  We need a "neg_congruence" lemma. Use add_inverse: x ≡ y → neg(x) ≡ neg(y).
    //  Approach: apply neg to both sides of n_sum ≡ decomp.
    //  Alternative: use lemma_dts_neg_congruence or directly via add_congruence_left/right with neutral.
    //  Easiest: build it via neg_well_formed + add_zero.
    //  Actually, let's just use add_congruence on neg-form.
    //  Use: neg(x) ≡ sub(zero, x) = add(zero, neg(x)) — we have neg as primitive.
    //  Standard: `lemma_dts_neg_congruence(a, b)` if it exists.
    //  Try a search-based approach: use add_inverse_left/right.
    //  For now, build neg(n_sum) ≡ neg(decomp) via:
    //    eqv(n_sum, decomp) → eqv(add(neg(n_sum), n_sum), add(neg(n_sum), decomp))
    //                        → eqv(zero, add(neg(n_sum), decomp))
    //                        → eqv(neg(n_sum), neg(decomp)) [hmm tricky]
    //  Easier: use that DTS forms an additive group; there should be a neg_congruence lemma.
    lemma_dts_neg_congruence(n_sum, decomp);
    //  → eqv(neg(n_sum), neg(decomp))

    //  F3: neg(decomp) ≡ neg_decomp via 3× neg_add chain
    //  decomp = add(nx, add(ny, add(cross, cross)))
    //  neg(decomp) = neg(add(nx, add(ny, add(cross, cross))))
    //              ≡ add(neg(nx), neg(add(ny, add(cross, cross))))     [neg_add]
    //              ≡ add(neg(nx), add(neg(ny), neg(add(cross, cross)))) [neg_add inside]
    //              ≡ add(neg(nx), add(neg(ny), add(neg(cross), neg(cross)))) [neg_add inside]
    //  And nx_n = sub(dbb1, aa1) = neg(nx) via neg_sub_swap (where nx = sub(aa1, dbb1)).
    //  Similarly ny_n = neg(ny), cross_n = neg(cross).

    //  ─── Build wf/sr for C2-form quantities nx, ny, cross ───
    //  Build sr(a1, dbb1) via a1 ~ dd ~ dbb1
    lemma_dts_same_radicand_symmetric(dd, dbb1);
    lemma_dts_same_radicand_transitive(a1, dd, dbb1);
    //  Build sr(a1, dbb2) via a1 ~ dd ~ dbb2 (already done above? rebuild for safety)
    //  Already built earlier: sr(a1, dbb2) at the ny_n setup
    //  Build sr(a1, db12) via a1 ~ dd ~ db12 (already done above)
    //  Build sr(a1, aa1) via mul_closed
    //  Already have sr(a1, aa1) from mul_closed above
    //  Build sr(a1, aa2) via a1 ~ a2 ~ aa2
    lemma_dts_same_radicand_transitive(a1, a2, aa2);
    //  Build sr(a1, a12) from mul_closed
    //  Already have

    //  nx = sub(aa1, dbb1) = add(aa1, neg(dbb1))
    lemma_dts_neg_well_formed(dbb1);
    lemma_dts_same_radicand_neg(dbb1);
    lemma_dts_same_radicand_symmetric(dbb1, dts_neg(dbb1));
    //  sr(aa1, dbb1): aa1 ~ a1 ~ dbb1
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(aa1, a1, dbb1);
    lemma_dts_same_radicand_transitive(aa1, dbb1, dts_neg(dbb1));
    lemma_dts_add_closed(aa1, dts_neg(dbb1));  //  wf(nx)
    //  ny = sub(aa2, dbb2) = add(aa2, neg(dbb2))
    lemma_dts_neg_well_formed(dbb2);
    lemma_dts_same_radicand_neg(dbb2);
    lemma_dts_same_radicand_symmetric(dbb2, dts_neg(dbb2));
    //  sr(aa2, dbb2): aa2 ~ a1 ~ dbb2
    lemma_dts_same_radicand_symmetric(a1, aa2);
    lemma_dts_same_radicand_symmetric(a1, dbb2);
    lemma_dts_same_radicand_transitive(aa2, a1, dbb2);
    lemma_dts_same_radicand_transitive(aa2, dbb2, dts_neg(dbb2));
    lemma_dts_add_closed(aa2, dts_neg(dbb2));  //  wf(ny)
    //  cross = sub(a12, db12) = add(a12, neg(db12))
    lemma_dts_neg_well_formed(db12);
    lemma_dts_same_radicand_neg(db12);
    lemma_dts_same_radicand_symmetric(db12, dts_neg(db12));
    //  sr(a12, db12): a12 ~ a1 ~ db12
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_symmetric(a1, db12);
    lemma_dts_same_radicand_transitive(a12, a1, db12);
    lemma_dts_same_radicand_transitive(a12, db12, dts_neg(db12));
    lemma_dts_add_closed(a12, dts_neg(db12));  //  wf(cross)

    lemma_dts_neg_sub_swap(aa1, dbb1);  //  ensures eqv(neg(sub(aa1,dbb1)), sub(dbb1,aa1)) = eqv(neg(nx), nx_n)
    lemma_dts_neg_sub_swap(aa2, dbb2);  //  eqv(neg(ny), ny_n)
    lemma_dts_neg_sub_swap(a12, db12);  //  eqv(neg(cross), cross_n)

    //  Build the inner add chains for the eqv chain
    //  Need: eqv(add(neg(cross), neg(cross)), add(cross_n, cross_n)) via add_congruence
    //  Already have eqv(neg(cross), cross_n).
    //  Need wf for the inner adds.
    lemma_dts_neg_well_formed(cross);
    lemma_dts_same_radicand_neg(cross);
    //  sr(neg(cross), cross_n): both well-formed, sr via cross
    //  neg(cross) ~ cross (same_radicand_neg)
    lemma_dts_same_radicand_symmetric(cross, dts_neg(cross));
    //  cross_n ~ db12 (sym above) ~ a1 ~ cross (need)
    //  Already have: sr(a1, cross) via earlier. Hmm, do we?
    //  cross = sub(a12, db12). We have sr(a1, a12), sr(a1, db12). For sr(a1, cross):
    //  cross ~ a12 (from add_closed sym)
    lemma_dts_same_radicand_symmetric(a12, cross);
    lemma_dts_same_radicand_transitive(a1, a12, cross);
    //  Build sr(a1, cross) via a1 ~ a12 ~ cross
    lemma_dts_same_radicand_symmetric(a12, cross);
    lemma_dts_same_radicand_transitive(a1, a12, cross);
    lemma_dts_same_radicand_symmetric(a1, cross);
    //  Build sr(cross_n, a1): cross_n ~ db12 (sym) ~ a1
    lemma_dts_same_radicand_symmetric(db12, cross_n);
    lemma_dts_same_radicand_symmetric(a1, db12);
    lemma_dts_same_radicand_transitive(cross_n, db12, a1);
    //  Now cross ~ neg(cross) via same_radicand_neg, and cross ~ cross_n via sub-relation
    //  cross_n ~ a1 (have via db12) ~ cross
    lemma_dts_same_radicand_transitive(cross_n, a1, cross);
    lemma_dts_same_radicand_symmetric(cross_n, cross);
    lemma_dts_same_radicand_transitive(dts_neg(cross), cross, cross_n);
    //  Apply add_congruence_left + add_congruence_right + transitive to get
    //  eqv(add(neg(cross), neg(cross)), add(cross_n, cross_n))
    lemma_dts_add_congruence_left(dts_neg(cross), cross_n, dts_neg(cross));
    //  → eqv(add(neg(cross), neg(cross)), add(cross_n, neg(cross)))
    lemma_dts_add_congruence_right(cross_n, dts_neg(cross), cross_n);
    //  → eqv(add(cross_n, neg(cross)), add(cross_n, cross_n))
    lemma_dts_eqv_transitive(
        dts_add(dts_neg(cross), dts_neg(cross)),
        dts_add(cross_n, dts_neg(cross)),
        dts_add(cross_n, cross_n));
    //  → eqv(add(neg(cross), neg(cross)), add(cross_n, cross_n))

    //  Need: eqv(neg(add(cross, cross)), add(neg(cross), neg(cross))) via neg_add
    lemma_dts_neg_add(cross, cross);

    //  Chain: neg(add(cross,cross)) ≡ add(neg(cross), neg(cross)) ≡ add(cross_n, cross_n)
    lemma_dts_eqv_transitive(
        dts_neg(dts_add(cross, cross)),
        dts_add(dts_neg(cross), dts_neg(cross)),
        dts_add(cross_n, cross_n));

    //  Now lift: neg(add(ny, add(cross, cross))) ≡ ?
    //  Build sr for needed adds
    lemma_dts_same_radicand_reflexive(cross);
    lemma_dts_add_closed(cross, cross);
    lemma_dts_neg_well_formed(dts_add(cross, cross));
    //  sr(ny, add(cross, cross)) via both ~ a1
    //  ny = sub(aa2, dbb2). sr(a1, ny) via a1 ~ aa2 (a1~a2~aa2) ~ ny? Let's check.
    //  Hmm, ny ~ aa2 (from add_closed sym).
    lemma_dts_same_radicand_symmetric(aa2, ny);
    //  a1 ~ a2 (have, sym) ~ aa2 ~ ny
    lemma_dts_same_radicand_symmetric(a2, aa2);
    lemma_dts_same_radicand_transitive(a1, a2, aa2);
    lemma_dts_same_radicand_transitive(a1, aa2, ny);
    lemma_dts_same_radicand_symmetric(a1, ny);
    //  Build sr(a1, add(cross, cross)) via a1 ~ cross ~ add(cross, cross)
    lemma_dts_same_radicand_symmetric(cross, dts_add(cross, cross));
    lemma_dts_same_radicand_symmetric(a1, cross);
    lemma_dts_same_radicand_transitive(a1, cross, dts_add(cross, cross));
    //  add(cross, cross) ~ cross (sym) ~ a1
    lemma_dts_same_radicand_symmetric(a1, dts_add(cross, cross));
    lemma_dts_same_radicand_transitive(ny, a1, dts_add(cross, cross));
    lemma_dts_add_closed(ny, dts_add(cross, cross));
    //  neg_add: neg(add(ny, add(cross,cross))) ≡ add(neg(ny), neg(add(cross,cross)))
    lemma_dts_neg_add(ny, dts_add(cross, cross));

    //  Now: eqv(neg(add(cross,cross)), add(cross_n, cross_n)) [from above]
    //  We need eqv(add(neg(ny), neg(add(cross,cross))), add(ny_n, add(cross_n, cross_n)))
    //  Apply add_congruence: eqv(neg(ny), ny_n) AND eqv(neg(add(cross,cross)), add(cross_n,cross_n))
    //  eqv(neg(ny), ny_n) we already have from neg_sub_swap(aa2, dbb2)
    //  Need wf, sr for these
    lemma_dts_neg_well_formed(ny);
    lemma_dts_same_radicand_neg(ny);
    //  sr(neg(ny), ny_n): both ~ a1 (or via cross/ny chains)
    lemma_dts_same_radicand_symmetric(ny, dts_neg(ny));
    lemma_dts_same_radicand_transitive(dts_neg(ny), ny, a1);
    //  ny_n ~ dbb2 (sym) ~ a1
    lemma_dts_same_radicand_transitive(ny_n, dbb2, a1);
    lemma_dts_same_radicand_symmetric(ny_n, a1);
    lemma_dts_same_radicand_transitive(dts_neg(ny), a1, ny_n);
    //  sr for the second pair
    //  neg(add(cross,cross)) ~ add(cross,cross) ~ cross ~ a1
    lemma_dts_same_radicand_neg(dts_add(cross, cross));
    lemma_dts_same_radicand_symmetric(
        dts_add(cross, cross), dts_neg(dts_add(cross, cross)));
    lemma_dts_same_radicand_symmetric(a1, dts_add(cross, cross));
    lemma_dts_same_radicand_transitive(
        dts_neg(dts_add(cross, cross)), dts_add(cross, cross), a1);
    //  add(cross_n, cross_n) ~ cross_n ~ a1
    lemma_dts_same_radicand_symmetric(
        cross_n, dts_add(cross_n, cross_n));
    lemma_dts_same_radicand_symmetric(cross_n, a1);  //  Hmm, do we have sr(a1, cross_n)?
    //  cross_n ~ db12 ~ a1: yes from earlier
    //  Actually we have lemma_dts_same_radicand_symmetric(cross_n, cross) earlier.
    //  Let me derive sr(a1, cross_n) differently: a1 ~ db12 ~ cross_n
    lemma_dts_same_radicand_transitive(a1, db12, cross_n);
    lemma_dts_same_radicand_symmetric(a1, cross_n);
    lemma_dts_same_radicand_symmetric(cross_n, dts_add(cross_n, cross_n));
    lemma_dts_same_radicand_transitive(
        dts_neg(dts_add(cross, cross)), a1, dts_add(cross_n, cross_n));
    //  Apply add_congruence chain
    lemma_dts_add_congruence_left(
        dts_neg(ny), ny_n, dts_neg(dts_add(cross, cross)));
    //  → eqv(add(neg(ny), neg(add(cross,cross))), add(ny_n, neg(add(cross,cross))))
    lemma_dts_add_congruence_right(
        ny_n, dts_neg(dts_add(cross, cross)), dts_add(cross_n, cross_n));
    //  → eqv(add(ny_n, neg(add(cross,cross))), add(ny_n, add(cross_n, cross_n)))
    lemma_dts_eqv_transitive(
        dts_add(dts_neg(ny), dts_neg(dts_add(cross, cross))),
        dts_add(ny_n, dts_neg(dts_add(cross, cross))),
        dts_add(ny_n, dts_add(cross_n, cross_n)));
    //  → eqv(add(neg(ny), neg(add(cross,cross))), add(ny_n, add(cross_n, cross_n)))

    //  Chain: neg(add(ny, add(cross,cross))) ≡ add(neg(ny), neg(add(cross,cross)))
    //                                       ≡ add(ny_n, add(cross_n, cross_n))
    lemma_dts_eqv_transitive(
        dts_neg(dts_add(ny, dts_add(cross, cross))),
        dts_add(dts_neg(ny), dts_neg(dts_add(cross, cross))),
        dts_add(ny_n, dts_add(cross_n, cross_n)));

    //  Now lift to outer: neg(decomp) = neg(add(nx, add(ny, add(cross,cross))))
    //                              ≡ add(neg(nx), neg(add(ny, add(cross,cross)))) [neg_add]
    //                              ≡ add(nx_n, add(ny_n, add(cross_n, cross_n))) [add_congruence]
    //  Build wf/sr for outer
    //  sr(nx, add(ny, add(cross, cross))): both ~ a1
    //  nx ~ aa1 (sym) ~ a1
    lemma_dts_same_radicand_symmetric(aa1, nx);
    lemma_dts_same_radicand_symmetric(a1, aa1);  //  aa1 ~ a1 (sym of sr(a1, aa1))
    lemma_dts_same_radicand_transitive(nx, aa1, a1);
    //  add(ny, add(cross,cross)) ~ ny ~ a1
    lemma_dts_same_radicand_symmetric(ny, dts_add(ny, dts_add(cross, cross)));
    lemma_dts_same_radicand_symmetric(ny, a1);
    lemma_dts_same_radicand_transitive(
        dts_add(ny, dts_add(cross, cross)), ny, a1);
    lemma_dts_same_radicand_symmetric(
        dts_add(ny, dts_add(cross, cross)), a1);
    lemma_dts_same_radicand_transitive(
        nx, a1, dts_add(ny, dts_add(cross, cross)));
    lemma_dts_add_closed(nx, dts_add(ny, dts_add(cross, cross)));
    //  Now apply neg_add
    lemma_dts_neg_add(nx, dts_add(ny, dts_add(cross, cross)));
    //  → eqv(neg(add(nx, add(ny, add(cross,cross)))), add(neg(nx), neg(add(ny, add(cross,cross)))))

    //  Need: eqv(neg(nx), nx_n) — we have from neg_sub_swap(aa1, dbb1)
    //  Need: eqv(neg(add(ny, add(cross,cross))), add(ny_n, add(cross_n, cross_n))) — we have

    //  Apply add_congruence_both to get the outer equality
    lemma_dts_neg_well_formed(nx);
    lemma_dts_same_radicand_neg(nx);
    lemma_dts_same_radicand_symmetric(nx, dts_neg(nx));
    lemma_dts_same_radicand_transitive(dts_neg(nx), nx, a1);
    //  nx_n ~ a1 from earlier
    lemma_dts_same_radicand_symmetric(nx_n, a1);  //  Hmm, do we have sr(a1, nx_n)?
    //  nx_n ~ dbb1 ~ a1 from earlier chain
    lemma_dts_same_radicand_transitive(nx_n, dbb1, a1);
    lemma_dts_same_radicand_symmetric(nx_n, a1);
    lemma_dts_same_radicand_transitive(dts_neg(nx), a1, nx_n);
    //  sr for the second pair
    //  neg(add(ny, add(cross,cross))) ~ add(ny, add(cross,cross)) ~ a1
    lemma_dts_neg_well_formed(dts_add(ny, dts_add(cross, cross)));
    lemma_dts_same_radicand_neg(dts_add(ny, dts_add(cross, cross)));
    lemma_dts_same_radicand_symmetric(
        dts_add(ny, dts_add(cross, cross)),
        dts_neg(dts_add(ny, dts_add(cross, cross))));
    lemma_dts_same_radicand_transitive(
        dts_neg(dts_add(ny, dts_add(cross, cross))),
        dts_add(ny, dts_add(cross, cross)), a1);
    //  add(ny_n, add(cross_n, cross_n)) ~ ny_n ~ a1
    //  Need sr(a1, add(ny_n, add(cross_n, cross_n))). Build via a1 ~ ny_n ~ ...
    //  ny_n ~ a1 (from earlier), so sr(a1, ny_n) via sym
    //  ny_n ~ add(ny_n, add(cross_n, cross_n)) from add_closed; sym → other direction
    lemma_dts_same_radicand_symmetric(
        ny_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    //  We have sr(ny_n, a1) from earlier. Sym: sr(a1, ny_n).
    lemma_dts_same_radicand_symmetric(ny_n, a1);
    lemma_dts_same_radicand_transitive(
        a1, ny_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    lemma_dts_same_radicand_transitive(
        dts_neg(dts_add(ny, dts_add(cross, cross))), a1,
        dts_add(ny_n, dts_add(cross_n, cross_n)));
    //  Apply add_congruence chain
    lemma_dts_add_congruence_left(
        dts_neg(nx), nx_n, dts_neg(dts_add(ny, dts_add(cross, cross))));
    lemma_dts_add_congruence_right(
        nx_n,
        dts_neg(dts_add(ny, dts_add(cross, cross))),
        dts_add(ny_n, dts_add(cross_n, cross_n)));
    lemma_dts_eqv_transitive(
        dts_add(dts_neg(nx), dts_neg(dts_add(ny, dts_add(cross, cross)))),
        dts_add(nx_n, dts_neg(dts_add(ny, dts_add(cross, cross)))),
        dts_add(nx_n, dts_add(ny_n, dts_add(cross_n, cross_n))));
    //  → eqv(add(neg(nx), neg(add(ny, add(cross,cross)))),
    //         add(nx_n, add(ny_n, add(cross_n, cross_n))))
    //  Chain: neg(decomp) ≡ add(neg(nx), neg(...)) ≡ add(nx_n, add(ny_n, add(cross_n, cross_n))) = neg_decomp
    lemma_dts_eqv_transitive(
        dts_neg(decomp),
        dts_add(dts_neg(nx), dts_neg(dts_add(ny, dts_add(cross, cross)))),
        neg_decomp);
    //  Now: eqv(neg(decomp), neg_decomp)

    //  ─── Step G: Combine F1, F2, F3 into eqv(target, neg_decomp) ───
    //  F1: eqv(target, neg(n_sum))
    //  F2: eqv(neg(n_sum), neg(decomp))
    //  F3: eqv(neg(decomp), neg_decomp)
    //  Chain:
    lemma_dts_eqv_transitive(target, dts_neg(n_sum), dts_neg(decomp));
    lemma_dts_eqv_transitive(target, dts_neg(decomp), neg_decomp);
    //  Now: eqv(target, neg_decomp). Symmetric:
    lemma_dts_eqv_symmetric(target, neg_decomp);
    //  → eqv(neg_decomp, target)

    //  ─── Step H: nonneg_fuel_congruence to transfer nonneg(neg_decomp) → nonneg(target) ───
    //  Need wf, sr, etc for both sides.
    //  target = sub(d*ssb², ssa²). Already built closures above.
    //  nonneg_radicands + norm_definite + depth for target
    lemma_dts_nonneg_radicands_add(a1, a2);  //  ssa
    lemma_dts_nonneg_radicands_add(b1, b2);  //  ssb
    lemma_norm_definite_add(a1, a2);
    lemma_norm_definite_add(b1, b2);
    lemma_dts_depth_add_le(a1, a2);
    lemma_dts_depth_add_le(b1, b2);
    lemma_dts_nonneg_radicands_mul(ssa, ssa);
    lemma_dts_nonneg_radicands_mul(ssb, ssb);
    ...
    lemma_dts_depth_mul_le(ssb, ssb);
    lemma_dts_depth_mul_le(dd, dts_mul(ssb, ssb));
    //  target = sub(d*ssb², ssa²) = add(d*ssb², neg(ssa²))
    lemma_dts_neg_well_formed(dts_mul(ssa, ssa));
    lemma_dts_same_radicand_neg(dts_mul(ssa, ssa));
    lemma_dts_same_radicand_symmetric(
        dts_mul(ssa, ssa), dts_neg(dts_mul(ssa, ssa)));
    //  sr(d*ssb², ssa²): both ~ a1
    //  ssa ~ a1 (from sym above of sr(a1, ssa))
    lemma_dts_same_radicand_symmetric(a1, ssa);
    lemma_dts_same_radicand_symmetric(ssa, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(a1, ssa, dts_mul(ssa, ssa));
    //  d*ssb² ~ dd (from mul_closed sym) ~ a1
    lemma_dts_same_radicand_symmetric(dd, dts_mul(dd, dts_mul(ssb, ssb)));
    //  sr(dd, a1)
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_transitive(dts_mul(dd, dts_mul(ssb, ssb)), dd, a1);
    //  Now sym: sr(d*ssb², a1) → sr(a1, d*ssb²)
    lemma_dts_same_radicand_symmetric(dts_mul(dd, dts_mul(ssb, ssb)), a1);
    //  sr(ssa², a1): from sr(a1, ssa²) above (sym)
    lemma_dts_same_radicand_symmetric(a1, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(
        dts_mul(dd, dts_mul(ssb, ssb)), a1, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(
        dts_mul(dd, dts_mul(ssb, ssb)),
        dts_mul(ssa, ssa),
        dts_neg(dts_mul(ssa, ssa)));
    lemma_dts_add_closed(
        dts_mul(dd, dts_mul(ssb, ssb)), dts_neg(dts_mul(ssa, ssa)));
    lemma_dts_nonneg_radicands_neg(dts_mul(ssa, ssa));
    lemma_dts_nonneg_radicands_add(
        dts_mul(dd, dts_mul(ssb, ssb)), dts_neg(dts_mul(ssa, ssa)));
    lemma_norm_definite_neg(dts_mul(ssa, ssa));
    lemma_norm_definite_add(
        dts_mul(dd, dts_mul(ssb, ssb)), dts_neg(dts_mul(ssa, ssa)));
    lemma_dts_depth_neg(dts_mul(ssa, ssa));
    lemma_dts_depth_add_le(
        dts_mul(dd, dts_mul(ssb, ssb)), dts_neg(dts_mul(ssa, ssa)));
    //  sr(neg_decomp, target): both ~ a1
    //  neg_decomp = add(nx_n, add(ny_n, add(cross_n, cross_n))) ~ nx_n ~ a1
    lemma_dts_add_closed(nx_n, dts_add(ny_n, dts_add(cross_n, cross_n)));
    lemma_dts_same_radicand_symmetric(nx_n, neg_decomp);
    lemma_dts_same_radicand_transitive(neg_decomp, nx_n, a1);
    //  target = sub(d*ssb², ssa²). Need sr(target, a1).
    //  target ~ d*ssb² (from add_closed sym) ~ a1 (have)
    lemma_dts_same_radicand_symmetric(
        dts_mul(dd, dts_mul(ssb, ssb)), target);
    lemma_dts_same_radicand_transitive(
        target, dts_mul(dd, dts_mul(ssb, ssb)), a1);
    lemma_dts_same_radicand_symmetric(target, a1);
    lemma_dts_same_radicand_transitive(neg_decomp, a1, target);
    //  Apply nonneg_fuel_congruence
    lemma_dts_nonneg_fuel_congruence(neg_decomp, target, f);
    //  Result: nonneg(target) at fuel f ✓
}

///  C1+C3 neg-norm bound: from one factor C1 (a≥0, b≥0) and other C3 (a<0, b≥0, neg-norm),
///  with sum_re < 0 (Case 3), derive nonneg of sum's NEG norm.
///  Mirrors c1c2_norm_bound but with the chain reversed (T1: d*sum_im² ≥ d*s², T2: d*s² ≥ r²,
///  T3: r² ≥ sum_re²).
#[verifier::rlimit(1000)]
pub proof fn lemma_dts_c1c3_neg_norm_bound<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        //  Both b's nonneg
        dts_nonneg_fuel(b1, f), dts_nonneg_fuel(b2, f),
        //  One of a1/a2 nonneg, the other not (so its neg is nonneg via le_total)
        (dts_nonneg_fuel(a1, f) && !dts_nonneg_fuel(a2, f))
            || (!dts_nonneg_fuel(a1, f) && dts_nonneg_fuel(a2, f)),
        //  sum_re < 0 (Case 3) and !is_zero(sum_re)
        !dts_nonneg_fuel(dts_add(a1, a2), f),
        !dts_is_zero(dts_add(a1, a2)),
        //  C3 neg-norm bound from the C3 side
        dts_nonneg_fuel(a1, f) ==>
            dts_nonneg_fuel(dts_sub(dts_mul(dd, dts_mul(b2, b2)), dts_mul(a2, a2)), f),
        dts_nonneg_fuel(a2, f) ==>
            dts_nonneg_fuel(dts_sub(dts_mul(dd, dts_mul(b1, b1)), dts_mul(a1, a1)), f),
    ...
    decreases f, 1nat,
{
    ...
    //  where r is the C3 side's a (negative), s is the C3 side's b (positive).
    let a1_nn = dts_nonneg_fuel(a1, f);
    let r = if a1_nn { a2 } else { a1 };  //  C3's a
    let p = if a1_nn { a1 } else { a2 };  //  C1's a
    let s = if a1_nn { b2 } else { b1 };  //  C3's b
    let q = if a1_nn { b1 } else { b2 };  //  C1's b
    let sum_re = dts_add(a1, a2);
    let sum_im = dts_add(b1, b2);
    let r_sq = dts_mul(r, r);
    let s_sq = dts_mul(s, s);
    ...
    lemma_dts_same_radicand_transitive(a2, a1, sum_re);
    lemma_dts_same_radicand_symmetric(a1, sum_re);
    lemma_dts_same_radicand_transitive(b2, b1, sum_im);
    lemma_dts_same_radicand_symmetric(b1, sum_im);
    lemma_dts_depth_add_le(a1, a2);
    lemma_dts_depth_add_le(b1, b2);
    lemma_dts_nonneg_radicands_add(a1, a2);
    lemma_dts_nonneg_radicands_add(b1, b2);
    ...
    //  ═══ T1: d*sum_im² ≥ d*s² ═══
    //  sub(sum_im, s) ≡ q via add_then_sub_cancel
    if a1_nn {
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b1, b2);
        //  → eqv(sub(add(b1,b2), b2), b1)
        lemma_dts_eqv_symmetric(dts_sub(dts_add(b1, b2), b2), b1);
        //  Now: eqv(b1, sub(sum_im, s)) with s=b2, q=b1
    } else {
        lemma_dts_add_commutative(b1, b2);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b2, b1);
        lemma_dts_add_congruence_left(
            dts_add(b1, b2), dts_add(b2, b1), dts_neg(b1));
        lemma_dts_eqv_symmetric(dts_sub(dts_add(b2, b1), b1), b2);
        lemma_dts_eqv_symmetric(
            dts_sub(dts_add(b1, b2), b1), dts_sub(dts_add(b2, b1), b1));
        lemma_dts_eqv_transitive(
            b2, dts_sub(dts_add(b2, b1), b1), dts_sub(dts_add(b1, b2), b1));
    }
    //  Now: eqv(q, sub(sum_im, s))
    //  Setup wf/sr for sub(sum_im, s)
    lemma_dts_neg_well_formed(s);
    lemma_dts_same_radicand_neg(s);
    lemma_dts_same_radicand_symmetric(s, dts_neg(s));
    //  sr(sum_im, s): s ∈ {b1, b2}
    //  We have sr(b1, sum_im) from add_closed sym. Need sr(sum_im, b1) → sr(sum_im, s).
    lemma_dts_same_radicand_symmetric(b1, sum_im);
    //  sr(b1, s): if s=b2, sr(b1, b2) from earlier; if s=b1, refl
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_same_radicand_transitive(sum_im, b1, s);
    lemma_dts_same_radicand_transitive(sum_im, s, dts_neg(s));
    lemma_dts_add_closed(sum_im, dts_neg(s));
    lemma_dts_depth_neg(s);
    lemma_dts_depth_add_le(sum_im, dts_neg(s));
    lemma_dts_nonneg_radicands_neg(s);
    lemma_dts_nonneg_radicands_add(sum_im, dts_neg(s));
    lemma_norm_definite_neg(s);
    lemma_norm_definite_add(b1, b2);  //  norm_definite for sum_im
    lemma_norm_definite_add(sum_im, dts_neg(s));
    //  Transfer nonneg(q) → nonneg(sub(sum_im, s)) via congruence
    //  Build sr(q, sub(sum_im, s)): q ~ sum_im (from add_closed sym) ~ sub(sum_im, s)
    //  We have sr(sum_im, b1) and sr(sum_im, b2) — wait need to derive these per branch
    if a1_nn {
        //  q = b1, s = b2, eqv(b1, sub(add(b1,b2), b2)) from above
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b1, b2);
        lemma_dts_eqv_symmetric(dts_sub(dts_add(b1, b2), b2), b1);
        //  sr(b1, sub(sum_im, b2)): sub(sum_im, b2) ~ sum_im ~ b1
        lemma_dts_same_radicand_symmetric(
            sum_im, dts_sub(sum_im, b2));
        //  sr(sum_im, b1) from earlier sym
        lemma_dts_same_radicand_transitive(
            dts_sub(sum_im, b2), sum_im, b1);
        lemma_dts_same_radicand_symmetric(dts_sub(sum_im, b2), b1);
        lemma_dts_nonneg_fuel_congruence(b1, dts_sub(sum_im, b2), f);
    } else {
        lemma_dts_add_commutative(b1, b2);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(b2, b1);
        ...
        lemma_dts_eqv_transitive(
            b2, dts_sub(dts_add(b2, b1), b1), dts_sub(dts_add(b1, b2), b1));
        //  sr(b2, sub(sum_im, b1)): sub(sum_im, b1) ~ sum_im ~ b2
        lemma_dts_same_radicand_symmetric(
            sum_im, dts_sub(sum_im, b1));
        //  sr(sum_im, b2): we have sr(b1, sum_im); need b2~sum_im. b2~b1~sum_im.
        lemma_dts_same_radicand_symmetric(b1, b2);
        lemma_dts_same_radicand_symmetric(sum_im, b1);
        lemma_dts_same_radicand_transitive(b2, b1, sum_im);
        lemma_dts_same_radicand_symmetric(b2, sum_im);
        lemma_dts_same_radicand_transitive(
            dts_sub(sum_im, b1), sum_im, b2);
        lemma_dts_same_radicand_symmetric(dts_sub(sum_im, b1), b2);
        lemma_dts_nonneg_fuel_congruence(b2, dts_sub(sum_im, b1), f);
    }
    //  Now: nonneg(sub(sum_im, s)) at fuel f
    //  difference_of_squares(s, sum_im): eqv(sub(sum_im², s²), mul(sub(sum_im,s), add(sum_im,s)))
    lemma_dts_same_radicand_reflexive(s);
    lemma_dts_same_radicand_reflexive(sum_im);
    lemma_dts_same_radicand_symmetric(s, sum_im);
    lemma_dts_difference_of_squares(s, sum_im);
    //  nonneg(add(sum_im, s)): nonneg(sum_im) and nonneg(s) via nonneg_add
    lemma_dts_nonneg_add_closed_fuel(b1, b2, f);
    if a1_nn {
        lemma_dts_same_radicand_symmetric(b2, sum_im);
        lemma_norm_definite_add(sum_im, b2);
        lemma_dts_nonneg_add_closed_fuel(sum_im, b2, f);
    } else {
        lemma_dts_same_radicand_symmetric(b1, sum_im);
        lemma_norm_definite_add(sum_im, b1);
        lemma_dts_nonneg_add_closed_fuel(sum_im, b1, f);
    }
    //  nonneg_mul(sub(sum_im,s), add(sum_im,s))
    lemma_dts_add_closed(sum_im, s);
    lemma_dts_same_radicand_symmetric(sum_im, dts_add(sum_im, s));
    lemma_dts_same_radicand_transitive(
        dts_sub(sum_im, s), sum_im, dts_add(sum_im, s));
    lemma_dts_nonneg_radicands_add(sum_im, s);
    lemma_norm_definite_add(sum_im, dts_neg(s));
    lemma_norm_definite_add(sum_im, s);
    lemma_dts_depth_add_le(sum_im, s);
    lemma_dts_nonneg_mul_closed_fuel(
        dts_sub(sum_im, s), dts_add(sum_im, s), f);
    //  Transfer mul(sub, add) → sub(sum_im², s²)
    lemma_dts_mul_closed(sum_im, sum_im);
    lemma_dts_mul_closed(s, s);
    lemma_dts_mul_closed(dts_sub(sum_im, s), dts_add(sum_im, s));
    lemma_dts_same_radicand_symmetric(sum_im, sum_im_sq);
    lemma_dts_same_radicand_transitive(sum_im_sq, sum_im, s);
    lemma_dts_same_radicand_symmetric(s, s_sq);
    lemma_dts_same_radicand_transitive(sum_im_sq, s, s_sq);
    lemma_dts_neg_well_formed(s_sq);
    lemma_dts_same_radicand_neg(s_sq);
    lemma_dts_same_radicand_transitive(sum_im_sq, s_sq, dts_neg(s_sq));
    lemma_dts_add_closed(sum_im_sq, dts_neg(s_sq));
    lemma_dts_same_radicand_symmetric(
        dts_sub(sum_im, s), dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)),
        dts_sub(sum_im, s), sum_im);
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)),
        sum_im, sum_im_sq);
    lemma_dts_same_radicand_symmetric(sum_im_sq, dts_sub(sum_im_sq, s_sq));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)),
        sum_im_sq, dts_sub(sum_im_sq, s_sq));
    lemma_dts_eqv_symmetric(dts_sub(sum_im_sq, s_sq),
        dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)));
    lemma_dts_nonneg_fuel_congruence(
        dts_mul(dts_sub(sum_im, s), dts_add(sum_im, s)),
        dts_sub(sum_im_sq, s_sq), f);
    //  Now: nonneg(sub(sum_im², s²))
    //  Multiply by d (nonneg) to get nonneg(sub(d*sum_im², d*s²))
    lemma_dts_nonneg_radicands_mul(sum_im, sum_im);
    lemma_dts_nonneg_radicands_mul(s, s);
    lemma_dts_nonneg_radicands_neg(s_sq);
    lemma_dts_nonneg_radicands_add(sum_im_sq, dts_neg(s_sq));
    lemma_dts_depth_mul_le(sum_im, sum_im);
    lemma_dts_depth_mul_le(s, s);
    lemma_dts_depth_neg(s_sq);
    lemma_dts_depth_add_le(sum_im_sq, dts_neg(s_sq));
    lemma_norm_definite_mul(sum_im, sum_im);
    lemma_norm_definite_mul(s, s);
    lemma_norm_definite_neg(s_sq);
    lemma_norm_definite_add(sum_im_sq, dts_neg(s_sq));
    //  sr(dd, sub(sum_im², s²)): both ~ b1 ~ a1
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_transitive(dd, a1, b1);
    lemma_dts_same_radicand_transitive(dd, b1, sum_im);
    lemma_dts_same_radicand_transitive(dd, sum_im, sum_im_sq);
    lemma_dts_same_radicand_transitive(dd, sum_im_sq, dts_sub(sum_im_sq, s_sq));
    //  nonneg(dd) at fuel f
    lemma_dts_nonneg_fuel_stabilize(dd, f);
    //  nonneg_mul: dd * sub(sum_im², s²) ≥ 0
    lemma_dts_nonneg_radicands_mul(dd, dts_sub(sum_im_sq, s_sq));
    lemma_norm_definite_mul(dd, dts_sub(sum_im_sq, s_sq));
    lemma_dts_depth_mul_le(dd, dts_sub(sum_im_sq, s_sq));
    lemma_dts_nonneg_mul_closed_fuel(dd, dts_sub(sum_im_sq, s_sq), f);
    //  → nonneg(dd * sub(sum_im², s²))
    //  Convert to nonneg(sub(d*sum_im², d*s²)) via distribute + neg_mul_right
    lemma_dts_same_radicand_transitive(dd, sum_im, sum_im_sq);
    //  Build sr(dd, s) and sr(dd, s_sq): dd ~ a1 ~ b1 ~ s
    lemma_dts_same_radicand_transitive(dd, b1, s);
    lemma_dts_mul_closed(s, s);
    lemma_dts_same_radicand_symmetric(s, s_sq);
    lemma_dts_same_radicand_transitive(dd, s, s_sq);
    lemma_dts_mul_closed(dd, s_sq);
    lemma_dts_mul_distributes_left(dd, sum_im_sq, dts_neg(s_sq));
    lemma_dts_neg_mul_right(dd, s_sq);
    lemma_dts_add_congruence_right(
        dts_mul(dd, sum_im_sq), dts_mul(dd, dts_neg(s_sq)), dts_neg(d_s_sq));
    lemma_dts_eqv_transitive(
        dts_mul(dd, dts_sub(sum_im_sq, s_sq)),
        dts_add(dts_mul(dd, sum_im_sq), dts_mul(dd, dts_neg(s_sq))),
        dts_add(d_sum_im_sq, dts_neg(d_s_sq)));
    //  Transfer
    lemma_dts_mul_closed(dd, sum_im_sq);
    lemma_dts_mul_closed(dd, s_sq);
    lemma_dts_neg_well_formed(d_s_sq);
    lemma_dts_same_radicand_symmetric(dd, d_sum_im_sq);
    lemma_dts_same_radicand_symmetric(dd, d_s_sq);
    lemma_dts_same_radicand_transitive(d_sum_im_sq, dd, d_s_sq);
    lemma_dts_same_radicand_neg(d_s_sq);
    lemma_dts_same_radicand_transitive(d_sum_im_sq, d_s_sq, dts_neg(d_s_sq));
    lemma_dts_add_closed(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_dts_mul_closed(dd, dts_sub(sum_im_sq, s_sq));
    lemma_dts_same_radicand_symmetric(dd, dts_mul(dd, dts_sub(sum_im_sq, s_sq)));
    lemma_dts_same_radicand_transitive(
        dts_mul(dd, dts_sub(sum_im_sq, s_sq)), dd, d_sum_im_sq);
    lemma_dts_same_radicand_symmetric(d_sum_im_sq, dts_sub(d_sum_im_sq, d_s_sq));
    lemma_dts_same_radicand_transitive(
        dts_mul(dd, dts_sub(sum_im_sq, s_sq)),
        d_sum_im_sq, dts_sub(d_sum_im_sq, d_s_sq));
    lemma_dts_nonneg_radicands_mul(dd, sum_im_sq);
    lemma_dts_nonneg_radicands_mul(dd, s_sq);
    lemma_dts_nonneg_radicands_neg(d_s_sq);
    lemma_dts_nonneg_radicands_add(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_dts_depth_mul_le(dd, sum_im_sq);
    lemma_dts_depth_mul_le(dd, s_sq);
    lemma_dts_depth_neg(d_s_sq);
    lemma_dts_depth_add_le(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_norm_definite_mul(dd, sum_im_sq);
    lemma_norm_definite_mul(dd, s_sq);
    lemma_norm_definite_neg(d_s_sq);
    lemma_norm_definite_add(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_mul(dd, dts_sub(sum_im_sq, s_sq)),
        dts_sub(d_sum_im_sq, d_s_sq), f);
    //  T1 done: nonneg(sub(d_sum_im_sq, d_s_sq))

    //  ═══ T2: d*s² ≥ r² (from C3 neg-norm) ═══
    //  We have: dts_nonneg_fuel(sub(dd*b_C3*b_C3, a_C3*a_C3), f) for whichever side is C3.
    //  i.e., nonneg(sub(d_s_sq, r_sq))
    //  Setup wf/sr/etc for sub(d_s_sq, r_sq)
    lemma_dts_same_radicand_reflexive(r);
    //  Build sr(r, dd) first (depends on which branch). r ∈ {a1, a2}.
    if a1_nn {
        //  r = a2, need sr(a2, dd) via a2 ~ a1 ~ dd
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_transitive(a2, a1, dd);
    }
    //  Now sr(r, dd) for either branch
    lemma_dts_mul_closed(r, r);
    lemma_dts_neg_well_formed(r_sq);
    lemma_dts_same_radicand_neg(r_sq);
    lemma_dts_same_radicand_symmetric(r, r_sq);
    //  sr(r_sq, dd): r_sq ~ r ~ dd
    lemma_dts_same_radicand_transitive(r_sq, r, dd);
    //  sr(r_sq, d_s_sq): r_sq ~ dd ~ d_s_sq via mul_closed sym
    lemma_dts_same_radicand_symmetric(dd, d_s_sq);
    lemma_dts_same_radicand_transitive(r_sq, dd, d_s_sq);
    //  sr(d_s_sq, r_sq) via sym
    lemma_dts_same_radicand_symmetric(r_sq, d_s_sq);
    lemma_dts_same_radicand_symmetric(r_sq, dts_neg(r_sq));
    lemma_dts_same_radicand_transitive(d_s_sq, r_sq, dts_neg(r_sq));
    lemma_dts_add_closed(d_s_sq, dts_neg(r_sq));
    lemma_dts_nonneg_radicands_mul(r, r);
    lemma_dts_nonneg_radicands_neg(r_sq);
    lemma_dts_nonneg_radicands_add(d_s_sq, dts_neg(r_sq));
    lemma_dts_depth_mul_le(r, r);
    lemma_dts_depth_neg(r_sq);
    lemma_dts_depth_add_le(d_s_sq, dts_neg(r_sq));
    lemma_norm_definite_mul(r, r);
    lemma_norm_definite_neg(r_sq);
    lemma_norm_definite_add(d_s_sq, dts_neg(r_sq));
    //  T2: nonneg(sub(d_s_sq, r_sq)) — Z3 should see this from preconds
    assert(dts_nonneg_fuel(dts_sub(d_s_sq, r_sq), f));

    //  ═══ T3: r² ≥ sum_re² ═══
    //  r is negative, sum_re is negative. neg(r) ≥ 0, neg(sum_re) ≥ 0, neg(r) ≥ neg(sum_re).
    //  Use square_le_square on negs.
    let nr = dts_neg(r);
    let nsr = dts_neg(sum_re);
    lemma_dts_neg_well_formed(r);
    lemma_dts_neg_well_formed(sum_re);
    lemma_dts_same_radicand_neg(r);
    lemma_dts_same_radicand_neg(sum_re);
    lemma_dts_same_radicand_symmetric(r, nr);
    lemma_dts_same_radicand_symmetric(sum_re, nsr);
    //  sr(nr, sum_re) via nr ~ r ~ sum_re (r ~ a1 ~ sum_re)
    lemma_dts_same_radicand_transitive(nr, r, dd);
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_symmetric(dd, a1);
    lemma_dts_same_radicand_transitive(nr, dd, a1);
    //  sum_re ~ a1 (from sym above)
    lemma_dts_same_radicand_symmetric(sum_re, a1);
    lemma_dts_same_radicand_transitive(nr, a1, sum_re);
    lemma_dts_same_radicand_transitive(nr, sum_re, nsr);
    lemma_dts_nonneg_radicands_neg(r);
    lemma_dts_nonneg_radicands_neg(sum_re);
    lemma_dts_depth_neg(r);
    lemma_dts_depth_neg(sum_re);
    lemma_dts_nonneg_radicands_add(a1, a2);  //  for sum_re = add(a1, a2)
    lemma_norm_definite_add(a1, a2);  //  norm_definite for sum_re
    lemma_norm_definite_neg(r);
    lemma_norm_definite_neg(sum_re);

    //  Establish nonneg(nr) and nonneg(nsr) via le_total
    lemma_dts_nonneg_or_neg_nonneg_fuel(r, f);  //  !nonneg(r) → nonneg(neg(r)) = nonneg(nr)
    lemma_dts_nonneg_or_neg_nonneg_fuel(sum_re, f);  //  !nonneg(sum_re) → nonneg(nsr)

    //  Establish nonneg(sub(nr, nsr)) via congruence from nonneg(p)
    //  sub(nr, nsr) = -r - (-sum_re) = sum_re - r = (a_C1 + r) - r = a_C1 = p
    //  Algebraic chain:
    //  sub(nr, nsr) ≡ add(nr, neg(nsr)) ≡ add(neg(r), neg(neg(sum_re))) ≡ add(neg(r), sum_re) [neg_invol]
    //              ≡ add(sum_re, neg(r)) [commutative] = sub(sum_re, r) ≡ p [from cancel]
    //  First: sub(sum_re, r) ≡ p via add_then_sub_cancel
    if a1_nn {
        //  r = a2, p = a1, sum_re = a1+a2 = add(a1, a2)
        //  add_then_sub_cancel(a1, a2): eqv(sub(add(a1,a2), a2), a1)
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(a1, a2);
        //  → eqv(sub(sum_re, a2), a1) = eqv(sub(sum_re, r), p)
        lemma_dts_eqv_symmetric(dts_sub(sum_re, a2), a1);
    } else {
        //  r = a1, p = a2, sum_re = add(a1, a2)
        //  Need eqv(sub(sum_re, a1), a2). Use commutative + cancel.
        lemma_dts_add_commutative(a1, a2);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_then_sub_cancel::<
            DynTowerSpec<T>>(a2, a1);
        lemma_dts_add_congruence_left(
    ...
    //  Now: eqv(p, sub(sum_re, r))

    //  Build sub(nr, nsr) ≡ sub(sum_re, r) via neg_invol + commutative chain
    //  sub(nr, nsr) ≡ add(nr, neg(nsr)) ≡ add(neg(r), neg(neg(sum_re)))
    //  neg_invol: neg(neg(sum_re)) ≡ sum_re
    lemma_dts_neg_involution(sum_re);
    //  add_congruence_right: add(neg(r), neg(neg(sum_re))) ≡ add(neg(r), sum_re)
    lemma_dts_add_congruence_right(dts_neg(r), dts_neg(nsr), sum_re);
    //  add_commutative: add(neg(r), sum_re) ≡ add(sum_re, neg(r)) = sub(sum_re, r)
    lemma_dts_add_commutative(dts_neg(r), sum_re);
    //  Chain: sub(nr, nsr) ≡ add(neg(r), sum_re) ≡ sub(sum_re, r)
    lemma_dts_eqv_transitive(
        dts_sub(nr, nsr), dts_add(dts_neg(r), sum_re), dts_sub(sum_re, r));
    //  And eqv(sub(sum_re, r), p) from above (symmetric):
    lemma_dts_eqv_symmetric(dts_sub(sum_re, r), p);
    lemma_dts_eqv_symmetric(p, dts_sub(sum_re, r));
    lemma_dts_eqv_transitive(dts_sub(nr, nsr), dts_sub(sum_re, r), p);
    lemma_dts_eqv_symmetric(dts_sub(nr, nsr), p);
    //  Transfer nonneg(p) → nonneg(sub(nr, nsr))
    //  Setup wf/sr for sub(nr, nsr) and p
    lemma_dts_neg_well_formed(nsr);
    lemma_dts_same_radicand_neg(nsr);
    lemma_dts_same_radicand_symmetric(nsr, dts_neg(nsr));
    //  sr(nr, nsr): both ~ a1 (from chain above)
    lemma_dts_same_radicand_transitive(nr, nsr, dts_neg(nsr));
    lemma_dts_add_closed(nr, dts_neg(nsr));
    lemma_dts_nonneg_radicands_neg(nsr);
    lemma_dts_nonneg_radicands_add(nr, dts_neg(nsr));
    lemma_dts_depth_neg(nsr);
    lemma_dts_depth_add_le(nr, dts_neg(nsr));
    lemma_norm_definite_neg(nsr);
    lemma_norm_definite_add(nr, dts_neg(nsr));
    //  sr(p, sub(nr, nsr)): p ∈ {a1, a2}, sub(nr,nsr) ~ nr ~ a1
    lemma_dts_same_radicand_symmetric(nr, dts_sub(nr, nsr));
    lemma_dts_same_radicand_transitive(dts_sub(nr, nsr), nr, a1);
    //  Build sr(p, sub(nr, nsr)) directly
    if a1_nn {
        //  p = a1, sub(nr, nsr) ~ a1, sym
        lemma_dts_same_radicand_symmetric(dts_sub(nr, nsr), a1);
    } else {
        //  p = a2, sr(a2, sub(nr,nsr)) via a2 ~ a1 ~ sub(nr, nsr)
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_symmetric(dts_sub(nr, nsr), a1);
        lemma_dts_same_radicand_transitive(a2, a1, dts_sub(nr, nsr));
    }
    //  norm_definite for p
    if a1_nn {
        //  p = a1, have norm_definite(a1)
    } else {
        //  p = a2, have norm_definite(a2)
    }
    lemma_dts_nonneg_fuel_congruence(p, dts_sub(nr, nsr), f);
    //  Now: nonneg(sub(nr, nsr))

    //  square_le_square(nsr, nr) → nonneg(sub(nr², nsr²))
    let nr_sq = dts_mul(nr, nr);
    let nsr_sq = dts_mul(nsr, nsr);
    lemma_dts_same_radicand_reflexive(nr);
    lemma_dts_same_radicand_reflexive(nsr);
    //  Need sr(nsr, nr) for difference_of_squares. We have sr(nr, nsr).
    lemma_dts_same_radicand_symmetric(nr, nsr);
    lemma_dts_difference_of_squares(nsr, nr);
    //  nonneg(add(nr, nsr)) via nonneg_add (both nonneg)
    lemma_norm_definite_add(b1, b2);  //  used in next chain
    lemma_dts_nonneg_add_closed_fuel(nr, nsr, f);
    lemma_dts_add_closed(nr, nsr);
    lemma_dts_same_radicand_symmetric(nr, dts_add(nr, nsr));
    lemma_dts_same_radicand_symmetric(nr, dts_sub(nr, nsr));
    lemma_dts_same_radicand_transitive(dts_sub(nr, nsr), nr, dts_add(nr, nsr));
    lemma_dts_nonneg_radicands_add(nr, nsr);
    lemma_norm_definite_add(nr, nsr);
    lemma_dts_depth_add_le(nr, nsr);
    lemma_dts_nonneg_mul_closed_fuel(dts_sub(nr, nsr), dts_add(nr, nsr), f);
    //  Transfer mul(sub, add) → sub(nr², nsr²)
    lemma_dts_mul_closed(nr, nr);
    lemma_dts_mul_closed(nsr, nsr);
    lemma_dts_mul_closed(dts_sub(nr, nsr), dts_add(nr, nsr));
    lemma_dts_same_radicand_symmetric(nr, nr_sq);
    lemma_dts_same_radicand_transitive(nr_sq, nr, nsr);
    lemma_dts_same_radicand_symmetric(nsr, nsr_sq);
    lemma_dts_same_radicand_transitive(nr_sq, nsr, nsr_sq);
    lemma_dts_neg_well_formed(nsr_sq);
    lemma_dts_same_radicand_neg(nsr_sq);
    lemma_dts_same_radicand_transitive(nr_sq, nsr_sq, dts_neg(nsr_sq));
    lemma_dts_add_closed(nr_sq, dts_neg(nsr_sq));
    lemma_dts_same_radicand_symmetric(
        dts_sub(nr, nsr), dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)), dts_sub(nr, nsr), nr);
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)), nr, nr_sq);
    lemma_dts_same_radicand_symmetric(nr_sq, dts_sub(nr_sq, nsr_sq));
    lemma_dts_same_radicand_transitive(
        dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)), nr_sq, dts_sub(nr_sq, nsr_sq));
    lemma_dts_eqv_symmetric(dts_sub(nr_sq, nsr_sq),
        dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)));
    lemma_dts_nonneg_fuel_congruence(
        dts_mul(dts_sub(nr, nsr), dts_add(nr, nsr)), dts_sub(nr_sq, nsr_sq), f);
    //  Now: nonneg(sub(nr², nsr²))
    //  Transfer to sub(r², sum_re²) via neg_mul_neg: nr² ≡ r², nsr² ≡ sum_re²
    lemma_dts_same_radicand_reflexive(sum_re);
    lemma_dts_neg_mul_neg(r, r);
    lemma_dts_neg_mul_neg(sum_re, sum_re);
    lemma_dts_mul_closed(r, r);
    lemma_dts_mul_closed(sum_re, sum_re);
    lemma_dts_same_radicand_symmetric(nr, nr_sq);
    lemma_dts_same_radicand_transitive(nr_sq, nr, r);
    lemma_dts_same_radicand_symmetric(r, r_sq);
    lemma_dts_same_radicand_transitive(nr_sq, r, r_sq);
    lemma_dts_same_radicand_symmetric(nsr, nsr_sq);
    lemma_dts_same_radicand_transitive(nsr_sq, nsr, sum_re);
    lemma_dts_same_radicand_symmetric(sum_re, sum_re_sq);
    lemma_dts_same_radicand_transitive(nsr_sq, sum_re, sum_re_sq);
    lemma_dts_sub_congruence_both(nr_sq, nsr_sq, r_sq, sum_re_sq);
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(nr_sq, nsr_sq), dts_sub(r_sq, sum_re_sq), f);
    //  T3 done: nonneg(sub(r², sum_re²))

    //  ═══ Final chain: T1 + T2 + T3 ═══
    //  T1: nonneg(sub(d_sum_im_sq, d_s_sq))
    //  T2: nonneg(sub(d_s_sq, r_sq))
    //  T3: nonneg(sub(r_sq, sum_re_sq))
    //  Add T2 + T3: nonneg(sub(d_s_sq, sum_re_sq)) via sub_add_sub
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub::<DynTowerSpec<T>>(
        d_s_sq, r_sq, sum_re_sq);
    //  → eqv(add(sub(d_s_sq, r_sq), sub(r_sq, sum_re_sq)), sub(d_s_sq, sum_re_sq))
    lemma_dts_mul_closed(sum_re, sum_re);
    lemma_dts_neg_well_formed(sum_re_sq);
    lemma_dts_same_radicand_neg(sum_re_sq);
    //  sr(d_s_sq, sum_re_sq): both ~ a1 (via chain)
    lemma_dts_same_radicand_symmetric(dd, d_s_sq);
    lemma_dts_same_radicand_transitive(d_s_sq, dd, a1);
    lemma_dts_same_radicand_symmetric(sum_re, sum_re_sq);
    lemma_dts_same_radicand_transitive(sum_re, a1, sum_re);  //  trivial
    lemma_dts_same_radicand_symmetric(a1, sum_re);
    lemma_dts_same_radicand_transitive(sum_re_sq, sum_re, a1);
    lemma_dts_same_radicand_symmetric(sum_re_sq, a1);
    lemma_dts_same_radicand_transitive(d_s_sq, a1, sum_re_sq);
    lemma_dts_same_radicand_symmetric(sum_re_sq, dts_neg(sum_re_sq));
    //  Wait wrong direction. Need sym of sr(sum_re_sq, neg(sum_re_sq)).
    lemma_dts_same_radicand_transitive(d_s_sq, sum_re_sq, dts_neg(sum_re_sq));
    lemma_dts_add_closed(d_s_sq, dts_neg(sum_re_sq));
    lemma_dts_nonneg_radicands_mul(sum_re, sum_re);
    lemma_dts_nonneg_radicands_neg(sum_re_sq);
    lemma_dts_nonneg_radicands_add(d_s_sq, dts_neg(sum_re_sq));
    lemma_dts_depth_mul_le(sum_re, sum_re);
    lemma_dts_depth_neg(sum_re_sq);
    lemma_dts_depth_add_le(d_s_sq, dts_neg(sum_re_sq));
    lemma_norm_definite_mul(sum_re, sum_re);
    lemma_norm_definite_neg(sum_re_sq);
    lemma_norm_definite_add(d_s_sq, dts_neg(sum_re_sq));
    //  Add the two T2, T3 nonneg subs
    //  Build wf for sub(r_sq, sum_re_sq) = add(r_sq, neg(sum_re_sq))
    //  sr(r_sq, neg(sum_re_sq)): r_sq ~ a1 ~ sum_re ~ sum_re_sq ~ neg(sum_re_sq)
    //  We have sr(r_sq, dd) from earlier. dd ~ a1 (have).
    lemma_dts_same_radicand_symmetric(a1, dd);
    lemma_dts_same_radicand_transitive(r_sq, dd, a1);
    //  sum_re ~ a1 (we have sr(a1, sum_re); sym → sr(sum_re, a1))
    //  sum_re_sq ~ sum_re via mul_closed sym
    lemma_dts_same_radicand_symmetric(sum_re, sum_re_sq);
    //  Build sr(a1, sum_re_sq) via a1 ~ sum_re ~ sum_re_sq
    lemma_dts_same_radicand_transitive(a1, sum_re, sum_re_sq);
    lemma_dts_same_radicand_transitive(r_sq, a1, sum_re_sq);
    //  Build sr(sum_re_sq, neg(sum_re_sq)) via mul_closed/sym
    lemma_dts_same_radicand_neg(sum_re_sq);
    lemma_dts_same_radicand_symmetric(sum_re_sq, dts_neg(sum_re_sq));
    lemma_dts_same_radicand_transitive(r_sq, sum_re_sq, dts_neg(sum_re_sq));
    lemma_dts_neg_well_formed(sum_re_sq);
    lemma_dts_add_closed(r_sq, dts_neg(sum_re_sq));
    //  sr(sub(d_s_sq, r_sq), sub(r_sq, sum_re_sq)) via a1
    lemma_dts_same_radicand_symmetric(d_s_sq, dts_sub(d_s_sq, r_sq));
    lemma_dts_same_radicand_transitive(dts_sub(d_s_sq, r_sq), d_s_sq, a1);
    lemma_dts_same_radicand_symmetric(r_sq, dts_sub(r_sq, sum_re_sq));
    //  sr(r_sq, a1)
    lemma_dts_same_radicand_symmetric(r_sq, r);
    //  Wait, we need sr(r_sq, a1). r_sq ~ r ~ a1 via the chain above.
    //  Actually we have sr(r, dd) and sr(a1, dd) → sr(r, a1)
    //  Or sr(r_sq, a1): Check what we have for r_sq in this scope.
    //  We have lemma_dts_same_radicand_symmetric(r, r_sq) earlier? Let me skip and assume Z3 handles it.
    lemma_dts_same_radicand_transitive(dts_sub(r_sq, sum_re_sq), r_sq, r);
    if a1_nn {
        //  r = a2, sr(a2, a1) via sym
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_transitive(dts_sub(r_sq, sum_re_sq), r, a1);
    } else {
        //  r = a1, trivial
        lemma_dts_same_radicand_transitive(dts_sub(r_sq, sum_re_sq), r, a1);
    }
    lemma_dts_same_radicand_symmetric(dts_sub(r_sq, sum_re_sq), a1);
    lemma_dts_same_radicand_transitive(
        dts_sub(d_s_sq, r_sq), a1, dts_sub(r_sq, sum_re_sq));
    lemma_dts_nonneg_radicands_add(d_s_sq, dts_neg(r_sq));
    lemma_dts_nonneg_radicands_add(r_sq, dts_neg(sum_re_sq));
    lemma_norm_definite_add(d_s_sq, dts_neg(r_sq));
    lemma_norm_definite_add(r_sq, dts_neg(sum_re_sq));
    lemma_dts_depth_add_le(d_s_sq, dts_neg(r_sq));
    lemma_dts_depth_add_le(r_sq, dts_neg(sum_re_sq));
    lemma_dts_nonneg_add_closed_fuel(
        dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq), f);
    //  Transfer via sub_add_sub eqv
    lemma_dts_add_closed(
        dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq));
    lemma_dts_same_radicand_symmetric(
        dts_sub(d_s_sq, r_sq),
        dts_add(dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq)));
    lemma_dts_same_radicand_transitive(
        dts_add(dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq)),
        dts_sub(d_s_sq, r_sq), d_s_sq);
    lemma_dts_same_radicand_symmetric(d_s_sq, dts_sub(d_s_sq, sum_re_sq));
    lemma_dts_same_radicand_transitive(
        dts_add(dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq)),
        d_s_sq, dts_sub(d_s_sq, sum_re_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_add(dts_sub(d_s_sq, r_sq), dts_sub(r_sq, sum_re_sq)),
        dts_sub(d_s_sq, sum_re_sq), f);
    //  Now: nonneg(sub(d_s_sq, sum_re_sq))

    //  Add T1 + (T2+T3): nonneg(sub(d_sum_im_sq, sum_re_sq)) via sub_add_sub
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub::<DynTowerSpec<T>>(
        d_sum_im_sq, d_s_sq, sum_re_sq);
    //  sr stuff for the new add
    lemma_dts_same_radicand_symmetric(d_sum_im_sq, dts_sub(d_sum_im_sq, d_s_sq));
    lemma_dts_same_radicand_transitive(
        dts_sub(d_sum_im_sq, d_s_sq), d_sum_im_sq, dd);
    lemma_dts_same_radicand_transitive(
        dts_sub(d_sum_im_sq, d_s_sq), dd, a1);
    //  sr(sub(d_sum_im_sq, d_s_sq), sub(d_s_sq, sum_re_sq)) via a1
    //  sub(d_s_sq, sum_re_sq) ~ d_s_sq ~ a1
    lemma_dts_same_radicand_symmetric(d_s_sq, dts_sub(d_s_sq, sum_re_sq));
    lemma_dts_same_radicand_transitive(
        dts_sub(d_s_sq, sum_re_sq), d_s_sq, a1);
    lemma_dts_same_radicand_symmetric(dts_sub(d_s_sq, sum_re_sq), a1);
    lemma_dts_same_radicand_transitive(
        dts_sub(d_sum_im_sq, d_s_sq), a1, dts_sub(d_s_sq, sum_re_sq));
    lemma_dts_nonneg_radicands_add(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_dts_nonneg_radicands_add(d_s_sq, dts_neg(sum_re_sq));
    lemma_norm_definite_add(d_sum_im_sq, dts_neg(d_s_sq));
    lemma_norm_definite_add(d_s_sq, dts_neg(sum_re_sq));
    ...
    lemma_dts_depth_add_le(d_s_sq, dts_neg(sum_re_sq));
    lemma_dts_nonneg_add_closed_fuel(
        dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq), f);
    //  Transfer via sub_add_sub eqv
    lemma_dts_add_closed(
        dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq));
    lemma_dts_same_radicand_symmetric(
        dts_sub(d_sum_im_sq, d_s_sq),
        dts_add(dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq)));
    lemma_dts_same_radicand_transitive(
        dts_add(dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq)),
        dts_sub(d_sum_im_sq, d_s_sq), d_sum_im_sq);
    //  Build wf for sub(d_sum_im_sq, sum_re_sq) = add(d_sum_im_sq, neg(sum_re_sq))
    //  sr(d_sum_im_sq, neg(sum_re_sq)) via d_sum_im_sq ~ dd ~ a1 ~ sum_re_sq ~ neg(sum_re_sq)
    lemma_dts_same_radicand_symmetric(dd, d_sum_im_sq);
    //  Need sr(dd, d_sum_im_sq) — flip from sr(d_sum_im_sq, dd) which I might not have
    //  Actually we built sr(dd, d_sum_im_sq) via mul_closed earlier in T1 setup
    //  Hmm wait, we built sr(d_sum_im_sq, dd) from sym. Let me just rebuild directly.
    lemma_dts_same_radicand_transitive(d_sum_im_sq, dd, a1);
    lemma_dts_same_radicand_transitive(d_sum_im_sq, a1, sum_re_sq);
    lemma_dts_same_radicand_transitive(d_sum_im_sq, sum_re_sq, dts_neg(sum_re_sq));
    lemma_dts_add_closed(d_sum_im_sq, dts_neg(sum_re_sq));
    lemma_dts_same_radicand_symmetric(d_sum_im_sq, dts_sub(d_sum_im_sq, sum_re_sq));
    lemma_dts_same_radicand_transitive(
        dts_add(dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq)),
        d_sum_im_sq, dts_sub(d_sum_im_sq, sum_re_sq));
    lemma_dts_nonneg_radicands_add(d_sum_im_sq, dts_neg(sum_re_sq));
    lemma_norm_definite_add(d_sum_im_sq, dts_neg(sum_re_sq));
    lemma_dts_depth_add_le(d_sum_im_sq, dts_neg(sum_re_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_add(dts_sub(d_sum_im_sq, d_s_sq), dts_sub(d_s_sq, sum_re_sq)),
        dts_sub(d_sum_im_sq, sum_re_sq), f);
    //  Now: nonneg(sub(d*sum_im², sum_re²)) ✓
}
...
///  Uses square_of_sum + d_dist_quad + sub_add_sub_4arg three times.
#[verifier::rlimit(500)]
                        ...
                        dts_sub(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2))))))),
{
    ...
    lemma_dts_mul_closed(b2, b2);
    lemma_dts_mul_closed(b1, b2);
    //  sr(dd, bb1), sr(dd, bb2), sr(dd, b12) via dd~a1~b1~b1·b1
    lemma_dts_same_radicand_symmetric(a1, dd);  //  sr(dd, a1)
    lemma_dts_same_radicand_transitive(dd, a1, b1);  //  sr(dd, b1)
    lemma_dts_same_radicand_transitive(dd, b1, bb1);  //  sr(dd, bb1)
    //  sr(b1, bb2) via b1~b2~bb2
    lemma_dts_same_radicand_transitive(b1, b2, bb2);
    lemma_dts_same_radicand_transitive(dd, b1, bb2);  //  sr(dd, bb2)
    lemma_dts_same_radicand_transitive(dd, b1, b12);  //  sr(dd, b12)
    lemma_dts_mul_closed(dd, bb1);
    lemma_dts_mul_closed(dd, bb2);
    lemma_dts_mul_closed(dd, b12);
    //  sr(a1, mul cells)
    ...
    lemma_dts_same_radicand_symmetric(dd, db12);
    lemma_dts_same_radicand_transitive(a1, dd, db12);
    //  sr between mul cells via a1
    lemma_dts_same_radicand_symmetric(a1, aa1);  //  sr(aa1, a1)
    lemma_dts_same_radicand_transitive(aa1, a1, aa2);
    lemma_dts_same_radicand_transitive(aa1, a1, a12);
    lemma_dts_same_radicand_transitive(aa1, a1, dbb1);
    lemma_dts_same_radicand_transitive(aa1, a1, dbb2);
    lemma_dts_same_radicand_transitive(aa1, a1, db12);
    //  sr(aa2, a1) via sym of sr(a1, aa2)
    lemma_dts_same_radicand_symmetric(a1, aa2);  //  sr(aa2, a1)
    lemma_dts_same_radicand_transitive(aa2, a1, dbb2);
    lemma_dts_same_radicand_transitive(aa2, a1, dbb1);
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_transitive(a12, a1, db12);
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(dbb1, a1, dbb2);
    //  Build pair adds: aa1+aa2, dbb1+dbb2, a12+a12, db12+db12
    lemma_dts_add_closed(aa1, aa2);
    lemma_dts_add_closed(dbb1, dbb2);
    lemma_dts_same_radicand_reflexive(a12);
    lemma_dts_add_closed(a12, a12);
    lemma_dts_same_radicand_reflexive(db12);
    lemma_dts_add_closed(db12, db12);
    //  sr(aa1, aa1+aa2), sr(a1, aa1+aa2)
    lemma_dts_same_radicand_symmetric(aa1, dts_add(aa1, aa2));
    lemma_dts_same_radicand_symmetric(a1, aa1);
    lemma_dts_same_radicand_transitive(a1, aa1, dts_add(aa1, aa2));
    lemma_dts_same_radicand_symmetric(dbb1, dts_add(dbb1, dbb2));
    lemma_dts_same_radicand_symmetric(a1, dbb1);
    lemma_dts_same_radicand_transitive(a1, dbb1, dts_add(dbb1, dbb2));
    lemma_dts_same_radicand_symmetric(a12, dts_add(a12, a12));
    lemma_dts_same_radicand_symmetric(a1, a12);
    lemma_dts_same_radicand_transitive(a1, a12, dts_add(a12, a12));
    lemma_dts_same_radicand_symmetric(db12, dts_add(db12, db12));
    lemma_dts_same_radicand_symmetric(a1, db12);
    lemma_dts_same_radicand_transitive(a1, db12, dts_add(db12, db12));
    //  Now build the OUTER squares: (aa1+aa2)+(a12+a12) and (dbb1+dbb2)+(db12+db12)
    //  sr(aa1+aa2, a12+a12) via aa1+aa2 ~ a1 ~ a12+a12
    lemma_dts_same_radicand_symmetric(a1, dts_add(aa1, aa2));
    lemma_dts_same_radicand_transitive(dts_add(aa1, aa2), a1, dts_add(a12, a12));
    lemma_dts_add_closed(dts_add(aa1, aa2), dts_add(a12, a12));
    //  sr(dbb1+dbb2, db12+db12)
    lemma_dts_same_radicand_symmetric(a1, dts_add(dbb1, dbb2));
    lemma_dts_same_radicand_transitive(dts_add(dbb1, dbb2), a1, dts_add(db12, db12));
    lemma_dts_add_closed(dts_add(dbb1, dbb2), dts_add(db12, db12));
    //  Add closures for the sub forms (LHS of sub_add_sub_4arg)
    lemma_dts_add_closed(a1, a2);  //  build ssa
    lemma_dts_same_radicand_reflexive(ssa);
    lemma_dts_mul_closed(ssa, ssa);
    lemma_dts_add_closed(b1, b2);
    lemma_dts_same_radicand_reflexive(ssb);
    lemma_dts_mul_closed(ssb, ssb);
    //  sr(dd, ssb²): sr(dd, b1) → sr(dd, ssb) → sr(dd, ssb²)
    lemma_dts_same_radicand_symmetric(b1, ssb);
    lemma_dts_same_radicand_transitive(dd, b1, ssb);
    lemma_dts_same_radicand_transitive(dd, ssb, dts_mul(ssb, ssb));
    lemma_dts_mul_closed(dd, dts_mul(ssb, ssb));
    ...
    lemma_dts_same_radicand_symmetric(ssa, dts_mul(ssa, ssa));
    lemma_dts_same_radicand_transitive(a1, ssa, dts_mul(ssa, ssa));
    //  sr(a1, ssb), sr(a1, ssb²), sr(a1, dd*ssb²)
    lemma_dts_same_radicand_transitive(a1, b1, ssb);
    lemma_dts_same_radicand_symmetric(b1, ssb);
    lemma_dts_same_radicand_symmetric(ssb, dts_mul(ssb, ssb));
    lemma_dts_same_radicand_transitive(a1, ssb, dts_mul(ssb, ssb));
    ...
    //  ─── Step 1: ssa² ≡ (aa1+aa2) + (a12+a12)  [square_of_sum] ───
    lemma_dts_square_of_sum(a1, a2);
    let big_a = dts_add(dts_add(aa1, aa2), dts_add(a12, a12));

    //  ─── Step 2: dd*ssb² ≡ (dbb1+dbb2) + (db12+db12)  [d_dist_quad] ───
    //  Need sr(b1, b2) and sr(b1, dd) for d_dist_quad's preconditions
    lemma_dts_same_radicand_symmetric(a1, b1);
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_dts_same_radicand_transitive(b1, a1, dd);
    lemma_dts_d_dist_quad(dd, b1, b2);
    let big_db = dts_add(dts_add(dbb1, dbb2), dts_add(db12, db12));

    //  ─── Step 3: sub(ssa², dd*ssb²) ≡ sub(big_a, big_db)  [sub_congruence_both] ───
    //  sr requirements: sr(ssa², big_a) and sr(dd*ssb², big_db).
    //  big_a is built from a1's family; ssa² also a1's family.
    //  big_db is built from a1's family (via dd, b1, etc); dd*ssb² also a1's family.
    //  Re-establish sr(a1, ...) for these compound terms.
    lemma_dts_same_radicand_symmetric(a1, dts_mul(ssa, ssa));  //  sr(ssa², a1)
    lemma_dts_same_radicand_symmetric(a1, dts_add(aa1, aa2));
    lemma_dts_same_radicand_transitive(a1, aa1, dts_add(aa1, aa2));
    lemma_dts_same_radicand_symmetric(a1, dts_add(a12, a12));
    lemma_dts_same_radicand_transitive(dts_add(aa1, aa2), a1, dts_add(a12, a12));
    //  big_a: add(add(aa1,aa2), add(a12,a12)). Need sr(big_a, a1) etc.
    lemma_dts_same_radicand_symmetric(dts_add(aa1, aa2),
        dts_add(dts_add(aa1, aa2), dts_add(a12, a12)));
    lemma_dts_same_radicand_transitive(big_a, dts_add(aa1, aa2), a1);
    lemma_dts_same_radicand_symmetric(big_a, a1);
    //  same for big_db
    lemma_dts_same_radicand_symmetric(a1, dts_add(dbb1, dbb2));
    lemma_dts_same_radicand_transitive(a1, dbb1, dts_add(dbb1, dbb2));
    lemma_dts_same_radicand_symmetric(a1, dts_add(db12, db12));
    lemma_dts_same_radicand_transitive(dts_add(dbb1, dbb2), a1, dts_add(db12, db12));
    lemma_dts_same_radicand_symmetric(dts_add(dbb1, dbb2),
        dts_add(dts_add(dbb1, dbb2), dts_add(db12, db12)));
    lemma_dts_same_radicand_transitive(big_db, dts_add(dbb1, dbb2), a1);
    lemma_dts_same_radicand_symmetric(big_db, a1);
    //  sr(ssa², big_a)
    lemma_dts_same_radicand_transitive(dts_mul(ssa, ssa), a1, big_a);
    //  sr(dd*ssb², big_db)
    lemma_dts_same_radicand_symmetric(a1, dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_symmetric(a1, dts_mul(dd, dts_mul(ssb, ssb)));
    lemma_dts_same_radicand_transitive(dts_mul(dd, dts_mul(ssb, ssb)), a1, big_db);
    //  Apply sub_congruence_both
    lemma_dts_sub_congruence_both(
        dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb)),
        big_a, big_db);
    //  → eqv(sub(ssa², dd*ssb²), sub(big_a, big_db))

    //  ─── Step 4: sub(big_a, big_db) ≡ sub((aa1+aa2),(dbb1+dbb2)) + sub((a12+a12),(db12+db12)) ───
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub_4arg::<DynTowerSpec<T>>(
        dts_add(aa1, aa2), dts_add(dbb1, dbb2),
        dts_add(a12, a12), dts_add(db12, db12));
    //  symmetric so we can chain forward
    lemma_dts_eqv_symmetric(
        dts_add(
            dts_sub(dts_add(aa1, aa2), dts_add(dbb1, dbb2)),
            dts_sub(dts_add(a12, a12), dts_add(db12, db12))),
        dts_sub(big_a, big_db));

    //  ─── Step 5a: sub(aa1+aa2, dbb1+dbb2) ≡ sub(aa1, dbb1) + sub(aa2, dbb2) = nx + ny ───
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub_4arg::<DynTowerSpec<T>>(
        aa1, dbb1, aa2, dbb2);
    lemma_dts_eqv_symmetric(
        dts_add(nx, ny),
    ...
    //  ─── Step 5b: sub(a12+a12, db12+db12) ≡ sub(a12, db12) + sub(a12, db12) = cross+cross ───
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub_4arg::<DynTowerSpec<T>>(
        a12, db12, a12, db12);
    lemma_dts_eqv_symmetric(
        dts_add(cross, cross),
    ...
    //  ─── Step 6: combine via add_congruence ───
    //  add(sub(aa1+aa2,dbb1+dbb2), sub(a12+a12,db12+db12))
    //  ≡ add(nx+ny, sub(a12+a12,db12+db12))   [add_congruence_left]
    //  ≡ add(nx+ny, cross+cross)              [add_congruence_right]
    lemma_dts_add_congruence_left(
        dts_sub(dts_add(aa1, aa2), dts_add(dbb1, dbb2)),
        dts_add(nx, ny),
        dts_sub(dts_add(a12, a12), dts_add(db12, db12)));
    lemma_dts_add_congruence_right(
        ...
        dts_sub(dts_add(a12, a12), dts_add(db12, db12)),
        dts_add(cross, cross));
    //  Chain: add(sub(aa1+aa2,dbb1+dbb2), sub(a12+a12,db12+db12))
    //       ≡ add(nx+ny, sub(a12+a12,db12+db12))
    //       ≡ add(nx+ny, cross+cross)
    lemma_dts_eqv_transitive(
        dts_add(
            dts_sub(dts_add(aa1, aa2), dts_add(dbb1, dbb2)),
            dts_sub(dts_add(a12, a12), dts_add(db12, db12))),
        dts_add(dts_add(nx, ny),
            dts_sub(dts_add(a12, a12), dts_add(db12, db12))),
        dts_add(dts_add(nx, ny), dts_add(cross, cross)));

    //  ─── Step 7: ssa² - dd*ssb² ≡ sub(big_a, big_db)               [from Step 3]
    //                            ≡ add(sub(aa1+aa2,dbb1+dbb2), sub(a12+a12,db12+db12)) [from Step 4]
    //                            ≡ add(nx+ny, cross+cross)            [from Step 6]
    //                            ≡ add(nx, add(ny, cross+cross))      [add_associative]
    //  ═══ Build the chain ═══
    //  Step 3 result: eqv(sub(ssa², dd*ssb²), sub(big_a, big_db))
    //  Step 4 result: eqv(sub(big_a, big_db),
    //                     add(sub(aa1+aa2,dbb1+dbb2), sub(a12+a12,db12+db12)))
    lemma_dts_eqv_transitive(
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))),
        dts_sub(big_a, big_db),
    ...
    //  ─── Step 8: re-associate (nx+ny)+(cross+cross) ≡ nx + (ny + (cross+cross)) ───
    lemma_dts_add_associative(nx, ny, dts_add(cross, cross));
    //  Chain final
    lemma_dts_eqv_transitive(
        dts_sub(dts_mul(ssa, ssa), dts_mul(dd, dts_mul(ssb, ssb))),
        dts_add(dts_add(nx, ny), dts_add(cross, cross)),
...
proof fn lemma_dts_nonneg_add_remaining<T: OrderedField>(
    ...
    ensures
        dts_nonneg_fuel(
            DynTowerSpec::Ext(
                Box::new(dts_add(a1, a2)),
                Box::new(dts_add(b1, b2)),
                Box::new(dd)),
            (f + 1) as nat),
    decreases f, 8nat,
{
    let sum_re = dts_add(a1, a2);
    let sum_im = dts_add(b1, b2);
    //  ═══ Infrastructure for sum components ═══
    lemma_dts_same_radicand_symmetric(a1, b1);
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_dts_same_radicand_transitive(b1, a1, a2);
    lemma_dts_add_closed(a1, a2);
    ...
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_norm_definite_add(b1, b2);
    //  sr(sum_re, sum_im): sum_re ~ a1 ~ b1 ~ sum_im
    lemma_dts_same_radicand_symmetric(a1, sum_re);
    lemma_dts_same_radicand_transitive(sum_re, a1, b1);
    lemma_dts_same_radicand_symmetric(b1, sum_im);
    lemma_dts_same_radicand_transitive(sum_re, b1, sum_im);
...
#[verifier::rlimit(80)]
pub proof fn lemma_dts_nonneg_add_closed_fuel<T: OrderedField>(
    x: DynTowerSpec<T>, y: DynTowerSpec<T>, fuel: nat,
)
    requires
    ...
    decreases fuel, 0nat,
{
    match (x, y) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            verus_algebra::inequalities::lemma_nonneg_add::<T>(r1, r2);
        }
        (DynTowerSpec::Ext(re1, im1, d), DynTowerSpec::Ext(re2, im2, _)) => {
            let f = (fuel - 1) as nat;
            ...
            //  Remaining: C2+C2, C1+C3, C3+C1, C2+C3, C3+C2, C3+C3.
            //  Dispatch to clean-context helper.
            lemma_dts_same_radicand_symmetric(a1, dd);
            lemma_dts_same_radicand_transitive(a1, b1, b2);
            lemma_dts_nonneg_add_remaining(a1, b1, a2, b2, dd, f);
            return;
        }
        _ => {}
    }
...
#[verifier::rlimit(120)]
proof fn lemma_dts_nonneg_mul_cc<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
...
#[verifier::rlimit(120)]
proof fn lemma_dts_nonneg_mul_bb<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
...
proof fn lemma_dts_nonneg_mul_bb_norm<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
    ...
    decreases f, 1nat,
{
    ...
    //  Congruence transfers to norm(product).
        ...
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1), a1, dd);
        lemma_dts_same_radicand_transitive(dts_mul(a1, a1), dd, dts_mul(dd, dts_mul(b1, b1)));
        verus_algebra::lemmas::additive_group_lemmas::lemma_sub_antisymmetric::<DynTowerSpec<T>>(
...
#[verifier::rlimit(200)]
pub proof fn lemma_dts_nonneg_mul_closed_fuel<T: OrderedField>(
    x: DynTowerSpec<T>, y: DynTowerSpec<T>, fuel: nat,
)
    requires
        fuel >= dts_depth(x) + 1, fuel >= dts_depth(y) + 1,
    ...
    decreases fuel, 0nat,
{
    match (x, y) {
        ...
        (DynTowerSpec::Ext(re1, im1, d), DynTowerSpec::Ext(re2, im2, _)) => {
            ...
            //  ═══ is_zero(im) shortcut: extracted to separate helper for rlimit ═══
            let im_val_check = dts_add(dts_mul(a1, b2), dts_mul(b1, a2));
            if dts_is_zero(im_val_check) {
                lemma_dts_nonneg_mul_iszero_im(a1, b1, a2, b2, dd, f);
                return;
            }
...
///  Helper for nonneg_mul when is_zero(im_val): product im is zero.
///  Cauchy-Schwarz step: from dd*b1²≥a1² and dd*b2²≥na2², derive
///  (dd*b1²)*(dd*b2²) ≥ na2²*a1² via le_mul_nonneg_monotone chain (inlined).
///  This gives Q² ≥ P² where Q=dd*b1*b2, P=a1*a2 (after square_mul congruences).
#[verifier::rlimit(200)]
proof fn lemma_dts_cauchy_schwarz_step<T: OrderedField>(
    a1_sq: DynTowerSpec<T>, na2_sq: DynTowerSpec<T>,
    dd_b1_sq: DynTowerSpec<T>, dd_b2_sq: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, nx: DynTowerSpec<T>, ny: DynTowerSpec<T>,
    a2_sq: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(a1_sq) + 1, f >= dts_depth(na2_sq) + 1,
        f >= dts_depth(dd_b1_sq) + 1, f >= dts_depth(dd_b2_sq) + 1,
        f >= dts_depth(dd) + 1,
        dts_well_formed(a1_sq), dts_well_formed(na2_sq),
        dts_well_formed(dd_b1_sq), dts_well_formed(dd_b2_sq), dts_well_formed(dd),
        dts_same_radicand(a1_sq, dd), dts_same_radicand(dd, dd_b1_sq),
        dts_same_radicand(dd, dd_b2_sq), dts_same_radicand(na2_sq, dd),
        dts_nonneg_radicands(a1_sq), dts_nonneg_radicands(na2_sq),
        dts_nonneg_radicands(dd_b1_sq), dts_nonneg_radicands(dd_b2_sq),
        dts_norm_definite(a1_sq), dts_norm_definite(na2_sq),
        dts_norm_definite(dd_b1_sq), dts_norm_definite(dd_b2_sq),
        //  Norm inequalities
        dts_nonneg_fuel(dts_sub(dd_b1_sq, a1_sq), f),  //  dd*b1² ≥ a1²
        dts_nonneg_fuel(dts_sub(dd_b2_sq, na2_sq), f),  //  dd*b2² ≥ na2²
        dts_nonneg_fuel(dd_b2_sq, f),  //  dd*b2² ≥ 0
        dts_nonneg_fuel(a1_sq, f),     //  a1² ≥ 0
    ensures
        //  Step 1: (dd*b1²)*(dd*b2²) ≥ a1²*(dd*b2²)
        dts_nonneg_fuel(dts_sub(dts_mul(dd_b1_sq, dd_b2_sq),
            dts_mul(a1_sq, dd_b2_sq)), f),
        //  Step 2: (dd*b2²)*a1² ≥ na2²*a1²
        dts_nonneg_fuel(dts_sub(dts_mul(dd_b2_sq, a1_sq),
            dts_mul(na2_sq, a1_sq)), f),
    decreases f, 1nat,
{
    //  Step 1: nonneg_mul(sub(dd_b1_sq, a1_sq), dd_b2_sq) → nonneg(neg(nx)*dd_b2_sq)
    //  = nonneg(sub(dd_b1_sq, a1_sq) * dd_b2_sq)
    //  Then distributes: sub(dd_b1_sq, a1_sq)*dd_b2_sq ≡ dd_b1_sq*dd_b2_sq - a1_sq*dd_b2_sq
    lemma_dts_same_radicand_transitive(a1_sq, dd, dd_b1_sq);
    lemma_dts_neg_well_formed(a1_sq);
    lemma_dts_same_radicand_neg(a1_sq);
    lemma_dts_same_radicand_symmetric(dd, dd_b1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dd);
    lemma_dts_same_radicand_transitive(dd_b1_sq, dd, a1_sq);
    lemma_dts_same_radicand_transitive(dd_b1_sq, a1_sq, dts_neg(a1_sq));
    lemma_dts_add_closed(dd_b1_sq, dts_neg(a1_sq));
    lemma_dts_nonneg_radicands_neg(a1_sq);
    lemma_dts_nonneg_radicands_add(dd_b1_sq, dts_neg(a1_sq));
    lemma_norm_definite_neg(a1_sq);
    lemma_norm_definite_add(dd_b1_sq, dts_neg(a1_sq));
    lemma_dts_depth_neg(a1_sq);
    lemma_dts_depth_add_le(dd_b1_sq, dts_neg(a1_sq));
    let neg_nx = dts_sub(dd_b1_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(dd_b1_sq, neg_nx);
    lemma_dts_same_radicand_transitive(neg_nx, dd_b1_sq, dd);
    lemma_dts_same_radicand_transitive(neg_nx, dd, dd_b2_sq);
    lemma_dts_nonneg_mul_closed_fuel(neg_nx, dd_b2_sq, f);
    //  Distributes: neg_nx * dd_b2_sq ≡ dd_b1_sq*dd_b2_sq + neg(a1_sq)*dd_b2_sq
    //  = dd_b1_sq*dd_b2_sq - a1_sq*dd_b2_sq via neg_mul_left
    lemma_dts_same_radicand_symmetric(dd, dd_b1_sq);
    lemma_dts_same_radicand_symmetric(dd, dd_b2_sq);
    lemma_dts_same_radicand_transitive(dd_b2_sq, dd, dd_b1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dd);
    lemma_dts_same_radicand_transitive(dd_b1_sq, dd, a1_sq);
    lemma_dts_same_radicand_transitive(dd_b1_sq, a1_sq, dts_neg(a1_sq));
    lemma_dts_mul_distributes_left(dd_b2_sq, dd_b1_sq, dts_neg(a1_sq));
    //  mul(dd_b2_sq, neg(a1_sq)) ≡ neg(mul(dd_b2_sq, a1_sq))
    lemma_dts_same_radicand_transitive(dd_b2_sq, dd, a1_sq);
    lemma_dts_neg_mul_right(dd_b2_sq, a1_sq);
    //  add(mul(dd_b2_sq, dd_b1_sq), mul(dd_b2_sq, neg(a1_sq)))
    //  ≡ add(mul(dd_b2_sq, dd_b1_sq), neg(mul(dd_b2_sq, a1_sq)))
    //  = sub(mul(dd_b2_sq, dd_b1_sq), mul(dd_b2_sq, a1_sq))
    lemma_dts_mul_closed(dd_b2_sq, dd_b1_sq);
    lemma_dts_same_radicand_transitive(dd_b2_sq, dd, a1_sq);
    lemma_dts_same_radicand_transitive(dd_b2_sq, a1_sq, dts_neg(a1_sq));
    lemma_dts_mul_closed(dd_b2_sq, dts_neg(a1_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, a1_sq);
    lemma_dts_mul_closed(dd_b2_sq, a1_sq);
    lemma_dts_neg_well_formed(dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_add_congruence_right(
        dts_mul(dd_b2_sq, dd_b1_sq),
        dts_mul(dd_b2_sq, dts_neg(a1_sq)),
        dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    //  Now: neg_nx*dd_b2_sq ≡ sub(dd_b2_sq*dd_b1_sq, dd_b2_sq*a1_sq)
    //  Transfer via commutativity: dd_b2_sq*dd_b1_sq ≡ dd_b1_sq*dd_b2_sq, dd_b2_sq*a1_sq ≡ a1_sq*dd_b2_sq
    lemma_dts_mul_commutative(dd_b2_sq, dd_b1_sq);
    lemma_dts_mul_commutative(dd_b2_sq, a1_sq);
    //  sub(dd_b2_sq*dd_b1_sq, dd_b2_sq*a1_sq) ≡ sub(dd_b1_sq*dd_b2_sq, a1_sq*dd_b2_sq)
    lemma_dts_same_radicand_symmetric(dd_b1_sq, dd);
    ...
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, dd_b1_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dd_b1_sq, dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, dd_b2_sq));
    //  same_radicand for sub_congruence_both pairs
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, dd_b1_sq), dd_b2_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, dd_b1_sq), dd, dd_b1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, dd_b1_sq), dd_b1_sq, dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dd_b2_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dd, a1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), a1_sq, dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(dd_b2_sq, dd_b1_sq), dts_mul(dd_b2_sq, a1_sq),
        dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq));
    //  Chain: neg_nx*dd_b2_sq ≡ sub(dd_b2_sq*dd_b1_sq, dd_b2_sq*a1_sq) ≡ sub(dd_b1_sq*dd_b2_sq, a1_sq*dd_b2_sq)
    lemma_dts_mul_closed(neg_nx, dd_b2_sq);
    lemma_dts_same_radicand_symmetric(neg_nx, dts_mul(neg_nx, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(neg_nx, dd_b2_sq), neg_nx, dd);
    //  mul(neg_nx, dd_b2_sq) ≡ mul(dd_b2_sq, neg_nx) via commutative
    lemma_dts_mul_commutative(neg_nx, dd_b2_sq);
    //  mul(dd_b2_sq, neg_nx) ≡ add(mul(dd_b2_sq, dd_b1_sq), mul(dd_b2_sq, neg(a1_sq))) via distributes (already called)
    //  Chain: mul(neg_nx, dd_b2_sq) ≡ mul(dd_b2_sq, neg_nx) ≡ add(...)
    lemma_dts_eqv_transitive(
        dts_mul(neg_nx, dd_b2_sq),
        dts_mul(dd_b2_sq, neg_nx),
        dts_add(dts_mul(dd_b2_sq, dd_b1_sq), dts_mul(dd_b2_sq, dts_neg(a1_sq))));
    //  add(mul(dd_b2_sq, dd_b1_sq), mul(dd_b2_sq, neg(a1_sq))) ≡ add(mul(dd_b2_sq, dd_b1_sq), neg(mul(dd_b2_sq, a1_sq)))
    //  via add_congruence_right from neg_mul_right (already called)
    lemma_dts_eqv_transitive(
        dts_mul(neg_nx, dd_b2_sq),
        dts_add(dts_mul(dd_b2_sq, dd_b1_sq), dts_mul(dd_b2_sq, dts_neg(a1_sq))),
        dts_add(dts_mul(dd_b2_sq, dd_b1_sq), dts_neg(dts_mul(dd_b2_sq, a1_sq))));
    lemma_dts_eqv_transitive(
        dts_mul(neg_nx, dd_b2_sq),
        dts_sub(dts_mul(dd_b2_sq, dd_b1_sq), dts_mul(dd_b2_sq, a1_sq)),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)));
    //  Transfer nonneg
    //  same_radicand(dd, sub(dd_b1_sq*dd_b2_sq, a1_sq*dd_b2_sq))
    lemma_dts_neg_well_formed(dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_neg(dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dd, dd_b1_sq, dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dd_b1_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dd, a1_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), a1_sq, dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq),
        dts_neg(dts_mul(a1_sq, dd_b2_sq)));
    lemma_dts_nonneg_radicands_mul(dd_b1_sq, dd_b2_sq);
    lemma_dts_nonneg_radicands_mul(a1_sq, dd_b2_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_add_closed(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(a1_sq, dd_b2_sq)));
    lemma_dts_same_radicand_symmetric(dts_mul(dd_b1_sq, dd_b2_sq),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)));
    lemma_dts_same_radicand_transitive(dd, dd_b1_sq, dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dd, dts_mul(dd_b1_sq, dd_b2_sq),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)));
    lemma_dts_same_radicand_transitive(dts_mul(neg_nx, dd_b2_sq), dd,
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_mul(neg_nx, dd_b2_sq),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)), f);

    //  Step 2: nonneg_mul(sub(dd_b2_sq, na2_sq), a1_sq)
    let neg_ny = dts_sub(dd_b2_sq, na2_sq);
    lemma_dts_same_radicand_symmetric(na2_sq, dd);
    lemma_dts_neg_well_formed(na2_sq);
    lemma_dts_same_radicand_neg(na2_sq);
    lemma_dts_same_radicand_symmetric(na2_sq, dd);
    lemma_dts_same_radicand_transitive(na2_sq, dd, dd_b2_sq);
    lemma_dts_same_radicand_symmetric(na2_sq, dd_b2_sq);
    lemma_dts_same_radicand_transitive(dd_b2_sq, na2_sq, dts_neg(na2_sq));
    lemma_dts_add_closed(dd_b2_sq, dts_neg(na2_sq));
    lemma_dts_nonneg_radicands_neg(na2_sq);
    lemma_dts_nonneg_radicands_add(dd_b2_sq, dts_neg(na2_sq));
    lemma_norm_definite_neg(na2_sq);
    lemma_norm_definite_add(dd_b2_sq, dts_neg(na2_sq));
    lemma_dts_depth_neg(na2_sq);
    lemma_dts_depth_add_le(dd_b2_sq, dts_neg(na2_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, neg_ny);
    lemma_dts_same_radicand_transitive(neg_ny, dd_b2_sq, dd);
    lemma_dts_same_radicand_transitive(neg_ny, dd, a1_sq);
    lemma_dts_nonneg_mul_closed_fuel(neg_ny, a1_sq, f);
    //  Distributes: neg_ny * a1_sq ≡ dd_b2_sq*a1_sq - na2_sq*a1_sq
    lemma_dts_same_radicand_symmetric(na2_sq, dd_b2_sq);
    lemma_dts_same_radicand_transitive(dd_b2_sq, na2_sq, dts_neg(na2_sq));
    lemma_dts_mul_distributes_left(a1_sq, dd_b2_sq, dts_neg(na2_sq));
    lemma_dts_same_radicand_symmetric(dd, a1_sq);
    lemma_dts_same_radicand_symmetric(na2_sq, dd);
    lemma_dts_same_radicand_transitive(a1_sq, dd, na2_sq);
    lemma_dts_neg_mul_right(a1_sq, na2_sq);
    lemma_dts_mul_closed(a1_sq, dd_b2_sq);
    lemma_dts_same_radicand_transitive(a1_sq, dd, na2_sq);
    lemma_dts_same_radicand_transitive(a1_sq, na2_sq, dts_neg(na2_sq));
    lemma_dts_mul_closed(a1_sq, dts_neg(na2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, na2_sq);
    lemma_dts_mul_closed(a1_sq, na2_sq);
    lemma_dts_neg_well_formed(dts_mul(a1_sq, na2_sq));
    lemma_dts_add_congruence_right(
        dts_mul(a1_sq, dd_b2_sq),
        dts_mul(a1_sq, dts_neg(na2_sq)),
        dts_neg(dts_mul(a1_sq, na2_sq)));
    //  Commutativity: a1_sq*dd_b2_sq ≡ dd_b2_sq*a1_sq, a1_sq*na2_sq ≡ na2_sq*a1_sq
    lemma_dts_mul_commutative(a1_sq, dd_b2_sq);
    lemma_dts_mul_commutative(a1_sq, na2_sq);
    lemma_dts_mul_closed(dd_b2_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, na2_sq);
    lemma_dts_same_radicand_symmetric(na2_sq, a1_sq);
    lemma_dts_mul_closed(na2_sq, a1_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, na2_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(na2_sq, dts_mul(na2_sq, a1_sq));
    //  same_radicand pairs for sub_congruence_both
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, dd_b2_sq), a1_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, dd_b2_sq), dd, dd_b2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, dd_b2_sq), dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, na2_sq), a1_sq, na2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, na2_sq), na2_sq, dts_mul(na2_sq, a1_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(a1_sq, dd_b2_sq), dts_mul(a1_sq, na2_sq),
        dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq));
    //  Chain eqv + transfer nonneg
    lemma_dts_mul_closed(neg_ny, a1_sq);
    lemma_dts_same_radicand_symmetric(neg_ny, dts_mul(neg_ny, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(neg_ny, a1_sq), neg_ny, dd);
    //  mul(neg_ny, a1_sq) ≡ mul(a1_sq, neg_ny) via commutative
    lemma_dts_mul_commutative(neg_ny, a1_sq);
    //  mul(a1_sq, neg_ny) ≡ add(mul(a1_sq, dd_b2_sq), mul(a1_sq, neg(na2_sq))) via distributes (already called)
    lemma_dts_eqv_transitive(
        dts_mul(neg_ny, a1_sq),
        dts_mul(a1_sq, neg_ny),
        dts_add(dts_mul(a1_sq, dd_b2_sq), dts_mul(a1_sq, dts_neg(na2_sq))));
    //  add(..., mul(a1_sq, neg(na2_sq))) ≡ add(..., neg(mul(a1_sq, na2_sq))) via add_congruence_right
    lemma_dts_eqv_transitive(
        dts_mul(neg_ny, a1_sq),
        dts_add(dts_mul(a1_sq, dd_b2_sq), dts_mul(a1_sq, dts_neg(na2_sq))),
        dts_add(dts_mul(a1_sq, dd_b2_sq), dts_neg(dts_mul(a1_sq, na2_sq))));
    lemma_dts_eqv_transitive(
        dts_mul(neg_ny, a1_sq),
        dts_sub(dts_mul(a1_sq, dd_b2_sq), dts_mul(a1_sq, na2_sq)),
        dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)));
    //  same_radicand(dd, sub(dd_b2_sq*a1_sq, na2_sq*a1_sq)) chain
    lemma_dts_neg_well_formed(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(na2_sq, a1_sq));
    ...
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dd_b2_sq, dd);
    lemma_dts_same_radicand_symmetric(na2_sq, dd);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dd, na2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), na2_sq, dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq),
        dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_nonneg_radicands_mul(dd_b2_sq, a1_sq);
    lemma_dts_nonneg_radicands_mul(na2_sq, a1_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_add_closed(dts_mul(dd_b2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_symmetric(dts_mul(dd_b2_sq, a1_sq),
        dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_transitive(dd, dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dd, dts_mul(dd_b2_sq, a1_sq),
        dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_transitive(dts_mul(neg_ny, a1_sq), dd,
        dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_mul(neg_ny, a1_sq),
        dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)), f);
}

///  Combines two le_mul results via commute + nonneg_add + sub_add_sub algebra identity:
///  nonneg(sub(A*C, B*C)) + nonneg(sub(C*B, D*B)) → nonneg(sub(A*C, D*B))
///  where A=dd_b1_sq, B=a1_sq (commuted to C), C=dd_b2_sq, D=na2_sq.
proof fn lemma_dts_cauchy_schwarz_combine<T: OrderedField>(
    dd_b1_sq: DynTowerSpec<T>, dd_b2_sq: DynTowerSpec<T>,
    a1_sq: DynTowerSpec<T>, na2_sq: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        f >= dts_depth(dd_b1_sq) + 1, f >= dts_depth(dd_b2_sq) + 1,
        f >= dts_depth(a1_sq) + 1, f >= dts_depth(na2_sq) + 1, f >= dts_depth(dd) + 1,
        dts_well_formed(dd_b1_sq), dts_well_formed(dd_b2_sq),
        dts_well_formed(a1_sq), dts_well_formed(na2_sq), dts_well_formed(dd),
        dts_same_radicand(dd_b1_sq, dd), dts_same_radicand(dd, dd_b2_sq),
        dts_same_radicand(a1_sq, dd), dts_same_radicand(na2_sq, dd),
        dts_nonneg_radicands(dd_b1_sq), dts_nonneg_radicands(dd_b2_sq),
        dts_nonneg_radicands(a1_sq), dts_nonneg_radicands(na2_sq),
        dts_norm_definite(dd_b1_sq), dts_norm_definite(dd_b2_sq),
        dts_norm_definite(a1_sq), dts_norm_definite(na2_sq),
        //  From cauchy_schwarz_step:
        dts_nonneg_fuel(dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)), f),
        dts_nonneg_fuel(dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq)), f),
    ensures
        dts_nonneg_fuel(dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(na2_sq, a1_sq)), f),
    decreases f, 2nat,
{
    //  Commute middle: a1_sq*dd_b2_sq ≡ dd_b2_sq*a1_sq
    lemma_dts_same_radicand_symmetric(a1_sq, dd);
    lemma_dts_same_radicand_transitive(a1_sq, dd, dd_b2_sq);
    lemma_dts_mul_commutative(a1_sq, dd_b2_sq);
    //  sr chains for mul_closed
    lemma_dts_same_radicand_transitive(dd_b1_sq, dd, dd_b2_sq);  //  dd_b1_sq ~ dd_b2_sq
    lemma_dts_mul_closed(dd_b1_sq, dd_b2_sq);
    lemma_dts_mul_closed(a1_sq, dd_b2_sq);
    //  sr(dd_b2_sq, a1_sq): dd_b2_sq ~ dd ~ a1_sq (via symmetric)
    lemma_dts_same_radicand_symmetric(dd, dd_b2_sq);
    lemma_dts_same_radicand_symmetric(a1_sq, dd);
    lemma_dts_same_radicand_transitive(dd_b2_sq, dd, a1_sq);
    lemma_dts_mul_closed(dd_b2_sq, a1_sq);
    //  sr(na2_sq, a1_sq): na2_sq ~ dd ~ a1_sq
    lemma_dts_same_radicand_symmetric(na2_sq, dd);
    lemma_dts_same_radicand_transitive(na2_sq, dd, a1_sq);
    lemma_dts_mul_closed(na2_sq, a1_sq);
    lemma_dts_eqv_reflexive(dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_reflexive(dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_symmetric(a1_sq, dts_mul(a1_sq, dd_b2_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, dd_b2_sq), a1_sq, dd_b2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(a1_sq, dd_b2_sq), dd_b2_sq,
        dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_sub_congruence_both(
        dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq),
        dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_nonneg_fuel_congruence(
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(a1_sq, dd_b2_sq)),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(dd_b2_sq, a1_sq)), f);
    //  nonneg_add(sub1, sub2) for transitivity
    let sub1 = dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(dd_b2_sq, a1_sq));
    let sub2 = dts_sub(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq));
    //  Infrastructure for nonneg_add
    lemma_dts_neg_well_formed(dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dd_b1_sq, dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dd_b1_sq, dd);
    //  sr(dd, mul(dd_b2_sq, a1_sq)): dd ~ dd_b2_sq ~ mul(dd_b2_sq, a1_sq)
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dd, dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dd, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(dd_b2_sq, a1_sq),
        dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    lemma_dts_nonneg_radicands_mul(dd_b1_sq, dd_b2_sq);
    lemma_dts_nonneg_radicands_mul(dd_b2_sq, a1_sq);
    lemma_dts_nonneg_radicands_mul(na2_sq, a1_sq);
    lemma_dts_nonneg_radicands_neg(dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_nonneg_radicands_add(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    lemma_dts_neg_well_formed(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_nonneg_radicands_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dd_b2_sq, dts_mul(dd_b2_sq, a1_sq));
    ...
    lemma_dts_same_radicand_symmetric(na2_sq, dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dd, na2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), na2_sq,
        dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq),
        dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(dd_b2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_nonneg_radicands_add(dts_mul(dd_b2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_norm_definite_mul(dd_b1_sq, dd_b2_sq);
    lemma_norm_definite_mul(dd_b2_sq, a1_sq);
    lemma_norm_definite_mul(na2_sq, a1_sq);
    lemma_norm_definite_neg(dts_mul(dd_b2_sq, a1_sq));
    lemma_norm_definite_add(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    lemma_norm_definite_neg(dts_mul(na2_sq, a1_sq));
    lemma_norm_definite_add(dts_mul(dd_b2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    //  same_radicand(sub1, sub2) for nonneg_add
    lemma_dts_same_radicand_symmetric(dts_mul(dd_b1_sq, dd_b2_sq), sub1);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(dd_b1_sq, dd_b2_sq), dd);
    //  sr(dd, mul(dd_b2_sq, a1_sq)) already established above
    lemma_dts_same_radicand_transitive(sub1, dd, dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_same_radicand_symmetric(dts_mul(dd_b2_sq, a1_sq), sub2);
    lemma_dts_same_radicand_transitive(sub1, dts_mul(dd_b2_sq, a1_sq), sub2);
    //  Depth bounds
    lemma_dts_depth_mul_le(dd_b1_sq, dd_b2_sq);
    lemma_dts_depth_mul_le(dd_b2_sq, a1_sq);
    lemma_dts_depth_mul_le(na2_sq, a1_sq);
    lemma_dts_depth_neg(dts_mul(dd_b2_sq, a1_sq));
    lemma_dts_depth_add_le(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(dd_b2_sq, a1_sq)));
    lemma_dts_depth_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_depth_add_le(dts_mul(dd_b2_sq, a1_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_nonneg_add_closed_fuel(sub1, sub2, f);
    //  sub_add_sub algebra identity: add(sub(A,B), sub(B,C)) ≡ sub(A,C)
    verus_algebra::lemmas::additive_group_lemmas::lemma_sub_add_sub::<DynTowerSpec<T>>(
        dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(dd_b2_sq, a1_sq), dts_mul(na2_sq, a1_sq));
    //  Transfer nonneg
    lemma_dts_add_closed(sub1, sub2);
    lemma_dts_same_radicand_symmetric(sub1, dts_add(sub1, sub2));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), sub1,
        dts_mul(dd_b1_sq, dd_b2_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dd, na2_sq);
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), na2_sq,
        dts_mul(na2_sq, a1_sq));
    lemma_dts_neg_well_formed(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_neg(dts_mul(na2_sq, a1_sq));
    lemma_dts_same_radicand_transitive(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(na2_sq, a1_sq),
        dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_add_closed(dts_mul(dd_b1_sq, dd_b2_sq), dts_neg(dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_symmetric(dts_mul(dd_b1_sq, dd_b2_sq),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_same_radicand_transitive(dts_add(sub1, sub2), dts_mul(dd_b1_sq, dd_b2_sq),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(na2_sq, a1_sq)));
    lemma_dts_nonneg_fuel_congruence(dts_add(sub1, sub2),
        dts_sub(dts_mul(dd_b1_sq, dd_b2_sq), dts_mul(na2_sq, a1_sq)), f);
}
...
#[verifier::rlimit(200)]
proof fn lemma_dts_nonneg_mul_iszero_im<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>, a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_norm_definite(a1), dts_norm_definite(b1),
        dts_norm_definite(a2), dts_norm_definite(b2), dts_norm_definite(dd),
        dts_norm_definite(DynTowerSpec::Ext(
            Box::new(a1), Box::new(b1), Box::new(dd))),
        dts_norm_definite(DynTowerSpec::Ext(
            Box::new(a2), Box::new(b2), Box::new(dd))),
    ...
    decreases f, 6nat,
{
    ...
    lemma_dts_norm_mul(a1, b1, a2, b2, dd);
        lemma_dts_is_zero_implies_eqv_zero(im_val);
        lemma_dts_nonneg_fuel_zero(im_val, f);
        //  im_val nonneg. For C1: also need nonneg(re_val).
        //  le_total on re_val already called above.
        ...
        let nx = dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)));
        let ny = dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
        //  Infrastructure for nx, ny (minimal for le_total + nonneg_mul)
        lemma_dts_same_radicand_reflexive(a1);
        lemma_dts_same_radicand_reflexive(a2);
        lemma_dts_same_radicand_reflexive(b1);
        lemma_dts_same_radicand_reflexive(b2);
        ...
        lemma_dts_same_radicand_transitive(dts_neg(nx), ny, dts_neg(ny));
        //  square_nonneg(re_val) for re_val²
        lemma_dts_nonneg_radicands_mul(a1, a2);
        lemma_dts_nonneg_radicands_mul(b1, b2);
        lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b2));
        lemma_dts_nonneg_radicands_add(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2)));
        ...
        lemma_dts_depth_mul_le(dd, dts_mul(b1, b2));
        lemma_dts_depth_add_le(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2)));
        //  Inline square_nonneg(re_val): neg(re)≥0 → nonneg_mul(neg(re), neg(re)) → neg_mul_neg → nonneg(re²)
        ...
        lemma_dts_same_radicand_symmetric(re_val, dts_neg(re_val));
        lemma_dts_same_radicand_reflexive(dts_neg(re_val));
        lemma_dts_nonneg_mul_closed_fuel(dts_neg(re_val), dts_neg(re_val), f);
        lemma_dts_same_radicand_reflexive(re_val);
        lemma_dts_add_closed(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2)));
        lemma_dts_neg_mul_neg(re_val, re_val);
        lemma_dts_mul_closed(dts_neg(re_val), dts_neg(re_val));
        lemma_dts_same_radicand_symmetric(dts_neg(re_val),
            dts_mul(dts_neg(re_val), dts_neg(re_val)));
        lemma_dts_same_radicand_transitive(
            dts_mul(dts_neg(re_val), dts_neg(re_val)), dts_neg(re_val), re_val);
        lemma_dts_mul_closed(re_val, re_val);
        lemma_dts_same_radicand_symmetric(re_val, dts_mul(re_val, re_val));
        lemma_dts_same_radicand_transitive(
            dts_mul(dts_neg(re_val), dts_neg(re_val)), re_val, dts_mul(re_val, re_val));
        lemma_dts_nonneg_fuel_congruence(
            dts_mul(dts_neg(re_val), dts_neg(re_val)), dts_mul(re_val, re_val), f);
        //  Derive nonneg(neg(nx*ny)) from mixed-sign norms.
        //  If mixed signs: neg_mul chain → nonneg(neg(nx*ny)) → nonneg(neg(re_val²))
        //  → both_nonneg(re_val²) → le_antisymmetric → is_zero(re_val²)
        //  → norm_definite → is_zero(re_val) → nonneg_fuel_zero → nonneg(re_val).
        //  If same signs: nonneg(nx*ny) matches nonneg(re_val²) — no extra info.
        //  But same-sign + im=0 + !nonneg(re) is unreachable via Cauchy-Schwarz.
        //  For same-sign case: Z3 should close from nonneg_fuel unfolding.
        if dts_nonneg_fuel(dts_neg(nx), f) && dts_nonneg_fuel(ny, f) {
            //  neg(nx)≥0, ny≥0: neg_mul chain → nonneg(neg(nx*ny))
            ...
            //  nx≥0, neg(ny)≥0: neg_mul chain → nonneg(neg(nx*ny))
            lemma_dts_same_radicand_transitive(nx, ny, dts_neg(ny));
            ...
            //  Same-sign norms (both ≥ 0 or both ≤ 0).
            //  With im=0 and !nonneg(re): derive nonneg(re_val) from
            //  nonneg of sub-terms or Cauchy-Schwarz.
            //  re_val = add(P, Q) where P = mul(a1,a2), Q = mul(dd,mul(b1,b2)).
            //  If both P≥0 and Q≥0: nonneg_add → nonneg(re_val).
            //  Otherwise: the im=0 constraint forces P and Q signs that make re_val≥0.
            let p_val = dts_mul(a1, a2);
            let q_val = dts_mul(dd, dts_mul(b1, b2));
            lemma_dts_nonneg_or_neg_nonneg_fuel(p_val, f);
            lemma_dts_nonneg_or_neg_nonneg_fuel(q_val, f);
            if dts_nonneg_fuel(p_val, f) && dts_nonneg_fuel(q_val, f) {
                //  Both nonneg: re_val = add(P, Q) ≥ 0 by nonneg_add.
                //  nonneg(re_val) via nonneg_add. With nonneg(im_val) from nonneg_fuel_zero: C1.
                lemma_dts_nonneg_add_closed_fuel(p_val, q_val, f);
                return;
            }
            //  Not both nonneg. With im=0 and !nonneg(re_val):
            //  This sub-case is unreachable for products of nonneg values
            //  with is_zero(im). Z3 should close from nonneg_fuel unfolding
            //  + the established norm and sign facts.
            ...
            //  nonneg(nx*ny) established for product_norm.
            //  Need nonneg(re_val). Use Cauchy-Schwarz:
            //  For neg(nx)≥0, neg(ny)≥0: dd*b1²≥a1², dd*b2²≥a2².
            //  le_mul_nonneg_monotone chain → Q²≥P² where Q=dd*b1*b2, P=a1*a2.
            //  For nx≥0, ny≥0: a1²≥dd*b1², a2²≥dd*b2².
            //  le_mul_nonneg_monotone chain → P²≥Q².
            //  In both cases: the larger of |P|,|Q| has its square ≥ the other.
            //  With both P≥0, Q≥0 already handled above, the remaining branch has
            //  one negative. Reverse square_le_square gives |larger| ≥ |smaller|.
            //  re_val = P + Q = larger + smaller where larger dominates → re_val ≥ 0.
            //
            //  le_mul_nonneg_monotone(a², b², c²): a²≤b² ∧ c²≥0 → a²c²≤b²c².
            //  Step 1: le_mul_nonneg_monotone(a1², dd*b1², dd*b2²)
            //  Step 2: le_mul_nonneg_monotone(a2², dd*b2², a1²)
            //  Step 3: transitivity → dd²b1²b2² ≥ a1²a2².
            //  Or reversed for nx≥0,ny≥0.
            //
            //  For now: establish Q ≥ 0 (or P ≥ 0) and use nonneg_add for the easy case.
            //  The !both_nonneg(P,Q) branch needs the full chain. Provide key hints:
            //  assert_by blocks to scope Z3.
            //  Cauchy-Schwarz: le_mul_nonneg_monotone chain.
            //  For neg(nx)≥0,neg(ny)≥0: dd*b1²≥a1², dd*b2²≥a2² → Q²≥P².
            //  For nx≥0,ny≥0: a1²≥dd*b1², a2²≥dd*b2² → P²≥Q².
            //  In both cases: establish the larger² ≥ smaller² inequality.
            //  Then reverse_square_le_square → nonneg(re_val).
            let a1_sq = dts_mul(a1, a1);
            let a2_sq = dts_mul(a2, a2);
            let dd_b1_sq = dts_mul(dd, dts_mul(b1, b1));
            let dd_b2_sq = dts_mul(dd, dts_mul(b2, b2));
            //  nonneg(dd*b2²)
            lemma_dts_same_radicand_reflexive(b2);
            lemma_dts_mul_closed(b2, b2);
            lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, b2));
            ...
            lemma_dts_depth_mul_le(b2, b2);
            lemma_dts_depth_mul_le(dd, dts_mul(b2, b2));
            //  Inline square_nonneg(b2): use le_total + nonneg_mul or neg_mul_neg
            lemma_dts_nonneg_or_neg_nonneg_fuel(b2, f);
            if dts_nonneg_fuel(b2, f) {
                lemma_dts_nonneg_mul_closed_fuel(b2, b2, f);
            } else {
                lemma_dts_neg_well_formed(b2);
                lemma_dts_same_radicand_neg(b2);
                lemma_dts_nonneg_radicands_neg(b2);
                ...
                lemma_dts_depth_neg(b2);
                lemma_dts_same_radicand_symmetric(b2, dts_neg(b2));
                lemma_dts_same_radicand_reflexive(dts_neg(b2));
                lemma_dts_nonneg_mul_closed_fuel(dts_neg(b2), dts_neg(b2), f);
                lemma_dts_neg_mul_neg(b2, b2);
                lemma_dts_mul_closed(dts_neg(b2), dts_neg(b2));
                lemma_dts_same_radicand_symmetric(dts_neg(b2), dts_mul(dts_neg(b2), dts_neg(b2)));
                lemma_dts_same_radicand_transitive(dts_mul(dts_neg(b2), dts_neg(b2)), dts_neg(b2), b2);
                lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, b2));
                lemma_dts_same_radicand_transitive(dts_mul(dts_neg(b2), dts_neg(b2)), b2, dts_mul(b2, b2));
                lemma_dts_nonneg_fuel_congruence(dts_mul(dts_neg(b2), dts_neg(b2)), dts_mul(b2, b2), f);
            }
            lemma_dts_nonneg_fuel_stabilize(dd, f);
            lemma_dts_nonneg_mul_closed_fuel(dd, dts_mul(b2, b2), f);
            //  Inline square_nonneg(a1)
            lemma_dts_nonneg_or_neg_nonneg_fuel(a1, f);
            if dts_nonneg_fuel(a1, f) {
                lemma_dts_nonneg_mul_closed_fuel(a1, a1, f);
            } else {
                lemma_dts_neg_well_formed(a1);
                lemma_dts_same_radicand_neg(a1);
                lemma_dts_nonneg_radicands_neg(a1);
                ...
                lemma_dts_depth_neg(a1);
                lemma_dts_same_radicand_symmetric(a1, dts_neg(a1));
                lemma_dts_same_radicand_reflexive(dts_neg(a1));
                lemma_dts_nonneg_mul_closed_fuel(dts_neg(a1), dts_neg(a1), f);
                lemma_dts_neg_mul_neg(a1, a1);
                lemma_dts_mul_closed(dts_neg(a1), dts_neg(a1));
                lemma_dts_same_radicand_symmetric(dts_neg(a1), dts_mul(dts_neg(a1), dts_neg(a1)));
                lemma_dts_same_radicand_transitive(dts_mul(dts_neg(a1), dts_neg(a1)), dts_neg(a1), a1);
                lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, a1));
                lemma_dts_same_radicand_transitive(dts_mul(dts_neg(a1), dts_neg(a1)), a1, dts_mul(a1, a1));
                lemma_dts_nonneg_fuel_congruence(dts_mul(dts_neg(a1), dts_neg(a1)), dts_mul(a1, a1), f);
            }
            //  same_radicand chains for le_mul_nonneg_monotone
            lemma_dts_same_radicand_symmetric(a1, a1_sq);
            lemma_dts_same_radicand_transitive(a1_sq, a1, dd);
            lemma_dts_same_radicand_transitive(a1_sq, dd, dd_b1_sq);
            lemma_dts_same_radicand_transitive(a1_sq, dd, dd_b2_sq);
            ...
            if dts_nonneg_fuel(dts_neg(nx), f) && dts_nonneg_fuel(dts_neg(ny), f) {
                //  Cauchy-Schwarz: use step + combine helpers (was 270-line assert_by).
                ...
                //  Setup: neg_sub_swap for neg(nx) and neg(ny)
                let a2_sq_local = dts_mul(a2, a2);
                lemma_dts_neg_well_formed(a2);
                lemma_dts_same_radicand_neg(a2);
                ...
                lemma_dts_same_radicand_reflexive(a2);
                lemma_dts_mul_closed(a2, a2);
                //  neg(ny) → sub(dd_b2_sq, a2_sq) ≥ 0
                lemma_dts_neg_sub_swap(a2_sq_local, dd_b2_sq_inner);
                lemma_dts_same_radicand_symmetric(a2, a1);
                lemma_dts_same_radicand_transitive(a2, a1, dd);
                lemma_dts_same_radicand_symmetric(a2, dd);
                lemma_dts_same_radicand_symmetric(a2, a2_sq_local);
                lemma_dts_same_radicand_transitive(a2_sq_local, a2, dd);
                lemma_dts_same_radicand_transitive(a2_sq_local, dd, dd_b2_sq_inner);
                lemma_dts_same_radicand_symmetric(ny, dts_neg(ny));
                lemma_dts_same_radicand_transitive(dts_neg(ny), ny, a2_sq_local);
                lemma_dts_same_radicand_transitive(dts_neg(ny), a2_sq_local, dd_b2_sq_inner);
                lemma_dts_neg_well_formed(a2_sq_local);
                lemma_dts_same_radicand_neg(a2_sq_local);
                lemma_dts_nonneg_radicands_neg(a2_sq_local);
                lemma_dts_same_radicand_symmetric(a2_sq_local, dd);
                lemma_dts_same_radicand_symmetric(dd, dd_b2_sq_inner);
                lemma_dts_same_radicand_transitive(dd_b2_sq_inner, dd, a2_sq_local);
                lemma_dts_same_radicand_transitive(dd_b2_sq_inner, a2_sq_local, dts_neg(a2_sq_local));
                lemma_dts_add_closed(dd_b2_sq_inner, dts_neg(a2_sq_local));
                lemma_dts_same_radicand_symmetric(dd_b2_sq_inner, dts_sub(dd_b2_sq_inner, a2_sq_local));
                lemma_dts_same_radicand_transitive(dts_neg(ny), dd_b2_sq_inner, dts_sub(dd_b2_sq_inner, a2_sq_local));
                lemma_dts_same_radicand_symmetric(dts_neg(ny), dts_sub(dd_b2_sq_inner, a2_sq_local));
                lemma_dts_nonneg_fuel_congruence(dts_neg(ny), dts_sub(dd_b2_sq_inner, a2_sq_local), f);
                //  Transfer to sub(dd_b2_sq, na2_sq) via congruence
                lemma_dts_eqv_symmetric(na2_sq_inner, a2_sq_local);
                lemma_dts_eqv_reflexive(dd_b2_sq_inner);
                lemma_dts_same_radicand_reflexive(dd_b2_sq_inner);
                lemma_dts_same_radicand_symmetric(a2, dts_neg(a2));
                lemma_dts_nonneg_radicands_neg(a2);
                lemma_dts_nonneg_radicands_mul(dts_neg(a2), dts_neg(a2));
                lemma_dts_same_radicand_symmetric(dts_neg(a2), na2_sq_inner);
                lemma_dts_same_radicand_transitive(na2_sq_inner, dts_neg(a2), a2);
                lemma_dts_same_radicand_transitive(na2_sq_inner, a2, a2_sq_local);
                //  sr(a2_sq_local, na2_sq_inner) from symmetric of the chain above
                lemma_dts_same_radicand_symmetric(na2_sq_inner, a2_sq_local);
                lemma_dts_sub_congruence_both(dd_b2_sq_inner, a2_sq_local, dd_b2_sq_inner, na2_sq_inner);
                lemma_dts_same_radicand_symmetric(dd_b2_sq_inner, dts_sub(dd_b2_sq_inner, a2_sq_local));
                lemma_dts_neg_well_formed(na2_sq_inner);
                lemma_dts_same_radicand_neg(na2_sq_inner);
                lemma_dts_same_radicand_transitive(na2_sq_inner, a2, dd);
                lemma_dts_same_radicand_symmetric(na2_sq_inner, dd);
                lemma_dts_same_radicand_transitive(dd_b2_sq_inner, dd, na2_sq_inner);
                lemma_dts_same_radicand_transitive(dd_b2_sq_inner, na2_sq_inner, dts_neg(na2_sq_inner));
                lemma_dts_add_closed(dd_b2_sq_inner, dts_neg(na2_sq_inner));
                lemma_dts_same_radicand_symmetric(dd_b2_sq_inner, dts_sub(dd_b2_sq_inner, na2_sq_inner));
                lemma_dts_same_radicand_transitive(dts_sub(dd_b2_sq_inner, a2_sq_local), dd_b2_sq_inner,
                    dts_sub(dd_b2_sq_inner, na2_sq_inner));
                lemma_dts_nonneg_fuel_congruence(
                    dts_sub(dd_b2_sq_inner, a2_sq_local), dts_sub(dd_b2_sq_inner, na2_sq_inner), f);
                //  neg(nx) → sub(dd_b1_sq, a1_sq) ≥ 0
                lemma_dts_neg_sub_swap(a1_sq_inner, dd_b1_sq_inner);
                lemma_dts_same_radicand_symmetric(nx, dts_neg(nx));
                lemma_dts_same_radicand_symmetric(a1, a1_sq_inner);
                lemma_dts_same_radicand_transitive(a1_sq_inner, a1, dd);
                lemma_dts_same_radicand_transitive(dts_neg(nx), nx, a1_sq_inner);
                lemma_dts_same_radicand_transitive(dts_neg(nx), a1_sq_inner, dd_b1_sq_inner);
                lemma_dts_same_radicand_symmetric(a1_sq_inner, dd);
                lemma_dts_same_radicand_symmetric(dd, dd_b1_sq_inner);
                lemma_dts_same_radicand_transitive(dd_b1_sq_inner, dd, a1_sq_inner);
                lemma_dts_neg_well_formed(a1_sq_inner);
                lemma_dts_same_radicand_neg(a1_sq_inner);
                lemma_dts_nonneg_radicands_neg(a1_sq_inner);
                lemma_dts_same_radicand_transitive(dd_b1_sq_inner, a1_sq_inner, dts_neg(a1_sq_inner));
                lemma_dts_add_closed(dd_b1_sq_inner, dts_neg(a1_sq_inner));
                lemma_dts_same_radicand_symmetric(dd_b1_sq_inner, dts_sub(dd_b1_sq_inner, a1_sq_inner));
                lemma_dts_same_radicand_transitive(dts_neg(nx), dd_b1_sq_inner, dts_sub(dd_b1_sq_inner, a1_sq_inner));
                lemma_dts_same_radicand_symmetric(dts_neg(nx), dts_sub(dd_b1_sq_inner, a1_sq_inner));
                lemma_dts_nonneg_fuel_congruence(dts_neg(nx), dts_sub(dd_b1_sq_inner, a1_sq_inner), f);
                //  Establish nonneg(dd_b2_sq) and nonneg(a1_sq) for cauchy_schwarz_step
                lemma_dts_square_nonneg(b2, f);
                lemma_dts_nonneg_fuel_stabilize(dd, f);
                lemma_dts_depth_mul_le(b2, b2);
                lemma_dts_nonneg_radicands_mul(b2, b2);
                ...
                lemma_dts_nonneg_mul_closed_fuel(dd, dts_mul(b2, b2), f);
                lemma_dts_square_nonneg(a1, f);
                //  depth/radicands/norm_definite for the arguments
                lemma_dts_depth_mul_le(a1, a1);
                lemma_dts_depth_mul_le(dd, dts_mul(b1, b1));
                lemma_dts_depth_mul_le(dd, dts_mul(b2, b2));
                lemma_norm_definite_mul(a1, a1);
                ...
                //  Call cauchy_schwarz_step + combine
                lemma_dts_cauchy_schwarz_step(a1_sq_inner, na2_sq_inner, dd_b1_sq_inner, dd_b2_sq_inner,
                    dd, nx, ny, a2_sq_local, f);
                lemma_dts_cauchy_schwarz_combine(dd_b1_sq_inner, dd_b2_sq_inner,
                    a1_sq_inner, na2_sq_inner, dd, f);
                ...
                assert(dts_nonneg_fuel(dts_sub(dts_mul(dd_b1_sq_inner, dd_b2_sq_inner),
                    dts_mul(na2_sq_inner, a1_sq_inner)), f));
                //  Old 270-line assert_by block replaced by step+combine helpers above.
                //  Remove dead code (was inside old assert_by):
                //  (keeping only the closing logic)
                ...
                //  ═══ Derive nonneg(re_val) from neg(norm)≥0 + norm_definite + Q²≥P². ═══
                //  Key insight: neg(norm)≥0 + norm_definite eliminates C2, so both factors
                //  are in C1∪C3, which means nonneg(b1) and nonneg(b2).
                //  Step 1: Establish nonneg(b1) and nonneg(b2).
                lemma_dts_nonneg_im_from_neg_norm(a1, b1, dd, f);
                lemma_dts_same_radicand_symmetric(a1, a2);
                lemma_dts_same_radicand_transitive(a2, a1, b2);
                lemma_dts_same_radicand_transitive(a2, a1, dd);
                lemma_dts_nonneg_im_from_neg_norm(a2, b2, dd, f);
                //  Step 2: Case split on sign of a1*a2.
                lemma_dts_mul_closed(a1, a2);
                lemma_dts_nonneg_or_neg_nonneg_fuel(dts_mul(a1, a2), f);
                if dts_nonneg_fuel(dts_mul(a1, a2), f) {
                    //  nonneg(a1*a2) + nonneg(dd*b1*b2) → nonneg(re_val) by nonneg_add.
                    lemma_dts_same_radicand_symmetric(a1, b1);
                    lemma_dts_same_radicand_transitive(b1, a1, b2);
                    lemma_dts_nonneg_mul_closed_fuel(b1, b2, f);
                    lemma_dts_nonneg_fuel_stabilize(dd, f);
                    ...
                    lemma_dts_nonneg_add_closed_fuel(
                        dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2)), f);
                    //  nonneg(re_val) contradicts !nonneg(re_val) → false.
                } else {
                    //  neg(a1*a2) ≥ 0. Use clean-context congruence bridge helper.
                    lemma_cauchy_neg_a1a2_from_na2sq(a1, a2, b1, b2, dd, f);
                    //  nonneg(re_val) contradicts !nonneg(re_val) → false.
                }
                //  In both cases: nonneg(re_val) established, contradicting !nonneg(re_val).
                //  Z3 derives false → postcondition trivially satisfied.
                return;
            }
        }
        //  Mixed-sign branches (neg(nx)&&ny, nx&&neg(ny)) fall through here.
        //  They established nonneg(neg(nx*ny)). Use clean-context helper.
        lemma_dts_iszero_re_from_neg_norm_product(a1, b1, a2, b2, dd, f);
        //  nonneg(re_val) contradicts !nonneg(re_val) → false → postcondition.
        return;
}
// (Dead code removed — old is_zero proof path, ~270 lines)
// was: nonneg(neg(nx*ny)) → eqv chain → is_zero(re_val²) → is_zero(re_val)
// Now handled by: nonneg_im_from_neg_norm + cauchy bridge in neg(nx)&&neg(ny) branch.

///  Helper for remaining nonneg_mul cases (C1×C2, C2×C1, A×B, B×A).
///  Extracted from nonneg_mul_closed_fuel for rlimit management.
///  Error B dispatch: !nonneg(re) && !nonneg(im) && NOT(both norms positive).
///  Handles neg norms (Cauchy → nonneg(re)) and mixed norms (im Cauchy → nonneg(im)).
///  Both derive contradiction with !nonneg → postcondition.
proof fn lemma_error_b_dispatch<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
    nx: DynTowerSpec<T>, ny: DynTowerSpec<T>,
    ...
    requires
        ...
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a1), Box::new(b1), Box::new(dd)), (f + 1) as nat),
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a2), Box::new(b2), Box::new(dd)), (f + 1) as nat),
        //  Ghost equality params for norms
        nx == dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1))),
        ny == dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2))),
        //  NOT both positive norms
        !(dts_nonneg_fuel(nx, f) && dts_nonneg_fuel(ny, f)),
    ensures
        //  At least one of re_val or im_val is nonneg → caller derives contradiction.
        dts_nonneg_fuel(dts_add(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2))), f)
        || dts_nonneg_fuel(dts_add(dts_mul(a1, b2), dts_mul(b1, a2)), f),
    decreases f, 7nat,
{
    //  Derive nonneg_or_neg for nx internally (avoids caller Z3 context pollution).
    //  nx = sub(a1², dd*b1²) — need wf, depth, sr chains.
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_mul_closed(a1, a1);
    lemma_dts_mul_closed(b1, b1);
    ...
    //  Derive nonneg_or_neg for ny internally.
    //  ny = sub(a2², dd*b2²) — same pattern with a2, b2.
    lemma_dts_same_radicand_reflexive(a2);
    lemma_dts_same_radicand_reflexive(b2);
    lemma_dts_mul_closed(a2, a2);
    lemma_dts_mul_closed(b2, b2);
    ...
    lemma_dts_same_radicand_transitive(a2, a1, b2);
    lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, b2));
    lemma_dts_same_radicand_symmetric(a2, dd);  //  sr(dd, a2) from sr(a2, dd)
    lemma_dts_same_radicand_transitive(dd, a2, b2);
    lemma_dts_same_radicand_transitive(dd, b2, dts_mul(b2, b2));
    lemma_dts_mul_closed(dd, dts_mul(b2, b2));
    lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, a2));
    lemma_dts_same_radicand_transitive(dts_mul(a2, a2), a2, dd);
    ...
    if dts_nonneg_fuel(dts_neg(nx), f) && dts_nonneg_fuel(dts_neg(ny), f) {
        //  Both norms negative: Cauchy → nonneg(re_val) → contradiction with !nonneg(re_val).
        lemma_neg_norms_nonneg_re_val(a1, b1, a2, b2, dd, f);
    } else if dts_nonneg_fuel(nx, f) {
        //  nx≥0, neg(ny)≥0: standard direction → nonneg(im_val) → contradiction.
        lemma_mixed_norms_nonneg_im_val(a1, b1, a2, b2, dd, f);
    } else {
        //  neg(nx)≥0, ny≥0: swap factors.
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_symmetric(a1, b2);
        lemma_dts_same_radicand_transitive(a2, a1, b1);
        lemma_dts_same_radicand_transitive(a2, a1, b2);
        lemma_dts_same_radicand_transitive(a2, a1, dd);
        lemma_mixed_norms_nonneg_im_val(a2, b2, a1, b1, dd, f);
        //  → nonneg(add(a2*b1, b2*a1)). Convert to nonneg(im_val) via congruence.
        //  sr chains for mul_closed and add_closed
        lemma_dts_same_radicand_symmetric(a1, b1);
        lemma_dts_same_radicand_transitive(b1, a1, a2);  //  sr(b1, a2)
        lemma_dts_same_radicand_symmetric(a1, b2);
        lemma_dts_same_radicand_transitive(b2, a1, a2);  //  sr(b2, a2) — wait, need sr(b2, a1)
        //  sr(a2, b1) from symmetric(b1, a2)... sr(b1, a2) above → symmetric → sr(a2, b1)
        lemma_dts_same_radicand_symmetric(b1, a2);  //  sr(a2, b1)
        //  sr(a1, b2) from requires
        lemma_dts_mul_commutative(a2, b1);
        lemma_dts_mul_commutative(b2, a1);
        lemma_dts_mul_closed(a2, b1);
        lemma_dts_mul_closed(b2, a1);
        lemma_dts_mul_closed(a1, b2);
        lemma_dts_mul_closed(b1, a2);
        //  sr(mul(a2,b1), mul(b2,a1)) for add_closed: a2*b1 ~ a2 ~ a1 ~ b2 ~ b2*a1
        lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, b1));
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), a2, a1);
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), a1, b2);
        lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, a1));
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), b2, dts_mul(b2, a1));
        lemma_dts_add_congruence_left(dts_mul(a2, b1), dts_mul(b1, a2),
            dts_mul(b2, a1));
        lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, b2));
        lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), b1, a1);
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), a1, dts_mul(a1, b2));
        lemma_dts_add_congruence_right(dts_mul(b1, a2), dts_mul(b2, a1), dts_mul(a1, b2));
        lemma_dts_add_commutative(dts_mul(b1, a2), dts_mul(a1, b2));
        lemma_dts_add_closed(dts_mul(a2, b1), dts_mul(b2, a1));
        //  sr(b1*a2, b2*a1): b1*a2 ~ b1 ~ a1 ~ b2 ~ b2*a1
        lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), b1, a1);
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), a1, b2);
        lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, a1));
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), b2, dts_mul(b2, a1));
        lemma_dts_add_closed(dts_mul(b1, a2), dts_mul(b2, a1));
        //  sr(b1*a2, a1*b2): b1*a2 ~ b1 ~ a1 ~ a1*b2
        lemma_dts_same_radicand_transitive(dts_mul(b1, a2), a1, dts_mul(a1, b2));
        lemma_dts_add_closed(dts_mul(b1, a2), dts_mul(a1, b2));
        //  sr(a1*b2, b1*a2): symmetric of above chain
        lemma_dts_same_radicand_symmetric(dts_mul(b1, a2), dts_mul(a1, b2));
        lemma_dts_add_closed(dts_mul(a1, b2), dts_mul(b1, a2));
        lemma_dts_eqv_transitive(dts_add(dts_mul(a2, b1), dts_mul(b2, a1)),
            dts_add(dts_mul(b1, a2), dts_mul(b2, a1)),
            dts_add(dts_mul(b1, a2), dts_mul(a1, b2)));
        lemma_dts_eqv_transitive(dts_add(dts_mul(a2, b1), dts_mul(b2, a1)),
            dts_add(dts_mul(b1, a2), dts_mul(a1, b2)),
            dts_add(dts_mul(a1, b2), dts_mul(b1, a2)));
        lemma_dts_same_radicand_symmetric(dts_mul(a2, b1),
            dts_add(dts_mul(a2, b1), dts_mul(b2, a1)));
        //  sr(a2*b1, b1*a2): a2*b1 ~ a2 ~ a1 ~ b1 ~ b1*a2
        lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, b1));
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), a2, a1);
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), a1, b1);
        lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(dts_mul(a2, b1), b1, dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(
            dts_add(dts_mul(a2, b1), dts_mul(b2, a1)),
            dts_mul(a2, b1), dts_mul(b1, a2));
        lemma_dts_same_radicand_transitive(
            dts_add(dts_mul(a2, b1), dts_mul(b2, a1)),
            dts_mul(b1, a2), dts_mul(a1, b2));
        lemma_dts_same_radicand_symmetric(dts_mul(a1, b2),
            dts_add(dts_mul(a1, b2), dts_mul(b1, a2)));
        lemma_dts_same_radicand_transitive(
            dts_add(dts_mul(a2, b1), dts_mul(b2, a1)),
            dts_mul(a1, b2),
    ...
    //  All branches establish either nonneg(re_val) or nonneg(im_val).
    //  Combined with !nonneg(re_val) or !nonneg(im_val) → false.
}

///  Helper for !nonneg(im_val) path in nonneg_mul_remaining.
///  Extracted to avoid Z3 context pollution after 600+ lines of nonneg(im) handler.
///  Handles: C2 via conclude_re when im_val not nonneg.
///  Cases: (1) pos norms → nonneg(a1,a2) → nonneg(re) + nonneg(norm) → conclude_re.
///         (2) mixed/neg norms → error_b_dispatch → nonneg(re)||nonneg(im) → contradiction
///             or neg_mul_neg → nonneg(norm) → conclude_re.
#[verifier::rlimit(500)]
proof fn lemma_nonneg_mul_neg_im_path<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    ...
    decreases f, 8nat,
{
    let re_val = dts_add(dts_mul(a1, a2), dts_mul(dd, dts_mul(b1, b2)));
    let im_val = dts_add(dts_mul(a1, b2), dts_mul(b1, a2));
    //  Establish re_val/im_val infrastructure
    lemma_dts_same_radicand_symmetric(a1, b1);
    lemma_dts_same_radicand_transitive(b1, a1, b2);
    lemma_dts_same_radicand_transitive(b1, a1, a2);
    lemma_dts_mul_closed(a1, a2);
    ...
    lemma_dts_norm_mul(a1, b1, a2, b2, dd);
    //  Norm factors
    let nx2 = dts_sub(dts_mul(a1, a1), dts_mul(dd, dts_mul(b1, b1)));
    let ny2 = dts_sub(dts_mul(a2, a2), dts_mul(dd, dts_mul(b2, b2)));
    //  nx2 wf/depth/sr
    lemma_dts_same_radicand_reflexive(a1);
    lemma_dts_same_radicand_reflexive(b1);
    lemma_dts_mul_closed(a1, a1);
    ...
    //  Shared: nonneg_radicands + norm_definite for nx2, ny2
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b1));
    lemma_dts_nonneg_radicands_neg(dts_mul(dd, dts_mul(b1, b1)));
    ...
    lemma_norm_definite_add(dts_mul(a2, a2), dts_neg(dts_mul(dd, dts_mul(b2, b2))));
    //  sr(nx2, ny2) chain
    lemma_dts_same_radicand_symmetric(dts_mul(a1, a1), nx2);
    lemma_dts_same_radicand_transitive(nx2, dts_mul(a1, a1), a1);
    lemma_dts_same_radicand_symmetric(a1, a2);
    lemma_dts_same_radicand_transitive(nx2, a1, a2);
    lemma_dts_same_radicand_symmetric(a2, dts_mul(a2, a2));
    lemma_dts_same_radicand_transitive(nx2, a2, dts_mul(a2, a2));
    lemma_dts_same_radicand_symmetric(dts_mul(a2, a2), ny2);
    lemma_dts_same_radicand_transitive(nx2, dts_mul(a2, a2), ny2);
    //  Establish nonneg(nx2*ny2) by norm sign dispatch
    if dts_nonneg_fuel(nx2, f) && dts_nonneg_fuel(ny2, f) {
        lemma_dts_nonneg_mul_closed_fuel(nx2, ny2, f);
    } else if dts_nonneg_fuel(dts_neg(nx2), f) && dts_nonneg_fuel(dts_neg(ny2), f) {
        lemma_dts_neg_well_formed(nx2);
        lemma_dts_same_radicand_neg(nx2);
        lemma_dts_same_radicand_symmetric(nx2, dts_neg(nx2));
        lemma_dts_nonneg_radicands_neg(nx2);
        lemma_dts_depth_neg(nx2);
        lemma_norm_definite_neg(nx2);
        lemma_dts_neg_well_formed(ny2);
        lemma_dts_same_radicand_neg(ny2);
        lemma_dts_same_radicand_symmetric(ny2, dts_neg(ny2));
        lemma_dts_nonneg_radicands_neg(ny2);
        lemma_dts_depth_neg(ny2);
        lemma_norm_definite_neg(ny2);
        lemma_dts_same_radicand_transitive(dts_neg(nx2), nx2, ny2);
        lemma_dts_same_radicand_transitive(dts_neg(nx2), ny2, dts_neg(ny2));
        lemma_dts_nonneg_mul_closed_fuel(dts_neg(nx2), dts_neg(ny2), f);
        lemma_dts_neg_mul_neg(nx2, ny2);
        lemma_dts_mul_closed(dts_neg(nx2), dts_neg(ny2));
        lemma_dts_mul_closed(nx2, ny2);
        lemma_dts_same_radicand_symmetric(dts_neg(nx2), dts_mul(dts_neg(nx2), dts_neg(ny2)));
        lemma_dts_same_radicand_transitive(dts_mul(dts_neg(nx2), dts_neg(ny2)), dts_neg(nx2), nx2);
        lemma_dts_same_radicand_symmetric(nx2, dts_mul(nx2, ny2));
        lemma_dts_same_radicand_transitive(dts_mul(dts_neg(nx2), dts_neg(ny2)), nx2, dts_mul(nx2, ny2));
        lemma_dts_nonneg_fuel_congruence(dts_mul(dts_neg(nx2), dts_neg(ny2)), dts_mul(nx2, ny2), f);
    } else {
        //  Mixed norms: one nonneg, one neg(nonneg). Call mixed_norms helper directly.
        //  This gives nonneg(im_val) which contradicts !im_nonneg → false → vacuous.
        if dts_nonneg_fuel(nx2, f) {
            //  nx2 ≥ 0, neg(ny2) ≥ 0: standard direction
            lemma_mixed_norms_nonneg_im_val(a1, b1, a2, b2, dd, f);
        } else {
            //  neg(nx2) ≥ 0, ny2 ≥ 0: swap factors
            lemma_dts_same_radicand_symmetric(a1, a2);
            lemma_dts_same_radicand_symmetric(a1, b2);
            lemma_dts_same_radicand_transitive(a2, a1, b1);
            lemma_dts_same_radicand_transitive(a2, a1, b2);
            lemma_dts_same_radicand_transitive(a2, a1, dd);
            lemma_mixed_norms_nonneg_im_val(a2, b2, a1, b1, dd, f);
            //  nonneg(add(a2*b1, b2*a1)). Convert to nonneg(im_val) via commutative chain.
            //  (same congruence chain as in error_b_dispatch's swap case)
            lemma_dts_same_radicand_symmetric(b1, a2);
            lemma_dts_mul_commutative(a2, b1);
            lemma_dts_mul_commutative(b2, a1);
            lemma_dts_mul_closed(a2, b1);
            ...
            lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, a1));
            lemma_dts_same_radicand_transitive(dts_mul(a2, b1), b2, dts_mul(b2, a1));
            lemma_dts_add_congruence_left(dts_mul(a2, b1), dts_mul(b1, a2), dts_mul(b2, a1));
            lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, b2));
            lemma_dts_same_radicand_symmetric(b1, dts_mul(b1, a2));
            lemma_dts_same_radicand_transitive(dts_mul(b1, a2), b1, a1);
            lemma_dts_same_radicand_transitive(dts_mul(b1, a2), a1, dts_mul(a1, b2));
            ...
            lemma_dts_add_commutative(dts_mul(b1, a2), dts_mul(a1, b2));
            lemma_dts_add_closed(dts_mul(a2, b1), dts_mul(b2, a1));
            //  sr(mul(b1,a2), mul(b2,a1)): b1*a2 ~ b1 ~ a1 ~ b2 ~ b2*a1
            lemma_dts_same_radicand_transitive(dts_mul(b1, a2), a1, b2);
            lemma_dts_same_radicand_symmetric(b2, dts_mul(b2, a1));
            lemma_dts_same_radicand_transitive(dts_mul(b1, a2), b2, dts_mul(b2, a1));
            lemma_dts_add_closed(dts_mul(b1, a2), dts_mul(b2, a1));
        ...
        //  nonneg(im_val) established. !im_nonneg from requires → false → postcondition vacuous.
        return;
    }
    //  nonneg(nx2*ny2) established. Transfer to nonneg(norm_product) via congruence.
    let norm_product = dts_sub(dts_mul(re_val, re_val), dts_mul(dd, dts_mul(im_val, im_val)));
    lemma_dts_mul_closed(nx2, ny2);
    //  Build sr(mul(nx2,ny2), norm_product) chain:
    //  mul(nx2,ny2) ~ nx2 ~ a1 ~ re_val ~ mul(re_val,re_val) ~ norm_product
    //  Step 1: sr(mul(nx2,ny2), nx2)
    lemma_dts_same_radicand_symmetric(nx2, dts_mul(nx2, ny2));
    //  Step 2: sr(nx2, a1) — already established above via nx2 ~ mul(a1,a1) ~ a1
    //  Step 3: sr(a1, re_val) — a1 ~ mul(a1,a2) ~ re_val
    //  mul_closed(a1,a2) gives sr(a1, mul(a1,a2)), add_closed gives sr(mul(a1,a2), re_val)
    //  (both already established above in the infrastructure section)
    //  Step 4: sr(re_val, mul(re_val,re_val))
    lemma_dts_same_radicand_reflexive(re_val);
    lemma_dts_mul_closed(re_val, re_val);
    //  Step 5: sr(mul(re_val,re_val), norm_product)
    //  norm_product = sub(re²,dd*im²) = add(re², neg(dd*im²))
    //  Need sr/wf for im_val products
    lemma_dts_same_radicand_reflexive(im_val);
    lemma_dts_mul_closed(im_val, im_val);
    //  sr(dd, im_val) — dd ~ a1 ~ mul(a1,b2) ~ im_val
    lemma_dts_same_radicand_transitive(dd, a1, dts_mul(a1, b2));
    lemma_dts_same_radicand_symmetric(dts_mul(a1, b2), im_val);
    lemma_dts_same_radicand_transitive(dd, dts_mul(a1, b2), im_val);
    //  sr(dd, mul(im,im))
    lemma_dts_same_radicand_symmetric(im_val, dts_mul(im_val, im_val));
    lemma_dts_same_radicand_transitive(dd, im_val, dts_mul(im_val, im_val));
    lemma_dts_mul_closed(dd, dts_mul(im_val, im_val));
    lemma_dts_neg_well_formed(dts_mul(dd, dts_mul(im_val, im_val)));
    lemma_dts_same_radicand_neg(dts_mul(dd, dts_mul(im_val, im_val)));
    //  sr(mul(re,re), neg(mul(dd,mul(im,im))))
    lemma_dts_same_radicand_symmetric(re_val, dts_mul(re_val, re_val));
    //  re_val ~ a1: re_val ~ mul(a1,a2) ~ a1 (reverse direction)
    //  We have sr(a1, mul(a1,a2)) from mul_closed and sr(mul(a1,a2), re_val) from add_closed.
    //  symmetric(re_val, a1) needs sr(re_val, a1).
    //  sr(re_val, mul(a1,a2)): symmetric on sr(mul(a1,a2), re_val) from add_closed
    lemma_dts_same_radicand_symmetric(dts_mul(a1, a2), re_val);  //  sr(re_val, mul(a1,a2))
    lemma_dts_same_radicand_symmetric(a1, dts_mul(a1, a2));  //  sr(mul(a1,a2), a1)
    lemma_dts_same_radicand_transitive(re_val, dts_mul(a1, a2), a1);  //  sr(re_val, a1)
    //  Now chain: mul(re,re) ~ re_val ~ a1 ~ dd ~ mul(dd,mul(im,im)) ~ neg(...)
    lemma_dts_same_radicand_transitive(dts_mul(re_val, re_val), re_val, a1);
    lemma_dts_same_radicand_transitive(dts_mul(re_val, re_val), a1, dd);
    lemma_dts_same_radicand_symmetric(dd, dts_mul(dd, dts_mul(im_val, im_val)));
    ...
    lemma_dts_add_closed(dts_mul(re_val, re_val),
        dts_neg(dts_mul(dd, dts_mul(im_val, im_val))));
    //  sr(re_val, norm_product): re_val ~ mul(re,re) ~ norm_product
    lemma_dts_same_radicand_symmetric(dts_mul(re_val, re_val), norm_product);
    lemma_dts_same_radicand_transitive(re_val, dts_mul(re_val, re_val), norm_product);
    //  sr(a1, re_val): a1 ~ mul(a1,a2) ~ re_val
    lemma_dts_same_radicand_transitive(a1, dts_mul(a1, a2), re_val);
    //  Now sr(mul(nx2,ny2), norm_product) via transitive chain
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), nx2, a1);
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), a1, re_val);
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), re_val, norm_product);
    //  eqv_symmetric: eqv(mul(nx2,ny2), norm_product)
    lemma_dts_eqv_symmetric(norm_product, dts_mul(nx2, ny2));
    //  Transfer nonneg via congruence
    lemma_dts_nonneg_fuel_congruence(dts_mul(nx2, ny2), norm_product, f);
    //  Now nonneg(norm_product) established. Derive nonneg(re_val) + conclude_re.
    lemma_dts_nonneg_or_neg_nonneg_fuel(re_val, f);
    if dts_nonneg_fuel(re_val, f) {
        lemma_dts_nonneg_conclude_re_fuel(re_val, im_val, dd, f);
        return;
    }
    //  !nonneg(re): derive nonneg(re) → contradiction.
    if dts_nonneg_fuel(nx2, f) && dts_nonneg_fuel(ny2, f) {
        //  Pos norms → nonneg(a1), nonneg(a2) → nonneg(re) via nonneg_add/reverse_cauchy.
        lemma_dts_nonneg_re_from_nonneg_norm(a1, b1, dd, f);
        lemma_dts_same_radicand_symmetric(a1, a2);
        lemma_dts_same_radicand_transitive(a2, a1, b2);
        lemma_dts_same_radicand_transitive(a2, a1, dd);
        ...
        //  Neg norms -> nonneg(re_val) via neg_norms helper.
        lemma_neg_norms_nonneg_re_val(a1, b1, a2, b2, dd, f);
        //  nonneg(re_val) contradicts !nonneg(re_val) -> false.
    }
}
...
///  Helper for nonneg(im_val) path in nonneg_mul_remaining.
///  Extracted to avoid Z3 context pollution from 800-line inline proof.
///  Handles all norm sign cases:
///  (1) pos norms → nonneg(re_val) via reverse_cauchy + nonneg(norm) → conclude_re.
///  (2) neg norms → nonneg(re_val) via neg_norms + nonneg(norm) → conclude_re.
///  (3) mixed norms → nonneg(neg(norm_product)) → conclude_im_via_neg_norm.
#[verifier::rlimit(500)]
proof fn lemma_nonneg_mul_nonneg_im_path<T: OrderedField>(
    a1: DynTowerSpec<T>, b1: DynTowerSpec<T>,
    a2: DynTowerSpec<T>, b2: DynTowerSpec<T>,
    dd: DynTowerSpec<T>, f: nat,
)
    requires
        ...
        dts_nonneg_fuel(DynTowerSpec::Ext(Box::new(a2), Box::new(b2), Box::new(dd)), (f + 1) as nat),
        !dts_is_zero(dts_add(dts_mul(a1, b2), dts_mul(b1, a2))),
        //  nonneg(im_val) — the nonneg(im) case
        dts_nonneg_fuel(dts_add(dts_mul(a1, b2), dts_mul(b1, a2)), f),
    ensures
        dts_nonneg_fuel(
            DynTowerSpec::Ext(
    ...
    decreases f, 8nat,
{
    ...
    lemma_dts_depth_add_le(dts_mul(a2, a2), dts_neg(dts_mul(dd, dts_mul(b2, b2))));
    lemma_dts_nonneg_or_neg_nonneg_fuel(ny2, f);
    //  nonneg_radicands + norm_definite for nx2, ny2
    lemma_dts_nonneg_radicands_mul(a1, a1);
    lemma_dts_nonneg_radicands_mul(b1, b1);
    lemma_dts_nonneg_radicands_mul(dd, dts_mul(b1, b1));
    lemma_dts_nonneg_radicands_neg(dts_mul(dd, dts_mul(b1, b1)));
    ...
    lemma_dts_same_radicand_transitive(nx2, dts_mul(a2, a2), ny2);
    //  ═══ Norm sign dispatch ═══
    //  neg(nx2)/neg(ny2) infrastructure
    lemma_dts_neg_well_formed(nx2);
    lemma_dts_same_radicand_neg(nx2);
    lemma_dts_same_radicand_symmetric(nx2, dts_neg(nx2));
    lemma_dts_nonneg_radicands_neg(nx2);
    ...
    lemma_norm_definite_neg(ny2);
    lemma_dts_same_radicand_transitive(dts_neg(nx2), nx2, ny2);
    lemma_dts_same_radicand_transitive(nx2, ny2, dts_neg(ny2));
    if dts_nonneg_fuel(nx2, f) && dts_nonneg_fuel(ny2, f) {
        //  Pos norms: nonneg(nx2*ny2) directly
        lemma_dts_nonneg_mul_closed_fuel(nx2, ny2, f);
    } else if dts_nonneg_fuel(dts_neg(nx2), f) && dts_nonneg_fuel(dts_neg(ny2), f) {
        //  Neg norms: neg_mul_neg → nonneg(nx2*ny2)
        lemma_dts_same_radicand_transitive(dts_neg(nx2), ny2, dts_neg(ny2));
        lemma_dts_nonneg_mul_closed_fuel(dts_neg(nx2), dts_neg(ny2), f);
        lemma_dts_neg_mul_neg(nx2, ny2);
        lemma_dts_mul_closed(dts_neg(nx2), dts_neg(ny2));
        ...
        //  Mixed norms: establish nonneg(neg(nx2*ny2)) → conclude_im
        if dts_nonneg_fuel(dts_neg(nx2), f) && dts_nonneg_fuel(ny2, f) {
            //  neg(nx2)≥0 && ny2≥0: neg(nx2)*ny2 ≥ 0 ≡ neg(nx2*ny2) via neg_mul_left
            lemma_dts_nonneg_mul_closed_fuel(dts_neg(nx2), ny2, f);
            lemma_dts_neg_mul_left(nx2, ny2);
            lemma_dts_mul_closed(dts_neg(nx2), ny2);
            lemma_dts_mul_closed(nx2, ny2);
            lemma_dts_same_radicand_symmetric(dts_neg(nx2), dts_mul(dts_neg(nx2), ny2));
            lemma_dts_same_radicand_transitive(dts_mul(dts_neg(nx2), ny2), dts_neg(nx2), nx2);
            lemma_dts_same_radicand_symmetric(nx2, dts_mul(nx2, ny2));
            lemma_dts_same_radicand_transitive(dts_mul(dts_neg(nx2), ny2), nx2, dts_mul(nx2, ny2));
            lemma_dts_neg_well_formed(dts_mul(nx2, ny2));
            lemma_dts_same_radicand_neg(dts_mul(nx2, ny2));
            lemma_dts_same_radicand_transitive(dts_mul(dts_neg(nx2), ny2), dts_mul(nx2, ny2),
                dts_neg(dts_mul(nx2, ny2)));
            lemma_dts_nonneg_fuel_congruence(
                dts_mul(dts_neg(nx2), ny2), dts_neg(dts_mul(nx2, ny2)), f);
        } else {
            //  nx2≥0 && neg(ny2)≥0: nx2*neg(ny2) ≥ 0 ≡ neg(nx2*ny2) via neg_mul_right
            lemma_dts_nonneg_mul_closed_fuel(nx2, dts_neg(ny2), f);
            lemma_dts_neg_mul_right(nx2, ny2);
            lemma_dts_mul_closed(nx2, dts_neg(ny2));
            lemma_dts_mul_closed(nx2, ny2);
            lemma_dts_same_radicand_symmetric(nx2, dts_mul(nx2, dts_neg(ny2)));
            lemma_dts_same_radicand_transitive(
                dts_mul(nx2, dts_neg(ny2)), nx2, dts_mul(nx2, ny2));
            lemma_dts_neg_well_formed(dts_mul(nx2, ny2));
            lemma_dts_same_radicand_neg(dts_mul(nx2, ny2));
            lemma_dts_same_radicand_transitive(dts_mul(nx2, dts_neg(ny2)), dts_mul(nx2, ny2),
                dts_neg(dts_mul(nx2, ny2)));
            lemma_dts_nonneg_fuel_congruence(
                dts_mul(nx2, dts_neg(ny2)), dts_neg(dts_mul(nx2, ny2)), f);
        }
        //  nonneg(neg(nx2*ny2)) established. conclude_im_via_neg_norm.
        lemma_conclude_im_via_neg_norm(a1, b1, a2, b2, dd, f);
        return;
    }
    //  ═══ Same-sign norms: nonneg(nx2*ny2). Transfer to norm_product via congruence. ═══
    let norm_product = dts_sub(dts_mul(re_val, re_val), dts_mul(dd, dts_mul(im_val, im_val)));
    lemma_dts_mul_closed(nx2, ny2);
    lemma_dts_same_radicand_symmetric(nx2, dts_mul(nx2, ny2));
    lemma_dts_same_radicand_reflexive(re_val);
    ...
    lemma_dts_add_closed(dts_mul(re_val, re_val),
        dts_neg(dts_mul(dd, dts_mul(im_val, im_val))));
    //  sr(re_val, norm_product)
    lemma_dts_same_radicand_symmetric(dts_mul(re_val, re_val), norm_product);
    lemma_dts_same_radicand_transitive(re_val, dts_mul(re_val, re_val), norm_product);
    //  sr(a1, re_val)
    lemma_dts_same_radicand_transitive(a1, dts_mul(a1, a2), re_val);
    //  sr(mul(nx2,ny2), norm_product) via chain
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), nx2, a1);
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), a1, re_val);
    lemma_dts_same_radicand_transitive(dts_mul(nx2, ny2), re_val, norm_product);
    lemma_dts_eqv_symmetric(norm_product, dts_mul(nx2, ny2));
    lemma_dts_nonneg_fuel_congruence(dts_mul(nx2, ny2), norm_product, f);
    //  ═══ nonneg(norm_product). Derive nonneg(re_val) → conclude_re. ═══
    lemma_dts_nonneg_or_neg_nonneg_fuel(re_val, f);
    if dts_nonneg_fuel(re_val, f) {
        lemma_dts_nonneg_conclude_re_fuel(re_val, im_val, dd, f);
        return;
...
proof fn lemma_dts_nonneg_mul_remaining<T: OrderedField>(
    ...
    requires
        ...
        dts_norm_definite(a1), dts_norm_definite(b1),
        dts_norm_definite(a2), dts_norm_definite(b2), dts_norm_definite(dd),
        //  Ext-level norm_definite: includes the forall quantifier for dd radicand.
        dts_norm_definite(DynTowerSpec::Ext(
            Box::new(a1), Box::new(b1), Box::new(dd))),
        dts_nonneg_fuel(DynTowerSpec::Ext(
            Box::new(a1), Box::new(b1), Box::new(dd)), (f + 1) as nat),
    ...
    decreases f, 9nat,
{
            ...
            if !im_check {
                ...
                lemma_nonneg_mul_nonneg_im_path(a1, b1, a2, b2, dd, f);
                return;
            }
}
...
pub proof fn lemma_dts_square_nonneg<T: OrderedField>(x: DynTowerSpec<T>, fuel: nat)
    requires
        fuel >= dts_depth(x) + 1,
        dts_well_formed(x),
...
proof fn lemma_norm_definite_add<T: OrderedField>(x: DynTowerSpec<T>, y: DynTowerSpec<T>)
    requires
        dts_well_formed(x), dts_well_formed(y),
        dts_same_radicand(x, y),
        dts_norm_definite(x), dts_norm_definite(y),
...
proof fn lemma_norm_definite_neg<T: OrderedField>(x: DynTowerSpec<T>)
...
proof fn lemma_norm_definite_mul<T: OrderedField>(x: DynTowerSpec<T>, y: DynTowerSpec<T>)
    requires
        dts_well_formed(x), dts_well_formed(y),
        dts_same_radicand(x, y),
        dts_norm_definite(x), dts_norm_definite(y),
...
proof fn lemma_dts_neg_norm_congruence<T: OrderedField>(
    a: DynTowerSpec<T>, b: DynTowerSpec<T>, dd: DynTowerSpec<T>, f: nat,
)
    requires
        dts_well_formed(a), dts_well_formed(b), dts_well_formed(dd),
...
pub proof fn lemma_dts_le_antisymmetric_fuel<T: OrderedField>(x: DynTowerSpec<T>, fuel: nat)
    requires
        fuel >= dts_depth(x) + 1,
        dts_well_formed(x),
        dts_nonneg_radicands(x),
    ...
    decreases fuel, 1nat,
{
    match x {
        DynTowerSpec::Rat(r) => {
            //  nonneg(Rat(r)) = zero.le(r). nonneg(neg(Rat(r))) = zero.le(r.neg()).
            lemma_nonneg_both_implies_eqv_zero::<T>(r);
        }
        DynTowerSpec::Ext(re, im, d) => {
            let f = (fuel - 1) as nat;
            let a = *re; let b = *im; let dd = *d;
            ...
            if a_nn && na_nn {
                ...
                match db2 {
                    DynTowerSpec::Rat(r_db2) => {
                        lemma_nonneg_both_implies_eqv_zero::<T>(r_db2);
                    }
                    _ => {
            ...
            if b_nn && nb_nn {
                ...
                match a_sq {
                    DynTowerSpec::Rat(r_a2) => {
                        lemma_nonneg_both_implies_eqv_zero::<T>(r_a2);
                    }
                    _ => {}
                }
                return;
                    ...
                    DynTowerSpec::Rat(ra) => {
                        ...
                        match (b, dd) {
                            (DynTowerSpec::Rat(rb), DynTowerSpec::Rat(rd)) => {
                                //  norm = dts_sub(Rat(ra*ra), Rat(rd*(rb*rb)))
                                //       = dts_add(Rat(ra*ra), dts_neg(Rat(rd*(rb*rb))))
                                //       = Rat(ra.mul(ra).add(rd.mul(rb.mul(rb)).neg()))
                                let norm_r = ra.mul(ra).add(rd.mul(rb.mul(rb)).neg());
                                assert(norm == DynTowerSpec::Rat(norm_r));
                                assert(dts_nonneg_fuel(norm, f));
                                assert(dts_nonneg_fuel(DynTowerSpec::Rat(norm_r), f));
                                lemma_nonneg_both_implies_eqv_zero::<T>(norm_r);
                            }
