    ...
    decreases x,
{
    match x {
        DynTowerSpec::Rat(r) => {
            ...
            //  Now: neg(r).eqv(neg(0))
            //  neg(0) = from_int_spec(0).neg() = ... need to show this is 0
            //  Actually: Rational { num: 0, den: 0 }.neg() = Rational { num: 0, den: 0 }
            //  So neg(0) == 0. And neg(r).eqv(neg(0)) && neg(0)==0 ==> neg(r).eqv(0).
            T::axiom_eq_implies_eqv(
                T::zero().neg(),
                T::zero(),
            );
            T::axiom_eqv_transitive(
    ...
    decreases a, b,
{
    match (a, b) {
        (DynTowerSpec::Rat(r1), DynTowerSpec::Rat(r2)) => {
            // TODO-GENERIC: was Rational::lemma_denom_positive(r1);
            // TODO-GENERIC: was Rational::lemma_denom_positive(r2);
            assert(r1.mul(r2.neg()).num
                == r1.mul(r2).neg().num) by (nonlinear_arith);
            assert(r1.mul(r2.neg()).den
                == r1.mul(r2).neg().den) by (nonlinear_arith);
        }
        (DynTowerSpec::Ext(re, im, d), DynTowerSpec::Rat(r)) => {
    ...
    },
{
    ...
    assert(dts_eqv(dts_neg(dts_mul(d, dts_mul(b2, ny))), dts_neg(dts_mul(db2, ny)))) by {
        verus_algebra::lemmas::additive_group_lemmas::lemma_sub_congruence::<DynTowerSpec<T>>(
        ...
        assert(dts_eqv(dts_add(dts_zero(), dts_neg(dts_mul(d, dts_mul(b2, ny)))),
            ...
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_zero_left::<DynTowerSpec<T>>(
                dts_neg(dts_mul(d, dts_mul(b2, ny))));
        };
                ...
                if dts_nonneg_fuel(dts_neg(nx), f) && dts_nonneg_fuel(ny, f) {
                    ...
                    if dts_nonneg_fuel(b1, f) && dts_nonneg_fuel(neg_b2, f) {
                        ...
                        if !dts_nonneg_fuel(b1, f) {
                            ...
                            //  !nonneg(re_val). This branch is unreachable by Cauchy-Schwarz,
                            //  but proving it requires an extensive chain.
                            //  For now: the product has neg(re)≥0, im≥0, norm>0, neg_norm<0.
                            //  No C1/C2/C3 case applies. Z3 should see this is impossible
                            //  given both factors are nonneg.
                            //  Help Z3: product_norm ≡ nx*ny (from norm_mul).
                            //  Provide explicit nonneg(product_norm) = nonneg(sub(re², dd*im²)):
                            //  Z3 already has the norm_mul identity and nonneg(nx*ny).
                            //  The postcondition should follow from the nonneg_fuel unfolding
                            //  of both factors at fuel f+1, which constrains re_val to be ≥ 0
                            //  when b1≥0 and b2≥0 and both norms ≤ 0.
                            //  TODO: if Z3 can't close this, add explicit Cauchy-Schwarz chain.
                            //  PLACEHOLDER — nonneg_mul_remaining will fail here until full
                            //  Cauchy-Schwarz proof is added.
                            return;
                        }
                    lemma_dts_nonneg_fuel_stabilize(dd, f);
    ...
    decreases fuel, 1nat,
{
    match x {
        DynTowerSpec::Rat(r) => {
            //  nonneg(Rat(r)) = 0.le(r). nonneg(neg(Rat(r))) = 0.le(neg(r)).
            //  0.le(neg(r)): 0 ≤ -r.num, so r.num ≤ 0.
            //  r.le(0): r.num * 1 ≤ 0 * r.denom() = r.num ≤ 0.
            let zero = T::zero();
            let neg_r = r.neg();
            assert(zero.num == 0 && zero.den == 0);
            assert(neg_r.num == -r.num && neg_r.den == r.den);
            assert(r.le(zero)) by (nonlinear_arith)
                requires zero.le(neg_r), zero.num == 0, neg_r.num == -r.num, neg_r.den == r.den;
            T::axiom_le_antisymmetric(zero, r);
            // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(r);
        }
        DynTowerSpec::Ext(re, im, d) => {
            let f = (fuel - 1) as nat;
            let a = *re; let b = *im; let dd = *d;
            ...
            if a_nn && na_nn {
                ...
                match db2 {
                    DynTowerSpec::Rat(r_db2) => {
                        ...
                        let zero_r = T::zero();
                        let neg_db2_r = r_db2.neg();
                        assert(r_db2.le(zero_r)) by (nonlinear_arith)
                            requires zero_r.le(neg_db2_r),
                                zero_r.num == 0, zero_r.den == 0,
                                neg_db2_r.num == -r_db2.num, neg_db2_r.den == r_db2.den;
                        T::axiom_le_antisymmetric(zero_r, r_db2);
                        // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(r_db2);
                    }
                    _ => {
            ...
            if b_nn && nb_nn {
                ...
                match a_sq {
                    DynTowerSpec::Rat(r_a2) => {
                        let zero_r = T::zero();
                        let neg_a2 = r_a2.neg();
                        assert(r_a2.le(zero_r)) by (nonlinear_arith)
                            requires zero_r.le(neg_a2),
                                zero_r.num == 0, zero_r.den == 0,
                                neg_a2.num == -r_a2.num, neg_a2.den == r_a2.den;
                        T::axiom_le_antisymmetric(zero_r, r_a2);
                        // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(r_a2);
                    }
                    _ => {}
                }
                return;
                    ...
                    DynTowerSpec::Rat(ra) => {
                        ...
                        match (b, dd) {
                            (DynTowerSpec::Rat(rb), DynTowerSpec::Rat(rd)) => {
                                ...
                                let zero_r = T::zero();
                                let neg_norm_r = norm_r.neg();
                                assert(norm_r.le(zero_r)) by (nonlinear_arith)
                                    requires zero_r.le(neg_norm_r),
                                        zero_r.num == 0, zero_r.den == 0,
                                        neg_norm_r.num == -norm_r.num, neg_norm_r.den == norm_r.den;
                                T::axiom_le_antisymmetric(zero_r, norm_r);
                                // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(norm_r);
                            }
...
proof fn lemma_rational_nonsquare_forces_zero<T: OrderedField>(
    alpha: T, beta: T, delta: T,
)
    requires
        alpha.mul(alpha).eqv(delta.mul(beta.mul(beta))),
        !dts_is_base_square(DynTowerSpec::Rat(delta)),
    ensures
        beta.eqv(T::zero()),
{
    // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(beta);
    if beta.num != 0 {
        //  beta ≢ 0. Construct witness s = alpha/beta.
        let s = alpha.div(beta);
        let alpha_sq = alpha.mul(alpha);
        let beta_sq = beta.mul(beta);
        let delta_beta_sq = delta.mul(beta_sq);
        //  Step 1: beta_sq ≢ 0
        // TODO-GENERIC: was Rational::lemma_eqv_zero_iff_num_zero(beta_sq);
        ...
        // TODO-GENERIC: Rational::lemma_div_congruence(alpha_sq, delta_beta_sq, beta_sq);
        //  Step 3: delta_beta_sq/beta_sq ≡ delta (div_mul_cancel)
        // TODO-GENERIC: Rational::lemma_div_mul_cancel(delta, beta_sq);
        //  Step 4: alpha_sq/beta_sq ≡ delta (transitivity)
        T::axiom_eqv_transitive(alpha_sq.div(beta_sq), delta_beta_sq.div(beta_sq), delta);
        ...
        // TODO-GENERIC: Rational::lemma_div_mul_assoc(alpha, beta, s);
        //  (alpha/beta)*s ≡ (alpha*s)/beta
        T::axiom_mul_commutative(s, s);
        //  s*s = s*s (trivial, but s = alpha/beta)
        T::axiom_mul_commutative(alpha, s);
        //  alpha*s ≡ s*alpha
        // TODO-GENERIC: Rational::lemma_div_mul_assoc(alpha, beta, alpha);
        //  (alpha/beta)*alpha ≡ (alpha*alpha)/beta = alpha_sq/beta
        //  So s*alpha ≡ alpha_sq/beta
        //  And (alpha*s)/beta ≡ (s*alpha)/beta ≡ (alpha_sq/beta)/beta
        let alpha_s = alpha.mul(s);
        let s_alpha = s.mul(alpha);
        ...
        // TODO-GENERIC: Rational::lemma_div_div(alpha_sq, beta, beta);
        ...
        T::axiom_eqv_transitive(alpha_s, s_alpha, alpha_sq.div(beta));
        //  (alpha*s)/beta ≡ (alpha_sq/beta)/beta [div_congruence on alpha*s ≡ alpha_sq/beta]
        // TODO-GENERIC: Rational::lemma_div_congruence(alpha_s, alpha_sq.div(beta), beta);
        //  s*s ≡ (alpha*s)/beta [div_mul_assoc(alpha, beta, s)]
        //  (alpha*s)/beta ≡ (alpha_sq/beta)/beta [div_congruence]
        //  (alpha_sq/beta)/beta ≡ alpha_sq/beta_sq [div_div]
        let ss = s.mul(s);
        T::axiom_eqv_transitive(ss, alpha_s.div(beta), alpha_sq.div(beta).div(beta));
        T::axiom_eqv_transitive(ss, alpha_sq.div(beta).div(beta), alpha_sq.div(beta_sq));
        //  s*s ≡ alpha_sq/beta_sq ≡ delta [transitivity]
        T::axiom_eqv_transitive(ss, alpha_sq.div(beta_sq), delta);
        //  Contradiction: s*s ≡ delta but delta is not a rational perfect square
        assert(dts_is_base_square(DynTowerSpec::Rat(delta)));
        assert(false);
    }
}
